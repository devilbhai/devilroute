# @devilroute/opencode-provider

> ## ⚠️ Deprecated — use [`@devilroute/opencode-plugin`](https://www.npmjs.com/package/@devilroute/opencode-plugin) instead
>
...
> Pre-1.0. The API may still change. See `CHANGELOG` in the DevilRoute repo for breaking notes.
...
# or
pnpm add -D @devilroute/opencode-provider
```
...
const config = buildDevilRouteOpenCodeConfig({
  baseURL: "http://localhost:20128", // or your DevilRoute deployment URL
...
const provider = createDevilRouteProvider({
  ...
  apiKey: process.env.DEVILROUTE_API_KEY!,
});

// Place `provider` under provider.devilroute in your opencode.json
```

If you already have an `opencode.json` on disk and want a non-destructive merge from the DevilRoute side, use `devilroute config opencode` from the CLI (ships with the main DevilRoute install) — it preserves comments and unrelated keys.

## API

### `createDevilRouteProvider(options): OpenCodeProviderEntry`

Returns the value to place under `provider.devilroute` inside `opencode.json`.

| Option        | Type                    | Required | Description                                                                                                  |
| ------------- | ----------------------- | -------- | ------------------------------------------------------------------------------------------------------------ |
| `baseURL`     | `string`                | Yes      | DevilRoute base URL. Accepts `http://host:port` **or** `http://host:port/v1`. Trailing slashes are tolerated. |
| `apiKey`      | `string`                | Yes      | DevilRoute API key. Use `sk_devilroute` for local installs that have `REQUIRE_API_KEY=false`.                  |
| `displayName` | `string`                | No       | Custom name shown in the OpenCode UI. Default: `"DevilRoute"`.                                                |
| `models`      | `string[]`              | No       | Override the surfaced model catalog. Default: 4 curated models — see `DEVILROUTE_DEFAULT_OPENCODE_MODELS`.    |
...
### `buildDevilRouteOpenCodeConfig(options): OpenCodeConfigDocument`

Same options as above, but returns a full document with `$schema` and the `provider.devilroute` wrapper, ready to write to `opencode.json`.
...
- `DEVILROUTE_PROVIDER_KEY` — `"devilroute"` (the key used under `provider.*`).
- `DEVILROUTE_PROVIDER_NPM` — `"@ai-sdk/openai-compatible"` (the runtime delegate).
- `OPENCODE_CONFIG_SCHEMA` — `"https://opencode.ai/config.json"`.
- `DEVILROUTE_DEFAULT_OPENCODE_MODELS` — readonly list of default model ids.
...
- **`401 Invalid API key`** — your DevilRoute instance has `REQUIRE_API_KEY=true` but the key you supplied doesn't exist there. Create one via the dashboard or set `REQUIRE_API_KEY=false` and use `sk_devilroute`.
