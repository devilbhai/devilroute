/**
 * OpenCode provider plugin for DevilRoute AI Gateway.
 *
 * Generates an OpenCode-compatible provider object that points to a running
 * DevilRoute instance. The output follows the OpenCode config schema
 * (https://opencode.ai/config.json) and delegates the runtime to
 * `@ai-sdk/openai-compatible` so OpenCode can drive any DevilRoute-exposed
 ...
 *     });
 *     // config -> { $schema, provider: { devilroute: { npm, name, options, models } } }
 *     ```
 *
 *  2. As a single-provider entry to merge into an existing opencode.json:
 *
 *     ```ts
 *     import { createDevilRouteProvider } from "@devilroute/opencode-provider";
 *     const provider = createDevilRouteProvider({ baseURL, apiKey });
 *     // provider -> the value to place under provider.devilroute in opencode.json
 *     ```
 *
...
export const DEVILROUTE_PROVIDER_NPM = "@ai-sdk/openai-compatible" as const;
 ...
 *
 * Curated set covering the most commonly deployed DevilRoute models. Synced
 * with the Alph4d0g/opencode-devilroute-auth DEVILROUTE_DEFAULT_MODELS constant
 * (https://github.com/Alph4d0g/opencode-devilroute-auth, MIT) and extended
 ...
 *
 * Mirrors the capability shape used by Alph4d0g/opencode-devilroute-auth
 * (https://github.com/Alph4d0g/opencode-devilroute-auth, MIT).
 */
 ...
 * Matches the context lengths used by DevilRoute's provider registry.
 */
export const DEVILROUTE_DEFAULT_MODEL_CONTEXT_LENGTHS: Record<string, number> = {
 ...
 * model via `DevilRouteProviderOptions.modelCapabilities`.
 */
export const DEVILROUTE_DEFAULT_MODEL_CAPABILITIES: Record<string, ModelCapabilities> = {
...
export interface DevilRouteProviderOptions {
  /** DevilRoute base URL, with or without trailing `/v1`. Required. */
  baseURL: string;
  /** DevilRoute API key. Required. Use `sk_devilroute` for local instances without REQUIRE_API_KEY. */
  apiKey: string;
  /** Override the display name shown in OpenCode. Default: `"DevilRoute"`. */
  displayName?: string;
   ...
   * `DEVILROUTE_DEFAULT_MODEL_CAPABILITIES` for ids in the default catalog;
   * for custom ids the override is used verbatim.
   */
  modelCapabilities?: Record<string, ModelCapabilities>;
  /**
   * Optional per-model context-length overrides (tokens). Takes precedence
   * over the static `DEVILROUTE_DEFAULT_MODEL_CONTEXT_LENGTHS` map but is
   ...
   * Emitted as `"devilroute/<id>"`. When omitted the key is not written.
   */
  model?: string;
  /**
...
/** Per-model entry written under `provider.devilroute.models[id]`. */
...
export interface OpenCodeProviderEntry {
  /** Identifier of the OpenCode runtime package that will speak to DevilRoute. */
  npm: typeof DEVILROUTE_PROVIDER_NPM;
...
export interface OpenCodeConfigDocument {
  ...
  /** Primary model for OpenCode, e.g. `"devilroute/claude-sonnet-4-5-thinking"`. */
  model?: string;
  /** Secondary / cheap model for OpenCode, e.g. `"devilroute/gemini-3-flash"`. */
  small_model?: string;
  provider: {
    [DEVILROUTE_PROVIDER_KEY]: OpenCodeProviderEntry;
  };
}

function requireNonEmpty(value: unknown, field: string): string {
  if (typeof value !== "string") {
    throw new TypeError(`@devilroute/opencode-provider: ${field} must be a string`);
  }
  const trimmed = value.trim();
  if (!trimmed) {
    throw new Error(`@devilroute/opencode-provider: ${field} is required and cannot be empty`);
  }
  return trimmed;
}
...
export function normalizeBaseURL(rawBaseURL: string): string {
  ...
  try {
    ...
    throw new Error(
      `@devilroute/opencode-provider: baseURL is not a valid URL: ${JSON.stringify(rawBaseURL)}`
    );
  }
  let base = trimmed;
  let end = base.length;
...
/**
 * Build the `provider.devilroute` entry for an OpenCode config document.
 * The returned object is JSON-serialisable and safe to embed verbatim.
 */
export function createDevilRouteProvider(options: DevilRouteProviderOptions): OpenCodeProviderEntry {
  ...
  for (const raw of modelList) {
    ...
    seen.add(id);
    const defaults = DEVILROUTE_DEFAULT_MODEL_CAPABILITIES[id] ?? {};
    ...
    const rawContextLength =
      ...
      DEVILROUTE_DEFAULT_MODEL_CONTEXT_LENGTHS[id];
    const contextLength =
  ...
  return {
    npm: DEVILROUTE_PROVIDER_NPM,
    name: options.displayName?.trim() || "DevilRoute",
...
/**
 * Build a full OpenCode config document (with `$schema` + `provider.devilroute`).
 * Useful when scaffolding a fresh `opencode.json`.
 *
 * When `options.model` / `options.smallModel` are supplied they are emitted as
 * top-level `model` / `small_model` keys prefixed with `"devilroute/"` so
 * OpenCode resolves them through the configured provider.
 */
export function buildDevilRouteOpenCodeConfig(
  options: DevilRouteProviderOptions
): OpenCodeConfigDocument {
  const doc: OpenCodeConfigDocument = {
    $schema: OPENCODE_CONFIG_SCHEMA,
    provider: {
      [DEVILROUTE_PROVIDER_KEY]: createDevilRouteProvider(options),
    },
  };

  if (options.model !== undefined) {
    const id = options.model.trim();
    if (id) doc.model = `${DEVILROUTE_PROVIDER_KEY}/${id}`;
  }

  if (options.smallModel !== undefined) {
    const id = options.smallModel.trim();
    if (id) doc.small_model = `${DEVILROUTE_PROVIDER_KEY}/${id}`;
  }
...
/**
 * Merge the DevilRoute provider entry (and optional `model` / `small_model`
 ...
 * present are not removed. If `existing.provider.devilroute` already exists it
...
export function mergeIntoExistingConfig(
  ...
  const partial = buildDevilRouteOpenCodeConfig(options);
  ...
  merged.provider = {
    ...existingProvider,
    [DEVILROUTE_PROVIDER_KEY]: partial.provider[DEVILROUTE_PROVIDER_KEY],
  };
 ...
 */
export const DEVILROUTE_MCP_DEFAULT_SCOPES = [
  "read:health",
  "read:combos",
  "read:quota",
  "read:usage",
...
export type DevilRouteMCPScope = (typeof DEVILROUTE_MCP_DEFAULT_SCOPES)[number] | string;

export interface DevilRouteMCPOptions {
  /** Absolute path to the MCP server entry point (TypeScript or compiled JS). */
  serverPath: string;
  /** DevilRoute API key forwarded to the MCP server as `DEVILROUTE_API_KEY`. */
  apiKey: string;
  /**
   * Management API key used for management-scoped operations.
   * When supplied it is forwarded as `DEVILROUTE_MANAGEMENT_API_KEY`.
   */
  managementApiKey?: string;
  /**
   * Comma-separated scope list passed as `DEVILROUTE_MCP_SCOPES`.
   * When omitted `DEVILROUTE_MCP_ENFORCE_SCOPES` is not set and all scopes are
   * available (development default). Pass an explicit list to restrict access.
   */
  scopes?: DevilRouteMCPScope[];
  /**
...
/**
 * Build the `mcp.servers.devilroute` entry for an OpenCode config document.
 *
 * @example
 * ```ts
 * const mcpEntry = createDevilRouteMCPEntry({
 *   serverPath: "/home/user/.local/share/devilroute/open-sse/mcp-server/server.ts",
 *   apiKey: "sk_devilroute",
 *   managementApiKey: "sk_manage_...",
 *   scopes: ["read:health", "read:combos", "execute:completions"],
 * });
 * // Place at config.mcp.servers.devilroute
 * ```
 */
export function createDevilRouteMCPEntry(options: DevilRouteMCPOptions): OpenCodeMCPServerEntry {
  ...
  const env: Record<string, string> = {
    DEVILROUTE_API_KEY: apiKey,
  };

  if (options.managementApiKey !== undefined) {
    const mgmtKey = options.managementApiKey.trim();
    if (mgmtKey) env.DEVILROUTE_MANAGEMENT_API_KEY = mgmtKey;
  }

  if (options.scopes !== undefined && options.scopes.length > 0) {
    env.DEVILROUTE_MCP_ENFORCE_SCOPES = "true";
    env.DEVILROUTE_MCP_SCOPES = options.scopes.join(",");
  }
...
async function fetchJSON<T>(url: string, apiKey: string, timeoutMs: number): Promise<T> {
  ...
  try {
    ...
    throw new Error(`@devilroute/opencode-provider: request to ${url} failed: ${message}`);
  } finally {
    clearTimeout(timer);
  }
}
 ...
 * The shape mirrors the subset of fields that DevilRoute's `/v1/models`
 * endpoint reliably provides across versions, normalised from both
 * camelCase and snake_case variants used by different DevilRoute releases.
 *
 * Attribution: field-variant normalisation logic adapted from
 * https://github.com/Alph4d0g/opencode-devilroute-auth (MIT).
 */
export interface DevilRouteLiveModel {
  id: string;
  name: string;
...
/**
 * Fetch the live model catalog from a running DevilRoute instance.
 *
 ...
 * `display_name`) field variants across DevilRoute versions.
 *
 * Useful for dynamically populating the `models` option of
 * `createDevilRouteProvider` / `buildDevilRouteOpenCodeConfig` instead of
 * relying on `DEVILROUTE_DEFAULT_OPENCODE_MODELS`.
 *
 * @param baseURL   - DevilRoute base URL (with or without `/v1`).
 * @param apiKey    - DevilRoute API key.
 * @param timeoutMs - Request timeout in milliseconds (default 5000).
 *
 * @example
 * ```ts
 * const models = await fetchLiveModels("http://localhost:20128", "sk_devilroute");
 * const config = buildDevilRouteOpenCodeConfig({
 *   baseURL: "http://localhost:20128",
 *   apiKey: "sk_devilroute",
 *   models,                    // DevilRouteLiveModel[] — contextLength auto-extracted
...
export async function fetchLiveModels(
  ...
  const models: DevilRouteLiveModel[] = [];
  for (const raw of rawList) {
    ...
    // Extract context_length from DevilRoute's /v1/models response.
    // DevilRoute returns context_length in snake_case for both synced
 ...
 */
export type DevilRouteCompressionOverride =
  | ""
  | "off"
  | "lite"
  | "standard"
...
export interface DevilRouteCombo {
  id: string;
  name: string;
  strategy: string;
  active: boolean;
  compressionOverride: DevilRouteCompressionOverride;
}

/**
 * Fetch the active routing combo list from a running DevilRoute instance.
 *
...
export async function listCombos(
  ...
  for (const raw of rawList) {
    ...
    const compressionOverride = VALID_COMPRESSION_OVERRIDES.has(rawOverride)
      ? (rawOverride as DevilRouteCompressionOverride)
      : "";
...
/**
 * Options for `createDevilRouteComboConfig`.
 * Mirrors the subset of combo fields exposed by the DevilRoute `/api/combos`
 * PATCH / POST payload that are safe to set programmatically.
 */
export interface DevilRouteComboConfigOptions {
...
/**
 * Build a typed combo payload suitable for DevilRoute's management API.
 *
 ...
 * ```
 */
export function createDevilRouteComboConfig(
  options: DevilRouteComboConfigOptions
): Record<string, unknown> {
 ...
 */
export interface DevilRouteRoleOverrides {
...
/** Per-role binding used by `createDevilRouteAgentBlock`. */
export interface DevilRouteAgentRole extends DevilRouteRoleOverrides {
  /** DevilRoute model id, e.g. `"claude-sonnet-4-5-thinking"`. */
  modelId: string;
...
/** Options for `createDevilRouteAgentBlock`. */
export interface DevilRouteAgentBlockOptions {
  /** Per-role bindings. Keys become entries under OpenCode's `agent` block. */
  roles: Record<string, DevilRouteAgentRole>;
}

/** Single entry inside the emitted OpenCode `agent` block. */
export interface OpenCodeAgentEntry extends DevilRouteRoleOverrides {
  /** Always emitted as `"devilroute/<modelId>"`. */
  model: string;
...
function buildAgentEntry(role: DevilRouteAgentRole): OpenCodeAgentEntry | undefined {
  if (!role || typeof role.modelId !== "string") return undefined;
  const modelId = role.modelId.trim();
  if (!modelId) return undefined;
  const entry: OpenCodeAgentEntry = { model: `${DEVILROUTE_PROVIDER_KEY}/${modelId}` };
 ...
 * specific DevilRoute model. Useful for `.opencode/agent/*.md` defaults and
 ...
 * @example
 * ```ts
 * const agentBlock = createDevilRouteAgentBlock({
 *   roles: {
 ...
 *   },
 * });
 * // -> { build: { model: "devilroute/claude-sonnet-4-5-thinking", temperature: 0.2 }, ... }
 * ```
 */
export function createDevilRouteAgentBlock(
  options: DevilRouteAgentBlockOptions
...
/**
 * Per-mode binding used by `createDevilRouteModesBlock`.
 *
 * @deprecated OpenCode's top-level `mode` block is deprecated in favour of
 * `agent`. Prefer `DevilRouteAgentRole` + `createDevilRouteAgentBlock`. This
 * type and the corresponding helper are kept for back-compat with configs
 * still using `mode:`.
 */
export interface DevilRouteMode extends DevilRouteAgentRole {}

/**
 * Options for `createDevilRouteModesBlock`.
 *
 * @deprecated See `DevilRouteMode`.
 */
export interface DevilRouteModesBlockOptions {
  /** Per-mode bindings. Keys become entries under OpenCode's deprecated top-level `mode` block. */
  modes: Record<string, DevilRouteMode>;
}
 ...
 * a specific DevilRoute model. Emits the same shape as the `agent` block since
 * OpenCode's schema treats them identically (both reference `AgentConfig`).
 *
 * Modes with empty / missing `modelId` are skipped.
 *
 * @deprecated OpenCode's top-level `mode` block is deprecated in favour of
 * `agent`. Prefer `createDevilRouteAgentBlock`. This helper is kept for
 * back-compat with configs still using `mode:`.
 *
 * @example
 * ```ts
 * const modesBlock = createDevilRouteModesBlock({
 *   modes: {
 ...
 * ```
 */
export function createDevilRouteModesBlock(
  options: DevilRouteModesBlockOptions
...
export default createDevilRouteProvider;
