 ...
 *
 * Covers the `createDevilRouteAuthHook(opts)` factory and its loader behaviour
...
import { createDevilRouteAuthHook } from "../src/index.js";

test("createDevilRouteAuthHook: default providerId is 'devilroute'", () => {
  const hook = createDevilRouteAuthHook();
  assert.equal(hook.provider, "opencode-devilroute");
});

test("createDevilRouteAuthHook: custom providerId binds to hook.provider (multi-instance)", () => {
  const hook = createDevilRouteAuthHook({ providerId: "devilroute-preprod" });
  assert.equal(hook.provider, "opencode-devilroute-preprod");
});

test("createDevilRouteAuthHook: methods[0] is type 'api' with label including displayName", () => {
  const hook = createDevilRouteAuthHook();
  ...
  assert.equal(m.label, "DevilRoute API Key");

  const custom = createDevilRouteAuthHook({ providerId: "devilroute-preprod" });
  assert.equal(custom.methods[0].label, "DevilRoute (opencode-devilroute-preprod) API Key");
});

test("createDevilRouteAuthHook: prompts[0] uses key='apiKey' per @opencode-ai/plugin contract", () => {
  ...
  assert.ok(
    typeof (p as { message: string }).message === "string" &&
      (p as { message: string }).message.includes("devilroute"),
...
test("loader: valid api auth → {apiKey, baseURL, fetch} when baseURL option set (T-04)", async () => {
  const hook = createDevilRouteAuthHook({ baseURL: "https://or.example.com/v1" });
  const result = await hook.loader!(
