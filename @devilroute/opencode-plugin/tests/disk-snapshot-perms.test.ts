...
import {
  ...
  diskSnapshotPath,
  type DevilRouteFetchCacheEntry,
} from "../src/index.js";

function makeEntry(): Omit<DevilRouteFetchCacheEntry, "expiresAt"> {
  return {
    rawModels: [],
    rawCombos: [],
...
test("defaultDiskSnapshotWriter writes an owner-only (no group/other) snapshot", async (t) => {
  ...
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "devilroute-disk-perms-"));
