...
import {
  ...
  invalidateDevilRouteFetchCache,
  forceSyncDevilRouteModels,
...
test("parseDevilRoutePluginOptions accepts autoSyncIntervalMs including 0", () => {
  assert.equal(parseDevilRoutePluginOptions({ autoSyncIntervalMs: 0 }).autoSyncIntervalMs, 0);
  assert.equal(parseDevilRoutePluginOptions({ autoSyncIntervalMs: 120_000 }).autoSyncIntervalMs, 120_000);
});

test("resolveDevilRoutePluginOptions defaults autoSyncIntervalMs to 300000", () => {
  const r = resolveDevilRoutePluginOptions({});
  assert.equal(r.autoSyncIntervalMs, DEFAULT_AUTO_SYNC_INTERVAL_MS);
});

test("resolveDevilRoutePluginOptions clamps low positive autoSyncIntervalMs", () => {
  const r = resolveDevilRoutePluginOptions({ autoSyncIntervalMs: 5000 });
  assert.equal(r.autoSyncIntervalMs, MIN_AUTO_SYNC_INTERVAL_MS);
});

test("invalidateDevilRouteFetchCache clears by baseURL prefix", () => {
  const cache: DevilRouteFetchCache = new Map();
  ...
  const removed = invalidateDevilRouteFetchCache(cache, "https://a.example/v1");
  assert.equal(removed, 1);
  assert.equal(cache.size, 1);
  assert.equal(cache.has("https://b.example/v1::def"), true);
});

test("forceSyncDevilRouteModels: fetches, populates cache, returns count", async () => {
  const cache: DevilRouteFetchCache = new Map();
  const resolved = resolveDevilRoutePluginOptions({
    providerId: "devilroute",
    baseURL: "https://devilroute.example/v1",
  ...
  const result = await forceSyncDevilRouteModels({
    resolved,
    cache,
    readAuthJson: async () => ({
      devilroute: { type: "api", key: "test-key" },
    }),
    fetcher: async () => [
  ...
  assert.equal(result.provider, "devilroute");
  assert.equal(cache.size, 1);
...
test("forceSyncDevilRouteModels: missing auth returns error", async () => {
  const cache: DevilRouteFetchCache = new Map();
  const resolved = resolveDevilRoutePluginOptions({
    providerId: "devilroute",
    baseURL: "https://devilroute.example/v1",
