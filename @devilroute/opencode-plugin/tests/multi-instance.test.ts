 ...
 *
 * Validates that two `DevilRoutePlugin(input, opts)` invocations with
 ...
 *
 *   "plugin": [
 *     ["@devilroute/opencode-plugin", {"providerId": "devilroute-prod",    "baseURL": "https://or.example/v1"}],
 *     ["@devilroute/opencode-plugin", {"providerId": "devilroute-preprod", "baseURL": "https://or-preprod.example/v1"}]
 *   ]
 *
 * Assertions:
...
import { DevilRoutePlugin } from "../src/index.js";

const fakeInput = {} as Parameters<typeof DevilRoutePlugin>[0];

test("multi-instance: two plugin invocations bind to their own providerId", async () => {
  const a = await DevilRoutePlugin(fakeInput, {
    providerId: "devilroute-prod",
    baseURL: "https://a.example/v1",
  });
  const b = await DevilRoutePlugin(fakeInput, {
    providerId: "devilroute-preprod",
    baseURL: "https://b.example/v1",
  });

  assert.equal(a.auth?.provider, "opencode-devilroute-prod");
  assert.equal(b.auth?.provider, "opencode-devilroute-preprod");
});
...
test("multi-instance: identical opts twice still yield independent objects", async () => {
  ...
  const first = await DevilRoutePlugin(fakeInput, { ...opts });
  const second = await DevilRoutePlugin(fakeInput, { ...opts });
...
test("multi-instance: invalid opts on one instance does not poison the other", async () => {
  ...
  await assert.rejects(
    () => DevilRoutePlugin(fakeInput, { providerId: "bad id!" } as never),
    /providerId/
  );
  const ok = await DevilRoutePlugin(fakeInput, {
