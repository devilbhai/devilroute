...
const RAW_MODELS: DevilRouteRawModelEntry[] = [
  {
    id: "openai/gpt-test",
...
test("options: managementReadToken is accepted and preserved", () => {
  const parsed = parseDevilRoutePluginOptions({ managementReadToken: MANAGEMENT_READ_TOKEN });
...
test("provider hook: management GET fetchers use managementReadToken while /v1 uses apiKey", async () => {
  ...
  const enrichmentFetcher: DevilRouteEnrichmentFetcher = async (_baseURL, token) => {
    calls.push(["pricing", token]);
    return new Map();
  };
  const compressionMetaFetcher: DevilRouteCompressionMetaFetcher = async (_baseURL, token) => {
    calls.push(["context", token]);
    return [];
  };
  const providersFetcher: DevilRouteProvidersFetcher = async (_baseURL, token) => {
...
test("config hook: managementReadToken stays out of provider inference and MCP config", async () => {
  ...
  const hook = createDevilRouteConfigHook(
    ...
    {
      readAuthJson: async () => ({
        "opencode-devilroute": { type: "api" as const, key: API_KEY },
      }),
      fetcher: async (_baseURL, token) => {
        calls.push(["models", token]);
        return RAW_MODELS;
  ...
  assert.equal(input.provider?.["opencode-devilroute"]?.options?.apiKey, API_KEY);
  assert.equal(
    input.mcp?.["opencode-devilroute"]?.headers?.Authorization,
    `Bearer ${API_KEY}`,
...
test("disk cache: snapshot written under management token A is rejected under token B", async () => {
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "devilroute-token-snapshot-"));
  ...
  try {
    ...
    const tokenAHook = createDevilRouteConfigHook(
    ...
    const tokenBHook = createDevilRouteConfigHook(
    ...
    assert.deepEqual(
      input.provider?.["opencode-devilroute"]?.models,
      {},
