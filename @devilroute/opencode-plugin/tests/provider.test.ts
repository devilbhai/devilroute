 ...
 *
 * Covers `createDevilRouteProviderHook(opts, deps)`:
...
const FIXTURE: DevilRouteRawModelEntry[] = [
  {
    id: "claude-primary",
    object: "model",
    owned_by: "combo",
...
function stubFetcher(payload: DevilRouteRawModelEntry[]): DevilRouteModelsFetcher & {
  callCount: () => number;
...
test("createDevilRouteProviderHook: default providerId is 'devilroute'", () => {
  const hook = createDevilRouteProviderHook(undefined, { combosFetcher: async () => [] });
  assert.equal(hook.id, "opencode-devilroute");
});

test("createDevilRouteProviderHook: custom providerId binds to hook.id (multi-instance)", () => {
  const a = createDevilRouteProviderHook(
    { providerId: "devilroute-preprod" },
    { combosFetcher: async () => [] }
  );
  const b = createDevilRouteProviderHook(
    { providerId: "devilroute-local" },
    { combosFetcher: async () => [] }
  );
  assert.equal(a.id, "opencode-devilroute-preprod");
  assert.equal(b.id, "opencode-devilroute-local");
});
...
test("models: extracts apiKey from ctx.auth (type=api) and calls fetcher with it", async () => {
  ...
  // ("devilroute"), not the OC-gate-prefixed hook.id ("opencode-devilroute") —
  // that prefix must never leak into anything DevilRoute's server parses.
...
test("models: returns {} when no baseURL resolvable (no opts.baseURL and no auth.baseURL)", async () => {
  const fetcher = stubFetcher(FIXTURE);
  const hook = createDevilRouteProviderHook({}, { fetcher, combosFetcher: async () => [] });
...
test("models: maps a sample /v1/models entry to ModelV2 (sanity)", async () => {
  ...
  // devilrouteProviderId ("devilroute") — the OC-gate prefix ("opencode-")
  // must stay OC-internal (hook.id / AuthHook.provider) and never leak into
  // anything DevilRoute's own server parses for credential lookup.
  const claude = out["devilroute/claude-primary"];
  ...
  assert.equal(claude.id, "devilroute/claude-primary");
  assert.equal(claude.name, "claude-primary");
  assert.equal(claude.providerID, "devilroute");
  ...
  // cost is zeroed (DevilRoute /v1/models has no pricing)
