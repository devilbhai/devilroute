 ...
 *
 * Covers `createDevilRouteFetchInterceptor` (URL-prefix gating, header merge,
...
import { createDevilRouteAuthHook, createDevilRouteFetchInterceptor } from "../src/index.js";
...
test("createDevilRouteFetchInterceptor: targets baseURL → Authorization header injected", async () => {
  const { calls, restore } = installFetchRecorder();
  try {
    const f = createDevilRouteFetchInterceptor({ apiKey: KEY, baseURL: BASE });
...
test("createDevilRouteFetchInterceptor: path-prefixed baseURL scopes auth to its normalized inference paths", async () => {
  const { calls, restore } = installFetchRecorder();
  try {
    const prefixedBase = "https://or.example.com/tenant-a/v1";
    const f = createDevilRouteFetchInterceptor({
      apiKey: KEY,
    ...
    const suffixingInterceptor = createDevilRouteFetchInterceptor({
      apiKey: KEY,
...
test("createDevilRouteFetchInterceptor: targets baseURL → Authorization OVERRIDES caller-supplied Bearer", async () => {
  const { calls, restore } = installFetchRecorder();
  try {
    const f = createDevilRouteFetchInterceptor({ apiKey: KEY, baseURL: BASE });
    await f(`${BASE}/chat/completions`, {
...
test("createDevilRouteFetchInterceptor: targets baseURL + body → Content-Type defaults to application/json", async () => {
  const { calls, restore } = installFetchRecorder();
  try {
    const f = createDevilRouteFetchInterceptor({ apiKey: KEY, baseURL: BASE });
    await f(`${BASE}/chat/completions`, {
...
test("createDevilRouteFetchInterceptor: caller-set Content-Type is NOT overwritten", async () => {
  const { calls, restore } = installFetchRecorder();
  try {
    const f = createDevilRouteFetchInterceptor({ apiKey: KEY, baseURL: BASE });
...
test("createDevilRouteFetchInterceptor: non-baseURL host → passthrough, no Authorization injected", async () => {
  const { calls, restore } = installFetchRecorder();
  try {
    const f = createDevilRouteFetchInterceptor({ apiKey: KEY, baseURL: BASE });
...
test("createDevilRouteFetchInterceptor: refuses suffix-spoof — `${base}-attacker.evil` does NOT match baseURL", async () => {
  const { calls, restore } = installFetchRecorder();
  try {
    const f = createDevilRouteFetchInterceptor({ apiKey: KEY, baseURL: BASE });
...
test("createDevilRouteFetchInterceptor: URL object input is handled", async () => {
  const { calls, restore } = installFetchRecorder();
  try {
    const f = createDevilRouteFetchInterceptor({ apiKey: KEY, baseURL: BASE });
...
test("createDevilRouteFetchInterceptor: Request input is handled (reads .url)", async () => {
  const { calls, restore } = installFetchRecorder();
  try {
    const f = createDevilRouteFetchInterceptor({ apiKey: KEY, baseURL: BASE });
...
test("createDevilRouteFetchInterceptor: trailing slash in baseURL is normalized", async () => {
  const { calls, restore } = installFetchRecorder();
  try {
    const f = createDevilRouteFetchInterceptor({
      apiKey: KEY,
...
test("createDevilRouteFetchInterceptor: GET without body does NOT set Content-Type", async () => {
  const { calls, restore } = installFetchRecorder();
  try {
    const f = createDevilRouteFetchInterceptor({ apiKey: KEY, baseURL: BASE });
...
test("loader: returns fetch fn when apiKey + baseURL both present (via opts)", async () => {
  const hook = createDevilRouteAuthHook({ baseURL: BASE });
...
test("loader: returns fetch fn when baseURL is stashed on the auth credential", async () => {
  ...
  const hook = createDevilRouteAuthHook();
...
test("loader: omits fetch fn when baseURL missing (apiKey-only return)", async () => {
  const hook = createDevilRouteAuthHook(); // no baseURL opt
  const result = await hook.loader!(async () => ({ type: "api", key: KEY }) as never, {} as never);
