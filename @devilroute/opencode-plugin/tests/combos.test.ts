 ...
 *
 * Covers:
 *   - `defaultDevilRouteCombosFetcher(baseURL, apiKey, timeoutMs?)`
 ...
 *   - `createDevilRouteProviderHook(opts, deps)` extension
...
import {
  ...
  defaultDevilRouteCombosFetcher,
  mapComboToModelV2,
...
const MODEL_PRIMARY: DevilRouteRawModelEntry = {
  id: "claude-primary",
  capabilities: {
    tool_calling: true,
    reasoning: true,
...
const MODEL_SECONDARY: DevilRouteRawModelEntry = {
  id: "claude-secondary",
  capabilities: {
    tool_calling: true,
    reasoning: false,
...
const MODEL_NO_TOOLS: DevilRouteRawModelEntry = {
  id: "gemini-3-flash",
...
): DevilRouteModelsFetcher & { callCount: () => number } {
  let n = 0;
...
function failingCombosFetcher(
  err = new Error("boom")
): DevilRouteCombosFetcher & { callCount: () => number } {
  let n = 0;
...
// ────────────────────────────────────────────────────────────────────────────
// defaultDevilRouteCombosFetcher — envelope tolerance + error surfacing
// ────────────────────────────────────────────────────────────────────────────

test("defaultDevilRouteCombosFetcher: parses {combos:[…]} envelope", async () => {
  ...
  try {
    const combos = await defaultDevilRouteCombosFetcher("https://or.example.com", "sk-test");
...
test("defaultDevilRouteCombosFetcher: parses bare array envelope", async () => {
  const originalFetch = globalThis.fetch;
  ...
  try {
    const combos = await defaultDevilRouteCombosFetcher("https://or.example.com/v1", "sk-test");
...
test("defaultDevilRouteCombosFetcher: strips trailing /v1 before /api/combos", async () => {
  const originalFetch = globalThis.fetch;
  let observedUrl = "";
  globalThis.fetch = (async (input: unknown) => {
  ...
  try {
    await defaultDevilRouteCombosFetcher("https://or.example.com/v1/", "sk-test");
...
test("defaultDevilRouteCombosFetcher: throws on non-2xx with status code in message", async () => {
  const originalFetch = globalThis.fetch;
  globalThis.fetch = (async () => {
  ...
  try {
    await assert.rejects(
      async () => {
        await defaultDevilRouteCombosFetcher("https://or.example.com", "sk-bad");
      },
      (err: unknown) => {
...
test("defaultDevilRouteCombosFetcher: throws when apiKey missing", async () => {
  await assert.rejects(
    async () => defaultDevilRouteCombosFetcher("https://or.example.com", ""),
    /apiKey required/
  );
});

test("defaultDevilRouteCombosFetcher: throws when baseURL missing", async () => {
  await assert.rejects(
    async () => defaultDevilRouteCombosFetcher("", "sk-test"),
    /baseURL required/
  );
});
...
test("mapComboToModelV2: api block matches providerId + baseURL", () => {
  ...
  assert.equal(m.providerID, "devilroute-preprod");
...
test("mapComboToModelV2: explicit member temperature=false drops combo temperature=false", () => {
  const tempFalse: DevilRouteRawModelEntry = {
    id: "no-temp",
...
// ────────────────────────────────────────────────────────────────────────────
// createDevilRouteProviderHook — combos merge + collision + soft-fail + cache
// ────────────────────────────────────────────────────────────────────────────
...
test("models() returns combo entries merged into the map", async () => {
  ...
  assert.ok(out["devilroute/claude-primary"]);
  assert.ok(out["devilroute/claude-secondary"]);
  assert.ok(out["devilroute/gemini-3-flash"]);
  assert.ok(out["devilroute/claude-tier"]);

  const combo = out["devilroute/claude-tier"];
  assert.equal(combo.name, "Claude Tier");
  assert.equal(combo.providerID, "devilroute");
...
test("models(): combo with unknown member ids degrades to all-false LCD posture", async () => {
  ...
  const out = await hook.models!({} as never, { auth: apiAuth("sk-z") as never });
  assert.ok(out["devilroute/phantom-combo"]);
  // With zero resolvable members, LCD = all-false (defensive posture).
  assert.equal(out["devilroute/phantom-combo"].capabilities.toolcall, false);
  assert.equal(out["devilroute/phantom-combo"].capabilities.reasoning, false);
  assert.equal(out["devilroute/phantom-combo"].limit.context, 0);
});
...
test("models(): hidden combos are excluded from the map", async () => {
  ...
  const out = await hook.models!({} as never, { auth: apiAuth("sk-z") as never });
  assert.ok(out["devilroute/visible"]);
  assert.ok(!out["devilroute/hidden"], "hidden combo must be omitted");
});
...
test("models(): combo name exactly matches raw model id → raw deleted, raw deleted, no warn", async () => {
  ...
  // the real DevilRoute payload where /v1/models pre-mirrors combos as
  // no-slash raw entries whose ids match /api/combos friendly names.
  const colliderCombo: DevilRouteRawCombo = {
    id: "uuid-collider",
  ...
  assert.ok(out["devilroute/claude-primary"], "combo surfaces under prefixed key");
  assert.equal(out["devilroute/claude-primary"].name, "claude-primary");
...
test("models(): two combos with same slug → second gets disambiguator suffix", async () => {
  ...
  const combos: DevilRouteRawCombo[] = [
    {
      id: "uuid-a",
      name: "Claude",
  ...
  assert.ok(out["devilroute/claude"], "first combo at prefixed slug");
  assert.ok(out["devilroute/claude-uuid"], "second combo disambiguated by id prefix");
});
...
test("models(): combos cached + reused within TTL (one combo fetch per TTL window)", async () => {
  ...
  assert.ok(second["devilroute/claude-tier"]);
});
...
test("models(): nested combo-ref context is the min of nested + raw members", async () => {
  ...
  const out = await hook.models!({} as never, { auth: apiAuth("sk-z") as never });
  const masterLight = out["devilroute/master-light"];
