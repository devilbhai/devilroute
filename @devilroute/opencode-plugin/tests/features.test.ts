 ...
 *
 * Surfaces tested:
 *   - `parseDevilRoutePluginOptions({ features: ... })`  → schema accept/reject
 *   - `applyEnrichment(model, entry)`                   → mutation semantics
 *   - `formatCompressionPipeline(steps)`                → display formatting
 *   - `createDevilRouteProviderHook` with mocked
 *     `enrichmentFetcher` / `compressionMetaFetcher`    → overlay applied,
 *                                                         off-by-default
 *                                                         gating works.
 *   - `createDevilRouteConfigHook` with `features.mcpAutoEmit:true`
 *                                                       → emits mcp entry
 *                                                       → falls back to
 *                                                         provider apiKey
 *                                                         when mcpToken
...
import {
  ...
  defaultDevilRouteEnrichmentFetcher,
  defaultDevilRouteCompressionMetaFetcher,
  ...
  type DevilRouteCompressionCombo,
...
test("parseDevilRoutePluginOptions: empty features object → preserved", () => {
  const r = parseDevilRoutePluginOptions({ features: {} });
  assert.deepEqual(r, { features: {} });
});

test("parseDevilRoutePluginOptions: all boolean features set → preserved", () => {
  const r = parseDevilRoutePluginOptions({
    features: {
      combos: true,
      enrichment: true,
...
test("parseDevilRoutePluginOptions: mcpToken string → preserved", () => {
  const r = parseDevilRoutePluginOptions({
...
test("parseDevilRoutePluginOptions: unknown features key → throws (strict)", () => {
  assert.throws(
    () =>
      parseDevilRoutePluginOptions({
        features: { combos: true, unknown_field: "oops" },
      }),
    /Invalid @devilroute\/opencode-plugin options/
  );
});

test("parseDevilRoutePluginOptions: non-boolean for boolean feature → throws", () => {
  assert.throws(
    () =>
      parseDevilRoutePluginOptions({
...
test("parseDevilRoutePluginOptions: empty mcpToken → throws (min 1)", () => {
  assert.throws(
    () => parseDevilRoutePluginOptions({ features: { mcpToken: "" } }),
    /Invalid @devilroute\/opencode-plugin options/
  );
});
...
const SAMPLE_RAW: DevilRouteRawModelEntry[] = [
  {
    id: "claude-sonnet-4-6",
    object: "model",
    created: 0,
...
test("provider hook: enrichment fetcher called when features.enrichment !== false", async () => {
  ...
  // #6859: dynamic-hook catalog keys use the unprefixed devilrouteProviderId.
  const m = out["devilroute/claude-sonnet-4-6"];
...
test("provider hook: enrichment fetcher NOT called when features.enrichment:false", async () => {
  ...
  assert.equal(
    out["devilroute/claude-sonnet-4-6"].name,
    "claude-sonnet-4-6",
    "raw id preserved"
  );
});
...
test("provider hook: compression metadata fetcher called when opted in", async () => {
  let called = 0;
  const compressionCombos: DevilRouteCompressionCombo[] = [
    {
      id: "default-caveman",
  ...
  const combo = out["devilroute/claude-primary"];
...
const stubAuthJson = (apiKey: string) => async () => ({
  "opencode-devilroute": { type: "api" as const, key: apiKey },
});
...
test("config hook: MCP auto-emit OFF by default (no mcp entry)", async () => {
  ...
  assert.ok(input.provider?.["opencode-devilroute"], "provider block written");
...
test("config hook: features.mcpAutoEmit:true writes mcp entry with provider apiKey", async () => {
  ...
  const input: { provider?: Record<string, unknown>; mcp?: Record<string, unknown> } = {};
  await hook(input as never);
  const entry = input.mcp?.["opencode-devilroute"] as
...
test("config hook: features.mcpToken overrides provider apiKey in mcp Bearer", async () => {
  ...
  const input: { provider?: Record<string, unknown>; mcp?: Record<string, unknown> } = {};
  await hook(input as never);
  const entry = input.mcp?.["opencode-devilroute"] as { headers: Record<string, string> };
  assert.equal(
...
test("config hook: existing operator mcp.<providerId> wins (no overwrite)", async () => {
  ...
  const input: { provider?: Record<string, unknown>; mcp?: Record<string, unknown> } = {
    mcp: { "opencode-devilroute": { type: "custom-user-entry", url: "https://manual.example/mcp" } },
  };
  await hook(input as never);
  assert.deepEqual(
    input.mcp?.["opencode-devilroute"],
...
test("config hook: features.mcpAutoEmit:true with /v1 in baseURL → strips correctly", async () => {
  const hook = createDevilRouteConfigHook(
    ...
    {
      readAuthJson: async () => ({
        "opencode-devilroute-preprod": { type: "api" as const, key: "sk-preprod" },
      }),
      fetcher: async () => SAMPLE_RAW,
      combosFetcher: async () => [],
      logger: { warn: () => {} },
  ...
  const input: { provider?: Record<string, unknown>; mcp?: Record<string, unknown> } = {};
  await hook(input as never);
  const entry = input.mcp?.["opencode-devilroute-preprod"] as { url: string };
  assert.equal(
    entry.url,
...
test("defaultDevilRouteEnrichmentFetcher: empty baseURL → empty map", async () => {
  const m = await defaultDevilRouteEnrichmentFetcher("", "sk", 100);
  assert.equal(m.size, 0);
});

test("defaultDevilRouteEnrichmentFetcher: empty apiKey → empty map", async () => {
  const m = await defaultDevilRouteEnrichmentFetcher("https://or.example.com", "", 100);
  assert.equal(m.size, 0);
});

test("defaultDevilRouteCompressionMetaFetcher: empty baseURL → empty array", async () => {
  const arr = await defaultDevilRouteCompressionMetaFetcher("", "sk", 100);
...
test("defaultDevilRouteEnrichmentFetcher: merges names from /api/pricing/models and prices from /api/pricing", async () => {
  ...
  try {
    const map = await defaultDevilRouteEnrichmentFetcher(
...
test("defaultDevilRouteEnrichmentFetcher: name-only when pricing endpoint 5xxs", async () => {
  const origFetch = globalThis.fetch;
  globalThis.fetch = (async (input: unknown) => {
    const url = typeof input === "string" ? input : (input as { url: string }).url;
    if (url.endsWith("/api/pricing/models")) {
  ...
  try {
    const map = await defaultDevilRouteEnrichmentFetcher("https://or.example.com", "sk-test", 5_000);
    const opus = map.get("cc/claude-opus-4-7");
...
test("defaultDevilRouteEnrichmentFetcher: pricing-only when catalog endpoint 5xxs", async () => {
  const origFetch = globalThis.fetch;
  globalThis.fetch = (async (input: unknown) => {
    const url = typeof input === "string" ? input : (input as { url: string }).url;
    if (url.endsWith("/api/pricing")) {
...
): DevilRouteEnrichmentMap {
  const map: DevilRouteEnrichmentMap = new Map();
...
test("canonicalDedupSet: drops canonical row when alias twin present", () => {
  ...
  const c2a = buildCanonicalToAliasMap(map);
  const raw: DevilRouteRawModelEntry[] = [
    { id: "cc/claude-opus-4-7" } as DevilRouteRawModelEntry,
    { id: "claude/claude-opus-4-7" } as DevilRouteRawModelEntry,
  ];
...
test("canonicalDedupSet: keeps standalone canonical row (no alias twin) — never hides a model", () => {
  ...
  const c2a = buildCanonicalToAliasMap(map);
  const raw: DevilRouteRawModelEntry[] = [
    { id: "claude/claude-opus-99" } as DevilRouteRawModelEntry, // canonical only — no `cc/claude-opus-99`
  ];
  const drop = canonicalDedupSet(raw, c2a);
...
test("canonicalDedupSet: multi-provider — drops all canonical twins where alias exists", () => {
  ...
  const raw: DevilRouteRawModelEntry[] = [
    { id: "cc/claude-opus-4-7" } as DevilRouteRawModelEntry,
    { id: "claude/claude-opus-4-7" } as DevilRouteRawModelEntry,
    { id: "cx/gpt-5.5" } as DevilRouteRawModelEntry,
    { id: "codex/gpt-5.5" } as DevilRouteRawModelEntry,
    { id: "pol/openai-large" } as DevilRouteRawModelEntry,
    { id: "pollinations/openai-large" } as DevilRouteRawModelEntry,
  ];
  const drop = canonicalDedupSet(raw, c2a);
  assert.equal(drop.has("claude/claude-opus-4-7"), true);
