 ...
 *
 * Covers `createDevilRouteConfigHook(opts, deps)`:
 *   - happy path: valid auth.json → mutates input.provider[id] with the
 *     stripped per-model shape (mirroring `@devilroute/opencode-provider`).
...
import {
  ...
  createDevilRouteConfigHook,
  createDevilRouteProviderHook,
  DevilRoutePlugin,
  resolveDevilRoutePluginOptions,
  type DevilRouteCombosFetcher,
  type DevilRouteEnrichmentEntry,
  type DevilRouteEnrichmentFetcher,
  type DevilRouteEnrichmentMap,
  type DevilRouteFetchCache,
  type DevilRouteModelsFetcher,
  type DevilRouteProviderConnection,
  type DevilRouteProvidersFetcher,
  type DevilRouteRawCombo,
  type DevilRouteRawModelEntry,
  type DevilRouteReadAuthJson,
  type DevilRouteStaticProviderEntry,
} from "../src/index.js";
...
const MODEL_CLAUDE: DevilRouteRawModelEntry = {
...
const MODEL_GEMINI: DevilRouteRawModelEntry = {
  id: "gemini-3-flash",
...
const COMBO_CLAUDE_TIER: DevilRouteRawCombo = {
...
): DevilRouteReadAuthJson & { callCount: () => number } {
  let n = 0;
  const f: DevilRouteReadAuthJson = async () => {
    n++;
...
function throwingReadAuthJson(): DevilRouteReadAuthJson & { callCount: () => number } {
  let n = 0;
  const f: DevilRouteReadAuthJson = async () => {
    n++;
...
function stubModelsFetcher(
  payload: DevilRouteRawModelEntry[]
): DevilRouteModelsFetcher & { callCount: () => number; callsBy: () => Array<[string, string]> } {
  let n = 0;
  const calls: Array<[string, string]> = [];
  const f: DevilRouteModelsFetcher = async (baseURL, apiKey) => {
    n++;
...
function stubCombosFetcher(
  payload: DevilRouteRawCombo[]
): DevilRouteCombosFetcher & { callCount: () => number; callsBy: () => Array<[string, string]> } {
  let n = 0;
  const calls: Array<[string, string]> = [];
  const f: DevilRouteCombosFetcher = async (baseURL, apiKey) => {
    n++;
    calls.push([baseURL, apiKey]);
    return payload;
  };
...
function throwingModelsFetcher(): DevilRouteModelsFetcher & { callCount: () => number } {
  let n = 0;
  const f: DevilRouteModelsFetcher = async () => {
    n++;
...
function throwingCombosFetcher(): DevilRouteCombosFetcher & { callCount: () => number } {
  let n = 0;
  const f: DevilRouteCombosFetcher = async () => {
    n++;
...
function stubEnrichmentFetcher(payload: DevilRouteEnrichmentMap): DevilRouteEnrichmentFetcher & {
  callCount: () => number;
  callsBy: () => Array<[string, string]>;
} {
  let n = 0;
  const calls: Array<[string, string]> = [];
  const f: DevilRouteEnrichmentFetcher = async (baseURL, apiKey) => {
    n++;
    calls.push([baseURL, apiKey]);
    return payload;
  };
...
function throwingEnrichmentFetcher(): DevilRouteEnrichmentFetcher & { callCount: () => number } {
  let n = 0;
  const f: DevilRouteEnrichmentFetcher = async () => {
    n++;
...
test("config: with valid auth.json + apiKey + baseURL → mutates input.provider[id] with stripped models block", async () => {
  const readAuthJson = stubReadAuthJson({
    "opencode-devilroute": { type: "api", key: "sk-test-1", baseURL: "https://or.example.com/v1" },
  });
  ...
  const hook = createDevilRouteConfigHook(
  ...
  const provider = (input as { provider: Record<string, DevilRouteStaticProviderEntry> }).provider;
  const entry = provider["opencode-devilroute"];
  assert.ok(entry, "input.provider['opencode-devilroute'] set");
  assert.equal(entry.npm, "@ai-sdk/openai-compatible");
  assert.equal(entry.name, "DevilRoute");
  ...
  const claude = entry.models["opencode-devilroute/claude-sonnet-4-6"];
  ...
  const combo = entry.models["devilroute/claude-tier"];
...
//     prefixed (`opencode-devilroute`). Without the fallback the lookup misses
...
test("config: auth.json under bare key (pre-prefix login) resolves via dual-key fallback", async () => {
  // Stored under bare `devilroute` (the key OC wrote before the auto-prefix fix),
  // but the resolved providerId is now `opencode-devilroute`.
  const readAuthJson = stubReadAuthJson({
    devilroute: { type: "api", key: "sk-bare-1", baseURL: "https://or.example.com/v1" },
  });
  ...
  const hook = createDevilRouteConfigHook(
    { providerId: "devilroute" }, // resolves to opencode-devilroute internally
    { readAuthJson, fetcher, combosFetcher, logger }
  );
  const input = makeInput();
  await hook(input);
...
test("config: prefixed key wins over bare key when both present (dual-key precedence)", async () => {
  const readAuthJson = stubReadAuthJson({
    "opencode-devilroute": { type: "api", key: "sk-prefixed", baseURL: "https://pref.example/v1" },
    devilroute: { type: "api", key: "sk-bare", baseURL: "https://bare.example/v1" },
  });
  const fetcher = stubModelsFetcher([MODEL_CLAUDE]);
  const combosFetcher = stubCombosFetcher([]);
  const logger = captureWarn();
  ...
  const entry = (input as { provider: Record<string, DevilRouteStaticProviderEntry> }).provider[
    "opencode-devilroute"
  ];
  assert.ok(entry);
  assert.equal(
...
test("config: existing input.provider[id] → no overwrite (respect manual override)", async () => {
  const manual = {
    ...
    name: "Manual DevilRoute",
    options: { baseURL: "http://manual/v1", apiKey: "manual-key" },
    models: { "manual-model": { name: "manual-model" } },
  };
  const readAuthJson = stubReadAuthJson({
    "opencode-devilroute": { type: "api", key: "sk-test", baseURL: "https://or.example/v1" },
  });
  const fetcher = stubModelsFetcher([MODEL_CLAUDE]);
  const combosFetcher = stubCombosFetcher([]);
  const logger = captureWarn();
  ...
  const input = makeInput({ "opencode-devilroute": manual });
  await hook(input);

  const provider = (input as { provider: Record<string, unknown> }).provider;
  assert.equal(provider["opencode-devilroute"], manual, "manual override preserved by reference");
...
test("config: fetchers throw → warn + emit stub entry with models: {}", async () => {
  ...
  // test below).
  const hook = createDevilRouteConfigHook(
    { providerId: "devilroute", features: { diskCache: false } },
    { readAuthJson, fetcher, combosFetcher, logger }
  );
  const input = makeInput();
  await hook(input);
...
test("config: combos fetcher throws → emit models-only catalog (no combos in models block)", async () => {
  ...
  assert.deepEqual(ids, [
    "opencode-devilroute/claude-sonnet-4-6",
    "opencode-devilroute/gemini-3-flash",
  ]);
  assert.equal(entry.models["devilroute/claude-tier"], undefined, "no combo entry");
  assert.ok(
    logger.entries.some((e) => String(e[0]).includes("/api/combos fetch failed")),
    "combos-fetch breadcrumb emitted"
  );
...
test("config: baseURL from auth.json takes precedence when opts.baseURL absent", async () => {
  const readAuthJson = stubReadAuthJson({
    "opencode-devilroute": { type: "api", key: "sk-test", baseURL: "https://creds.example/v1" },
  });
  const fetcher = stubModelsFetcher([MODEL_CLAUDE]);
  const combosFetcher = stubCombosFetcher([]);
  const logger = captureWarn();
  ...
  const hook = createDevilRouteConfigHook(
    { providerId: "devilroute" }, // NO opts.baseURL
    { readAuthJson, fetcher, combosFetcher, logger }
  );
  const input = makeInput();
  await hook(input);
...
test("config: opts.baseURL wins over auth.json's stored baseURL", async () => {
  ...
  const hook = createDevilRouteConfigHook(
    { providerId: "devilroute", baseURL: "https://opts.example/v1" },
    { readAuthJson, fetcher, combosFetcher, logger }
  );
  const input = makeInput();
  await hook(input);
...
test("config: no baseURL resolvable (no opts, no auth.json baseURL) → no-op", async () => {
  const readAuthJson = stubReadAuthJson({
    "opencode-devilroute": { type: "api", key: "sk-test" }, // NO baseURL on the credential
  });
  const fetcher = stubModelsFetcher([MODEL_CLAUDE]);
  const combosFetcher = stubCombosFetcher([]);
  const logger = captureWarn();
...
test("config: multi-instance — two plugins with different providerIds publish to their own keys without collision", async () => {
  const readAuthJson = stubReadAuthJson({
    "opencode-devilroute-prod": {
      type: "api",
      key: "sk-prod",
      baseURL: "https://prod.example/v1",
    },
    "opencode-devilroute-preprod": {
      type: "api",
      key: "sk-preprod",
  ...
  const hookA = createDevilRouteConfigHook(
    { providerId: "devilroute-prod" },
    { readAuthJson, fetcher, combosFetcher, logger }
  );
  const hookB = createDevilRouteConfigHook(
    { providerId: "devilroute-preprod" },
    { readAuthJson, fetcher, combosFetcher, logger }
  );
  ...
  const provider = (input as { provider: Record<string, DevilRouteStaticProviderEntry> }).provider;
  assert.ok(provider["opencode-devilroute-prod"], "prod block present");
  assert.ok(provider["opencode-devilroute-preprod"], "preprod block present");
  assert.equal(provider["opencode-devilroute-prod"].options.apiKey, "sk-prod");
  assert.equal(provider["opencode-devilroute-preprod"].options.apiKey, "sk-preprod");
  assert.equal(provider["opencode-devilroute-prod"].options.baseURL, "https://prod.example/v1");
  assert.equal(
    provider["opencode-devilroute-preprod"].options.baseURL,
    "https://preprod.example/v1"
  );
  assert.notEqual(
    provider["opencode-devilroute-prod"],
    provider["opencode-devilroute-preprod"],
...
test("config + provider share cache: second call uses cached fetch result (single fetch per TTL)", async () => {
  const readAuthJson = stubReadAuthJson({
    "opencode-devilroute": { type: "api", key: "sk-shared", baseURL: "https://or.example/v1" },
  });
  const fetcher = stubModelsFetcher([MODEL_CLAUDE]);
  const combosFetcher = stubCombosFetcher([COMBO_CLAUDE_TIER]);
  ...
  const configHook = createDevilRouteConfigHook(
    { providerId: "devilroute", baseURL: "https://or.example/v1", modelCacheTtl: 60_000 },
    { readAuthJson, fetcher, combosFetcher, cache: sharedCache, logger }
  );
  const providerHook = createDevilRouteProviderHook(
    { providerId: "devilroute", baseURL: "https://or.example/v1", modelCacheTtl: 60_000 },
...
test("provider → config order also dedupes (cache populated by provider, consumed by config)", async () => {
  const readAuthJson = stubReadAuthJson({
    "opencode-devilroute": { type: "api", key: "sk-reverse", baseURL: "https://or.example/v1" },
  });
  const fetcher = stubModelsFetcher([MODEL_CLAUDE]);
  const combosFetcher = stubCombosFetcher([]);
  const sharedCache: DevilRouteFetchCache = new Map();
...
test("buildStaticProviderEntry: stripped per-model shape matches sibling @devilroute/opencode-provider", () => {
  const resolved = resolveDevilRoutePluginOptions({
    providerId: "devilroute",
    displayName: "DevilRoute",
  });
  ...
  assert.equal(block.name, "DevilRoute");
  ...
  const claude = block.models["opencode-devilroute/claude-sonnet-4-6"];
...
test("buildStaticProviderEntry: hidden combos are excluded", () => {
  ...
  assert.equal(block.models["devilroute/claude-tier"], undefined);
  assert.ok(block.models["opencode-devilroute/claude-sonnet-4-6"]);
});
...
test("buildStaticProviderEntry: emits release_date when raw carries it; omits when null", () => {
  const resolved = resolveDevilRoutePluginOptions({ providerId: "devilroute" });
  const withDate: DevilRouteRawModelEntry = {
    ...MODEL_CLAUDE,
    id: "claude-with-date",
  ...
  assert.equal(block.models["opencode-devilroute/claude-with-date"].release_date, "2026-02-19");
  assert.equal(block.models["opencode-devilroute/gemini-3-flash"].release_date, undefined);
});

test("buildStaticProviderEntry: combo modalities = intersection of members (LCD)", () => {
  const resolved = resolveDevilRoutePluginOptions({ providerId: "devilroute" });
  const TEXT_ONLY: DevilRouteRawModelEntry = {
    id: "text-only",
  ...
  const combo = block.models["devilroute/mixed-tier"];
...
// ────────────────────────────────────────────────────────────────────────────
// Integration: DevilRoutePlugin factory now exposes config hook
// ────────────────────────────────────────────────────────────────────────────

test("DevilRoutePlugin factory exposes config hook alongside auth + provider", async () => {
  const hooks = await DevilRoutePlugin({} as never, { providerId: "devilroute" });
...
test("config: auth.json entry of wrong type (oauth) → no-op", async () => {
  const readAuthJson = stubReadAuthJson({
    "opencode-devilroute": { type: "oauth", refresh: "r", access: "a", expires: 0 },
  });
  const fetcher = stubModelsFetcher([MODEL_CLAUDE]);
  const combosFetcher = stubCombosFetcher([]);
  const logger = captureWarn();
  ...
  const hook = createDevilRouteConfigHook(
    { providerId: "devilroute", baseURL: "https://or.example/v1" },
    { readAuthJson, fetcher, combosFetcher, logger }
  );
  const input = makeInput();
  await hook(input);
...
test("config: initialises input.provider when undefined", async () => {
  const readAuthJson = stubReadAuthJson({
    "opencode-devilroute": { type: "api", key: "sk", baseURL: "https://or.example/v1" },
  });
  const fetcher = stubModelsFetcher([MODEL_CLAUDE]);
  const combosFetcher = stubCombosFetcher([]);
  const logger = captureWarn();
  ...
  assert.ok(provider!["opencode-devilroute"]);
});
...
test("config: enrichment fetched + name overlaid on raw-model entries", async () => {
  ...
  const enrichmentFetcher = stubEnrichmentFetcher(
    new Map<string, DevilRouteEnrichmentEntry>([
  ...
  assert.ok(entry);
  assert.equal(entry.models["opencode-devilroute/claude-sonnet-4-6"].name, "Claude Sonnet 4.6");
  assert.equal(entry.models["opencode-devilroute/gemini-3-flash"].name, "Gemini 3 Flash");
  // Combo names still come from /api/combos — enrichment overlay does NOT touch combos.
  assert.equal(entry.models["devilroute/claude-tier"].name, "Claude Tier");
...
test("config: features.enrichment=false skips enrichment fetch + keeps raw-id names", async () => {
  ...
  const hook = createDevilRouteConfigHook(
    { providerId: "devilroute", features: { enrichment: false } },
    { readAuthJson, fetcher, combosFetcher, enrichmentFetcher, logger }
  );
  const input = makeInput();
  await hook(input);
  ...
  assert.equal(
    entry.models["opencode-devilroute/claude-sonnet-4-6"].name,
    "claude-sonnet-4-6",
    "raw id retained"
  );
});
...
function stubProvidersFetcher(payload: DevilRouteProviderConnection[]): DevilRouteProvidersFetcher & {
  callCount: () => number;
} {
  let n = 0;
  const f: DevilRouteProvidersFetcher = async () => {
    n++;
    return payload;
  };
  return Object.assign(f, { callCount: () => n });
...
function throwingProvidersFetcher(): DevilRouteProvidersFetcher & { callCount: () => number } {
  let n = 0;
  const f: DevilRouteProvidersFetcher = async () => {
    n++;
    throw new Error("ETIMEDOUT");
...
const MODEL_CC_OPUS: DevilRouteRawModelEntry = {
  id: "cc/claude-opus-4-7",
  capabilities: { tool_calling: true, reasoning: true, temperature: true },
  context_length: 200_000,
};
const MODEL_NV_LLAMA: DevilRouteRawModelEntry = {
...
test("config: usableOnly=true → drops models for non-usable providers, keeps usable + unknown", async () => {
  ...
  const hook = createDevilRouteConfigHook(
    { providerId: "devilroute", baseURL: "https://or.example/v1", features: { usableOnly: true } },
    { readAuthJson, fetcher, combosFetcher, enrichmentFetcher, providersFetcher }
  );
...
test("config: diskCache hydrates stale snapshot when /v1/models throws", async () => {
  ...
  const hook = createDevilRouteConfigHook(
    { providerId: "devilroute", features: { diskCache: true } },
    {
      readAuthJson,
      fetcher,
      combosFetcher,
  ...
  assert.ok(
    entry.models["opencode-devilroute/claude-sonnet-4-6"],
...
test("config: providerTag (default-on) prepends '<provider> - ' to enriched raw-model names", async () => {
  ...
  assert.equal(entry.models["opencode-devilroute/gemini-3-flash"].name, "Gemini - Gemini 3 Flash");
...
test("config: providerTag=false suppresses the suffix", async () => {
  ...
  const hook = createDevilRouteConfigHook(
    { providerId: "devilroute", features: { providerTag: false } },
    { readAuthJson, fetcher, combosFetcher, enrichmentFetcher, logger }
  );
  const input = makeInput();
  await hook(input);
...
test("config: providerTag falls back to UPPER(alias) when providerDisplayName missing", async () => {
  ...
  // a slot DevilRoute hasn't curated a human label for yet. We still
  ...
  assert.equal(entry.models["opencode-devilroute/claude-sonnet-4-6"].name, "CC - Claude Sonnet 4.6");
});
...
test("config: providerTag is idempotent — second hook call doesn't double-suffix", async () => {
  ...
  const hook = createDevilRouteConfigHook(
    { providerId: "devilroute", modelCacheTtl: 60_000 },
    { readAuthJson, fetcher, combosFetcher, enrichmentFetcher, cache: sharedCache, logger }
  );

  const inputA = makeInput();
  await hook(inputA);
  const entryA = (inputA as { provider: Record<string, DevilRouteStaticProviderEntry> }).provider[
    "opencode-devilroute"
  ];
  assert.equal(
    entryA.models["opencode-devilroute/claude-sonnet-4-6"].name,
    "Claude - Claude Sonnet 4.6"
  );
  ...
  await hook(inputB);
  const entryB = (inputB as { provider: Record<string, DevilRouteStaticProviderEntry> }).provider[
    "opencode-devilroute"
  ];
  assert.equal(
    entryB.models["opencode-devilroute/claude-sonnet-4-6"].name,
    "Claude - Claude Sonnet 4.6"
  );
});
...
test("buildStaticProviderEntry: nested combo-ref context is the bottleneck across the graph", () => {
  const resolved = resolveDevilRoutePluginOptions({ providerId: "devilroute" });
  const rawModels: DevilRouteRawModelEntry[] = [
    {
      id: "raw-big",
      context_length: 200_000,
      max_output_tokens: 64_000,
  ...
  const rawCombos: DevilRouteRawCombo[] = [
    {
      id: "tiny-combo",
      name: "TinyCombo",
  ...
  const parent = block.models["devilroute/parent"];
