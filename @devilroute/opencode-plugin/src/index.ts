/**
 * OpenCode plugin for the DevilRoute AI Gateway.
 *
 * Implements the official `@opencode-ai/plugin` Plugin contract (auth +
 * provider + config hooks) to drive a running DevilRoute instance from
 ...
 *                  pass-through (DevilRoute is the source of truth — no
 ...
 *
 *  1. Single-instance (default `providerId: "devilroute"`):
 *
 *     ```json
 *     {
 *       "$schema": "https://opencode.ai/config.json",
 *       "plugin": ["@devilroute/opencode-plugin"]
 *     }
 *     ```
 *
 ...
 *       "$schema": "https://opencode.ai/config.json",
 *       "plugin": [
 *         ["@devilroute/opencode-plugin", { "providerId": "devilroute" }],
 *         ["@devilroute/opencode-plugin", { "providerId": "devilroute-preprod" }]
 *       ]
 *     }
 *     ```
 *
 ...
 *
 * Companion library: `@devilroute/opencode-provider` (build-time config generator)
 ...
 *                     different `providerId` ("devilroute", "devilroute-preprod",
 *                     "devilroute-local"). Maps to `ProviderHook.id` and
 ...
 *  - `baseURL`        Override base URL for this DevilRoute instance. When
 ...
 *                           JSONL file at
 *                           `~/.local/share/opencode/plugins/devilroute-debug-{providerId}.jsonl`.
 ...
 *                           (covers DevilRoute's canonical Anthropic aliases).
 *
 ...
 * import `DevilRoutePluginOptions` as before; only the source of truth
 * shifted from a hand-written interface to `z.infer<typeof optionsSchema>`.
 */
export type DevilRoutePluginOptions = z.infer<typeof optionsSchema>;
 ...
 */
export const DEVILROUTE_FEATURE_DEFAULTS = {
...
/** Union of the boolean feature-flag keys declared in `DEVILROUTE_FEATURE_DEFAULTS`. */
export type DevilRouteFeatureFlag = keyof typeof DEVILROUTE_FEATURE_DEFAULTS;
...
export function resolveEffectiveFeatureFlags(
  features?: DevilRoutePluginOptions["features"]
): Record<DevilRouteFeatureFlag, boolean> {
  const f: Partial<Record<DevilRouteFeatureFlag, boolean>> = features ?? {};
  const out = {} as Record<DevilRouteFeatureFlag, boolean>;
  for (const key of Object.keys(DEVILROUTE_FEATURE_DEFAULTS) as DevilRouteFeatureFlag[]) {
    const val = f[key];
    out[key] = typeof val === "boolean" ? val : DEVILROUTE_FEATURE_DEFAULTS[key];
  }
  return out;
}

export const DEVILROUTE_PROVIDER_KEY = "devilroute" as const;
 ...
 */
export function resolveDevilRoutePluginOptions(opts?: DevilRoutePluginOptions): Required<
  Pick<DevilRoutePluginOptions, "providerId" | "displayName" | "modelCacheTtl" | "autoSyncIntervalMs">
> & {
  /**
   * #6859: the UNPREFIXED provider id ("devilroute", "devilroute-preprod", …).
   ...
   * `provider.<id>` block). `devilrouteProviderId` MUST be used everywhere an
   * identifier reaches or represents something DevilRoute's own server parses
   * (model `id` prefix, `ModelV2.providerID`, combo catalog keys in the
   * dynamic provider hook) — DevilRoute's `parseModel()` has no alias for
   * "opencode-<x>", so a prefixed id there is unrecoverable and credential
   * lookup fails with "No credentials for opencode-<x>".
   */
  devilrouteProviderId: string;
} & Pick<DevilRoutePluginOptions, "baseURL" | "managementReadToken" | "features"> {
  const rawProviderId = opts?.providerId ?? DEVILROUTE_PROVIDER_KEY;
  const devilrouteProviderId = trimLeadingOpencodePrefix(rawProviderId);
  // OC 1.17.8+ native-adapter gate rejects providerID not in
  // {openai, anthropic, opencode*}. Silently prefix so existing
  // configs (providerId: "devilroute") keep working.
  const providerId = rawProviderId.startsWith("opencode-")
    ? rawProviderId
    : `opencode-${rawProviderId}`;
  const displayName =
    opts?.displayName ??
    (providerId === `opencode-${DEVILROUTE_PROVIDER_KEY}`
      ? "DevilRoute"
      : `DevilRoute (${providerId})`);
  const modelCacheTtl =
...
export type ResolvedDevilRoutePluginOptions = ReturnType<typeof resolveDevilRoutePluginOptions>;

/**
 * Strip a leading "opencode-" prefix (added only for the OC native-adapter
 * gate — see `resolveDevilRoutePluginOptions`) so the returned id is safe to
 * embed in anything DevilRoute's own server parses. A user-supplied
 * `providerId: "opencode-devilroute"` (already prefixed) resolves to the same
 * unprefixed "devilroute" as the default, matching `providerId`'s own
 ...
 * direct factory call) into the validated `DevilRoutePluginOptions` shape.
 *
 ...
 *
 * Validation happens at plugin invocation time (inside `DevilRoutePlugin`),
 ...
 */
export function parseDevilRoutePluginOptions(opts: unknown): DevilRoutePluginOptions {
  ...
  if (!result.success) {
    const errs = result.error.issues
      ...
      .join("; ");
    throw new Error(`Invalid @devilroute/opencode-plugin options: ${errs}`);
  }
  return result.data;
}

/**
 * Internal coercion shim. Delegates to `parseDevilRoutePluginOptions` to keep
 ...
 */
function coercePluginOptions(opts?: PluginOptions): DevilRoutePluginOptions {
  return parseDevilRoutePluginOptions(opts);
}
 ...
 * Covers DevilRoute's canonical Anthropic aliases: `cc/`, `claude/`,
 ...
 *     the multi-instance bug in opencode-devilroute-auth@1.2.1 which pinned
 *     `DEVILROUTE_PROVIDER_ID = "devilroute"` at module scope).
 *   - `methods[0]` is the `api` flavor (no OAuth flow; DevilRoute issues bearer
 ...
 */
export function createDevilRouteAuthHook(opts?: DevilRoutePluginOptions): AuthHook {
  const { providerId, displayName, baseURL, features } = resolveDevilRoutePluginOptions(opts);
        ...
        prompts: [
          {
            type: "text",
            key: "apiKey",
            message: `DevilRoute API key (${providerId})`,
          },
        ],
      },
    ],
    loader: async (getAuth, _provider) => {
      ...
      if (
        ...
        if (wantFetchInterceptor) {
          composedFetch = createDevilRouteFetchInterceptor({
            apiKey,
 ...
 */
export function invalidateDevilRouteFetchCache(
  cache: DevilRouteFetchCache,
  baseURL?: string,
): number {
  if (!baseURL) {
    const n = cache.size;
 ...
 */
export async function resolveDevilRouteRuntimeAuth(
  resolved: ResolvedDevilRoutePluginOptions,
  readAuthJson?: DevilRouteReadAuthJson,
  ...
  if (resolved.devilrouteProviderId && !lookupKeys.includes(resolved.devilrouteProviderId)) {
    lookupKeys.push(resolved.devilrouteProviderId);
  }
...
/**
 * Force-refresh DevilRoute catalog: clear memory + disk cache, re-fetch /v1/models
 * (and optional management endpoints), and repopulate the shared cache.
 * OpenCode equivalent of Pi `/omni sync`.
 */
export async function forceSyncDevilRouteModels(args: {
  resolved: ResolvedDevilRoutePluginOptions;
  cache: DevilRouteFetchCache;
  readAuthJson?: DevilRouteReadAuthJson;
  fetcher?: DevilRouteModelsFetcher;
  combosFetcher?: DevilRouteCombosFetcher;
  autoCombosFetcher?: DevilRouteAutoCombosFetcher;
  enrichmentFetcher?: DevilRouteEnrichmentFetcher;
  compressionMetaFetcher?: DevilRouteCompressionMetaFetcher;
  providersFetcher?: DevilRouteProvidersFetcher;
  now?: () => number;
}): Promise<{
  ok: boolean;
  count: number;
  ...
  const fetcher = args.fetcher ?? defaultDevilRouteModelsFetcher;
  const combosFetcher = args.combosFetcher ?? defaultDevilRouteCombosFetcher;
  const autoCombosFetcher = args.autoCombosFetcher ?? defaultDevilRouteAutoCombosFetcher;
  const enrichmentFetcher = args.enrichmentFetcher ?? defaultDevilRouteEnrichmentFetcher;
  const compressionMetaFetcher =
    args.compressionMetaFetcher ?? defaultDevilRouteCompressionMetaFetcher;
  const providersFetcher = args.providersFetcher ?? defaultDevilRouteProvidersFetcher;
  ...
  const auth = await resolveDevilRouteRuntimeAuth(
    resolved,
  ...
  if (!auth) {
    return {
      ...
      count: 0,
      combos: 0,
      provider: resolved.devilrouteProviderId,
      clearedMemory: 0,
      clearedDisk: false,
      error:
        "No DevilRoute credentials/baseURL available. Run `opencode connect devilroute` or set plugin baseURL.",
    };
  }

  const clearedMemory = invalidateDevilRouteFetchCache(cache, auth.baseURL);
  // Clear residual entries from prior baseURL history as well.
  const clearedAll = invalidateDevilRouteFetchCache(cache);
  let clearedDisk = false;
  if (wantDiskCache) {
    clearedDisk = await clearDiskSnapshot(resolved.providerId);
    if (resolved.devilrouteProviderId !== resolved.providerId) {
      clearedDisk = (await clearDiskSnapshot(resolved.devilrouteProviderId)) || clearedDisk;
    }
  }

  try {
    const rawModels = await fetcher(auth.baseURL, auth.apiKey, 10_000);
    let rawCombos: DevilRouteRawCombo[] = [];
    if (wantCombos) {
      try {
        rawCombos = await combosFetcher(auth.baseURL, auth.managementReadToken, 10_000);
      } catch (err) {
        console.warn("[devilroute-plugin] force sync: combos fetch failed", err);
      }
    }
    let rawAutoCombos: DevilRouteRawAutoCombo[] = [];
    if (wantAutoCombos) {
      try {
    ...
    let rawEnrichment: DevilRouteEnrichmentMap = new Map();
    if (wantEnrichment) {
      try {
    ...
    let rawCompressionCombos: DevilRouteCompressionCombo[] = [];
    ...
    let rawConnections: DevilRouteProviderConnection[] = [];
    if (wantUsableOnly) {
      try {
    ...
    console.warn(
      `[devilroute-plugin] force sync ok providerId=${resolved.providerId} ` +
...
export function createDevilRouteSyncModelsTool(args: {
  resolved: ResolvedDevilRoutePluginOptions;
  cache: DevilRouteFetchCache;
  ...
  return tool({
    description:
      "Force-refresh the DevilRoute model catalog (OpenCode equivalent of Pi `/omni sync`). " +
    ...
    async execute(toolArgs) {
      const result = await forceSyncDevilRouteModels({ resolved, cache });
      const reason = toolArgs.reason ? ` reason=${toolArgs.reason}` : "";
      if (!result.ok) {
        return {
          title: "DevilRoute sync failed",
          output:
            `DevilRoute model sync failed for provider=${result.provider}.${reason}\n` +
            `${result.error ?? "unknown error"}`,
          metadata: result,
        };
      }
      return {
        title: `DevilRoute sync: ${result.count} models`,
        output:
          `DevilRoute models synced.` +
 ...
 */
export function startDevilRouteAutoSync(args: {
  resolved: ResolvedDevilRoutePluginOptions;
  cache: DevilRouteFetchCache;
  intervalMs?: number;
}): () => void {
  ...
  const tick = () => {
    ...
    inFlight = (async () => {
      ...
      if (!result.ok) {
        console.warn(
          `[devilroute-plugin] auto-sync failed providerId=${resolved.providerId}: ${result.error}`,
        );
        return;
      }
      ...
      if (result.count !== lastCount) {
        console.warn(
          `[devilroute-plugin] auto-sync catalog size changed ${lastCount} → ${result.count} ` +
      ...
      .catch((err) => {
        console.warn("[devilroute-plugin] auto-sync tick error", err);
      })
      .finally(() => {
        inFlight = null;
      });
  ...
  console.warn(
    `[devilroute-plugin] auto-sync enabled intervalMs=${intervalMs} providerId=${resolved.providerId}`,
  );
...
export const DevilRoutePlugin: Plugin = async (_input, options) => {
  const resolved = resolveDevilRoutePluginOptions(coercePluginOptions(options));
  ...
  // Each `DevilRoutePlugin(...)` invocation gets its OWN cache via closure,
  // so prod + preprod side-by-side instances do NOT collide.
  const sharedCache: DevilRouteFetchCache = new Map();
  ...
  startDevilRouteAutoSync({ resolved, cache: sharedCache });

  const syncTool = createDevilRouteSyncModelsTool({ resolved, cache: sharedCache });
  const bareProviderId = resolved.devilrouteProviderId;
  ...
  const baseConfigHook = createDevilRouteConfigHook(resolved, { cache: sharedCache });
  const configWithSyncCommand = async (input: Config) => {
    ...
    if (!cfg.command["omni-sync"]) {
      cfg.command["omni-sync"] = {
        description: "Force-refresh DevilRoute model catalog (like Pi /omni sync)",
        template:
          `Force-refresh the DevilRoute model catalog now using the devilroute_sync_models tool ` +
    ...
    if (!cfg.command["omni-autosync"]) {
      cfg.command["omni-autosync"] = {
        description: "Show DevilRoute auto-sync / cache status",
        template:
          `Report DevilRoute discovery status for provider ${bareProviderId}: ` +
          `autoSyncIntervalMs=${resolved.autoSyncIntervalMs}, modelCacheTtl=${resolved.modelCacheTtl}. ` +
          `If the user asked to refresh now, call devilroute_sync_models.`,
      };
    }
  };

  return {
    auth: createDevilRouteAuthHook(resolved),
    provider: createDevilRouteProviderHook(resolved, { cache: sharedCache }),
    config: configWithSyncCommand,
    tool: {
      devilroute_sync_models: syncTool,
    },
  };
};
 ...
 * constants (DEVILROUTE_PROVIDER_KEY, DEFAULT_MODEL_CACHE_TTL_MS) + types +
 ...
 */
const DevilRouteV1Plugin = {
  id: "@devilroute/opencode-plugin",
  server: DevilRoutePlugin,
};

export default DevilRouteV1Plugin;
...
/**
 * Raw shape of a `/v1/models` entry from DevilRoute. Captured verbatim from
 ...
 */
export interface DevilRouteRawModelEntry {
  id: string;
  object?: string;
  owned_by?: string;
  root?: string | null;
 ...
 * DevilRoute instance. Surfaced as a dependency so unit tests can inject a
 * stub without monkey-patching global `fetch`.
 *
 * Why we inline this instead of using `@devilroute/opencode-provider`'s
 ...
 */
export type DevilRouteModelsFetcher = (
  baseURL: string,
  apiKey: string,
  timeoutMs?: number
) => Promise<DevilRouteRawModelEntry[]>;

/**
 * Default fetcher: `GET <baseURL>/v1/models` with bearer auth + AbortController
 * timeout. Accepts both the `{object:"list", data:[…]}` envelope DevilRoute
 * emits today and a bare-array envelope (defensive — keeps the plugin
 * working if a future DevilRoute build trims the wrapper). Anything that
 * isn't an object with a string `id` is filtered out silently.
 */
export const defaultDevilRouteModelsFetcher: DevilRouteModelsFetcher = async (
  baseURL,
  apiKey,
  timeoutMs = 10_000
) => {
  if (!apiKey) throw new Error("@devilroute/opencode-plugin: apiKey required to fetch /v1/models");
  if (!baseURL) throw new Error("@devilroute/opencode-plugin: baseURL required to fetch /v1/models");
  ...
  try {
    ...
    if (!res.ok) {
      throw new Error(
        `@devilroute/opencode-plugin: GET ${url} failed: ${res.status} ${res.statusText}`
      );
    }
    ...
    const rawList: unknown[] = Array.isArray(body)
      ...
      : body && typeof body === "object" && Array.isArray((body as { data?: unknown }).data)
        ...
        : [];
    const out: DevilRouteRawModelEntry[] = [];
    for (const r of rawList) {
      if (r && typeof r === "object" && typeof (r as { id?: unknown }).id === "string") {
        out.push(r as DevilRouteRawModelEntry);
      }
    }
    return out;
  } finally {
 ...
 *   5. `thinking` from DevilRoute has no 1:1 ModelV2 slot. We OR it into
 *      `reasoning` so thinking-only models still surface a non-false
 *      reasoning flag.
 *   6. `last_updated` from DevilRoute has no ModelV2 slot — dropped (the
 ...
 *   7. `temperature: true` per DevilRoute convention (OpenAI-compat mode
 ...
 *      Unknown strings (future DevilRoute additions) are ignored — when the
 *      server adds new modalities we can map them here without breaking
 *      existing entries.
 *   9. `status: "active"` — DevilRoute doesn't tier models alpha/beta on
...
/**
 * Raw shape of a single combo entry as returned by DevilRoute's `/api/combos`.
 *
 * Schema established via a live probe against
 * an DevilRoute `/api/combos` endpoint with a management-scoped key
 ...
 * bare array envelope so the plugin keeps working if a future DevilRoute
 * build trims the wrapper (mirrors the same pattern in the sibling
 * `@devilroute/opencode-provider#listCombos`).
 *
 ...
 */
export interface DevilRouteRawComboMemberRef {
...
export interface DevilRouteRawCombo {
  id: string;
  name?: string;
  ...
  models?: DevilRouteRawComboMemberRef[];
  /** Hidden combos are excluded from the OC model picker. */
  isHidden?: boolean;
  /** When DevilRoute attaches a lifecycle hint we forward it; today it doesn't. */
  release_date?: string;
  /**
 ...
 * `DevilRouteModelsFetcher` so unit tests can inject a stub instead of
 * monkey-patching global `fetch`.
 */
export type DevilRouteCombosFetcher = (
  baseURL: string,
  apiKey: string,
  timeoutMs?: number
) => Promise<DevilRouteRawCombo[]>;
 ...
 * plugin working if a future DevilRoute build trims the wrapper).
 *
 * Differences from `defaultDevilRouteModelsFetcher`:
 ...
 */
export const defaultDevilRouteCombosFetcher: DevilRouteCombosFetcher = async (
  baseURL,
  apiKey,
  timeoutMs = 10_000
) => {
  if (!apiKey) throw new Error("@devilroute/opencode-plugin: apiKey required to fetch /api/combos");
  if (!baseURL)
    throw new Error("@devilroute/opencode-plugin: baseURL required to fetch /api/combos");
  ...
  try {
    ...
    const rawList: unknown[] = Array.isArray(body)
      ...
      : body && typeof body === "object" && Array.isArray((body as { combos?: unknown }).combos)
        ...
        : [];
    const out: DevilRouteRawCombo[] = [];
    for (const r of rawList) {
      if (r && typeof r === "object" && typeof (r as { id?: unknown }).id === "string") {
        out.push(r as DevilRouteRawCombo);
      }
    }
    return out;
  } finally {
...
export function mapComboToModelV2(
  combo: DevilRouteRawCombo,
  members: DevilRouteRawModelEntry[],
  providerId: string,
  baseURL: string,
  apiFormat?: { anthropicPrefixes?: string[] }
): ModelV2 {
...
/**
 * Raw shape of an auto combo entry as returned by DevilRoute's
 ...
 */
export interface DevilRouteRawAutoCombo {
  ...
  /** MAX of candidates' context windows, served by newer DevilRoute builds.
 ...
 */
export type DevilRouteAutoCombosFetcher = (
  baseURL: string,
  apiKey: string,
  timeoutMs?: number
) => Promise<DevilRouteRawAutoCombo[]>;
 ...
 */
export const defaultDevilRouteAutoCombosFetcher: DevilRouteAutoCombosFetcher = async (
  baseURL,
  apiKey,
  timeoutMs = 5_000
) => {
  ...
  try {
    ...
    if (res.status === 404) {
      console.warn(
        `[devilroute-plugin] /api/combos/auto not available (404) — auto combos disabled`
      );
      return [];
    }
    if (!res.ok) {
      console.warn(
        `[devilroute-plugin] /api/combos/auto failed: ${res.status} ${res.statusText} — auto combos disabled`
      );
      return [];
    }
    const body = (await res.json()) as unknown;
    const rawList: unknown[] = Array.isArray(body)
      ...
      : body && typeof body === "object" && Array.isArray((body as { combos?: unknown }).combos)
        ? ((body as { combos: unknown[] }).combos as unknown[])
        : [];
    const out: DevilRouteRawAutoCombo[] = [];
    for (const r of rawList) {
      if (r && typeof r === "object" && typeof (r as { id?: unknown }).id === "string") {
        out.push(r as DevilRouteRawAutoCombo);
      }
    }
    return out;
  } catch (err) {
    ...
    console.warn(
      `[devilroute-plugin] /api/combos/auto fetch failed: ${err instanceof Error ? err.message : String(err)} — auto combos disabled`
    );
    return [];
  } finally {
    clearTimeout(timer);
 ...
 * DevilRoute builds). MUST be positive: OpenCode's overflow guard treats
...
export function mapAutoComboToStaticEntry(
  autoCombo: DevilRouteRawAutoCombo
): DevilRouteStaticModelEntry {
...
/**
 * Per-model enrichment overlay derived from DevilRoute's
 ...
 */
export interface DevilRouteEnrichmentEntry {
   ...
   * Populated by `defaultDevilRouteEnrichmentFetcher` from
...
export type DevilRouteEnrichmentMap = Map<string, DevilRouteEnrichmentEntry>;

export type DevilRouteEnrichmentFetcher = (
  baseURL: string,
  apiKey: string,
  timeoutMs?: number
) => Promise<DevilRouteEnrichmentMap>;
 ...
 * other.
 */
export const defaultDevilRouteEnrichmentFetcher: DevilRouteEnrichmentFetcher = async (
  baseURL,
  apiKey,
  timeoutMs = 10_000
) => {
  const out: DevilRouteEnrichmentMap = new Map();
    ...
    if (res.ok) {
      ...
      if (providers && typeof providers === "object") {
        for (const [providerAlias, slot] of Object.entries(providers)) {
          ...
          // DevilRoute hasn't curated a label for this slot.
          ...
          for (const m of models) {
            ...
            const entry: DevilRouteEnrichmentEntry = {
              providerAlias,
              providerCanonical,
            };
      ...
      if (body && typeof body === "object" && !Array.isArray(body)) {
        for (const [providerAlias, slot] of Object.entries(body as Record<string, unknown>)) {
          ...
          for (const [modelId, raw] of Object.entries(slot as Record<string, unknown>)) {
            ...
            const parsed: NonNullable<DevilRouteEnrichmentEntry["pricing"]> = {};
            // DevilRoute `/api/pricing` keys:
            //   input         → cost.input
            //   output        → cost.output
...
async function writeStartupDiagnostics(params: {
  ...
  enrichment: DevilRouteEnrichmentMap;
  autoCombos: DevilRouteRawAutoCombo[];
  features?: DevilRoutePluginOptions["features"];
}): Promise<void> {
  const {
    providerId,
  ...
  lines.push(
    `features(effective): ` +
      (Object.keys(effectiveFlags) as DevilRouteFeatureFlag[])
  ...
  try {
    ...
    const diagPath = path.join(diagDir, "plugins", "devilroute-startup-diagnostics.log");
    let existing = "";
    try {
...
// Re-export from naming.ts — thin wrapper preserving DevilRouteEnrichmentEntry signature
export function shortProviderLabel(
  enrichment: DevilRouteEnrichmentEntry | undefined
): string | undefined {
 ...
 *
 * DevilRoute's `/api/pricing/models` is keyed by short ALIAS (`cc`, `cx`,
...
export function buildCanonicalToAliasMap(
  enrichment: DevilRouteEnrichmentMap | undefined
): Map<string, string> {
...
): DevilRouteEnrichmentEntry | undefined {
...
export function canonicalDedupSet(
  rawModels: ReadonlyArray<DevilRouteRawModelEntry>,
  canonicalToAlias: Map<string, string>
): Set<string> {
 ...
 *
 * Real example: DevilRoute's `pricing['cohere']` slot lists 10 curated
 ...
 * a minimal `DevilRouteEnrichmentEntry` whenever the direct lookup
...
export function buildAliasIndex(
  enrichment: DevilRouteEnrichmentMap | undefined
): Map<string, DevilRouteEnrichmentEntry> {
  const out = new Map<string, DevilRouteEnrichmentEntry>();
  if (!enrichment) return out;
  for (const entry of enrichment.values()) {
    const alias = typeof entry.providerAlias === "string" ? entry.providerAlias.trim() : "";
...
export function resolveProviderTagEntry(
  rawId: string,
  direct: DevilRouteEnrichmentEntry | undefined,
  aliasIndex: Map<string, DevilRouteEnrichmentEntry>,
  canonicalToAlias?: Map<string, string>
): DevilRouteEnrichmentEntry | undefined {
  if (direct) {
...
export interface DevilRouteCompressionStep {
...
export interface DevilRouteCompressionCombo {
  id: string;
  name?: string;
  description?: string;
  pipeline: DevilRouteCompressionStep[];
  isDefault?: boolean;
}

export type DevilRouteCompressionMetaFetcher = (
  baseURL: string,
  apiKey: string,
  timeoutMs?: number
) => Promise<DevilRouteCompressionCombo[]>;
 ...
 */
export const defaultDevilRouteCompressionMetaFetcher: DevilRouteCompressionMetaFetcher = async (
  baseURL,
  apiKey,
  timeoutMs = 10_000
) => {
  const empty: DevilRouteCompressionCombo[] = [];
  ...
  try {
    ...
    const list = Array.isArray(body)
      ...
      : Array.isArray((body as { combos?: unknown[] })?.combos)
        ...
        : Array.isArray((body as { data?: unknown[] })?.data)
          ...
          : [];
    const out: DevilRouteCompressionCombo[] = [];
    for (const raw of list) {
      ...
      const steps: DevilRouteCompressionStep[] = [];
      for (const step of pipeline) {
        ...
        const entry: DevilRouteCompressionStep = { engine };
        if (typeof intensity === "string" && intensity.length > 0) {
          entry.intensity = intensity;
        }
        steps.push(entry);
      }
      const combo: DevilRouteCompressionCombo = { id, pipeline: steps };
 ...
 * DevilRoute knows but the plugin doesn't.
 *
 ...
 */
export function formatCompressionPipeline(pipeline: DevilRouteCompressionStep[]): string {
...
// `features.usableOnly` filter. Returns the operator's installed DevilRoute
...
export interface DevilRouteProviderConnection {
  /** Connection UUID. */
  id: string;
...
export type DevilRouteProvidersFetcher = (
  baseURL: string,
  apiKey: string,
  timeoutMs?: number
) => Promise<DevilRouteProviderConnection[]>;
 ...
 */
export const defaultDevilRouteProvidersFetcher: DevilRouteProvidersFetcher = async (
  baseURL,
  apiKey,
  timeoutMs = 10_000
) => {
  const empty: DevilRouteProviderConnection[] = [];
  if (!baseURL || !apiKey) return empty;
  const root = baseURL.replace(/\/v1\/?$/, "").replace(/\/$/, "");
    ...
    const list = Array.isArray(body)
      ...
      : Array.isArray((body as { connections?: unknown[] })?.connections)
        ...
        : Array.isArray((body as { data?: unknown[] })?.data)
          ? (body as { data: unknown[] }).data
          : [];
    const out: DevilRouteProviderConnection[] = [];
    for (const raw of list) {
      if (!raw || typeof raw !== "object") continue;
 ...
 * (`claude/sonnet-4`) depending on the DevilRoute deployment's `/v1/models`
 * surface shape.
 *
...
export function usableProviderAliasSet(
  connections: DevilRouteProviderConnection[],
  enrichment: DevilRouteEnrichmentMap | undefined
): {
  aliases: Set<string>;
  ...
  if (enrichment) {
    ...
    // populated by `defaultDevilRouteEnrichmentFetcher`. Every entry carries
...
export function isUsableCombo(
  ...
  // The provider id is folded INTO the full model string by DevilRoute's
 ...
 * (e.g. `devilroute/MASTER`, `devilroute/MASTER-LIGHT`), guaranteeing uniqueness
 ...
 *
 * NOTE: the key MUST carry the OWNING provider prefix (`devilroute/…`), never a
 ...
 * "Unable to determine provider", whereas `devilroute/MASTER` resolves provider=
 * `devilroute` and the openai-compatible adapter strips the prefix and sends the
 ...
 * OC user may register prod + preprod DevilRoute side-by-side with distinct
 ...
 *     DevilRoutePlugin instance — sharing the cache means /v1/models +
 *     /api/combos each hit the gateway exactly ONCE per TTL refresh, not
 *     twice.
 */
export interface DevilRouteFetchCacheEntry {
  rawModels: DevilRouteRawModelEntry[];
  rawCombos: DevilRouteRawCombo[];
  rawAutoCombos: DevilRouteRawAutoCombo[];
  /** Display-name + pricing overlay from /api/pricing/models. Empty Map when feature is disabled or fetch failed. */
  rawEnrichment: DevilRouteEnrichmentMap;
  /** Compression combos from /api/context/combos. Empty array when feature is disabled or fetch failed. */
  rawCompressionCombos: DevilRouteCompressionCombo[];
  /** Provider connections from /api/providers. Empty array when feature is disabled or fetch failed. */
  rawConnections: DevilRouteProviderConnection[];
  expiresAt: number;
}

export type DevilRouteFetchCache = Map<string, DevilRouteFetchCacheEntry>;
 ...
 *   - **Combo / model ID collisions: combos win.** DevilRoute treats combos
 ...
 *     `/api/combos` requires a management-scoped key and DevilRoute may
 ...
 *             outside tests today).
 */
export function createDevilRouteProviderHook(
  opts?: DevilRoutePluginOptions,
  deps: {
    fetcher?: DevilRouteModelsFetcher;
    combosFetcher?: DevilRouteCombosFetcher;
    autoCombosFetcher?: DevilRouteAutoCombosFetcher;
  ...
  const resolved = resolveDevilRoutePluginOptions(opts);
  const fetcher = deps.fetcher ?? defaultDevilRouteModelsFetcher;
  ...
  const combosFetcher = deps.combosFetcher ?? defaultDevilRouteCombosFetcher;
  const autoCombosFetcher = deps.autoCombosFetcher ?? defaultDevilRouteAutoCombosFetcher;
  const enrichmentFetcher = deps.enrichmentFetcher ?? defaultDevilRouteEnrichmentFetcher;
  const compressionMetaFetcher =
    deps.compressionMetaFetcher ?? defaultDevilRouteCompressionMetaFetcher;
  const providersFetcher = deps.providersFetcher ?? defaultDevilRouteProvidersFetcher;
  ...
  const cache: DevilRouteFetchCache = deps.cache ?? new Map();
  ...
  return {
    ...
    async models(_provider, ctx) {
      ...
      if (!baseURL) {
        console.warn(
          `[devilroute-plugin] provider.models(${resolved.providerId}): ` +
      ...
      let rawModels: DevilRouteRawModelEntry[];
      let rawCombos: DevilRouteRawCombo[];
      let rawAutoCombos: DevilRouteRawAutoCombo[];
      let rawEnrichment: DevilRouteEnrichmentMap;
      let rawCompressionCombos: DevilRouteCompressionCombo[];
      let rawConnections: DevilRouteProviderConnection[];
      if (cached && cached.expiresAt > t) {
        ...
        // management-scoped key and DevilRoute may not have any combos
        ...
        if (wantCombos) {
          try {
            ...
            console.warn(
              "[devilroute-plugin] combos fetch failed, falling back to models-only catalog",
              err
            );
          }
        }
        ...
        // may not exist yet on older DevilRoute versions).
        rawAutoCombos = [];
        if (wantAutoCombos) {
          try {
        ...
        if (wantEnrichment) {
          try {
            ...
            console.warn(
              "[devilroute-plugin] enrichment fetch failed, falling back to raw ids",
              err
            );
          }
        }
        ...
        if (wantCompressionMeta) {
          try {
            ...
            console.warn("[devilroute-plugin] compression-metadata fetch failed", err);
          }
        }
        ...
        if (wantUsableOnly) {
          try {
            ...
            console.warn(
              "[devilroute-plugin] /api/providers fetch failed; usableOnly filter disabled for this refresh",
              err
            );
          }
        }
        ...
        // the dynamic pipeline fired and how much catalog DevilRoute returned.
        // Emitted once per cache miss (TTL refresh) — quiet on cache hits.
        console.warn(
          `[devilroute-plugin] catalog refreshed for providerId=${resolved.providerId} baseURL=${baseURL}: ` +
      ...
      const rawModelById = new Map<string, DevilRouteRawModelEntry>();
      ...
      for (const entry of rawModels) {
        ...
        const model = mapRawModelToModelV2(entry, {
          ...
          providerId: resolved.devilrouteProviderId,
          baseURL,
        ...
        // prefixed id on `model.id` (e.g. `devilroute/claude-primary`), so we
      ...
      // compression metadata is present). DevilRoute returns at most one
      ...
      for (let pass = 0; pass < MAX_COMBO_PASSES && pending.length > 0; pass++) {
        ...
        for (const combo of pending) {
          ...
          const memberEntries: DevilRouteRawModelEntry[] = [];
          ...
          for (const step of memberSteps) {
            ...
            if (stepKind === "combo-ref") {
              ...
              } as unknown as DevilRouteRawModelEntry);
              continue;
            }
          ...
          // Apply enrichment overlay to combos too (DevilRoute's
          ...
          const comboKey = buildComboKey(combo, usedComboKeys, resolved.devilrouteProviderId);
          ...
          if (Object.prototype.hasOwnProperty.call(models, comboKey)) {
            ...
            if (!isIntentionalDedup) {
              ...
              if (!collisionWarned.has(dedupeKey)) {
                ...
                console.warn(
                  `[devilroute-plugin] combo key "${comboKey}" collides with a model id; combo wins.`
                );
              }
            }
          }
      ...
      if (pending.length > 0) {
        console.warn(
          `[devilroute-plugin] ${pending.length} combo(s) could not resolve all nested combo-refs after ${MAX_COMBO_PASSES} passes; they will advertise context=0 to avoid over-claiming.`
        );
      }
...
// same-origin DevilRoute inference requests only
// ────────────────────────────────────────────────────────────────────────────
 ...
 * a secret bound to both the configured DevilRoute origin and these intended
 * inference endpoints, and MUST NOT leak elsewhere.
 *
 * Ported from Alph4d0g's `opencode-devilroute-auth@1.2.1` `createFetchInterceptor`
 ...
 *     any caller-supplied Bearer for the same DevilRoute provider.
 *
 ...
 */
export function createDevilRouteFetchInterceptor(config: {
  apiKey: string;
  baseURL: string;
}): typeof fetch {
 ...
 * validator throws on them, breaking DevilRoute → Gemini chains transparently.
 *
 * Source: behavioural reverse-engineering from Alph4d0g's
 * opencode-devilroute-auth@1.2.1 (dist/src/plugin.js:517).
 */
 ...
 *              interceptor from `createDevilRouteFetchInterceptor`).
 */
    ...
    try {
      ...
      if (typeof initBody === "string") {
        ...
        if (!geminiStreamingWarningEmitted) {
          ...
          console.warn(
            "[devilroute-plugin] sanitizeGemini: streaming Request body, skipping schema strip (Gemini may reject)"
          );
        }
        return inner(input, init);
      } else if (
...
// `@devilroute/opencode-provider/src/index.ts` (`createDevilRouteProvider`,
 ...
 * `OpenCodeModelEntry` exported by `@devilroute/opencode-provider`. Stripped
 ...
 * when DevilRoute didn't surface a value, NOT emitted as `undefined` — the
 * resulting JSON must be diffable across DevilRoute deployments without
 * `undefined` noise.
 */
/** Modalities accepted by OC's static catalog reader (see `@opencode-ai/sdk`). */
export type DevilRouteModalityKind = "text" | "audio" | "image" | "video" | "pdf";

const STATIC_MODALITY_VALUES: ReadonlySet<DevilRouteModalityKind> = new Set([
  "text",
  "audio",
  "image",
  "video",
...
function normaliseModalities(raw: unknown): DevilRouteModalityKind[] {
  if (!Array.isArray(raw)) return [];
  const out: DevilRouteModalityKind[] = [];
  const seen = new Set<string>();
  for (const v of raw) {
    if (typeof v !== "string") continue;
    const lower = v.toLowerCase() as DevilRouteModalityKind;
...
export interface DevilRouteStaticModelEntry {
   ...
   * model KEY already carries the provider prefix (e.g. `devilroute/MASTER`),
  ...
  tool_call?: boolean;
  /**
   * Per-million-token cost. Maps from DevilRoute `/api/pricing` shape:
   ...
   * DevilRoute's `input_modalities` / `output_modalities` on `/v1/models`.
   ...
   */
  modalities?: {
    input: DevilRouteModalityKind[];
    output: DevilRouteModalityKind[];
  };
}

/**
 * Static `provider.<id>` block written to `input.provider` by the config hook.
 * Mirrors `OpenCodeProviderEntry` from `@devilroute/opencode-provider`.
 *
 *   - `npm` is always `"@ai-sdk/openai-compatible"` — DevilRoute exposes an
 ...
 */
export interface DevilRouteStaticProviderEntry {
  npm: "@ai-sdk/openai-compatible";
  name: string;
  options: {
    baseURL: string;
  ...
  models: Record<string, DevilRouteStaticModelEntry>;
}
 ...
 *
 * Mapping rules (per the sibling `createDevilRouteProvider` output spec):
 *
...
export function buildStaticProviderEntry(
  rawModels: DevilRouteRawModelEntry[],
  rawCombos: DevilRouteRawCombo[],
  opts: ReturnType<typeof resolveDevilRoutePluginOptions>,
  baseURL: string,
  apiKey: string,
  enrichment?: DevilRouteEnrichmentMap,
  compressionCombos?: DevilRouteCompressionCombo[],
  connections?: DevilRouteProviderConnection[],
  rawAutoCombos?: DevilRouteRawAutoCombo[]
): DevilRouteStaticProviderEntry {
  const models: Record<string, DevilRouteStaticModelEntry> = {};
  ...
  // Build a name-set of every non-hidden combo from `/api/combos`. DevilRoute
  ...
  for (const raw of rawModels) {
    ...
    // model KEY (e.g. `devilroute/claude-opus-4`) is what OC uses to recover
    // (providerID, modelID) when the user selects a model.
    const entry: DevilRouteStaticModelEntry = { name: displayName };
    ...
    // Modalities — emit when DevilRoute surfaced any. Without this field
    ...
    // DevilRoute field names to OC's static-schema field names.
    const pricing = enrichmentEntry?.pricing;
    if (pricing && (typeof pricing.input === "number" || typeof pricing.output === "number")) {
      const cost: NonNullable<DevilRouteStaticModelEntry["cost"]> = {
  ...
  const resolvedStaticCombosByName = new Map<string, DevilRouteStaticModelEntry>();
  ...
  for (let pass = 0; pass < MAX_STATIC_COMBO_PASSES && pendingStatic.length > 0; pass++) {
    ...
    for (const combo of pendingStatic) {
      ...
      // provider and the KEY prefix (`devilroute/<slug>`) is what OC parses.
      const entry: DevilRouteStaticModelEntry = { name: displayName };
      ...
      if (hasMembers) {
        ...
        const intersect = (sets: Set<DevilRouteModalityKind>[]): DevilRouteModalityKind[] => {
      ...
      // `opts.providerId` reaches DevilRoute's server doubled
      // (`opencode-devilroute/opencode-devilroute/<slug>`), and `parseModel()`
      // resolves credentials for the nonexistent provider `opencode-devilroute`
      // instead of `devilroute`. See #7976.
      models[buildComboKey(combo, usedComboKeys, opts.devilrouteProviderId)] = entry;
  ...
  if (pendingStatic.length > 0) {
    console.warn(
      `[devilroute-plugin] ${pendingStatic.length} combo(s) in the static catalog could not resolve all nested combo-refs after ${MAX_STATIC_COMBO_PASSES} passes; they will be omitted.`
    );
  }
    ...
    for (const autoCombo of rawAutoCombos) {
      ...
      if (models[key]) {
        ...
        if (!reportedCollisions.has(key)) {
          ...
          console.warn(
            `[devilroute-plugin] auto combo key "${key}" collides with an existing model; auto combo wins.`
          );
        }
      }
      models[key] = entry;
 ...
 *
 * Exported as a dependency-injectable function on `createDevilRouteConfigHook`
...
// `${OPENCODE_DATA_DIR ?? ~/.local/share/opencode}/plugins/devilroute-<providerId>.json`.
...
interface DevilRouteDiskSnapshot {
  v: 2;
  ...
  rawEnrichment: Array<[string, DevilRouteEnrichmentEntry]>;
  rawCompressionCombos: DevilRouteCompressionCombo[];
  rawConnections: DevilRouteProviderConnection[];
...
export function diskSnapshotPath(providerId: string): string {
  ...
  return path.join(dir, "plugins", `devilroute-${providerId}.json`);
}
...
export type DevilRouteDiskSnapshotWriter = (
  providerId: string,
  entry: Omit<DevilRouteFetchCacheEntry, "expiresAt">,
  identityFingerprint: string
) => Promise<void>;

export type DevilRouteDiskSnapshotReader = (
  providerId: string,
  identityFingerprint: string
) => Promise<Omit<DevilRouteFetchCacheEntry, "expiresAt"> | undefined>;
...
export const defaultDiskSnapshotWriter: DevilRouteDiskSnapshotWriter = async (
  providerId,
  entry,
  identityFingerprint
) => {
  try {
    ...
    const snapshot: DevilRouteDiskSnapshot = {
      v: 2,
      identityFingerprint,
...
export const defaultDiskSnapshotReader: DevilRouteDiskSnapshotReader = async (
  providerId,
  identityFingerprint
) => {
  try {
    ...
    const parsed = JSON.parse(body) as Partial<DevilRouteDiskSnapshot>;
    if (
      !parsed ||
      parsed.v !== 2 ||
...
export const noopDiskSnapshotWriter: DevilRouteDiskSnapshotWriter = async () => {};
...
function debugLogPath(providerId: string): string {
  return join(debugLogDir(), `devilroute-debug-${providerId}.jsonl`);
}

function debugStatePath(providerId: string): string {
  return join(debugLogDir(), `devilroute-debug-${providerId}.state.json`);
}
...
export function debugLogSetEnabled(providerId: string, enabled: boolean): void {
  try {
    ...
    console.warn(`[devilroute-plugin] debugLogSetEnabled failed: ${(err as Error).message}`);
  }
}
...
export function debugLogAppend(entry: DebugLogEntry): void {
  try {
    ...
    console.warn(`[devilroute-plugin] debugLogAppend failed: ${(err as Error).message}`);
  }
}
...
export function debugLogClear(providerId: string): void {
  try {
    ...
    console.warn(`[devilroute-plugin] debugLogClear failed: ${(err as Error).message}`);
  }
}
...
export const noopDiskSnapshotReader: DevilRouteDiskSnapshotReader = async () => undefined;

export type DevilRouteReadAuthJson = () => Promise<AuthJsonShape | undefined | null>;

export const defaultReadAuthJson: DevilRouteReadAuthJson = async () => {
  const dir = process.env.OPENCODE_DATA_DIR ?? path.join(os.homedir(), ".local/share/opencode");
 ...
 *   - `fetcher`          — replaces `defaultDevilRouteModelsFetcher`.
 *   - `combosFetcher`    — replaces `defaultDevilRouteCombosFetcher`.
 *   - `now`              — clock for cache TTL (default `Date.now`).
 *   - `cache`            — shared fetch-result cache (see
 *                          `DevilRouteFetchCache`). Pass the same Map the
 *                          provider hook owns to dedupe round-trips.
 *   - `logger`           — `{warn}` sink for breadcrumb capture in tests.
 *                          Defaults to `console`.
 */
export function createDevilRouteConfigHook(
  opts?: DevilRoutePluginOptions,
  deps: {
    readAuthJson?: DevilRouteReadAuthJson;
    fetcher?: DevilRouteModelsFetcher;
    ...
    compressionMetaFetcher?: DevilRouteCompressionMetaFetcher;
    providersFetcher?: DevilRouteProvidersFetcher;
    diskSnapshotReader?: DevilRouteDiskSnapshotReader;
    diskSnapshotWriter?: DevilRouteDiskSnapshotWriter;
    now?: () => number;
    cache?: DevilRouteFetchCache;
  ...
  return async (input: Config) => {
    ...
    if (existingProviders && existingProviders[resolved.providerId] !== undefined) {
      logger.warn(
        `[devilroute-plugin] config shim skipped: provider.${resolved.providerId} already set by user`
      );
      return;
    }
    ...
    if (authJson === null) {
      logger.warn("[devilroute-plugin] config shim: auth.json failed to parse; treating as missing");
      authJson = undefined;
    }

    // Try both prefixed (e.g. opencode-devilroute) and unprefixed (e.g. devilroute)
    // keys so a user who ran `/connect devilroute` before the auto-prefix fix
    ...
    if (!apiKey) {
      ...
      logger.warn(
        `[devilroute-plugin] config shim skipped: no apiKey for providerId=${resolved.providerId}`
      );
      return;
    }
    // Management-plane catalog reads may use a narrower read-only token.
    ...
    if (!baseURL) {
      logger.warn(
        `[devilroute-plugin] config shim skipped: no baseURL for providerId=${resolved.providerId}`
      );
      return;
    }
    ...
    if (cached && cached.expiresAt > t) {
      ...
      try {
        ...
        logger.warn(
          "[devilroute-plugin] config shim: /v1/models fetch failed; publishing stub provider entry",
          err
        );
        rawModels = [];
      ...
      try {
        ...
        logger.warn(
          "[devilroute-plugin] config shim: /api/combos fetch failed; publishing models-only static catalog",
          err
        );
      }
      ...
      if (wantEnrichment) {
        try {
          ...
          logger.warn(
            "[devilroute-plugin] config shim: /api/pricing/models fetch failed; publishing raw-id static catalog",
            err
          );
        }
      }
      ...
      if (wantCompressionMeta) {
        try {
          ...
          logger.warn(
            "[devilroute-plugin] config shim: /api/context/combos fetch failed; publishing combos without compression suffix",
            err
          );
        }
      }
      ...
      if (wantUsableOnly) {
        try {
          ...
          logger.warn(
            "[devilroute-plugin] config shim: /api/providers fetch failed; usableOnly filter disabled for this refresh",
            err
          );
        }
      }
      ...
      if (modelsFetchThrew && wantDiskCache) {
        ...
        if (snapshot && snapshot.rawModels.length > 0) {
          logger.warn(
            `[devilroute-plugin] config shim: /v1/models unreachable; using stale disk cache (${snapshot.rawModels.length} models)`
          );
    ...
    if (features.mcpAutoEmit === true) {
      ...
      if (!mcpKey) {
        logger.warn(
          `[devilroute-plugin] mcp auto-emit skipped: no Bearer token for providerId=${resolved.providerId}`
        );
      } else {
        ...
        if (inputWithMcp.mcp[resolved.providerId] !== undefined) {
          logger.warn(
            `[devilroute-plugin] mcp auto-emit skipped: mcp.${resolved.providerId} already set by user`
          );
        } else {
