...
adapter the primary transport. Set `DEVILROUTE_BROWSER_POOL` to one of the same values to
...
`X-DevilRoute-Claude-Web-*` headers (`open-sse/executors/claude-web/stream.ts:364`).
...
1. Start DevilRoute with `npm run dev` or a built installation.
