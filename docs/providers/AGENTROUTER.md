...
For most users, **no special setup is required**. DevilRoute ships a built-in
...
DevilRoute also supports AgentRouter (and similar relays) through the **Claude Code
...
- DevilRoute running with the `ENABLE_CC_COMPATIBLE_PROVIDER` feature flag enabled
  (see below).
...
setting an environment variable before starting DevilRoute:
...
**`Invalid JSON response from provider (reset after Ns)` from the devilroute logs** —
...
model actually resolves in the devilroute logs (`ROUTING` tag shows
