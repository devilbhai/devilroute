...
> Feature design + implementation notes for DevilRoute's operator-level proxy
> subscription flow. This is the v1 cut: a single operator pastes subscription
> links, picks a mode (global or rule), and DevilRoute binds the resulting proxy
...
Today, DevilRoute's proxy pool is hand-curated: every node lives in
...
into DevilRoute and re-bind them whenever the upstream list changes.

The goal of v1 is to make DevilRoute first-class for **operator-supplied**
...
| U5 | Operator | supply a local sing-box/clash SOCKS5 endpoint | SS/VMess/Trojan/VLESS nodes (which DevilRoute's dispatcher can't speak natively) become usable through a local kernel bridge |
