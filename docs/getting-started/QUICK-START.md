# Quick Start: Get DevilRoute Running in 3 Minutes

> **TL;DR**: Install → Connect a free provider → Point your IDE to DevilRoute. Done.

---

## Step 1: Install DevilRoute
...
## Step 2: Start DevilRoute

```bash
devilroute
```

DevilRoute starts at `http://localhost:20128`. The dashboard opens automatically.

---

## Step 3: Connect a Free Provider

You can use DevilRoute **without paying anything** by connecting a free provider.
...
From [API Keys](http://localhost:20128/dashboard/api-manager), create a new key. Store this key since it will not appear again. Do note that this key is for tools to access DevilRoute, not to access upstream providers.
...
## Step 5: Point Your IDE or CLI to DevilRoute
...
That's it! Your IDE now uses DevilRoute with automatic provider selection.
...
3. In the Continue.dev chat pane, select `DevilRoute - Auto` and you will make requests to DevilRoute.
...
2. Now let's launch Codex, but configured for DevilRoute. Run:

```
devilroute launch-codex --model auto
```

You can do this manually via `codex` and command line parameters to specify endpoint and api key, but with the above command, DevilRoute takes care of everything for you.

3. The CLI should be sending requests to DevilRoute now.

### Confirm your tool is routing to DevilRoute
...
- **[Auto-Combo Guide](./AUTO-COMBO-GUIDE.md)** — Let DevilRoute pick the best AI for you
...
`auto` tells DevilRoute to automatically pick the best provider for each request. It considers speed, cost, quality, and availability. See the [Auto-Combo Guide](./AUTO-COMBO-GUIDE.md) for details.

### "How much does it cost?"

DevilRoute itself is **free and open-source**. You only pay for the providers you use. Many providers have free tiers — see the [Free Tiers Guide](./FREE-TIERS-GUIDE.md).

### "Can I use it with Claude Code / Cursor / Copilot?"

**Yes!** DevilRoute works with any tool that supports OpenAI format. Just set the base URL to `http://localhost:20128/v1`. See the [CLI Tools Guide](../reference/CLI-TOOLS.md) for specific setup instructions.

### "What if a provider goes down?"

DevilRoute automatically skips failed providers and tries the next one. You don't need to do anything. See the [Auto-Combo Guide](./AUTO-COMBO-GUIDE.md) for details.
