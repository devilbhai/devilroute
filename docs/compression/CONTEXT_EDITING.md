...
Delegated **Context Editing** is a Claude-only context-management feature. Unlike DevilRoute's local
...
tool-use / tool-result blocks from its own running context window. DevilRoute only attaches a body
parameter (`context_management.edits[]`); Claude does the actual clearing against its own tokenizer.

This is a delegated capability by nature: other providers reject the parameter, so DevilRoute scopes
...
DevilRoute injects a single edit into the outbound Anthropic Messages body:
...
DevilRoute already emits on Claude requests.
...
left as-is. If a `clear_thinking_20251015` edit is also present, DevilRoute stable-sorts the
...
After a Claude response, DevilRoute records how much context the provider actually cleared. This is
...
| ----------------- | --------------------------------------------------- | ------------------------------------------- |
| Where it runs     | In DevilRoute, before the request leaves the proxy   | In the provider (Claude), server-side       |
...
The two are complementary: local engines compress the bytes DevilRoute sends; Context Editing lets
