...
RTK compression is DevilRoute's command-aware compression engine for terminal and tool output. It is
...
`~118,000` standard tokens to `~23,900` RTK tokens, which is `79.7%` saved (`~80%`). DevilRoute uses
...
Within the same scope, RTK TOML schema v1 filters take precedence over DevilRoute JSON filters. TOML
...
- `DEVILROUTE_RTK_TRUST_PROJECT_FILTERS=1` is set.
...
DevilRoute can parse, validate, test, and install declarative filter files using RTK TOML schema v1.
...
DevilRoute receives tool output after the client has already captured it, so `filter_stderr = true`
...
`rtkEngine` (`@devilroute/open-sse/services/compression/engines/rtk`) is a
...
- User directory: `~/.devilroute/rtk/filters/`
