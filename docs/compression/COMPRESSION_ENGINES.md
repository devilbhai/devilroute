...
DevilRoute compression is built around engine contracts. A mode can run one engine directly
...
actually reach `devilroute_ccr_retrieve`** (`callerSupportsCcrRetrieve()` in
...
with a `22-87%` range, and a `~46%` input-compression tool. DevilRoute uses the Caveman input-side
...
DevilRoute install; when the optional stack is absent, LLMLingua still fail-opens. Only the three
...
- **Global npm (`npm i -g devilroute`) / standalone** — run the install command above inside
...
| Tool                                | Scope               | Purpose                          |
| ----------------------------------- | ------------------- | -------------------------------- |
| `devilroute_compression_status`      | `read:compression`  | Settings, analytics, cache stats |
| `devilroute_compression_configure`   | `write:compression` | Update global settings           |
| `devilroute_set_compression_engine`  | `write:compression` | Set mode and optional pipeline   |
| `devilroute_list_compression_combos` | `read:compression`  | List compression combos          |
| `devilroute_compression_combo_stats` | `read:compression`  | Read combo/engine analytics      |
