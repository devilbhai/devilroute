...
Embedded services are locally-installed process sidecar tools that DevilRoute installs, supervises, and
exposes as first-class routing targets. Unlike external providers (which are reached over the internet
via API keys), embedded services run on the same machine as DevilRoute and communicate over loopback.
...
| --------------- | ----------------------------------------------- | :----------: | ------------------------------------------------------------------------------------------------------------------ |
| **9Router**     | `9router`                                      |    20130     | AI router that DevilRoute can use as a sub-provider. Models exposed as `9router/{sub}/{model}`                     |
...
- DevilRoute installs them under `DATA_DIR/services/{name}/` (isolated from DevilRoute's own `package.json`)
- DevilRoute spawns and monitors them as child processes
- DevilRoute injects an ephemeral API key into the child's environment and rotates it without downtime (where applicable)
...
| API key management                    | DevilRoute generates, encrypts at-rest (AES-256-GCM), and injects via env |
...
`package.json` and `node_modules/`. Does not conflict with DevilRoute's own deps.
...
the next time DevilRoute boots (if the service is installed).
...
- Strips incoming `cookie` and `authorization` headers (no leakage of DevilRoute session)
...
2. Verify `npm` is in PATH: `which npm` from the same user account that runs DevilRoute.
...
DevilRoute's default port 20128.
...
- `DATA_DIR` or its parent is not writable by the DevilRoute process.
- Running inside Docker rootless without write access to the mapped volume.

**Fix:**

1. Check `DATA_DIR` (default: `~/.devilroute/`): `ls -la ~/.devilroute/`
2. Ensure the DevilRoute process user owns the directory: `chown -R $USER ~/.devilroute/`
...
3. Try the install manually: `npm install {package}@latest --prefix ~/.devilroute/services/{name}/`.
...
Yes. Both services follow the same local-first model as DevilRoute itself. They run
on the same machine and communicate over loopback. "Production" here means the VPS
or local server where DevilRoute is deployed, not a remote cloud provider.
...
2. Check structured logs in DevilRoute's pino output filtered by
   `service:supervisor` namespace.
3. Inspect the DB row: `sqlite3 ~/.devilroute/devilroute.db "SELECT * FROM version_manager WHERE tool='9router'"`.
