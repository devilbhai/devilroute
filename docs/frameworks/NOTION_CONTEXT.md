...
DevilRoute can connect to a **Notion** workspace as a **context source** — a read/write
...
blocks — all proxied through DevilRoute (with retry, timeout, and error classification)
...
> DevilRoute to access with that integration (Notion's permission model is share-based,
> not workspace-wide).
...
DevilRoute saves the token and immediately runs a 1-result test search; if Notion
...
`open-sse/mcp-server/server.ts` only when `DEVILROUTE_MCP_ENFORCE_SCOPES=true`; the
caller's allowed scopes come from `DEVILROUTE_MCP_SCOPES` (comma-separated) or the
