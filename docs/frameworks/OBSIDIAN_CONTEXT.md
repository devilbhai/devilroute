...
DevilRoute connects to an **Obsidian** vault as a **context source** — a local Markdown
...
vault sync (driven by `src/lib/obsidianSync.ts`). Enabling it points DevilRoute at a
...
per call (per-API-key first, then global). Tools that hit the DevilRoute **sync server**
(the four `obsidian_sync_*` tools) additionally require the sync auth token configured
in DevilRoute settings.
...
| `obsidian_sync_status`       | DevilRoute sync server status: running, vault name, port, uptime, last sync.          |
...
`open-sse/mcp-server/server.ts`, gated on `DEVILROUTE_MCP_ENFORCE_SCOPES=true`, with
allowed scopes sourced from `DEVILROUTE_MCP_SCOPES` or the API key's scope context. See
