...
DevilRoute ships a WordPress-style plugin system. Plugins are self-contained
...
Plugins live under the DevilRoute data directory:
...
`<home>/.devilroute/plugins`, where `<home>` is taken from the `HOME` /
