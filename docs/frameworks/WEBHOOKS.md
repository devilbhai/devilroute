...
DevilRoute can fire HTTP webhooks on platform events. Use them to integrate with
...
When a webhook has a `secret`, DevilRoute signs the JSON body and sends:
...
> Header names use the `X-Webhook-*` prefix (not `X-DevilRoute-*`). The signature
