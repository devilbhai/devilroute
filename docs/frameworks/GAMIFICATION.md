...
DevilRoute includes a local-first gamification layer that rewards users for
...
| Community Servers | Federate with external DevilRoute instances                      |
...
All tables live in the main DevilRoute SQLite database, created by migration
...
Follows the standard DevilRoute pattern — imports `getDbInstance()` from
...
The raw token is returned to the user exactly once at creation time. DevilRoute
...
All routes follow the standard DevilRoute pattern:
