...
Traffic Inspector is DevilRoute's built-in HTTPS traffic debugger — a Charles Proxy / mitmweb / HTTP Toolkit-like tool that is **LLM-aware** and **agent-aware**. It lives at `/dashboard/tools/traffic-inspector` and receives live traffic from up to 5 simultaneous capture sources.
...
| Feature                                                             | mitmweb | Charles | Fiddler | **DevilRoute Traffic Inspector** |
| ------------------------------------------------------------------- | :-----: | :-----: | :-----: | :-----------------------------: |
| Web-based                                                           |    ✓    |    ✗    |    ✗    |                ✓                |
| Open-source                                                         |    ✓    |    ✗    | partial |                ✓                |
...
| **Integrated with DevilRoute** routing, fallback, cost               |    ✗    |    ✗    |    ✗    |                ✓                |
...
interface LlmMetadata {
  ...
  mappedTo: string | null; // x-devilroute-mapped header
  costEstimateUsd: number | null; // estimated cost based on DevilRoute pricing
}
```
...
If DevilRoute crashes while system-wide proxy mode is active:
...
| POST   | `/requests/{id}/replay`     | Re-execute the same request through DevilRoute router                               |
