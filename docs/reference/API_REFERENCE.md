...
Complete reference for all DevilRoute API endpoints.
...
| ------------------------ | --------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `X-DevilRoute-No-Cache`   | Request   | Set to `true` to bypass cache                                                                                                                                                     |
| `x-devilroute-no-memory`  | Request   | Set to `true` to skip memory + skills injection for this request (mirrors no-cache; avoids the per-call token/cost overhead)                                                      |
| `X-DevilRoute-Progress`   | Request   | Set to `true` for progress events                                                                                                                                                 |
| `X-Session-Id`           | Request   | Sticky session key for external session affinity                                                                                                                                  |
| `x_session_id`           | Request   | Underscore variant also accepted (direct HTTP)                                                                                                                                    |
| `X-DevilRoute-Session-Id` | Request   | Caller-supplied session/conversation tag (also feeds memory). When present, persisted verbatim to `call_logs.session_tag` for per-session cost attribution (#8249) — never synthesized when absent |
| `Idempotency-Key`        | Request   | Dedup key (5s window)                                                                                                                                                             |
| `X-Request-Id`           | Request   | Alternative dedup key                                                                                                                                                             |
| `X-DevilRoute-Cache`      | Response  | `HIT` or `MISS` (non-streaming)                                                                                                                                                   |
| `X-DevilRoute-Idempotent` | Response  | `true` if deduplicated                                                                                                                                                            |
| `X-DevilRoute-Progress`   | Response  | `enabled` if progress tracking on                                                                                                                                                 |
| `X-DevilRoute-Session-Id` | Response  | Effective session ID used by DevilRoute                                                                                                                                            |
| `X-DevilRoute-Request-Id` | Response  | Request correlation id (when known)                                                                                                                                               |
| `X-DevilRoute-Version`    | Response  | DevilRoute build version (always present)                                                                                                                                          |
| `X-DevilRoute-Cost-Saved` | Response  | USD the cache avoided on a HIT (cache hits only)                                                                                                                                  |
| `X-DevilRoute-Decision`   | Response  | Routing trace: `strategy=<name>; provider=<alias>; latency_ms=<n>` (`<name>` is the combo strategy, or `single` for a non-combo request) — always present on completion responses |

> Nginx note: if you rely on underscore headers (for example `x_session_id`), enable `underscores_in_headers on;`.

> **Cost telemetry headers:** non-streaming success responses also carry the `X-DevilRoute-*` cost-telemetry set — `X-DevilRoute-Response-Cost` (USD, fixed 10 decimals; `0.0000000000` for free/unpriced), `X-DevilRoute-Tokens-In` / `X-DevilRoute-Tokens-Out`, `X-DevilRoute-Model`, `X-DevilRoute-Provider`, `X-DevilRoute-Latency-Ms`, `X-DevilRoute-Cache-Hit`, and `X-DevilRoute-Fallback-Attempts` (only when > 0), plus `X-DevilRoute-Request-Id` and `X-DevilRoute-Version`. These are emitted by chat completions, `/v1/responses`, `/v1/messages`, **and the media endpoints** — `/v1/embeddings`, `/v1/images/generations`, `/v1/audio/speech`, `/v1/audio/transcriptions`, `/v1/rerank`, `/v1/videos/generations`, `/v1/music/generations`, and `/v1/moderations` (always cost `0`). Media cost is computed per modality (per-image, per-second, per-character, per search-unit) when pricing is available, otherwise `0` (fail-open).

> **Cache-hit cost semantics:** on a semantic-cache HIT (`X-DevilRoute-Cache-Hit: true`) no upstream call is made, so `X-DevilRoute-Response-Cost` is `0.0000000000` (the **incremental** cost of serving the hit). The original/would-have-been cost is reported separately in `X-DevilRoute-Cost-Saved`. Billing consumers should sum `X-DevilRoute-Response-Cost` (hits cost nothing); cache analytics can aggregate `X-DevilRoute-Cost-Saved`.

### `x-devilroute-compression`
...
```
X-DevilRoute-Compression: <mode>; source=<source>
```
...
- Remote media URLs must be public HTTPS. DevilRoute fetches them server-side with redirect
...
For thinking-capable Claude models, `/v1/models` also advertises a **no-thinking** variant whose id is prefixed with `claude-3-devilroute-no-thinking/`:

```
claude-3-devilroute-no-thinking/<provider>/<model>
```
...
For clients that cannot attach `Authorization: Bearer ...`, DevilRoute also accepts API keys in the URL via either query-string compatibility (`?token=...`, `?apiKey=...`, `?api_key=...`, `?key=...`) or the dedicated `/api/v1/vscode/{token}/...` endpoints documented below.
...
wscat -c "ws://localhost:20128/v1/responses?api_key=<DEVILROUTE_API_KEY>"
# (or: -H "Authorization: Bearer <DEVILROUTE_API_KEY>")
...
a ChatGPT account`). Send the **bare** id (e.g. `gpt-5.5`). DevilRoute's bridge is
...
Point the Codex CLI at DevilRoute by adding a custom provider with WebSocket
support to `~/.codex/config.toml` (use a separate `CODEX_HOME` to avoid touching
an existing config):

```toml
model = "gpt-5.5"                 # bare id — NOT "codex/gpt-5.5"
model_provider = "devilroute"

[model_providers.devilroute]
name = "DevilRoute (WS)"
...
env_key = "DEVILROUTE_API_KEY"             # holds the DevilRoute API key (Bearer)
```

```bash
export DEVILROUTE_API_KEY=sk-...           # an DevilRoute API key (any key if REQUIRE_API_KEY=false)
codex exec "Responda apenas: PONG"
```

The CLI upgrades `base_url + /responses` to a WebSocket and DevilRoute tunnels it
...
```json
{
  ...
  "backupPath": "/home/user/.devilroute/backups/env-repair-2026-04-11.bak"
}
```
...
- URL-based tokens may appear in reverse-proxy logs, browser history, and telemetry outside DevilRoute. Treat them as a compatibility option, not the default authentication mode.
...
Outbound webhook subscriptions for DevilRoute events (request completion, quota exhaustion, key rotation, etc.).
...
Cloud agent tasks (Claude Code, Codex Cloud, OpenHands, etc.) executed remotely on behalf of DevilRoute users.
...
DevilRoute exposes three independent temporary-failure mechanisms; the management endpoints below let operators read and override them:
...
Skill framework for extending DevilRoute with custom executable handlers, plus marketplace integrations.
...
DevilRoute ships an embedded Model Context Protocol server with 3 transports (stdio, SSE, streamable-http) and scoped tools. The dashboard endpoints below read status/audit data and proxy the HTTP transports.
...
DevilRoute exposes an A2A (Agent-to-Agent) JSON-RPC 2.0 endpoint plus a REST wrapper for inspection/dashboard use.

### JSON-RPC

```bash
POST /a2a
Authorization: Bearer your-api-key   # optional unless DEVILROUTE_API_KEY is set
Content-Type: application/json
...
**Auth:** the REST helpers run without management auth (dashboard-readable); the JSON-RPC `/a2a` route uses Bearer `DEVILROUTE_API_KEY` if configured.
...
Manage CLI tools that integrate with DevilRoute (antigravity, chipotle, commandCode,
...
Manage DevilRoute plugins (third-party extensions).
...
Inspect the runtime guardrails (PII detection, prompt injection detection, vision bridging). Guardrails run on every request; per-call opt-out is via the `x-devilroute-disabled-guardrails` request header — there is no persisted enable/disable surface.
