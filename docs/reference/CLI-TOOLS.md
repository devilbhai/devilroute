---
title: "CLI Tools — DevilRoute"
version: 3.8.40
lastUpdated: 2026-06-28
---

# CLI Tools — DevilRoute

Last updated: 2026-06-28

DevilRoute integrates with three categories of CLI tools spread across three dedicated dashboard pages:

| Page           | Route                   | Concept                                                                   | Count        |
| -------------- | ----------------------- | ------------------------------------------------------------------------- | ------------ |
| **CLI Code's** | `/dashboard/cli-code`   | Coding tools you point at DevilRoute (Client → CLI → DevilRoute → Provider) | 21           |
| **CLI Agents** | `/dashboard/cli-agents` | Autonomous agents you point at DevilRoute (same flow, broader scope)       | 6            |
| **ACP Agents** | `/dashboard/acp-agents` | CLIs that DevilRoute spawns as backend via stdio/ACP (reverse flow)        | see registry |
...
Claude / Codex / OpenCode / Cline / KiloCode / Continue / Hermes Agent / Goose / ...
           │
           ▼  (all point to DevilRoute)
    http://YOUR_SERVER:20128/v1
           │
           ▼  (DevilRoute routes to the right provider)
    Anthropic / OpenAI / Gemini / DeepSeek / Groq / Mistral / ...

ACP Agents (reverse spawn flow):
    Client request → DevilRoute → spawns CLI via stdio/ACP → response
```
...
You do not have to write each tool's config by hand. DevilRoute ships a `setup-*`
command per supported CLI that reads the **live** model catalog from a running
DevilRoute (local or remote) and writes the tool's own config on your machine:

```bash
devilroute setup-codex        devilroute setup-claude       devilroute setup-opencode
devilroute setup-cline        devilroute setup-kilo         devilroute setup-continue
devilroute setup-cursor       devilroute setup-roo          devilroute setup-crush
devilroute setup-goose        devilroute setup-qwen         devilroute setup-aider
```

Each accepts `--remote <url> --api-key <key>` (configure a local tool against a
remote DevilRoute), `--dry-run` (preview without writing), and `--port`. Tools
...
`devilroute launch` (Claude Code) and `devilroute launch-codex` (Codex) spawn the CLI
...
This page (renamed from `/dashboard/agents`) shows CLIs that DevilRoute can **spawn** as backend execution engines via stdio/ACP protocol. The catalog is maintained separately in `src/lib/acp/registry.ts` and is **not** the same as `CLI_TOOLS`.
...
| `POST /api/cli-tools/grok-build-settings`   | Grok Build (~/.grok/config.toml, `[model.devilroute]`) |
...
### Step 1 — Get an DevilRoute API Key
...
```bash
# DevilRoute Universal Endpoint
export OPENAI_BASE_URL="http://localhost:20128/v1"
export OPENAI_API_KEY="sk-your-devilroute-key"
export ANTHROPIC_BASE_URL="http://localhost:20128"
export ANTHROPIC_AUTH_TOKEN="sk-your-devilroute-key"
export GEMINI_BASE_URL="http://localhost:20128/v1"
export GEMINI_API_KEY="sk-your-devilroute-key"
```
...
mkdir -p ~/.claude && cat > ~/.claude/settings.json << EOF
{
  "env": {
    ...
    "ANTHROPIC_AUTH_TOKEN": "sk-your-devilroute-key"
  }
}
EOF
```
...
> Use `opencode run "your prompt" --model devilroute/claude-sonnet-4-5-thinking --variant high`
...
mkdir -p ~/.cline/data && cat > ~/.cline/data/globalState.json << EOF
{
  ...
  "openAiApiKey": "sk-your-devilroute-key"
}
EOF
```
...
Or use the DevilRoute dashboard → **CLI Tools → Cline → Apply Config**.
...
```bash
kilocode --api-base http://localhost:20128/v1 --api-key sk-your-devilroute-key
```
...
```json
{
  ...
  "kilo-code.apiKey": "sk-your-devilroute-key"
}
```

Or use the DevilRoute dashboard → **CLI Tools → KiloCode → Apply Config**.
...
Use this when VS Code Insiders is configured for custom endpoint models and you want DevilRoute to work without a custom header field.
...
**Example using the tokenized DevilRoute alias:**
...
[
  {
    ...
    "family": "gpt-4",
    "version": "1.0.0",
    "url": "http://localhost:20128/api/v1/vscode/sk-your-devilroute-key/chat/completions",
    "modelsUrl": "http://localhost:20128/api/v1/vscode/sk-your-devilroute-key/models",
...
- Replace `sk-your-devilroute-key` with an API key created in DevilRoute.
...
# The CLI uses its own auth — DevilRoute is not needed as backend for Kiro CLI itself.
# Use kiro-cli alongside DevilRoute for other tools.
kiro-cli status
```

For the **Kiro IDE** desktop app, use the MITM endpoint exposed by DevilRoute
under `/dashboard/cli-tools → Kiro`.

---

## 10. Internal DevilRoute CLI

The `devilroute` binary provides commands for server lifecycle, setup, diagnostics, and provider management. Entry point: `bin/devilroute.mjs`.

```bash
devilroute                              # Start server (default port 20128)
devilroute setup                        # Interactive setup wizard
devilroute doctor                       # Check config, DB, ports, runtime
devilroute providers list               # Configured provider connections
devilroute providers test-all           # Test every active connection
devilroute reset-password               # Reset the admin password
devilroute logs                         # Stream request logs
devilroute health                       # Detailed health (breakers, cache, memory)
devilroute --version                    # Print version
devilroute --help                       # Show all commands
```
...
```bash
devilroute setup                        # Interactive setup wizard
devilroute setup --non-interactive      # CI/automation mode (reads env vars + flags)
devilroute setup --password '<value>'   # Set admin password directly
devilroute setup --add-provider \
  --provider openai \
  --api-key '<value>' \
...
| Var                 | Purpose                                                        |
| ------------------- | -------------------------------------------------------------- |
| `DEVILROUTE_API_KEY` | Provider API key (bound to `--api-key` via Commander `.env()`) |
| `DATA_DIR`          | Override the DevilRoute data directory                          |

All other non-interactive inputs are passed as flags, not environment variables:
`--password`, `--provider`, `--provider-name`, `--provider-base-url`, `--default-model`
(see the `devilroute setup` options above).

### Diagnostics

```bash
devilroute doctor                       # Check config, DB, ports, runtime, memory, liveness
devilroute doctor --json                # Machine-readable JSON
devilroute doctor --no-liveness         # Skip the HTTP health probe
devilroute doctor --host 0.0.0.0        # Override liveness host
devilroute doctor --liveness-url <url>  # Full health endpoint URL override
```
...
```bash
devilroute providers available                       # DevilRoute provider catalog
devilroute providers available --search openai       # Filter catalog by id/name/alias/category
devilroute providers available --category api-key    # Filter by category (api-key, oauth, free, ...)
...
devilroute providers test-all                        # Test every active connection
devilroute providers validate                        # Local-only structural validation
```

> `providers available` reads the DevilRoute catalog; `providers list/test/test-all/validate`
> read the local SQLite database directly and do not require the server to be running.

### Recovery & Reset

```bash
devilroute reset-password                # Reset the admin password (also: devilroute-reset-password)
devilroute reset-encrypted-columns       # Show warning + dry-run for encrypted credential reset
devilroute reset-encrypted-columns --force  # Actually null out encrypted credentials in SQLite
```

### Credential Export (⚠ handle with care)

```bash
devilroute auth export                                 # Show warning + confirmation gate — no DB access
devilroute auth export --force                          # Export ALL connections' DECRYPTED credentials to stdout as JSON
devilroute auth export --force --id <id>                 # Export only the matching connection
devilroute auth export --force --format env               # Emit DEVILROUTE_<PROVIDER>_<FIELD>=<value> lines
devilroute auth export --force --out creds.json           # Write to a file (created with 0600 permissions)
```
...
These assume a running DevilRoute server, unless noted otherwise:

```bash
devilroute status                       # Comprehensive runtime status
devilroute logs                         # Stream request logs (--json, --search, --follow)
devilroute config show                  # Display current configuration

devilroute provider list                # List available providers (alias of providers list)
devilroute provider add                 # Register DevilRoute as a provider on a tool
devilroute keys add | list | remove     # Manage API keys
devilroute models [provider]            # List models (--json, --search)
devilroute combo list | switch | create | delete

devilroute backup                       # Snapshot config + DB
devilroute restore                      # Restore from a previous snapshot

devilroute health                       # Detailed health (breakers, cache, memory)
devilroute quota                        # Provider quota usage
devilroute cache                        # Cache status
devilroute cache clear                  # Clear semantic + signature caches

devilroute mcp status | restart         # MCP server status / restart
devilroute a2a status | card            # A2A server status / agent card

devilroute tunnel list | create | stop  # Manage tunnels (cloudflare/tailscale/ngrok)
devilroute env show | get <k> | set <k> <v>  # Inspect / set env vars (temporary)

devilroute test                         # Provider connectivity smoke test
devilroute update                       # Check for updates
devilroute completion                   # Generate shell completion
```
...
Ready-to-paste examples with a tokenized DevilRoute URL:
...
| Error                                        | Cause                   | Fix                                              |
| -------------------------------------------- | ----------------------- | ------------------------------------------------ |
| `Connection refused`                         | DevilRoute not running   | `devilroute serve`                                |
