...
TPROXY transparent decrypt is DevilRoute's **5th capture mode** for the
...
configuration change** — but the process must trust the dynamic CA DevilRoute
...
> and installs under the pre-existing `devilroute-mitm.crt` trust-store slot
> (superseding the old single leaf there — no dual-trust cleanup needed),
> kept fully separate from TPROXY's own `devilroute-tproxy-ca.crt` slot below.
...
- Generates a long-lived CA via `generateMitmCa()` (CN `"DevilRoute MITM CA"`,
...
`devilroute-tproxy-ca.crt` (constant `TPROXY_CA_CERT_NAME`) — kept separate from
the static MITM cert's slot (`devilroute-mitm.crt`) so the two never clobber each
other.
...
host means anything DevilRoute intercepts can be decrypted. It is gated behind the
...
- Confirm the process trusts the dynamic CA. The CA is installed under
  `devilroute-tproxy-ca.crt`; apps with their own trust store (Firefox/Chrome NSS)
...
mode reverts the rules; if DevilRoute was killed mid-session, use the AgentBridge
