...
DevilRoute CLI commands authenticate against the local management API using a
`HMAC-SHA256(machine-id, salt)` token sent via the `x-devilroute-cli-token`
request header.

This allows CLI subcommands (`devilroute status`, `devilroute providers`, etc.)
...
3. The CLI sends the token as `x-devilroute-cli-token` on every request to
...
Set `DEVILROUTE_CLI_SALT` to rotate the derived token without code changes.
...
export DEVILROUTE_CLI_SALT="my-secret-salt-2026"

# Verify new token is in use
devilroute status
```

Default salt: `devilroute-cli-auth-v1`
...
**Opt-out:** set `DEVILROUTE_DISABLE_CLI_TOKEN=true` (env or `.env`) to disable the CLI
