...
description: "Maintainer attestation for the AI-detected potential-malware findings raised against devilroute and the v3.8.6 mitigations applied at each flagged call site."
---

# Socket.dev / supply-chain finding attestation

This document is the maintainer-authored attestation for the six
`AI-detected potential malware` findings raised against `devilroute@3.8.5` and
the mitigations applied in `devilroute@3.8.6`. It exists so:
...
Caddy. The fact that they exist in DevilRoute is documented at
...
A `DEVILROUTE_ZED_IMPORT_LEGACY_ONE_STEP=true` env flag preserves the v3.8.5
...
who already use Zed and want to mirror their provider keys into DevilRoute
without re-pasting.
...
toolkit." Within DevilRoute they are only used by the MITM cert install path
...
(`DEVILROUTE_BUILD_PROFILE=minimal`) replaces
...
## §5 — DevilRoute Cloud Sync credential write-back (`api/keys/[id]/route.js`)
...
1. **HMAC verification**: `verifyCloudSignature(rawBody, sigHeader)` checks
   the `X-Cloud-Sig` header (`HMAC-SHA256(DEVILROUTE_CLOUD_SYNC_SECRET,
...
2. **Secret-field opt-in**: `accessToken` / `refreshToken` /
   ...
   `DEVILROUTE_CLOUD_SYNC_SECRETS=true`. The default mode syncs only
...
**Why we keep it**: Cloud Sync is the only way for an DevilRoute Cloud tenant
...
The resulting bundle is intended to be published as `devilroute-secure`. See
