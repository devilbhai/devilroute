...
DevilRoute controls which **browser origins** may read cross-origin responses
...
taking seriously, but DevilRoute's exposure is **narrower than the generic case**,
...
> `Access-Control-Allow-Credentials: true`. DevilRoute's shared CORS path never
> does.
...
- If DevilRoute runs behind a reverse proxy / tunnel (nginx, Caddy, Cloudflare
...
## Example: reverse proxy in front of DevilRoute

CORS is enforced by DevilRoute itself, so the proxy generally should **not** add or
rewrite `Access-Control-*` headers (double headers break browsers). Terminate TLS
and forward — let DevilRoute answer preflight:
...
Set the allowed browser origins in DevilRoute (`CORS_ALLOWED_ORIGINS` or the
