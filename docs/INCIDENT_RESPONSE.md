# Incident Response Runbook — DevilRoute (2026-06-18)
...
| **SEV-3** | Latent bug or near-miss; no current user impact but error budget at risk. | Memory leak trending up; circuit breaker tripping on one provider. | Slack `#devilroute-ops` (next standup) | 7 d |
...
| Grafana SLO dashboards | SLO burn-rate panels | Slack `#devilroute-ops` |
...
| User report (support@) | Manual triage | Slack `#devilroute-triage` |
...
1. Check the authz-inventory endpoint:
   `curl https://api.devilroute.dev/api/settings/authz-inventory | jq`.
...
written handoff to the incoming in `#devilroute-ops-handoff` covering:
