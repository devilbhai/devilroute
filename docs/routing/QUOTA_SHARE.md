...
**Problem it solves:** DevilRoute proxies many API keys against the same upstream
...
- All persistence is in the existing DevilRoute SQLite DB (`DATA_DIR/storage.sqlite`).
...
1. `localStorage.getItem("devilroute:quota-share:pools")` is non-empty.
