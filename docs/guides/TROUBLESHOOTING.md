...
Common problems and solutions for DevilRoute.

---

## Quick Reference

**New to DevilRoute?** Start here — these solve 90% of problems:

| I see this              | What it means                       | What to do                                                                                        |
| ----------------------- | ----------------------------------- | ------------------------------------------------------------------------------------------------- |
| "Can't connect"         | DevilRoute isn't running             | Run `devilroute` or `docker restart devilroute`                                                     |
...
| EACCES: permission denied                           | Set `DATA_DIR=/path/to/writable/dir` to override `~/.devilroute`                                                                                          |
...
| `dlopen` / `slice is not valid mach-o file` (macOS) | Run `cd $(npm root -g)/devilroute/app && npm rebuild better-sqlite3 && devilroute` — see [macOS native module rebuild](#macos-native-module-rebuild) below |
...
HTTP-request-looking links. DevilRoute's `README.md` ships inside the npm package (it is
listed in `package.json` → `files`), so it lands at `node_modules/devilroute/README.md` on
...
1. **Stop the notifications** — exclude the install directory in your antivirus
   ...
   DevilRoute data dir (`~/.devilroute/`).
...
`%LOCALAPPDATA%\Programs\DevilRoute` (including hash-suffixed package directories from the
...
1. **Verify your download first** (rules out a tampered file). Every release publishes
   `latest.yml`, whose `sha512` field (base64) covers the `DevilRoute.Setup.<version>.exe`
...
**Cause:** You are running a Node.js version outside DevilRoute's approved secure runtime floor. The most common case is running an older Node 22 or 24 patch level that falls below the patched security floor DevilRoute requires.
...
3. Reinstall DevilRoute: `npm install -g devilroute`
4. Restart: `devilroute`
...
**Fix (v3.5.5+):** DevilRoute now uses undici's own `fetch()` function when a proxy dispatcher is active, ensuring consistent behavior. Update to v3.5.5+.

### MITM proxy under WSL: desktop apps on the Windows host are not intercepted

**Cause:** The MITM proxy and its CA certificate install into the environment where DevilRoute runs. Under WSL that environment is the Linux guest, while the AI desktop apps (Kiro, Trae, Copilot, Zed, …) run on the Windows host. The host apps do not trust the guest's certificate store and do not route through the guest's system proxy, so desktop interception does not engage there.

**Recommendation:** Run DevilRoute natively on the same OS as the desktop apps you want to intercept (Windows for Windows apps; macOS/Linux likewise). Keeping DevilRoute inside WSL while targeting host apps requires manually trusting the generated CA certificate on the Windows host and pointing each host app's network/proxy settings at the WSL proxy endpoint — an unsupported, fragile setup.
...
DevilRoute auto-refreshes tokens. If issues persist:
...
2. Verify `CLOUD_URL` points to your cloud endpoint (e.g., `https://devilroute.dev`)
...
When many concurrent requests hit a rate-limited provider, DevilRoute uses mutex + auto rate-limiting to serialize requests and prevent cascading failures. This is automatic for API key providers.

---

## Optional RAG / LLM failure taxonomy (16 problems)

Some DevilRoute users place the gateway in front of RAG or agent stacks. In those setups it is common to see a strange pattern: DevilRoute looks healthy (providers up, routing profiles ok, no rate limit alerts) but the final answer is still wrong.
...
1. When you investigate a bad response, capture:
   ...
   - route or provider combo in DevilRoute
   - any RAG context used downstream (retrieved documents, tool calls, etc)
2. Map the incident to one or two WFGY ProblemMap numbers (`No.1` … `No.16`).
3. Store the number in your own dashboard, runbook, or incident tracker next to the DevilRoute logs.
...
You can ignore this section if you do not run RAG or agent pipelines behind DevilRoute.
...
2. Restart DevilRoute so the new env values are picked up
...
3. Restart DevilRoute and re-test from **Dashboard → CLI Tools**
...
- Run `devilroute providers` from the CLI to re-trigger the OAuth flow, or
...
### DEVILROUTE_WS_BRIDGE_SECRET missing in production
...
**Cause:** The `DEVILROUTE_WS_BRIDGE_SECRET` env var is missing from the production environment.

**Fix:**

1. Generate a random secret: `openssl rand -hex 32`
2. Set `DEVILROUTE_WS_BRIDGE_SECRET=<random-secret>` in the production server env (and any client that talks to the bridge)
3. Restart DevilRoute
