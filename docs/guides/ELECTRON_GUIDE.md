...
DevilRoute ships a cross-platform desktop app (Windows / macOS / Linux) built on
...
| App version        | `3.8.0`                    |
| App id             | `online.devilroute.desktop` |
| Product name       | `DevilRoute`                |
...
- Windows: `%APPDATA%\devilroute`
- Linux: `$XDG_CONFIG_HOME/devilroute` or `~/.devilroute`
- macOS: `~/.devilroute`
...
- Close button minimizes to tray; the tray menu has **Open DevilRoute**, **Open Dashboard** (external browser), **Server Port** submenu, **Check for Updates**, **Quit**.
...
- `connect-src 'self' http://localhost:* http://127.0.0.1:* ws://localhost:* ws://127.0.0.1:* https://*.devilroute.online https://*.devilroute.dev`
- Dev mode adds `'unsafe-eval'` to `script-src` only

## Auto-update

Uses `electron-updater` with the GitHub provider (`diegosouzapw/DevilRoute`).
...
- `DevilRoute Setup X.Y.Z.exe`, `DevilRoute-X.Y.Z-portable.exe` (Windows)
...
Releases are published to GitHub Releases (`diegosouzapw/DevilRoute`), which is also where `electron-updater` checks for new versions.
