...
> **TL;DR**: DevilRoute ranks its **free** providers by model quality using **Arena AI
...
DevilRoute aggregates 160+ providers, many of which expose a **free tier** (no-auth,
...
Leaderboard categories map to DevilRoute task categories:

| Arena leaderboard | DevilRoute task categories                         |
| ----------------- | ------------------------------------------------- |
...
- It is **non-blocking and never fatal** — if the upstream fetch fails, DevilRoute keeps
...
stripped and a small alias map expands canonical names into the variants DevilRoute uses
