---
title: "CLI Integrations — point any coding CLI at DevilRoute"
version: 3.8.40
...
DevilRoute ships a family of `setup-*` commands that configure a coding
CLI (Codex, Claude Code, OpenCode, Cline, …) to use DevilRoute as its backend — so
the tool talks to **one** endpoint and DevilRoute routes to the right provider with
auto-fallback. Each command reads the **live** model catalog from a running
DevilRoute (local or remote) and writes the tool's own config file on **your**
...
There are also two launchers — `devilroute launch` (Claude Code) and
`devilroute launch-codex` (Codex) — that spawn the CLI with the right env injected,
...
- [Remote Mode](./REMOTE-MODE.md) — drive a remote DevilRoute (VPS / Tailnet) from your laptop

---

## Master table

Every command honours the **active context** (set with `devilroute connect`, see
...
|---------|------|----------------|-----------|-----------------|
| `devilroute setup-codex` | OpenAI Codex CLI | `~/.codex/<name>.config.toml` — one profile per compatible text model (`codex --profile <name>`) | `--remote` `--api-key` `--only` `--dry-run` `--port` `--codex-home` | Both |
| `devilroute setup-claude` | Claude Code | `~/.claude/profiles/<name>/settings.json` — one profile per matched model (`CLAUDE_CONFIG_DIR`) | `--remote` `--api-key` `--only` `--dry-run` `--port` `--claude-home` | Both |
| `devilroute setup-opencode` | OpenCode (openai-compatible) | `~/.config/opencode/opencode.json` — `devilroute` provider with every catalog model (`opencode -m devilroute/<model>`) | `--remote` `--api-key` `--only` `--model` `--dry-run` `--port` | Both |
| `devilroute setup-cline` | Cline | `~/.cline/data/{globalState,secrets}.json` (CLI mode) + prints VS Code extension settings | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--cline-dir` | Both |
| `devilroute setup-kilo` | Kilo Code | `~/.local/share/kilo/auth.json` (CLI) + merges `kilocode.*` into VS Code `settings.json` if present | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--auth-path` `--vscode-settings` | Both |
| `devilroute setup-continue` | Continue / `cn` CLI | `~/.continue/config.yaml` — `provider: openai` models, key via `${{ secrets.DEVILROUTE_API_KEY }}` | `--remote` `--api-key` `--only` `--dry-run` `--port` `--config-path` | Both |
| `devilroute setup-cursor` | Cursor | Nothing — prints the in-app steps (Cursor config is opaque SQLite) | `--remote` `--api-key` `--only` `--port` | Both |
| `devilroute setup-roo` | Roo Code | `~/.devilroute/roo-settings.json` (import doc) + sets `roo-cline.autoImportSettingsPath` if a VS Code `settings.json` exists | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--import-path` `--vscode-settings` | Both |
| `devilroute setup-crush` | Crush | `~/.config/crush/crush.json` — `openai-compat` provider, key via `$DEVILROUTE_API_KEY` | `--remote` `--api-key` `--only` `--dry-run` `--port` `--config-path` | Both |
| `devilroute setup-goose` | Goose | `~/.config/goose/config.yaml` (`GOOSE_PROVIDER`/`OPENAI_HOST`/`GOOSE_MODEL`) + prints env recipe | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--config-path` | Both |
| `devilroute setup-aider` | Aider | `~/.aider.conf.yml` (`openai-api-base` + `model: openai/<id>`) + prints env recipe | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--config-path` | Both |
| `devilroute setup-qwen` | Qwen Code | `~/.qwen/settings.json` — V4 `modelProviders.openai` array + `DEVILROUTE_API_KEY` in `~/.qwen/.env` | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--config-path` `--env-path` | Both |
| `devilroute launch` | Claude Code | Nothing — spawns `claude` with `ANTHROPIC_BASE_URL`/`ANTHROPIC_AUTH_TOKEN` injected | `--remote` `--api-key` `--token` `--profile` `--port` | Both |
| `devilroute launch-codex` | OpenAI Codex CLI | Nothing — spawns `codex` with the `devilroute` provider injected via `-c` flags | `--remote` `--api-key` `--profile` (`-p`) `--port` | Both |

Notes on flags (verified in the command source):

- `--remote <url>` — fetch the catalog from a remote DevilRoute (overrides `--port`
  and the active context). `--api-key <key>` supplies the credential for that
  server (defaults to the `DEVILROUTE_API_KEY` env var, or the active context's token).
...
- `--port <port>` — local DevilRoute port (default `20128`, ignored when `--remote`
...
> There is also a richer plugin integration — `devilroute setup opencode` — which
> installs `@devilroute/opencode-plugin`. They are different commands; the table
...
With DevilRoute running on `localhost:20128`, just run the setup command for your
...
Point any setup command at a remote DevilRoute with `--remote` + `--api-key`. The
...
DevilRoute exposes the OpenAI surface at `/v1`, the Anthropic surface at the root,
...
| `setup-codex`, `launch-codex` (`model_providers.devilroute.base_url`) | with `/v1` | Yes |
...
When you update with `devilroute update` (after confirming, or with `--apply`),
DevilRoute runs the install with `--include=optional` baked in:

```bash
npm install -g devilroute@latest --include=optional
```

This is **not** a flag you pass to `devilroute update` — it is always applied by the
...
Other `devilroute update` flags (verified in source): `--check` (exit 1 if
...
- [Codex CLI configuration](./CODEX-CLI-CONFIGURATION.md) — the one-time `[model_providers.devilroute]` base setup
