...
This guide covers adding Kiro (AWS-hosted AI coding assistant) accounts to DevilRoute,
...
## How DevilRoute Solves This (v3.8.0+)

Starting with v3.8.0, DevilRoute calls `registerClient()` (AWS SSO OIDC) during every
Kiro connection import. This gives each DevilRoute connection its own dedicated OIDC
...
2. Choose one of:
   ...
   - **Auto-Import** — click the button; DevilRoute reads credentials from the
...
4. DevilRoute validates the key and saves the connection.
...
(`clientSecretExpiresAt`). DevilRoute stores this timestamp in `providerSpecificData`
...
- Ensure DevilRoute can reach `https://oidc.us-east-1.amazonaws.com` (or the configured
...
- The key must be able to call `ListAvailableProfiles`; otherwise DevilRoute cannot
