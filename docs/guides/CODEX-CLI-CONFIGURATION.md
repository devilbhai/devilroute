---
title: "Codex CLI — Configuration with DevilRoute"
version: 3.8.49
lastUpdated: 2026-07-26
---

# Codex CLI — Configuration with DevilRoute

Complete guide for using the Codex CLI pointed at DevilRoute as an OpenAI-compatible backend.
...
**DevilRoute solves this transparently:**

```
Codex CLI
  → wire_api = "responses"
  → POST /v1/responses (DevilRoute)
    → DevilRoute Responses ↔ Chat Completions transformer
    → POST /chat/completions (DeepSeek / Mistral / GLM / Kimi / any provider)
```

You never need a separate translation proxy when using DevilRoute. **All models use `wire_api = "responses"`** — DevilRoute handles the rest.
...
| Model                                | DevilRoute ID                         | Context window         | `auto_compact` | `tool_output_limit` |
| ------------------------------------ | ------------------------------------ | ---------------------- | -------------- | ------------------- |
...
All Codex models in DevilRoute use the `cx/` prefix:

| Codex CLI name          | DevilRoute model    |
| ----------------------- | ------------------ |
...
Other providers use their own prefix (`kmc/`, `glm/`, `ds/`, `ollamacloud/`, `opencode-go/`, `mistral/`) — the prefix matches the DevilRoute provider alias.
...
## Generating profiles automatically with `devilroute setup-codex`

If you run DevilRoute on a VPS, you can auto-generate profile files from the live model catalog:

```bash
# From a VPS (uses local DevilRoute on port 20128)
devilroute setup-codex

# From any machine — point at your VPS
devilroute setup-codex --remote http://100.x.x.x:20128 --api-key sk-xxx

# Preview without writing files
devilroute setup-codex --remote http://100.x.x.x:20128 --dry-run

# Only generate GLM and Kimi profiles
devilroute setup-codex --only glm,kimi

# Write to a custom directory
devilroute setup-codex --codex-home /path/to/.codex
```
...
DevilRoute can also **auto-sync** these same profile files after a successful provider model discovery/import changes the live catalog. This is **opt-in and off by default**: toggle it from the **CLI Code dashboard** ("CLI profile auto-sync" → Codex), or set `DEVILROUTE_AUTO_SYNC_CODEX_PROFILES=true` (it also honors `CLI_ALLOW_CONFIG_WRITES`, on by default). When enabled it only writes separate `~/.codex/*.config.toml` profile files; it never changes the active/default `~/.codex/config.toml`, Codex-lb settings, auth, or provider selection.

---

## Launching Codex with `devilroute launch-codex`

Health-checks your DevilRoute instance before launching Codex:

```bash
# Launch against local DevilRoute (default port 20128)
devilroute launch-codex

# Launch with a specific profile
devilroute launch-codex --profile kimi-k27

# Launch against a remote VPS
devilroute launch-codex --remote http://100.x.x.x:20128/v1 --api-key sk-xxx

# Pass extra args to codex
devilroute launch-codex --profile glm52 -- --yolo "fix this bug"
```
...
[model_providers.devilroute.http_headers]
"X-Custom-Header" = "value"

# Headers read from env vars
[model_providers.devilroute.env_http_headers]
"X-Trace-Id" = "TRACE_ID"

# Extra URL query params (useful for Azure api-version)
[model_providers.devilroute.query_params]
...
```toml
[model_providers.devilroute-main]
base_url = "http://192.168.0.1:20128/v1"
env_key  = "DEVILROUTE_API_KEY"

[model_providers.devilroute-tailscale]
base_url = "http://100.x.x.x:20128/v1"
env_key  = "DEVILROUTE_API_KEY"
```
...
Two DevilRoute defaults can silently sabotage multi-hour Codex CLI sessions. Neither is a Codex CLI setting — both live on the DevilRoute side. Users migrating a config from upstream proxies that pin accounts and disable idle cutoffs often hit both and conclude DevilRoute “cannot sustain a long session.”
...
Critical detail: DevilRoute’s synthetic SSE **heartbeat does not reset** the idle clock. Only a real upstream body chunk updates `lastChunkTime`. A quiet model that is still “thinking” looks identical to a stalled upstream from the watchdog’s point of view.
...
In the DevilRoute process environment (`.env` / compose / systemd):
...
Restart DevilRoute after changing these env vars.
...
2. **Raise / disable idle cutoffs** in DevilRoute’s environment:
...
3. Keep the usual Codex `config.toml` (`wire_api = "responses"`, correct `base_url`, `DEVILROUTE_API_KEY`) — no Codex-side affinity/idle knobs exist for these two behaviors.
4. Restart DevilRoute, then start the long Codex task.
...
When the idle watchdog fires, DevilRoute logs a line shaped like:

```text
[STREAM] Idle timeout: no data from codex for 600000ms (model: cx/gpt-5.5)
```

Grep for `Idle timeout: no data from` (or the code `stream_idle_timeout` / error name `StreamIdleTimeoutError`). The provider segment is whatever DevilRoute used for that request (`codex`, another provider id, or `provider` if unknown) — it is not always the literal string `codex`.
...
Verify the model exists in DevilRoute with the correct prefix. Use `devilroute models list` or open `/dashboard/providers/<provider>`.

**`Authentication error`**
Confirm `DEVILROUTE_API_KEY` is exported: `echo $DEVILROUTE_API_KEY`.

**`Connection refused`**
Verify DevilRoute is running and the `base_url` host/port is correct for your network (local vs Tailscale vs VPS).
...
See [Long-running tasks](#long-running-tasks). Enable session affinity (TTL above task length) and raise or disable `STREAM_IDLE_TIMEOUT_MS` / `FETCH_BODY_TIMEOUT_MS`. Grep DevilRoute logs for `Idle timeout: no data from`.
