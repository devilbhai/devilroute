---
title: "📖 Setup Guide — DevilRoute"
version: 3.8.40
lastUpdated: 2026-06-28
---

# 📖 Setup Guide — DevilRoute

> Complete setup reference for DevilRoute. For the quick version, see the [Quick Start in README](../README.md#-quick-start).
...
The [AUR package](https://aur.archlinux.org/packages/devilroute-bin) installs DevilRoute and provides a systemd user service.
...
DevilRoute ships a desktop wrapper built on Electron 41 + electron-builder 26.10. Available scripts (workspace root):
...
devilroute providers test-batch
```

Combined with env vars (`INITIAL_PASSWORD`, `DEVILROUTE_WS_BRIDGE_SECRET`, etc.), this lets you spin up an DevilRoute instance fully scriptable.
...
| `devilroute setup`       | Guided CLI onboarding for password and first provider          |
| `devilroute doctor`      | Run local health checks without starting the server            |
| `devilroute providers`   | Discover, list, validate, and test providers from CLI          |
| `devilroute config`      | CLI tool configuration — list, get, set, validate configs      |
| `devilroute status`      | Offline status dashboard — version, DB, tools, config          |
| `devilroute logs`        | Stream usage logs from the API (supports `--follow`)           |
| `devilroute update`      | Check for or apply DevilRoute updates                           |
| `devilroute provider`    | Manage provider connections — add, list, remove, test, default |
...
```bash
devilroute setup --non-interactive --password "$DEVILROUTE_PASSWORD"
devilroute setup --non-interactive --add-provider --provider openai --api-key "$OPENAI_API_KEY"
devilroute setup --non-interactive --add-provider --provider openai --api-key "$OPENAI_API_KEY" --test-provider
```
...
```bash
devilroute doctor
devilroute doctor --json
devilroute doctor --no-liveness
```
...
devilroute providers available --search openai
devilroute providers available --category api-key
devilroute providers list
devilroute providers test <id-or-name>
...
Instead of pasting the base URL and key by hand, let DevilRoute write each tool's
own config from the live model catalog. One command per tool:

```bash
devilroute setup-codex        # ~/.codex/<name>.config.toml profiles
devilroute setup-claude       # ~/.claude/profiles/<name>/settings.json
devilroute setup-opencode     # ~/.config/opencode/opencode.json (openai-compatible)
devilroute setup-cline        # Cline CLI + VS Code extension settings
devilroute setup-kilo         # Kilo Code
devilroute setup-continue     # ~/.continue/config.yaml (Continue / cn)
devilroute setup-cursor       # prints Cursor's in-app steps
devilroute setup-roo          # Roo Code import + autoImport pointer
devilroute setup-crush        # ~/.config/crush/crush.json
devilroute setup-goose        # ~/.config/goose/config.yaml
devilroute setup-aider        # ~/.aider.conf.yml
devilroute setup-qwen         # ~/.qwen/settings.json + ~/.qwen/.env
```

Each accepts `--remote <url> --api-key <key>` to configure a local tool against a
**remote** DevilRoute, plus `--dry-run` to preview. The launchers
...
devilroute_get_health        # Should return system health
devilroute_list_combos       # Should return active combos
...
For Claude Code-compatible upstreams (`anthropic-compatible-cc-*`), DevilRoute derives the outbound `X-Stainless-Timeout` header from the resolved fetch timeout so provider-side read timeouts stay aligned with your env configuration.

For third-party Claude Code-compatible reverse proxies, DevilRoute keeps the default `anthropic-beta` set conservative and, when `Client Cache Control` is left on `Auto`, only forwards client-provided `cache_control` markers. Enable the per-connection "Enable redact-thinking beta" toggle only when the upstream specifically requires redacted Claude thinking streams.
...
> **Note:** For streaming requests, `FETCH_TIMEOUT_MS` only covers connection setup / waiting for the first upstream response. Once the stream is active, DevilRoute will only abort on an actual stall (`STREAM_IDLE_TIMEOUT_MS`) or Undici body inactivity (`FETCH_BODY_TIMEOUT_MS`).

### Reverse Proxy Compatibility

If you run DevilRoute behind Nginx, Caddy, Cloudflare, or another reverse proxy, make sure the proxy timeouts are also higher than your DevilRoute stream/fetch timeouts.
