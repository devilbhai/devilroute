---
title: "🐳 Docker Guide — DevilRoute"
version: 3.8.40
lastUpdated: 2026-06-28
---

# 🐳 Docker Guide — DevilRoute
...
DevilRoute ships four Compose profiles. Pick the one that matches your environment.

| Profile          | Service          | When to use                                                                                                                       | Command                                      |
| ---------------- | ---------------- | --------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------- |
| `base` (default) | `devilroute-base` | Headless server / minimal runtime, no provider CLIs bundled                                                                       | `docker compose --profile base up -d`        |
| `cli`            | `devilroute-cli`  | Agentic workflows that call `devilroute providers/setup/doctor` and bundled CLIs (Codex, Claude Code, Droid, OpenClaw)             | `docker compose --profile cli up -d`         |
| `host`           | `devilroute-host` | Linux hosts that want `network_mode`-like access to host CLIs by mounting `~/.local/bin`, `~/.codex`, `~/.claude`, etc. read-only | `docker compose --profile host up -d`        |
...
DevilRoute relies on Redis to back the distributed rate limiter and shared cache. The `redis` service is **always defined** in `docker-compose.yml` (it has no profile gate) and starts alongside any other profile.
...
Defaults exported by `runner-base`: `PORT=20128`, `HOSTNAME=0.0.0.0`, `NODE_OPTIONS=--max-old-space-size=512`, `DATA_DIR=/app/data`, `DEVILROUTE_MIGRATIONS_DIR=/app/migrations`.

Memory behavior in Docker:

- `NODE_OPTIONS=--max-old-space-size=512` is baked into the image as a fallback.
- The actual server process is started by the standalone launcher, which reads `DEVILROUTE_MEMORY_MB` and appends `--max-old-space-size=<DEVILROUTE_MEMORY_MB>`.
- Node uses the last repeated `--max-old-space-size` value, so setting `DEVILROUTE_MEMORY_MB` controls the effective Docker heap limit.
- If `DEVILROUTE_MEMORY_MB` is unset, the launcher uses `512`.
...
| ----------------------------- | --------------------------------------------------------------------------------------------------- | ------------------------ |
| `DEVILROUTE_WS_BRIDGE_SECRET`  | Shared secret for the WebSocket bridge. **Required in production** — set to a strong random string. | unset (must be provided) |
...
| `AUTO_UPDATE_HOST_REPO_DIR`   | Host path mounted into `cli` profile at `/workspace/devilroute` for self-update workflows            | `.` (current directory)  |
| `DEVILROUTE_MEMORY_MB`         | Runtime Node heap ceiling for the Docker standalone server; overrides the image fallback above      | `512`                    |
| `DASHBOARD_PORT` / `API_PORT` | Override exposed ports for dashboard (20128) and API (20129)                                        | `20128` / `20129`        |
| `DEVILROUTE_BASE_PATH`         | URL subpath when the app is published behind a reverse proxy (e.g. `/devilroute`)                    | _(empty = root)_         |
| `NEXT_PUBLIC_BASE_URL`        | Public browser origin including the subpath (e.g. `https://host/devilroute`)                         | unset                    |
...
Next.js `basePath` is compiled into the standalone bundle. DevilRoute records the baked
...
`DEVILROUTE_BASE_PATH` when the container starts. When they differ and the image was
...
`docker-compose.yml` forwards `DEVILROUTE_BASE_PATH` as a Docker build-arg and as a
...
prefix). Traefik should route `PathPrefix(`/devilroute`)` to the container without
`StripPrefix`, so Next.js receives `/devilroute/...` and serves assets from
`/devilroute/_next/...`.
...
Caddy sets the standard forwarding headers for the upstream container. DevilRoute uses
...
protection. Only enable `DEVILROUTE_TRUST_PROXY` for advanced deployments where you intentionally
want DevilRoute to derive the public origin from trusted forwarded headers instead of explicit
configuration.
...
- **SQLite WAL Mode:** `docker stop` should be allowed to finish so DevilRoute can checkpoint the latest changes back into `storage.sqlite`. The bundled Compose files already set a 40s stop grace period. If you run the image directly, keep `--stop-timeout 40`.
