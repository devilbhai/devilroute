---
title: "Remote Mode — Drive a remote DevilRoute from your laptop"
version: 3.8.40
...
Run the `devilroute` CLI on your laptop while DevilRoute itself runs somewhere else
(a VPS, a home server, another machine on your Tailnet). You log in once with
`devilroute connect`, and from then on **every** CLI command targets that remote
server — same commands, same output, just executed against the remote.

There is no second tool to install: remote mode is the regular `devilroute` CLI
...
- **Contexts** store one server each (`~/.devilroute/config.json`, `chmod 600`).
  `devilroute contexts use <name>` switches the active server; `default` is local.
...
Generate a scoped token in the dashboard (or with `devilroute tokens create`) and
...
that uses the bundled Antigravity desktop client, not just DevilRoute.)
...
There are two supported ways to connect Antigravity to a remote DevilRoute.
...
paste the `devilroute-cred-v1.…` blob into the **Step 2** field (it accepts either
a callback URL or a credential blob). DevilRoute decodes it, runs the Cloud Code
...
`redirect_uri` with their upstream OAuth app. DevilRoute cannot change it — the
...
`localhost` there means **the machine running the browser**, while DevilRoute's PKCE
...
- **`20128`** (the dashboard port) makes the origin true-localhost, which is what
  makes DevilRoute start the PKCE callback server at all — a LAN origin never
  reaches that branch.
...
`devilroute configure` reads the **active server's** live model catalog and writes
...
(`DEVILROUTE_API_KEY`) — the secret is never written to disk. For the one-time
base Codex setup (the `[model_providers.devilroute]` block), see
...
| ----------- | -------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Codex       | `devilroute setup-codex`    | `~/.codex/<name>.config.toml` profiles (per model)                                                                                                                   |
| Claude Code | `devilroute setup-claude`   | `~/.claude/profiles/<name>/settings.json` (per model)                                                                                                                |
| OpenCode    | `devilroute setup-opencode` | `~/.config/opencode/opencode.json` — the `devilroute` openai-compatible provider with every catalog model (run `opencode -m devilroute/<model>`)                       |
| Cline       | `devilroute setup-cline`    | `~/.cline/data/{globalState,secrets}.json` (CLI mode) + prints the VS Code extension settings to paste (OpenAI-compatible, Base URL **without** `/v1`)               |
| Kilo Code   | `devilroute setup-kilo`     | `~/.local/share/kilo/auth.json` (CLI) + VS Code `kilocode.*` settings — OpenAI-compatible, Base URL **with** `/v1`                                                   |
| Continue    | `devilroute setup-continue` | `~/.continue/config.yaml` (VS Code/JetBrains + `cn` CLI) — `provider: openai`, `apiBase` **with** `/v1`, key via `${{ secrets.DEVILROUTE_API_KEY }}`                  |
| Cursor      | `devilroute setup-cursor`   | prints the in-app steps (Settings → Models → Override OpenAI Base URL **with** `/v1` + key + model). Cursor config is opaque SQLite — chat panel only                |
| Roo Code    | `devilroute setup-roo`      | writes a Roo import JSON (`~/.devilroute/roo-settings.json`) + sets `roo-cline.autoImportSettingsPath` + prints UI steps (OpenAI-compatible, Base URL **with** `/v1`) |
| Crush       | `devilroute setup-crush`    | `~/.config/crush/crush.json` — `openai-compat` provider, `base_url` **with** `/v1`, key via `$DEVILROUTE_API_KEY`                                                     |
| Goose       | `devilroute setup-goose`    | `~/.config/goose/config.yaml` (`GOOSE_PROVIDER=openai` + `OPENAI_HOST` **without** `/v1` + `GOOSE_MODEL`) + env recipe                                               |
| Aider       | `devilroute setup-aider`    | `~/.aider.conf.yml` (`openai-api-base` **without** `/v1` + `model: openai/<id>`) + env recipe (`aider --message --yes`)                                              |
| Qwen Code   | `devilroute setup-qwen`     | `~/.qwen/settings.json` V4 `modelProviders.openai` entry + `DEVILROUTE_API_KEY` in `~/.qwen/.env`                                                                     |
...
> OpenCode also has a richer **plugin** integration: `devilroute setup opencode`
> (now remote-aware via `--remote`) installs `@devilroute/opencode-plugin`.
> `setup-opencode` is the lightweight openai-compatible alternative. The API key
> is referenced via `{env:DEVILROUTE_API_KEY}` — never written to disk.

---

## Managing contexts (switch between servers)

A **context** is a saved server (baseUrl + credential + scope). `devilroute connect`
creates one and makes it active; from then on every command targets it. Manage and
switch between them with `devilroute contexts`:
...
> revoke the token on the server with `devilroute tokens revoke <id>` to actually
> kill access.
...
- `devilroute connect` reuses the login brute-force lockout + audit logging.
...
- The local context file is `~/.devilroute/config.json` (`chmod 600`); tokens are
