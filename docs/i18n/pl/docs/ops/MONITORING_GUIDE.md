...
> **TL;DR**: DevilRoute dostarcza wbudowane monitorowanie kondycji, autopilota providerów, śledzenie limitów (quota) oraz haki obserwowalności. Ten przewodnik obejmuje dashboard, alerty i rozwiązywanie problemów.
...
DevilRoute ma **3 warstwy monitorowania**:
...
│  └─ Narzędzia MCP: devilroute_get_health, devilroute_get_session_snapshot │
└──────────────────────────────────────────────────────────────┘
```
...
DevilRoute obsługuje **3 kanały alertów**:
...
Utwórz `~/.devilroute/dashboard.json`:
...
- Śledzenie quota w DevilRoute może być **nieaktualne** — prawda providera jest upstream
...
- Uruchom `sqlite3 ~/.devilroute/storage.sqlite "PRAGMA integrity_check;"`
- Jeśli "ok" — fałszywy alarm, health check jest zbyt rygorystyczny
- Jeśli cokolwiek innego — **zatrzymaj DevilRoute** i postępuj według [przewodnika disaster recovery](./DATABASE_GUIDE.md#disaster-recovery)
