---
title: "Przewodnik wdrażania DevilRoute na Fly.io"
version: 3.8.40
lastUpdated: 2026-06-28
---

# Przewodnik wdrażania DevilRoute na Fly.io

Ten dokument opisuje rzeczywisty proces wdrażania DevilRoute na Fly.io i obejmuje dwa scenariusze:
...
Przewodnik opiera się na zweryfikowanej, działającej konfiguracji bieżącego projektu. Nazwa aplikacji to `devilroute`.
...
- Adres dostępu: `https://devilroute.fly.dev/`
...
- `app = 'devilroute'` określa, do której aplikacji Fly kierowane jest wdrożenie
...
```toml
app = 'devilroute-yourname'
```

Uwaga:

- Upewnij się, że aplikacja widoczna w konsoli odpowiada wartości `app` w `fly.toml`
- Jeśli wcześniej używałeś innej nazwy, np. `oroute`, nie myl jej z `devilroute`
...
```powershell
flyctl apps create devilroute
```

Jeśli zmieniłeś nazwę aplikacji, zastąp `devilroute` wybraną nazwą.
...
Te parametry były używane w rzeczywistych wdrożeniach bieżącej aplikacji `devilroute`:
...
- `DEVILROUTE_WS_BRIDGE_SECRET` (wymagany w produkcji — służy do uwierzytelniania mostu WebSocket)
...
| `DEVILROUTE_WS_BRIDGE_SECRET`     | Wymagana w produkcji | Sekret uwierzytelniania mostu WebSocket                    |
...
| ---------------------- | --------------------------- |
| `DATA_DIR`             | `/data`                     |
| `NEXT_PUBLIC_BASE_URL` | `https://devilroute.fly.dev` |
...
1. **Ustaw `NEXT_PUBLIC_BASE_URL` na publiczną domenę HTTPS**
   ...
   ```powershell
   flyctl secrets set NEXT_PUBLIC_BASE_URL=https://devilroute.fly.dev -a devilroute
   ```

   Jeśli używasz własnej domeny, zastąp ją odpowiednią domeną (np. `https://devilroute.yourdomain.com`).
...
2. **Skonfiguruj URL callbacku w konsoli dostawcy**
   ...
   - `https://devilroute.fly.dev/callback`
...
- Przeznaczone dla bieżącego projektu `devilroute`
...
flyctl secrets set `
  ...
  DEVILROUTE_WS_BRIDGE_SECRET=$wsBridgeSecret `
  DATA_DIR=/data `
  NEXT_PUBLIC_BASE_URL=https://devilroute.fly.dev `
  -a devilroute
```
...
```bash
flyctl secrets set DEVILROUTE_WS_BRIDGE_SECRET=$(openssl rand -hex 32) -a devilroute
```

Uwagi:

- `DEVILROUTE_WS_BRIDGE_SECRET` jest wymagany w produkcji; jego brak zepsuje handshake mostu WebSocket

Jeśli chcesz też ustawić hasło początkowe:

```powershell
flyctl secrets set INITIAL_PASSWORD=your-strong-password -a devilroute
```
...
```powershell
flyctl secrets set KEY=value -a devilroute
```
...
Jeśli bieżące repozytorium jest forkiem i chcesz synchronizować aktualizacje z upstream `https://github.com/diegosouzapw/DevilRoute`, postępuj według poniższego przepływu.
...
5. `flyctl deploy`
6. `flyctl status -a devilroute`
7. `flyctl logs --no-tail -a devilroute`
...
```powershell
flyctl status -a devilroute
```

### 10.2 Podgląd logów startowych

```powershell
flyctl logs --no-tail -a devilroute
```
...
```powershell
try {
  (Invoke-WebRequest -Uri "https://devilroute.fly.dev" -MaximumRedirection 5 -UseBasicParsing).StatusCode
} catch {
...
- Przeglądasz inną aplikację (np. `oroute` zamiast `devilroute`)
...
1. `flyctl auth login`
2. `flyctl apps create devilroute`
3. `flyctl secrets set ... -a devilroute`
4. `flyctl deploy`
5. `flyctl logs --no-tail -a devilroute`
