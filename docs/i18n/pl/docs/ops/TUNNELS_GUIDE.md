...
DevilRoute może udostępnić swój lokalny serwer (`http://localhost:20128`) w publicznym
...
Wszystkie trzy backendy są zarządzane w procesie — DevilRoute uruchamia/zatrzymuje
...
- **Auto-install.** Przy pierwszym użyciu DevilRoute pobiera najnowszą binarkę
...
login` + `cloudflared tunnel route dns ...`) nie są orkiestrowane przez
  DevilRoute. URL-e są efemeryczne i zmieniają się przy każdym restarcie.
...
| Variable                                             | Purpose                                                                                           |
| ---------------------------------------------------- | ------------------------------------------------------------------------------------------------- |
| `CLOUDFLARED_BIN`                                    | Nadpisuje ścieżkę do binarki. Jeśli ustawiona i poprawna, DevilRoute używa jej zamiast pobierania. |
...
trzeba skonfigurować w dashboardzie ngrok — sam DevilRoute tylko przekazuje
...
1. Zainstaluj Tailscale (albo pozwól DevilRoute to zrobić — patrz endpoint `install` poniżej).
2. Zaloguj się (`tailscale login` lub przez endpoint `login` DevilRoute).
...
cache haseł MITM DevilRoute (`getCachedPassword` / `setCachedPassword`) na
...
Gdy udostępniasz DevilRoute przez tunel, dashboard i przepływy OAuth muszą
...
i zrestartuj DevilRoute przed rozpoczęciem OAuth. Dla efemerycznych Cloudflare Quick
...
uruchomienie więcej niż jednego backendu jest dozwolone; DevilRoute śledzi każdy
niezależnie.
...
DevilRoute próbuje auto-instalacji przy pierwszym użyciu. Jeśli instalacja jest zablokowana
