...
DevilRoute wybiera sterownik SQLite przy starcie w 5-stopniowym łańcuchu fallback:
...
2. **`better-sqlite3` zainstalowany w runtime** (w `~/.devilroute/runtime/`)
...
- **Windows EBUSY**: `npm install -g devilroute@latest` może się nie udać, jeśli
  `better_sqlite3.node` poprzedniej wersji jest zablokowany przez działający proces. Instalacja
  runtime w `~/.devilroute/runtime/` omija globalną pamięć podręczną npm.
- **Brak narzędzi do budowania**: Niektóre środowiska (korporacyjny Windows bez VS Build
  ...
  gwarantują, że DevilRoute i tak się uruchomi, nawet gdy to się nie uda.
...
Przed załadowaniem pliku `.node` zainstalowanego w runtime DevilRoute odczytuje pierwsze 8
...
DEVILROUTE_SKIP_POSTINSTALL=1 npm install -g devilroute

# Force-reinstall runtime better-sqlite3
rm -rf ~/.devilroute/runtime
devilroute  # will reinstall on next start

# Check what driver is active
devilroute config db-info  # (if CLI command exists)
```
