...
> **TL;DR**: DevilRoute używa **SQLite z journalingiem WAL** jako głównego magazynu, z szyfrowaniem **AES-256-GCM** w spoczynku dla wrażliwych pól. Ten przewodnik obejmuje schemat, migracje, kopie zapasowe/odzyskiwanie oraz runbooki operacyjne.
...
DevilRoute wybrał SQLite zamiast PostgreSQL/MySQL z kilku powodów:
...
Dla wdrożeń **jednoużytkownikowych, jednainstancyjnych** (główny przypadek użycia DevilRoute) SQLite jest prostszy i szybszy.
...
| OS      | Ścieżka                                                      |
| ------- | ------------------------------------------------------------ |
| Linux   | `~/.devilroute/storage.sqlite`                                |
| macOS   | `~/.devilroute/storage.sqlite`                                |
| Windows | `%USERPROFILE%\.devilroute\storage.sqlite`                    |
...
```bash
DATA_DIR=/custom/path devilroute
```
...
Baza DevilRoute ma **94 moduły domenowe** w `src/lib/db/`. Każdy moduł:
...
DevilRoute ma **94 pliki modułów** w `src/lib/db/`. Poniżej próbka kluczowych modułów; pełna lista w listingu katalogu:
...
DevilRoute używa **wersjonowanych, idempotentnych migracji** w `src/lib/db/migrations/`. Każda migracja to pojedynczy plik SQL o nazwie `NNN_description.sql`.
...
1. Tworzy tabelę `_devilroute_migrations`, jeśli nie istnieje
...
> **Zmiany niekompatybilne wstecz** (np. usuwanie kolumn) są trudne. DevilRoute NIE wspiera downgrade — po zastosowaniu migracji zmiana schematu jest trwała. Planuj odpowiednio.
...
DevilRoute używa **`migrateLegacyEncryptedString()`**, aby przezroczyście obsługiwać dwa schematy szyfrowania:
...
devilroute backup create --name pre-migration
...
```bash
# Via CLI
devilroute restore pre-migration
...
devilroute backup auto enable --cron "0 2 * * *" --retention 7
```
...
| ------------------------------------------- | --------- | ------------------------------------------------------------------------------------------------------------ |
| `DEVILROUTE_BACKUP_SCHEDULE_JOB_INTERVAL_MS` | `30000`   | Interwał ticka w ms (min `5000`). Musi być krótszy niż 60 s, aby niezawodnie trafiać w pasującą minutę cron. |
...
```bash
sqlite3 ~/.devilroute/storage.sqlite ".backup /backups/devilroute-hot.db"
```

Używa online backup API SQLite — bezpieczne podczas działania DevilRoute.
...
Długo działające instancje DevilRoute zyskują na okazjonalnym `VACUUM`:

```bash
sqlite3 ~/.devilroute/storage.sqlite "VACUUM;"
```
...
```bash
sqlite3 ~/.devilroute/storage.sqlite "PRAGMA integrity_check;"
# Should print: ok
```
...
```bash
sqlite3 ~/.devilroute/storage.sqlite ".recover" > recovered.sql
sqlite3 recovered.db < recovered.sql
mv recovered.db ~/.devilroute/storage.sqlite
```
...
```bash
devilroute sync pull --merge   # or: devilroute backup restore <backup-id>
```
...
sqlite3 ~/.devilroute/storage.sqlite "PRAGMA wal_checkpoint(TRUNCATE);"
```
...
```bash
sqlite3 ~/.devilroute/storage.sqlite "SELECT * FROM api_keys LIMIT 5;"
```

### Liczba wierszy we wszystkich tabelach

```bash
sqlite3 ~/.devilroute/storage.sqlite <<EOF
...
```bash
# Stop DevilRoute first
devilroute stop

# Delete the DB file
rm ~/.devilroute/storage.sqlite*
...
- Zabij drugi proces
- Jeśli problem się utrzymuje, zrestartuj DevilRoute
...
1. **Zatrzymaj DevilRoute** (zapobiegaj dalszym próbom)
2. **Sprawdź stan DB** przez `sqlite3`
3. **Napraw ręcznie** częściową migrację
4. **Uruchom ponownie** DevilRoute (migracja zostanie ponowiona)
