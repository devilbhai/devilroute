---
title: "🌐 Przewodnik po proxy DevilRoute"
version: 3.8.40
lastUpdated: 2026-06-28
---

# 🌐 Przewodnik po proxy DevilRoute

> **Omijaj blokady geograficzne, chroń tożsamość i kieruj ruch AI przez dowolne proxy — bez złożonej konfiguracji.**

DevilRoute zawiera pełny system zarządzania proxy, który pozwala kierować ruch do upstreamowych dostawców AI przez proxy HTTP, HTTPS lub SOCKS5. Niezależnie od tego, czy jesteś w zablokowanym regionie, potrzebujesz rotacji IP, czy fingerprintingu stealth — ten przewodnik obejmuje wszystko.
...
DevilRoute obsługuje konfigurację proxy w **czterech niezależnych zakresach**, rozwiązywanych w kolejności priorytetu:
...
Gdy DevilRoute wysyła żądanie do upstreamowego dostawcy, wywołuje `resolveProxyForConnectionFromRegistry()`, które sprawdza kolejne poziomy w kolejności:
...
Proxy są uwzględnione w systemie **Backup/Restore**. Gdy eksportujesz konfigurację DevilRoute:
...
Jeśli skonfigurowałeś proxy w starszej wersji (przed rejestrem), DevilRoute migruje je automatycznie:
...
1. **Sync** — DevilRoute pobiera zwalidowane proxy z API 1proxy
...
DevilRoute nie tylko kieruje ruch przez proxy — sprawia, że ruch wygląda na legalny:
...
Dla dostawców używających wzorca CLIProxyAPI DevilRoute obsługuje trzy tryby upstream proxy:

| Tryb          | Opis                                              |
| ------------- | ------------------------------------------------- |
| `native`      | DevilRoute sam obsługuje routing proxy (domyślnie) |
...
To normalne przy tanich proxy zrywających idle connections. DevilRoute już to obsługuje przez:
...
Upewnij się, że proxy jest skonfigurowane **przed** startem flow OAuth. DevilRoute kieruje wymianę tokenów OAuth przez skonfigurowane proxy. Najpierw ustaw globalne lub provider-level proxy, potem połącz.
...
Mechanizm **proxy fast-fail** DevilRoute (`src/lib/proxyHealth.ts`) wykrywa martwe proxy w <2s szybkim sprawdzeniem połączenia TCP, potem **cache’uje wynik**, by uniknąć narzutu na każde żądanie.
...
```ts
import { getAllProxyHealthStatuses, invalidateProxyHealth } from "devilroute/proxyHealth";
...
DevilRoute śledzi użycie per-proxy, by operatorzy mogli diagnozować wzorce routingu, skoki latencji i powtarzające się awarie.

### Co jest śledzone

Dla każdego żądania przez skonfigurowane proxy DevilRoute zapisuje:
...
```bash
# Recent proxy events
curl -H "Authorization: Bearer $DEVILROUTE_KEY" \
...
Gdy do zakresu przypisano wiele proxy, DevilRoute używa **strategii rotacji**, by wybrać, którego użyć na każde żądanie. Strategia jest konfigurowana na poziomie zakresu (global, per-provider, per-account, per-combo).
...
```ts
import { rotateOneproxyProxy } from "devilroute/oneproxyRotator";
...
```ts
import { resetSequentialIndex } from "devilroute/oneproxyRotator";
...
```ts
import { failOneproxyProxy } from "devilroute/oneproxyRotator";
