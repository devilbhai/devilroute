---
title: "DevilRoute — Przewodnik wdrożenia na VM z Cloudflare"
version: 3.8.40
lastUpdated: 2026-06-28
---

# DevilRoute — Przewodnik wdrożenia na VM z Cloudflare
...
Kompletny przewodnik instalacji i konfiguracji DevilRoute na VM (VPS) z domeną zarządzaną przez Cloudflare.
...
## 2. Instalacja DevilRoute

### 2.1 Utwórz katalog konfiguracji

```bash
mkdir -p /opt/devilroute
```

### 2.2 Utwórz plik zmiennych środowiskowych

```bash
cat > /opt/devilroute/.env << 'EOF'
# === Security ===
...
DEVILROUTE_WS_BRIDGE_SECRET=REPLACE-WITH-WS-BRIDGE-SECRET  # REQUIRED em produção: usado pelo Codex Responses WS bridge
...
# DEVILROUTE_PUBLIC_BASE_URL=https://llms.seudominio.com

# === Cloud Sync (optional) ===
# CLOUD_URL=https://cloud.devilroute.online
# NEXT_PUBLIC_CLOUD_URL=https://cloud.devilroute.online
EOF
```
...
```bash
docker ps | grep devilroute
docker logs devilroute --tail 20
```
...
```bash
cat > /etc/nginx/sites-available/devilroute << 'NGINX'
...
Utrzymuj limity czasu strumienia reverse proxy w zgodzie ze zmiennymi timeout DevilRoute. Jeśli podniesiesz
`FETCH_TIMEOUT_MS` / `STREAM_IDLE_TIMEOUT_MS`, podnieś też `proxy_read_timeout` / `proxy_send_timeout`
powyżej tego samego progu.

DevilRoute używa `NEXT_PUBLIC_BASE_URL` jako kanonicznego, przeglądarkowego originu dla callbacków OAuth
...
`DEVILROUTE_TRUST_PROXY` tylko wtedy, gdy DevilRoute nie jest bezpośrednio osiągalny przez klientów, a Twój proxy
...
# Enable DevilRoute
ln -sf /etc/nginx/sites-available/devilroute /etc/nginx/sites-enabled/devilroute
...
```bash
docker logs -f devilroute          # Real-time stream
docker logs devilroute --tail 50   # Last 50 lines
```
...
docker cp devilroute:/app/data ./backup-$(date +%F)

# Or compress the entire volume
docker run --rm -v devilroute-data:/data -v $(pwd):/backup \
  alpine tar czf /backup/devilroute-data-$(date +%F).tar.gz /data
```

### Przywracanie z kopii zapasowej

```bash
docker stop devilroute
docker run --rm -v devilroute-data:/data -v $(pwd):/backup \
  alpine sh -c "rm -rf /data/* && tar xzf /backup/devilroute-data-YYYY-MM-DD.tar.gz -C /"
docker start devilroute
```
...
Zobacz też [TUNNELS_GUIDE.md](./TUNNELS_GUIDE.md) — przewodnik po Cloudflare Tunnel w tym repozytorium. Samodzielny worker `devilrouteCloud/` znajduje się w osobnym repozytorium towarzyszącym.
...
| 20128 | DevilRoute   | Tylko localhost (przez nginx) |
