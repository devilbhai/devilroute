...
1. `npm stage list devilroute` — znajdź stage id (wypisywany też w podsumowaniu workflow).
...
`devilroute` w trybie stage-only, żeby wycieknięty długotrwały token nie mógł `npm publish`
bezpośrednio skądkolwiek — CI może tylko stage'ować; tylko 2FA właściciela wypuszcza.

**Playbook zepsutego artefaktu (bez zmian):** `npm deprecate devilroute@<bad> "<reason> — use <fixed>"`
...
- [ ] Jeśli źródłowe angielskie docs się zmieniły, uruchom `npm run i18n:run` (wymaga `DEVILROUTE_TRANSLATION_API_KEY` w `.env`) przed tagowaniem
...
- [ ] Test na świeżej instalacji: usuń `~/.devilroute/devilroute.db` i uruchom `npm run dev`
...
> **Notatka operatorska:** zdalny katalog obrazu VPS pozostaje `/usr/lib/node_modules/devilroute/app/`.
...
- [ ] `devilroute --tray` bootuje na macOS (systray2 instalowany do `~/.devilroute/runtime/`)
- [ ] `devilroute --tray` bootuje na Linux (wymaga DISPLAY; graceful error jeśli nie ustawione)
- [ ] `devilroute --tray` bootuje na Windows (PowerShell NotifyIcon, bez dodatkowych binarek)
- [ ] `devilroute config tray enable` tworzy wpis autostart; disable go usuwa
- [ ] `npm install -g devilroute@<this-version>` uruchamia postinstall bez fatalnego wyjścia
- [ ] Ścieżka update zachowuje optional deps: `devilroute update --apply` i auto-updater
...
- [ ] `devilroute status` działa bez `.env` (ścieżka tokenu CLI, tylko loopback)
...
- [ ] Wszystkie 10 plików `skills/devilroute*/SKILL.md` są publicznie pobieralne przez raw GitHub URL
