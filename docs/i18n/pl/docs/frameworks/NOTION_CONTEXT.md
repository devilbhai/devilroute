...
DevilRoute może połączyć się z workspace'em **Notion** jako **źródłem kontekstu** — bazą
...
oraz dopisywać nowe bloki — wszystko proksowane przez DevilRoute (z retry, timeoutem
...
> strony/bazy, do których DevilRoute ma mieć dostęp (model uprawnień Notion opiera się
...
`POST` DevilRoute zapisuje token i od razu wykonuje testowe wyszukiwanie z 1 wynikiem;
...
`open-sse/mcp-server/server.ts` tylko gdy `DEVILROUTE_MCP_ENFORCE_SCOPES=true`;
dozwolone scope'y wywołującego pochodzą z `DEVILROUTE_MCP_SCOPES` (rozdzielone
