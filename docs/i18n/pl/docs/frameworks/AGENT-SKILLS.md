---
title: "Katalog Agent Skills DevilRoute"
version: 3.8.40
lastUpdated: 2026-06-28
---

# Katalog Agent Skills DevilRoute

> **Source of truth:** `src/lib/agentSkills/` (catalog, generator, parsers) + `skills/` directory (SKILL.md files)
> **Last updated:** 2026-06-28 — v3.8.40

Agent Skills to ustrukturyzowane pliki SKILL.md, które uczą zewnętrzne agenty, klientów MCP oraz orkiestratory A2A, jak korzystać z REST API i CLI DevilRoute. W przeciwieństwie do [Omni Skills](./SKILLS.md) (które są definicjami narzędzi LLM wykonywanymi wewnątrz DevilRoute), Agent Skills to _katalog dokumentacji_ — statyczny markdown, który można wstrzyknąć bezpośrednio do kontekstu agenta.
...
- **Surowy URL GitHub** do pobrania na żywo: `https://raw.githubusercontent.com/diegosouzapw/DevilRoute/refs/heads/main/skills/{id}/SKILL.md`
...
| Tool                              | Description                                        |
| :-------------------------------- | :------------------------------------------------- |
| `devilroute_agent_skills_list`     | List skills (optional `category` / `area` filters) |
| `devilroute_agent_skills_get`      | Get metadata + SKILL.md for one skill by `id`      |
| `devilroute_agent_skills_coverage` | Coverage stats (API/CLI have/total)                |
...
| :------------------- | :----------------- | :---------------------- |
| `cli-serve`          | cli-serve          | `devilroute serve`       |
| `cli-health`         | cli-health         | `devilroute health`      |
| `cli-providers`      | cli-providers      | `devilroute providers`   |
| `cli-keys`           | cli-keys           | `devilroute keys`        |
| `cli-models`         | cli-models         | `devilroute models`      |
| `cli-chat`           | cli-chat           | `devilroute chat`        |
| `cli-routing`        | cli-routing        | `devilroute routing`     |
| `cli-resilience`     | cli-resilience     | `devilroute resilience`  |
| `cli-compression`    | cli-compression    | `devilroute compression` |
| `cli-contexts`       | cli-contexts       | `devilroute contexts`    |
| `cli-cost-usage`     | cli-cost-usage     | `devilroute cost`        |
| `cli-mcp`            | cli-mcp            | `devilroute mcp`         |
| `cli-a2a`            | cli-a2a            | `devilroute a2a`         |
| `cli-tunnel`         | cli-tunnel         | `devilroute tunnel`      |
| `cli-backup-sync`    | cli-backup-sync    | `devilroute backup`      |
| `cli-policy-audit`   | cli-policy-audit   | `devilroute policy`      |
| `cli-batches`        | cli-batches        | `devilroute batch`       |
| `cli-eval`           | cli-eval           | `devilroute eval`        |
| `cli-plugins-skills` | cli-plugins-skills | `devilroute plugins`     |
| `cli-setup`          | cli-setup          | `devilroute setup`       |
...
curl "http://your-devilroute/api/agent-skills" | jq '.skills[] | {id, name, category}'

# Get SKILL.md for context injection
curl "http://your-devilroute/api/agent-skills/omni-providers/raw" > omni-providers.md
```
...
const result = await client.callTool("devilroute_agent_skills_list", { category: "api" });
...
resp = requests.post("http://your-devilroute/a2a", json={
    "jsonrpc": "2.0", "id": "1",
    "method": "message/send",
...
```bash
BASE="https://raw.githubusercontent.com/diegosouzapw/DevilRoute/refs/heads/main/skills"
...
- [MCP-SERVER.md](./MCP-SERVER.md) — katalog narzędzi MCP (narzędzia `devilroute_agent_skills_*`)
