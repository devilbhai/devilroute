...
DevilRoute zapewnia trwałą pamięć konwersacyjną powiązaną z kluczem API (oraz
...
> Klient może wyłączyć pamięć dla pojedynczego żądania nagłówkiem `x-devilroute-no-memory`
...
- `searchSemanticMemory(query, topK, scope)` — embeduje zapytanie, przeszukuje
  kolekcję filtrowaną po `kind = "devilroute_memory"` oraz opcjonalnie po
...
`qdrantApiKey`, `qdrantCollection` domyślnie `"devilroute_memory"`,
...
- `devilroute_memory_search` — `{apiKeyId, query?, type?, maxTokens?, limit?}`
...
- `devilroute_memory_add` — `{apiKeyId, sessionId?, type, key, content,
metadata?}` → opakowuje `createMemory()`. Przyjmuje tylko 4 kanoniczne typy:
  `factual`, `episodic`, `procedural`, `semantic` (D17).
- `devilroute_memory_clear` — `{apiKeyId, type?, olderThan?}` → listuje pasujące
...
- Do twardego usunięcia użyj `DELETE /api/memory/[id]` lub `devilroute_memory_clear`.
...
Silnik memory DevilRoute wspiera **cztery źródła embeddingów** (`src/lib/memory/embedding/`). Każde ma inne kompromisy w **latencji, koszcie, jakości modelu i złożoności setupu**.
