...
Traffic Inspector to wbudowany debugger ruchu HTTPS w DevilRoute — narzędzie w stylu Charles Proxy / mitmweb / HTTP Toolkit, które jest **świadome LLM** (**LLM-aware**) i **świadome agentów** (**agent-aware**). Znajduje się pod `/dashboard/tools/traffic-inspector` i odbiera ruch na żywo z maksymalnie 5 jednoczesnych źródeł przechwytywania.
...
| **Zintegrowany z DevilRoute** routing, fallback, cost                   |    ✗    |    ✗    |    ✗    |                ✓                |
...
Jeśli DevilRoute crashnie przy aktywnym trybie proxy systemowego:
...
| POST   | `/requests/{id}/replay`     | Ponownie wykonuje to samo żądanie przez router DevilRoute                          |
