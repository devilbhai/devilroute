...
Usługi wbudowane to lokalnie instalowane procesy sidecar, które DevilRoute instaluje, nadzoruje i
udostępnia jako pełnoprawne cele routingu. W przeciwieństwie do zewnętrznych providerów (osiąganych przez internet
przez klucze API), usługi wbudowane działają na tej samej maszynie co DevilRoute i komunikują się przez loopback.
...
| --------------- | ---------------------------------------------- | :-----------: | -------------------------------------------------------------------------------------------------------------------------- |
| **9Router**     | `9router`                                      |     20130     | Router AI, którego DevilRoute może używać jako sub-providera. Modele jako `9router/{sub}/{model}`                           |
...
- DevilRoute instaluje je pod `DATA_DIR/services/{name}/` (odizolowane od własnego `package.json` DevilRoute)
- DevilRoute uruchamia je i monitoruje jako procesy potomne
- DevilRoute wstrzykuje efemeryczny klucz API do środowiska potomka i rotuje go bez przestoju (gdzie dotyczy)
...
| Zarządzanie kluczem API                   | DevilRoute generuje, szyfruje at-rest (AES-256-GCM) i wstrzykuje przez env |
...
`package.json` i `node_modules/`. Nie koliduje z zależnościami DevilRoute.
...
przy następnym boot DevilRoute (jeśli jest zainstalowana).
...
- Usuwa przychodzące nagłówki `cookie` i `authorization` (brak wycieku sesji DevilRoute)
...
2. Zweryfikuj, że `npm` jest w PATH: `which npm` z tego samego konta użytkownika, które uruchamia DevilRoute.
...
domyślnym portem DevilRoute 20128.
...
- `DATA_DIR` lub jego rodzic nie jest zapisywalny przez proces DevilRoute.
- Uruchomienie w Docker rootless bez zapisu do zamapowanego volume.

**Naprawa:**

1. Sprawdź `DATA_DIR` (domyślnie: `~/.devilroute/`): `ls -la ~/.devilroute/`
2. Upewnij się, że użytkownik procesu DevilRoute jest właścicielem katalogu: `chown -R $USER ~/.devilroute/`
...
3. Spróbuj instalacji ręcznie: `npm install {package}@latest --prefix ~/.devilroute/services/{name}/`.
...
Tak. Obie usługi idą tym samym modelem local-first co sam DevilRoute. Działają
na tej samej maszynie i komunikują się przez loopback. „Production” oznacza tu VPS
lub lokalny serwer, na którym wdrożono DevilRoute, a nie zdalnego providera chmurowego.
...
2. Sprawdź strukturalne logi w output pino DevilRoute filtrowane po
   namespace `service:supervisor`.
3. Podejrzyj wiersz DB: `sqlite3 ~/.devilroute/devilroute.db "SELECT * FROM version_manager WHERE tool='9router'"`.
