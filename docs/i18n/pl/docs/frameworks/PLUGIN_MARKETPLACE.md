...
DevilRoute dostarcza system wtyczek w stylu WordPressa. Wtyczki to samodzielne
...
Wtyczki mieszkają w katalogu danych DevilRoute:

```
~/.devilroute/plugins/<plugin-name>/
  ├─ plugin.json
  └─ index.js          # (or whatever manifest.main points to)
```

`getDefaultPluginDir()` (`src/lib/plugins/scanner.ts`) rozwiązuje to do
`<home>/.devilroute/plugins`, gdzie `<home>` pochodzi ze zmiennych środowiskowych `HOME` /
...
| `requires`         | object?   | `{ devilroute?, permissions[] }`                             |
...
install (POST /api/plugins, path)
  ...
  → atomic rename into ~/.devilroute/plugins/<name> → insert DB row
