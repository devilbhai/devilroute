...
> **TL;DR**: ACP pozwala DevilRoute uruchamiać agentów CLI (np. Claude Code, Codex) jako procesy potomne zamiast korzystać z API HTTP. Daje to transport typu „CLI-as-backend”.

---

## Czym jest ACP?

ACP (Agent Client Protocol) to transport **"CLI-as-backend"** dla DevilRoute. Zamiast przechwytywać wywołania HTTP API do dostawców AI, ACP **uruchamia agentów CLI jako procesy potomne** i przekazuje prompty przez ich natywny interfejs.
...
Po wykryciu ACP może służyć jako transport dla dowolnego obsługiwanego providera. DevilRoute automatycznie użyje ACP, gdy CLI jest dostępne.
...
interface CliAgentInfo {
  ...
  providerAlias: string; // Provider ID in DevilRoute
...
2. **Sprawdź właściciela**: upewnij się, że DevilRoute ma uprawnienia odczytu/wykonania
