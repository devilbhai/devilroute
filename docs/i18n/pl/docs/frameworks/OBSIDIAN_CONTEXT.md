...
DevilRoute łączy się z vaultem **Obsidian** jako **źródłem kontekstu** — lokalną bazą
...
wskazuje DevilRoute na lokalny katalog vaultu i generuje losową parę username/password WebDAV:
...
w **serwer sync** DevilRoute (cztery narzędzia `obsidian_sync_*`) dodatkowo wymagają
tokenu auth sync skonfigurowanego w ustawieniach DevilRoute.
...
| `obsidian_sync_status`       | Status serwera sync DevilRoute: running, nazwa vaultu, port, uptime, last sync.               |
...
`open-sse/mcp-server/server.ts`, bramkowane przez `DEVILROUTE_MCP_ENFORCE_SCOPES=true`, z
dozwolonymi scope pochodzącymi z `DEVILROUTE_MCP_SCOPES` lub kontekstu scope klucza API. Zobacz
