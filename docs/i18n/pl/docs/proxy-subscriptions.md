...
> proxy w DevilRoute. To jest cięcie v1: pojedynczy operator wkleja linki
> subskrypcji, wybiera tryb (global lub rule), a DevilRoute wiąże wynikową pulę
...
Dziś pula proxy DevilRoute jest ręcznie kuratorowana: każdy węzeł żyje w
...
DevilRoute i ponownie je wiązać przy każdej zmianie listy upstream.

Celem v1 jest uczynienie DevilRoute first-class dla subskrypcji
...
| U5  | Operator | podać lokalny endpoint SOCKS5 sing-box/clash                          | węzły SS/VMess/Trojan/VLESS (których dispatcher DevilRoute nie mówi natywnie) stawały się używalne przez lokalny mostek kernela |
