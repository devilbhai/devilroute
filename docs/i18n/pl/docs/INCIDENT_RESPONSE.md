# Runbook reagowania na incydenty — DevilRoute (2026-06-18)
...
| **SEV-3** | Uśpiony błąd lub near-miss; brak bieżącego wpływu na użytkownika, ale budżet błędów zagrożony.               | Wyciek pamięci w trendzie wzrostowym; circuit breaker wyłącza jednego dostawcę. | Slack `#devilroute-ops` (następny standup) | 7 d                          |
...
| Panele SLO w Grafana              | Panele burn-rate SLO                 | Slack `#devilroute-ops`                |
...
Alerty **nie** idą na prywatne DM. Domyślny kanał to `#devilroute-ops`; PagerDuty
...
2. **Zadeklaruj**. Opublikuj w `#devilroute-ops`:
   ```
...
3. **Stabilizuj** przed diagnozą główną przyczyny. Preferowana kolejność:
   - Odetnij zły deploy: `kubectl rollout undo deploy/devilroute` (lub
   ...
   - Włącz tryb degradacji: ustaw
     `DEVILROUTE_DEGRADATION_MODE=lite` (pomija niekrytyczne middleware).
...
- **Wewnętrzna**: `#devilroute-ops` jest źródłem prawdy. Wątek na incydent na
  deklarację; wszystkie aktualizacje w wątku.
- **Zewnętrzna** (gdy dotyczy użytkowników zewnętrznych): status page
  (status.devilroute.example — zastąp prawdziwym URL, gdy będzie live). SEV-1
