...
DevilRoute kontroluje, które **origin przeglądarki** mogą odczytywać odpowiedzi cross-origin
...
traktować poważnie, ale ekspozycja DevilRoute jest **węższa niż w przypadku ogólnym**,
...
> `Access-Control-Allow-Credentials: true`. Wspólna ścieżka CORS DevilRoute nigdy
> tego nie robi.
...
- Jeśli DevilRoute działa za reverse proxy / tunnel (nginx, Caddy, Cloudflare
...
```bash
# Vite (5173) + Next.js (3000) dev servers calling a local DevilRoute
...
## Przykład: reverse proxy przed DevilRoute

CORS jest egzekwowany przez samo DevilRoute, więc proxy generalnie **nie powinno** dodawać ani
przepisywać nagłówków `Access-Control-*` (podwójne nagłówki psują przeglądarki). Terminuj TLS
i forwarduj — niech DevilRoute odpowiada na preflight:

```nginx
# nginx — forward to DevilRoute; do NOT inject Access-Control-* here
location / {
...
Ustaw dozwolone originy przeglądarki w DevilRoute (`CORS_ALLOWED_ORIGINS` lub
