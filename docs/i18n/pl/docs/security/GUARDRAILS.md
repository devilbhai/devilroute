...
między DevilRoute a upstream providerami. Każdy guardrail może sprawdzać (oraz
...
- nagłówek `x-devilroute-disabled-guardrails` (lub legacy
...
- **`garak` (advisory)** — uruchamia garak `--probes promptinject,dan,leakreplay`
  przeciw lokalnej instancji DevilRoute (`http://localhost:20128/v1`). Bramkowany
