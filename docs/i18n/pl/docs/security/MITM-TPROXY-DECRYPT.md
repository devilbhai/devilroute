...
TPROXY transparent decrypt to **5. tryb przechwytywania** DevilRoute w stosie MITM
...
zmiany konfiguracji** — ale musi ufać dynamicznemu CA instalowanemu przez DevilRoute
...
"TPROXY decrypt requires Linux + root + the native addon", a reszta DevilRoute
działa normalnie.
...
> i instaluje pod istniejącym slotem trust-store `devilroute-mitm.crt`
> (zastępując stary pojedynczy leaf — bez potrzeby dual-trust cleanup),
> w pełni oddzielony od slotu TPROXY `devilroute-tproxy-ca.crt` opisanego poniżej.
...
- Generuje długowieczne CA przez `generateMitmCa()` (CN `"DevilRoute MITM CA"`,
...
`devilroute-tproxy-ca.crt` (stała `TPROXY_CA_CERT_NAME`) — oddzielonym od
slotu statycznego certu MITM (`devilroute-mitm.crt`), żeby te dwa nigdy się nie nadpisywały.
...
| **Dedicated CA slot**            | The dynamic CA installs to `devilroute-tproxy-ca.crt`, never clobbering the static MITM cert.                                                                                                                                                                                                                                                                       |
...
host, oznacza, że wszystko przechwycone przez DevilRoute może zostać odszyfrowane. Jest bramkowane przez
...
- Potwierdź, że proces ufa dynamicznemu CA. CA jest instalowane pod
  `devilroute-tproxy-ca.crt`; aplikacje z własnym trust store (Firefox/Chrome NSS)
...
trybu cofa reguły; jeśli DevilRoute zostało zabite w trakcie sesji, użyj akcji AgentBridge
