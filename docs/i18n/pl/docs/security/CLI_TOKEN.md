...
Komendy CLI DevilRoute uwierzytelniają się względem lokalnego management API za pomocą
tokenu `HMAC-SHA256(machine-id, salt)` wysyłanego w nagłówku żądania
`x-devilroute-cli-token`.

Dzięki temu podkomendy CLI (`devilroute status`, `devilroute providers` itd.)
...
3. CLI wysyła token jako `x-devilroute-cli-token` w każdym żądaniu do
...
Ustaw `DEVILROUTE_CLI_SALT`, aby obrócić wyprowadzony token bez zmian w kodzie.
...
Domyślna sól: `devilroute-cli-auth-v1`
...
**Opt-out:** ustaw `DEVILROUTE_DISABLE_CLI_TOKEN=true` (env lub `.env`), aby całkowicie wyłączyć
