...
Wszystkie trasy management API DevilRoute są klasyfikowane do jednego z trzech
...
operator wystawił DevilRoute za tunelem nginx/Cloudflare/Tailscale i wyciekł JWT,
albo auth był źle skonfigurowany — atakujący zamienia „wywołaj API” na „uruchom
komendę na hoście” (remote code execution). DevilRoute zamyka to, egzekwując
...
Jeśli uruchamiasz DevilRoute za reverse proxy lub tunelem (nginx, Caddy, Cloudflare
...
- `403 LOCAL_ONLY` w logach DevilRoute dla jednej z tych ścieżek to strażnik
