...
> **TL;DR**: DevilRoute agreguje darmowe plany od 50+ providerów. Podłącz wielu darmowych providerów, aby uzyskać nielimitowane darmowe AI z automatycznym fallbackiem.
...
DevilRoute **agreguje** te darmowe plany w jeden endpoint. Zamiast rejestrować się w 10 różnych serwisach, podłączasz je wszystkie do DevilRoute i używasz `model: "auto"`, aby automatycznie wybrać najlepszą darmową opcję dla każdego żądania.
...
Siła DevilRoute to **stackowanie darmowych planów**. Zamiast polegać na jednym providerze, podłączasz wielu darmowych providerów i pozwalasz DevilRoute automatycznie wybrać najlepszego dla każdego żądania.
...
Następnie użyj `model: "auto"`, a DevilRoute:
...
## Jak DevilRoute usprawnia darmowe plany

### 1. Automatyczny fallback

Jeśli jeden darmowy provider jest zajęty lub niedostępny, DevilRoute automatycznie próbuje następnego. Nie musisz nic robić.

### 2. Inteligentny routing

DevilRoute wybiera **najlepszego darmowego providera** dla każdego żądania na podstawie:
...
Funkcja **compression** w DevilRoute oszczędza 15–95% tokenów. Dzięki temu darmowy limit starcza **5–20× dłużej**.

### 4. Obsługa wielu kont

Jeśli masz wiele kont u tego samego providera, DevilRoute traktuje każde jako osobnego kandydata. To podwaja lub potroja Twój darmowy limit.
...
**Tak!** To oficjalne darmowe plany providerów. DevilRoute tylko ułatwia korzystanie z nich wszystkich naraz.
...
Nie ma haczyka! Providerzy oferują darmowe plany, żeby przyciągnąć użytkowników. DevilRoute tylko ułatwia korzystanie z nich wszystkich naraz.
...
- **[Przewodnik Auto-Combo](./AUTO-COMBO-GUIDE.md)** — pozwól DevilRoute wybrać najlepsze AI
