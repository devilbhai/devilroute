...
**Problem, który rozwiązuje:** DevilRoute proxy’uje wiele kluczy API względem
...
- Cała persystencja w istniejącej bazie SQLite DevilRoute (`DATA_DIR/storage.sqlite`).
...
1. `localStorage.getItem("devilroute:quota-share:pools")` jest niepuste.
