...
zakresowany do konta stał się transportem podstawowym. Ustaw `DEVILROUTE_BROWSER_POOL` na jedną z tych samych wartości, aby
...
nagłówkach `X-DevilRoute-Claude-Web-*` (`open-sse/executors/claude-web/stream.ts:364`).
...
1. Uruchom DevilRoute przez `npm run dev` lub zbudowaną instalację.
