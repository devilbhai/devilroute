...
Dla większości użytkowników **nie jest wymagana żadna specjalna konfiguracja**. DevilRoute dostarcza wbudowany
...
DevilRoute obsługuje też AgentRouter (i podobne przekaźniki) przez typ providera **Claude Code
...
- DevilRoute uruchomione z włączoną flagą funkcji `ENABLE_CC_COMPATIBLE_PROVIDER`
  (patrz poniżej).
...
ustawiając zmienną środowiskową przed uruchomieniem DevilRoute:
...
**`Invalid JSON response from provider (reset after Ns)` w logach devilroute** —
...
providera `agentrouter`, który już ma poprawny obraz sieciowy. Sprawdź w logach devilroute,
