...
> **TL;DR**: Silnik kompresji DevilRoute jest **podłączalny (pluggable)** — możesz rejestrować własne silniki, dostarczać language packi dla nowych języków i składać stacked pipelines. Ten przewodnik pokazuje jak.
...
````ts
import type { CompressionEngine } from "devilroute/compression/engines/types";
import { registerCompressionEngine } from "devilroute/compression/engines/registry";
...
```
~/.devilroute/compression/engines/my-engine.ts    # User-level
...
} from "@devilroute/open-sse/services/compression/engines/registry";
...
Kompresja w stylu Caveman używa **pakietów reguł zależnych od języka**, aby obsługiwać wypełniacze (fillers), hedging i rozwlekłe wzorce w każdym języku naturalnym. DevilRoute dostarcza **6 language packów**: `en`, `es`, `fr`, `de`, `ja`, `pt-BR`.
...
```ts
import { loadRulePack } from "devilroute/compression/ruleLoader";
...
```
~/.devilroute/compression/rules/hi/filler.json  # User-level
...
DevilRoute wybiera **JEDEN tryb na żądanie** na podstawie konfiguracji, progów auto-trigger i override'ów combo.
...
`@devilroute/open-sse/services/compression/strategySelector`):

```ts
import { applyStackedCompression } from "@devilroute/open-sse/services/compression/strategySelector";
...
Silniki kompresji DevilRoute w README przypisują zasługi kilku projektom upstream
(„inspired by RTK, Caveman, LLMLingua-2, Troglodita”). Częste pytanie kontrybutorów:
**gdy upstream RTK doda nowy filtr toola albo Caveman doda rule pack, jak to trafia do DevilRoute?**
...
reguła/filtr w formacie DevilRoute** (zob.
...
własne warstwy DevilRoute `smartcrusher` / `toon` / `tabular`. Nie ma żywego
...
1. **Nie vendoruj** — wyraź regułę/filtr upstream w formacie DevilRoute.
