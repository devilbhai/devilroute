...
W przeciwieństwie do lokalnych silników kompresji DevilRoute (Caveman, RTK, LLMLingua, stacked
...
kontekstu. DevilRoute dołącza jedynie parametr w body (`context_management.edits[]`); faktyczne
czyszczenie wykonuje Claude względem własnego tokenizera.

Z natury jest to zdolność delegowana: inni providerzy odrzucają ten parametr, więc DevilRoute
...
DevilRoute wstrzykuje pojedynczą edycję do wychodzącego body Anthropic Messages:
...
Beta jest reklamowana nagłówkiem `anthropic-beta: context-management-2025-06-27`, który DevilRoute
...
DevilRoute stabilnie sortuje edycję `clear_thinking` na początek, bo Anthropic wymaga, by
...
Po odpowiedzi Claude DevilRoute zapisuje, ile kontekstu provider faktycznie usunął. To **nie** jest
...
| ----------------------- | ----------------------------------------------------- | -------------------------------------------- |
| Gdzie działa            | W DevilRoute, zanim żądanie opuści proxy               | U providera (Claude), po stronie serwera     |
...
Te dwa podejścia są komplementarne: lokalne silniki kompresują bajty wysyłane przez DevilRoute;
