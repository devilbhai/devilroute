...
Kompresja DevilRoute opiera się na kontraktach silników. Tryb może uruchomić jeden silnik bezpośrednio
...
dowodzą, że faktycznie może dotrzeć do `devilroute_ccr_retrieve`** (`callerSupportsCcrRetrieve()` w
...
z zakresem `22-87%` oraz narzędzie kompresji wejścia na poziomie `~46%`. DevilRoute używa liczby
...
całą instalację DevilRoute; gdy opcjonalny stos jest nieobecny, LLMLingua nadal fail-openuje. Tylko trzy
...
- **Global npm (`npm i -g devilroute`) / standalone** — uruchom powyższą komendę install wewnątrz
...
| Tool                                | Scope               | Purpose                                 |
| ----------------------------------- | ------------------- | --------------------------------------- |
| `devilroute_compression_status`      | `read:compression`  | Ustawienia, analytics, statystyki cache |
| `devilroute_compression_configure`   | `write:compression` | Aktualizacja ustawień globalnych        |
| `devilroute_set_compression_engine`  | `write:compression` | Ustawienie trybu i opcjonalnego potoku  |
| `devilroute_list_compression_combos` | `read:compression`  | Lista combo kompresji                   |
| `devilroute_compression_combo_stats` | `read:compression`  | Odczyt analytics combo/silnika          |
