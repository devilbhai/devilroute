---
title: "🗜️ Przewodnik po kompresji promptów — DevilRoute"
version: 3.8.40
lastUpdated: 2026-06-28
---

# 🗜️ Przewodnik po kompresji promptów — DevilRoute
...
DevilRoute implementuje modularny potok kompresji promptów, który działa **proaktywnie** zanim żądania trafią do upstreamowych providerów. Oszczędności tokenów zachodzą więc przejrzyście — bez zmian w Twoim workflow.
...
DevilRoute dokumentuje oszczędności kompresji z dwóch źródeł: benchmarków projektów upstream oraz
własnej kompozycji silników DevilRoute.
...
Dla nakładających się payloadów narzędzi/kontekstu domyślne combo DevilRoute układa silniki w stos:
...
Wyślij nagłówek żądania `x-devilroute-compression`, aby nadpisać plan kompresji dla pojedynczego
...
Zastosowany plan jest zwracany w nagłówku odpowiedzi `X-DevilRoute-Compression: <mode>; source=<source>`,
...
Poza 7 standardowymi trybami DevilRoute zawiera kilka zaawansowanych systemów kompresji,
...
} from "@devilroute/open-sse/services/compression/cachingAware";
...
```ts
import { applyAging } from "@devilroute/open-sse/services/compression/progressiveAging";
