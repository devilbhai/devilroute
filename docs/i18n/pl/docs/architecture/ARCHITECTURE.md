---
title: "Architektura DevilRoute"
version: 3.8.40
lastUpdated: 2026-06-28
---

# Architektura DevilRoute
...
DevilRoute to lokalna brama routingu AI i panel (dashboard) zbudowane na Next.js.
...
flowchart LR
    ...
    subgraph Router[DevilRoute Local Process]
...
DevilRoute może instalować, nadzorować i routować do lokalnie działających procesów narzędzi AI
...
- Builder metadanych odpowiedzi DevilRoute: `src/domain/devilrouteResponseMeta.ts`
...
- plik: `${DATA_DIR}/storage.sqlite` (lub `$XDG_CONFIG_HOME/devilroute/storage.sqlite` gdy ustawione, w przeciwnym razie `~/.devilroute/storage.sqlite`)
...
flowchart LR
    ...
    subgraph ContainerOrProcess[DevilRoute Runtime]
        Next[Next.js Server\nPORT=20128]
...
> DevilRoute v3.8.0. Kanoniczna i stale aktualizowana lista: zob.
...
- końcowa odpowiedź klienta zwrócona przez DevilRoute; odpowiedzi streamowane są przechowywane w tej samej zwartej formie podsumowania
...
1. `usageDb` i `localDb` współdzielą tę samą politykę katalogu bazowego (`DATA_DIR` -> `XDG_CONFIG_HOME/devilroute` -> `~/.devilroute`) z migracją plików legacy.
...
5. Katalog `open-sse/` jest publikowany jako **pakiet npm workspace** `@devilroute/open-sse`. Kod źródłowy importuje go przez `@devilroute/open-sse/...` (rozwiązywane przez Next.js `transpilePackages`). Ścieżki plików w tym dokumencie nadal używają nazwy katalogu `open-sse/` dla spójności.
...
- Build obrazu Docker: `docker build -t devilroute .`
