...
DevilRoute ma potok autoryzacji zależny od trasy, który bramkuje każde żądanie API. Klasyfikacja jest **deterministyczna** i **fail-closed** — wszystko, czego nie da się sklasyfikować, trafia do `MANAGEMENT` i wymaga sesji albo tokenu klasy management. Ta strona opisuje model dla inżynierów utrzymujących trasy lub projektujących nowe endpointy.
...
Walidowane przez `isValidApiKey()` / `extractApiKey()` w `src/sse/services/auth.ts` i reeksportowane przez `src/shared/utils/apiAuth.ts`. Walidator akceptuje też zmienne środowiskowe `DEVILROUTE_API_KEY` / `ROUTER_API_KEY` jako trwałe klucze passthrough (issue #1350).
...
Incoming request → src/proxy.ts
  → runAuthzPipeline() in src/server/authz/pipeline.ts
    1. Strip trusted internal headers (x-devilroute-auth-*, x-devilroute-route-class)
    ...
    8. Otherwise: POLICIES[routeClass].evaluate(ctx)
       - allow  → stamp x-devilroute-auth-{kind,id,label,scopes} → NextResponse.next()
       - reject → JSON error w/ correlation_id (dashboard pages → 302 /login)
```

Zaufane nagłówki wewnętrzne (zdefiniowane w `src/server/authz/headers.ts`) są **usuwane z przychodzących żądań** przed klasyfikacją — klienci nie mogą wcześniej ustawić `x-devilroute-auth-*`, aby podszyć się pod subject.
...
metadanych żądania lub `DEVILROUTE_MCP_SCOPES`.
...
x-devilroute-route-class:    PUBLIC | CLIENT_API | MANAGEMENT
```

Dla uwierzytelnionych żądań nagłówki żądania upstream (po stronie handlera) zawierają także:

```
x-devilroute-auth-kind:      client_api_key | dashboard_session | management_key | anonymous
x-devilroute-auth-id:        key_<last-4> | "dashboard" | "anonymous"
x-devilroute-auth-label:     (optional)
x-devilroute-auth-scopes:    comma-separated list
```
