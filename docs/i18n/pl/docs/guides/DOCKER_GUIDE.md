---
title: "🐳 Przewodnik Docker — DevilRoute"
version: 3.8.40
lastUpdated: 2026-06-28
---

# 🐳 Przewodnik Docker — DevilRoute
...
DevilRoute dostarcza cztery profile Compose. Wybierz ten, który pasuje do Twojego środowiska.

| Profil            | Usługa           | Kiedy używać                                                                                                                              | Polecenie                                    |
| ----------------- | ---------------- | ----------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------- |
| `base` (domyślny) | `devilroute-base` | Serwer headless / minimalne runtime, bez dołączonych CLI providerów                                                                       | `docker compose --profile base up -d`        |
| `cli`             | `devilroute-cli`  | Przepływy agentowe wywołujące `devilroute providers/setup/doctor` oraz dołączone CLI (Codex, Claude Code, Droid, OpenClaw)                 | `docker compose --profile cli up -d`         |
| `host`            | `devilroute-host` | Hosty Linux z dostępem do CLI hosta w stylu `network_mode` przez montowanie `~/.local/bin`, `~/.codex`, `~/.claude` itd. tylko do odczytu | `docker compose --profile host up -d`        |
...
DevilRoute korzysta z Redis jako zaplecza rozproszonego rate limitera i współdzielonej pamięci podręcznej. Usługa `redis` jest **zawsze zdefiniowana** w `docker-compose.yml` (bez bramki profilu) i startuje razem z każdym innym profilem.

| Szczegół             | Wartość                           |
| -------------------- | --------------------------------- |
| Image                | `redis:7-alpine`                  |
| Container name       | `devilroute-redis`                 |
| Internal port        | `6379`                            |
| Host port (override) | `REDIS_PORT` (defaults to `6379`) |
| Volume               | `devilroute-redis-data` → `/data`  |
...
| Image                  | `devilroute:prod` (built from `runner-cli` target)                                  |
| Redis container        | `devilroute-redis-prod` (`redis:8.6.2`, dedicated `redis-prod-data` volume)         |
| Data volume            | `devilroute-prod-data` (named, persisted across rebuilds)                           |
...
```bash
docker build --target runner-base -t devilroute:base .
docker build --target runner-cli  -t devilroute:cli  .
```

Domyślne wartości eksportowane przez `runner-base`: `PORT=20128`, `HOSTNAME=0.0.0.0`, `NODE_OPTIONS=--max-old-space-size=512`, `DATA_DIR=/app/data`, `DEVILROUTE_MIGRATIONS_DIR=/app/migrations`.

Zachowanie pamięci w Dockerze:

- `NODE_OPTIONS=--max-old-space-size=512` jest wbudowane w obraz jako fallback.
- Właściwy proces serwera uruchamia launcher standalone, który czyta `DEVILROUTE_MEMORY_MB` i dopisuje `--max-old-space-size=<DEVILROUTE_MEMORY_MB>`.
- Node używa ostatniej powtórzonej wartości `--max-old-space-size`, więc ustawienie `DEVILROUTE_MEMORY_MB` kontroluje efektywny limit heapa w Dockerze.
- Gdy `DEVILROUTE_MEMORY_MB` nie jest ustawione, launcher używa `512`.
...
| ----------------------------- | -------------------------------------------------------------------------------------------- | ------------------------ |
| `DEVILROUTE_WS_BRIDGE_SECRET`  | Współdzielony sekret mostu WebSocket. **Wymagany w produkcji** — ustaw silny, losowy ciąg.   | unset (must be provided) |
...
| `AUTO_UPDATE_HOST_REPO_DIR`   | Ścieżka hosta montowana w profilu `cli` pod `/workspace/devilroute` na potrzeby self-update   | `.` (current directory)  |
| `DEVILROUTE_MEMORY_MB`         | Sufit heapa Node w runtime dla serwera standalone Dockera; nadpisuje fallback obrazu powyżej | `512`                    |
| `DASHBOARD_PORT` / `API_PORT` | Nadpisanie eksponowanych portów dashboardu (20128) i API (20129)                             | `20128` / `20129`        |
| `DEVILROUTE_BASE_PATH`         | Podścieżka URL, gdy aplikacja jest publikowana za reverse proxy (np. `/devilroute`)           | _(empty = root)_         |
| `NEXT_PUBLIC_BASE_URL`        | Publiczny origin przeglądarki wraz z podścieżką (np. `https://host/devilroute`)               | unset                    |
...
Next.js `basePath` jest kompilowany do bundla standalone. DevilRoute zapisuje wbudowaną
...
`DEVILROUTE_BASE_PATH` przy starcie kontenera. Gdy się różnią, a obraz był
...
`docker-compose.yml` przekazuje `DEVILROUTE_BASE_PATH` jako Docker build-arg oraz jako
...
prefiksu). Traefik powinien routować `PathPrefix(`/devilroute`)` do kontenera bez
`StripPrefix`, żeby Next.js otrzymywał `/devilroute/...` i serwował assety z
`/devilroute/_next/...`.

Healthcheck Dockera sonduje `/api/monitoring/health` z prefiksem aktywnego
`DEVILROUTE_BASE_PATH`.

## Docker Compose z Caddy (HTTPS Auto-TLS)

DevilRoute można bezpiecznie udostępnić dzięki automatycznemu provisionowaniu SSL w Caddy. Upewnij się, że rekord DNS A domeny wskazuje na IP Twojego serwera.
...
services:
  devilroute:
    ...
    environment:
      ...
      - BASE_URL=http://devilroute:20128
...
Caddy ustawia standardowe nagłówki forwardingu dla kontenera upstream. DevilRoute używa
...
związanej z sesją. Włączaj `DEVILROUTE_TRUST_PROXY` tylko w zaawansowanych wdrożeniach, gdzie świadomie
chcesz, by DevilRoute wyprowadzał publiczny origin z zaufanych nagłówków forwarded zamiast z jawnej
konfiguracji.
...
- Quick Tunnels nie są automatycznie przywracane po restarcie DevilRoute ani kontenera. Włącz je ponownie z dashboardu, gdy potrzeba.
...
- Ustaw `CLOUDFLARED_BIN=/absolute/path/to/cloudflared`, jeśli chcesz, by DevilRoute używał istniejącego binarium zamiast pobierać własne.
...
- **Tryb SQLite WAL:** `docker stop` powinien móc się dokończyć, żeby DevilRoute mógł zrobić checkpoint najnowszych zmian z powrotem do `storage.sqlite`. Dołączone pliki Compose ustawiają już 40s grace period stopu. Przy bezpośrednim uruchomieniu obrazu zachowaj `--stop-timeout 40`.
