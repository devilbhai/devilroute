---
title: "Claude Code CLI — konfiguracja z DevilRoute"
version: 3.8.40
lastUpdated: 2026-07-24
---

# Claude Code CLI — konfiguracja z DevilRoute

Skieruj CLI **Claude Code** (`claude`) na DevilRoute — lokalnie lub na zdalny VPS —
...
```bash
# Launch Claude Code against a local DevilRoute (auto-detects the active context)
devilroute launch

# Against a remote DevilRoute (after `devilroute connect <host>`, this is automatic)
devilroute launch --remote http://192.168.0.15:20128 --api-key oma_live_xxx

# Generate per-model profiles, then launch one
devilroute setup-claude            # writes ~/.claude/profiles/<name>/settings.json
devilroute launch --profile glm52  # Claude Code using glm/glm-5.2 via DevilRoute
```
...
| `ANTHROPIC_AUTH_TOKEN`                       | Wysyłany jako `Authorization: Bearer …` — użyj tokenu dostępu / klucza API DevilRoute       |
...
`devilroute launch` ustawia to wszystko za Ciebie: rozwiązuje bazowy URL + token
z aktywnego kontekstu (więc `devilroute connect <vps>`, a potem `devilroute launch`
...
picker `/model` zwykle pokazuje **tylko** modele Claude/Anthropic z DevilRoute — `kimi/kimi-k2.6`
lub `glm/glm-5.2` się nie pojawią, mimo że routują poprawnie.

DevilRoute może odzwierciedlić dowolny włączony model (i combo) pod identyfikatorem `claude/…`,
aby przeszedł ten filtr i pojawił się w pickerze:

```
kimi/kimi-k2.6            →  claude/kimi/kimi-k2.6      "Kimi K2.6 (DevilRoute)"
glm/glm-5.2              →  claude/glm/glm-5.2         "GLM 5.2 (DevilRoute)"
<combo "custo-otimizado"> →  claude/combo/custo-otimizado
```

Gdy wybierzesz jeden z nich w Claude Code, DevilRoute zdejmuje opakowanie `claude/`
...
wygrywa), więc zwykły DevilRoute nie podwaja katalogu dla klientów, które nie używają
Claude Code:
...
{
  "env": {
    "ANTHROPIC_BASE_URL": "http://<your DevilRoute>:20128",
...
`devilroute setup-claude` pobiera żywy katalog `/v1/models` i zapisuje jeden
...
> `devilroute launch --profile <name>` (wstrzykuje `ANTHROPIC_AUTH_TOKEN` z
> aktywnego kontekstu) albo sam wyeksportuj `ANTHROPIC_AUTH_TOKEN` i uruchom
> `CLAUDE_CONFIG_DIR=~/.claude/profiles/<name> claude`.

**Auto-sync po discovery modeli (opt-in).** DevilRoute może regenerować te same
...
auto-sync” → Claude Code) albo ustaw `DEVILROUTE_AUTO_SYNC_CLAUDE_PROFILES=true` (honoruje też
...
devilroute setup-claude --remote http://192.168.0.15:20128 --api-key oma_live_xxx

# Only some providers
devilroute setup-claude --only glm,kimi

# Preview without writing
devilroute setup-claude --dry-run

# Launch a profile
devilroute launch --profile kimi-k27
```
...
Claude Code routuje do poziomów możliwości. Zmapuj każdy na model DevilRoute przez env /
...
Po uruchomieniu `devilroute connect <host>` (zobacz
[Tryb zdalny](./REMOTE-MODE.md)), `devilroute launch` i `devilroute setup-claude`
...
`/v1`**, i zrestartuj `claude` (env jest odczytywane raz przy starcie). `devilroute launch`
robi to za Ciebie.
...
DevilRoute odmawia zgadywania. Napraw na jeden z dwóch sposobów: przypnij prefiksowane id przez
...
`DEVILROUTE_PREFER_CLAUDE_CODE_FOR_UNPREFIXED_CLAUDE_MODELS=true` (domyślnie wyłączone;
...
**Błędy auth** — profil nie przechowuje tokenu. Użyj `devilroute launch --profile`
