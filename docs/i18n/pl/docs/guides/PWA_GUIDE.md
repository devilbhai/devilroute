...
DevilRoute jest dostarczany jako w pełni instalowalna Progressive Web App. Gdy otworzysz dashboard w dowolnej przeglądarce mobilnej — Android (Chrome) lub iOS (Safari) — możesz wybrać „Dodaj do ekranu głównego” i uzyskać doświadczenie zbliżone do natywnej aplikacji, bez sklepu z aplikacjami.

## Czym jest PWA?

Progressive Web App zamienia webowy dashboard DevilRoute w coś, co wygląda i działa jak natywna aplikacja mobilna. Po zainstalowaniu:
...
1. Otwórz dashboard DevilRoute w Chrome: `http://YOUR_IP:20128`
2. Chrome automatycznie pokaże baner **"Add DevilRoute to Home screen"**, albo:
   - Stuknij menu **⋮** (trzy kropki) → **"Add to Home screen"** lub **"Install app"**
3. Potwierdź monity
4. DevilRoute pojawi się na ekranie głównym jako samodzielna aplikacja

### iOS (Safari)

1. Otwórz dashboard DevilRoute w Safari: `http://YOUR_IP:20128`
2. Stuknij przycisk **Share** (kwadrat ze strzałką)
3. Przewiń w dół i stuknij **"Add to Home Screen"**
4. Nadaj nazwę (domyślnie „DevilRoute”) i stuknij **Add**
5. DevilRoute pojawi się na ekranie głównym z ikoną aplikacji

### Desktop (Chrome / Edge)

1. Otwórz dashboard DevilRoute
2. Kliknij **ikona instalacji** na pasku adresu (lub ⋮ → "Install DevilRoute...")
3. Potwierdź monity
4. DevilRoute otworzy się jako samodzielne okno — bez kart i paska adresu
...
DevilRoute zawiera service worker (`sw.js`), który zapewnia inteligentne buforowanie:
...
DevilRoute dostarcza ikony zoptymalizowane pod każdą platformę:
...
{
  "name": "DevilRoute",
  "short_name": "DevilRoute",
  "description": "DevilRoute is an AI gateway for multi-provider LLMs. One endpoint for all your AI providers.",
  "start_url": "/",
  "scope": "/",
  "display": "fullscreen",
  "orientation": "any",
...
- **Wersjonowanie cache**: `devilroute-pwa-v2` — zwiększ tę wartość, aby wymusić świeży cache przy aktualizacji
...
Przy uruchamianiu DevilRoute na Androidzie przez Termux PWA działa bezproblemowo:

1. Uruchom DevilRoute w Termux: `npx devilroute`
...
Zainstaluj PWA na dowolnym urządzeniu, które ma dostęp przeglądarkowy do serwera DevilRoute:
