...
> **TL;DR**: DevilRoute śledzi zużycie tokenów każdego żądania, liczy koszt, egzekwuje limity per klucz API i pokazuje analitykę na dashboardzie. Ten przewodnik wyjaśnia, jak to działa.
...
Każde żądanie przechodzące przez DevilRoute generuje **rekord użycia**, który rejestruje:
...
Dla providerów, które nie zwracają usage (niektóre providery web-cookie), DevilRoute **szacuje** tokeny heurystyką `~4 chars per token` (zob. `open-sse/services/autoCombo/pipelineRouter.ts`).

### Tokeny z cache

DevilRoute śledzi `cached_tokens` osobno od `prompt_tokens`, ponieważ:
...
Dla modeli bez danych cenowych DevilRoute przechodzi na **szacowanie kosztu** wewnętrznymi średnimi stawkami (źródło: dane cenowe LiteLLM).
...
| Narzędzie               | Opis                                            |
| ----------------------- | ----------------------------------------------- |
| `devilroute_cost_report` | Generuje raport kosztów per klucz za dany okres |
| `devilroute_check_quota` | Zwraca bieżący status limitu dla klucza API     |
