---
title: "Codex CLI — konfiguracja z DevilRoute"
version: 3.8.49
lastUpdated: 2026-07-26
---

# Codex CLI — konfiguracja z DevilRoute

Kompletny przewodnik po używaniu Codex CLI skierowanego na DevilRoute jako backend zgodny z OpenAI.
...
**DevilRoute rozwiązuje to w sposób przezroczysty:**
...
Przy DevilRoute nie potrzebujesz osobnego proxy tłumaczącego. **Wszystkie modele używają `wire_api = "responses"`** — resztę obsługuje DevilRoute.
...
| Model                                | ID DevilRoute                         | Okno kontekstu           | `auto_compact` | `tool_output_limit` |
| ------------------------------------ | ------------------------------------ | ------------------------ | -------------- | ------------------- |
...
Wszystkie modele Codex w DevilRoute używają prefiksu `cx/`:

| Nazwa w Codex CLI       | Model DevilRoute    |
| ----------------------- | ------------------ |
...
Inni providerzy używają własnego prefiksu (`kmc/`, `glm/`, `ds/`, `ollamacloud/`, `opencode-go/`, `mistral/`) — prefiks odpowiada aliasowi providera w DevilRoute.
...
## Automatyczne generowanie profili przez `devilroute setup-codex`

Gdy DevilRoute działa na VPS, możesz automatycznie wygenerować pliki profili z żywego katalogu modeli:
...
DevilRoute może też **automatycznie synchronizować** te same pliki profili po udanym discovery/import modeli providera, gdy zmienia się żywy katalog. To opcja **opt-in, domyślnie wyłączona**: włącz ją w **dashboardzie CLI Code** („CLI profile auto-sync” → Codex) albo ustaw `DEVILROUTE_AUTO_SYNC_CODEX_PROFILES=true` (uwzględnia też `CLI_ALLOW_CONFIG_WRITES`, domyślnie włączone). Po włączeniu zapisuje tylko osobne pliki profili `~/.codex/*.config.toml`; nigdy nie zmienia aktywnego/domyślnego `~/.codex/config.toml`, ustawień Codex-lb, auth ani wyboru providera.

---

## Uruchamianie Codex przez `devilroute launch-codex`

Sprawdza health instancji DevilRoute przed uruchomieniem Codex:
...
Dwa domyślne ustawienia DevilRoute mogą cicho psuć wielogodzinne sesje Codex CLI. Żadne nie jest ustawieniem Codex CLI — oba leżą po stronie DevilRoute. Użytkownicy migrujący konfigurację z upstreamowych proxy, które pinują konta i wyłączają idle cutoff, często trafiają na oba i wnioskują, że DevilRoute „nie utrzyma długiej sesji”.
...
Krytyczny szczegół: syntetyczny SSE **heartbeat DevilRoute nie resetuje** zegara idle. Tylko prawdziwy chunk body z upstream aktualizuje `lastChunkTime`. Cichy model, który nadal „myśli”, wygląda dla watchdogu tak samo jak zawieszony upstream.
...
W środowisku procesu DevilRoute (`.env` / compose / systemd):
...
Zrestartuj DevilRoute po zmianie tych zmiennych env.
...
2. **Podnieś / wyłącz idle cutoff** w środowisku DevilRoute:
...
3. Zostaw zwykły Codex `config.toml` (`wire_api = "responses"`, poprawny `base_url`, `DEVILROUTE_API_KEY`) — po stronie Codex nie ma pokręteł affinity/idle dla tych dwóch zachowań.
4. Zrestartuj DevilRoute, potem uruchom długie zadanie Codex.
...
Gdy odpala się idle watchdog, DevilRoute loguje linię w stylu:

```text
[STREAM] Idle timeout: no data from codex for 600000ms (model: cx/gpt-5.5)
```

Grepuj `Idle timeout: no data from` (albo kod `stream_idle_timeout` / nazwę błędu `StreamIdleTimeoutError`). Segment providera to to, czego DevilRoute użyło dla tego requestu (`codex`, inny id providera albo `provider`, gdy nieznany) — nie zawsze jest to dosłowny string `codex`.
...
Sprawdź, czy model istnieje w DevilRoute z poprawnym prefiksem. Użyj `devilroute models list` albo otwórz `/dashboard/providers/<provider>`.

**`Authentication error`**
Potwierdź, że `DEVILROUTE_API_KEY` jest wyeksportowany: `echo $DEVILROUTE_API_KEY`.

**`Connection refused`**
Sprawdź, czy DevilRoute działa i że host/port w `base_url` jest poprawny dla Twojej sieci (lokalnie vs Tailscale vs VPS).
...
Zobacz [Zadania długotrwałe](#zadania-długotrwałe). Włącz session affinity (TTL powyżej długości zadania) i podnieś lub wyłącz `STREAM_IDLE_TIMEOUT_MS` / `FETCH_BODY_TIMEOUT_MS`. Grepuj logi DevilRoute pod kątem `Idle timeout: no data from`.
