...
Kompletny przewodnik po konfiguracji providerów, tworzeniu combo, integracji narzędzi CLI i wdrażaniu DevilRoute.
...
**Wskazówka:** Używaj Opus do złożonych zadań, Sonnet dla szybkości. DevilRoute śledzi limit per model!

Trasy zgodne z Claude i Claude Code zachowują poziom myślenia `max` dla modeli Opus i Sonnet
Modele Haiku nie akceptują poziomu wysiłku `max`, więc DevilRoute obniża to
...
1. Subskrybuj: [Moonshot AI](https://platform.kimi.ai?aff=devilroute)
2. Pobierz klucz API → Dashboard → Add API Key
...
```json
{
  "env": {
    ...
    "ANTHROPIC_AUTH_TOKEN": "your-devilroute-api-key"
  }
}
```
...
```json
{
  "agents": {
    "defaults": {
      "model": { "primary": "devilroute/if/kimi-k2.7-code" }
    }
  },
  "models": {
    "providers": {
      "devilroute": {
        ...
        "apiKey": "your-devilroute-api-key",
...
cp .env.example ~/.devilroute/.env

# Start server
devilroute
# Or with custom port:
devilroute --port 3000
```

CLI automatycznie ładuje `.env` z `~/.devilroute/.env` lub `./.env`.

### Odinstalowywanie

Gdy nie potrzebujesz już DevilRoute, mamy dwa szybkie skrypty do czystego usunięcia:

| Polecenie                | Działanie                                                                        |
| ------------------------ | -------------------------------------------------------------------------------- |
| `npm run uninstall`      | Usuwa aplikację systemową, ale **zachowuje DB i konfiguracje** w `~/.devilroute`. |
| `npm run uninstall:full` | Usuwa aplikację ORAZ trwale **kasuje wszystkie konfiguracje, klucze i bazy**.    |

> Uwaga: Aby uruchomić te polecenia, przejdź do folderu projektu DevilRoute (jeśli klonowałeś) i je wykonaj. Alternatywnie, przy instalacji globalnej możesz po prostu uruchomić `npm uninstall -g devilroute`.
...
export DATA_DIR="/var/lib/devilroute"
export PORT="20128"
...
npm run start
# Or: pm2 start npm --name devilroute -- start
```
...
pm2 start npm --name devilroute -- start

# Or with custom memory limit
DEVILROUTE_MEMORY_MB=512 pm2 start npm --name devilroute -- start
...
docker build -t devilroute:cli .

# Portable mode (recommended)
docker run -d --name devilroute -p 20128:20128 --env-file ./.env -v devilroute-data:/app/data devilroute:cli
```
...
Użytkownicy Void Linux mogą spakować i zainstalować DevilRoute natywnie przez framework cross-kompilacji `xbps-src`. Automatyzuje to build standalone Node.js wraz z wymaganymi natywnymi bindingami `better-sqlite3`.
...
| --------------------------------------- | ------------------------------------ | ------------------------------------------------------------------------------------------------------------------------ |
| `JWT_SECRET`                            | `devilroute-default-secret-change-me` | Sekret podpisu JWT (**zmień w produkcji**)                                                                               |
| `INITIAL_PASSWORD`                      | `CHANGEME`                           | Hasło pierwszego logowania                                                                                               |
| `DATA_DIR`                              | `~/.devilroute`                       | Katalog danych (db, usage, logi)                                                                                         |
...
| `NEXT_PUBLIC_CLOUD_URL`                 | `https://devilroute.dev`              | Bazowy URL endpointu cloud sync (zastępuje legacy `CLOUD_URL`)                                                           |
...
| `DEVILROUTE_MEMORY_MB`                   | `512`                                | Limit sterty Node.js w MB                                                                                                |
...
### Łączenie peerów DevilRoute

Inną bramę DevilRoute możesz dodać jako provider **Custom OpenAI-compatible**. Użyj
...
```bash
# gateway-a
DEVILROUTE_INSTANCE_ID=gateway-a
DEVILROUTE_PEER_URLS=http://gateway-b:20128/v1
DEVILROUTE_PEER_MAX_HOPS=4
```

```bash
# gateway-b
DEVILROUTE_INSTANCE_ID=gateway-b
DEVILROUTE_PEER_URLS=http://gateway-a:20128/v1
DEVILROUTE_PEER_MAX_HOPS=4
```

Tylko żądania wysyłane na jawnie dozwolony URL peeru otrzymują nagłówek
`X-DevilRoute-Peer-Trace`. Brama odrzuca powtórzone ID instancji lub wyczerpany budżet hopów
...
do wielu uruchomionych instancji DevilRoute.
...
- Quick Tunnels nie są automatycznie przywracane po restarcie DevilRoute lub kontenera; włącz je ponownie z dashboardu w razie potrzeby
...
- **Semantic Cache** — automatycznie cache'uje odpowiedzi non-streaming z temperature=0 (pomiń przez `X-DevilRoute-No-Cache: true`)
- **Idempotencja żądań** — deduplikuje żądania w ciągu 5s przez nagłówek `Idempotency-Key` lub `X-Request-Id`
- **Śledzenie postępu** — opcjonalne zdarzenia SSE `event: progress` przez nagłówek `X-DevilRoute-Progress: true`

---

### Translator Playground

Dostęp: **Dashboard → Translator**. Debuguj i wizualizuj, jak DevilRoute tłumaczy żądania API między providerami.
...
DevilRoute akceptuje też `x_session_id` i zwraca efektywny klucz sesji w `X-DevilRoute-Session-Id`.
...
DevilRoute implementuje odporność na poziomie providera w pięciu komponentach:
...
4. **Wait For Cooldown** — jeśli każde połączenie-kandydat jest już w cooldown, DevilRoute może poczekać na najwcześniejszy cooldown i automatycznie ponowić to samo żądanie klienta.
...
- Migruj DevilRoute między maszynami
...
DevilRoute obsługuje transkrypcję audio przez endpoint zgodny z OpenAI:
...
DevilRoute obcina go do tego limitu przed wysłaniem żądania upstream.
...
DevilRoute ma wbudowany **auto-router oparty na score**, który wybiera najlepszy model dla każdego żądania spośród podłączonych providerów — bez utrzymywania combo. Wystarczy wysłać żądanie z jednym z prefiksów `auto/*`, a DevilRoute złoży wirtualne combo w locie, oceniając kandydatów pod kątem opóźnienia, kosztu, success rate, dopasowania kontekstu, przydatności modelu do zadania, ostatnich błędów, quoty i stanu circuit breakera.
...
curl -X POST http://localhost:20128/v1/chat/completions \
  -H "Authorization: Bearer $DEVILROUTE_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "model": "auto/coding",
...
DevilRoute jest jednocześnie **serwerem MCP** (Model Context Protocol) i **serwerem A2A** (Agent-to-Agent JSON-RPC 2.0). Każde IDE lub host agentów zgodny z MCP może wywoływać narzędzia DevilRoute bezpośrednio — bez dodatkowego wrappera.

### Transporty MCP

- **SSE**: `http://localhost:20128/api/mcp/sse`
- **Streamable HTTP**: `http://localhost:20128/api/mcp/stream`
- **stdio**: `devilroute --mcp` (dla wtyczek IDE preferujących stdio)
...
DevilRoute udostępnia rozszerzalny **framework skills** (`src/lib/skills/`), dzięki czemu agenci i endpoint A2A mogą uruchamiać rutyny domenowe (np. `code-review`, `summarize`, `extract-facts`, `web-research`).
...
DevilRoute przechowuje **długoterminową pamięć konwersacyjną** z hybrydowym retrieval:
...
Subskrybuj zdarzenia DevilRoute do monitoringu i automatyzacji w czasie rzeczywistym.
...
- Każdy payload zawiera `X-DevilRoute-Signature` (HMAC-SHA256) do weryfikacji
...
DevilRoute integruje się z cloud coding agents (**OpenAI Codex Cloud**, **Devin**, **Jules**, **Antigravity**), dzięki czemu możesz wysyłać długotrwałe zadania z tego samego dashboardu, który obsługuje lokalny routing.

- Twórz zadania w **Dashboard → Cloud Agents** lub przez `POST /api/v1/agents/tasks`
- Śledź status, logi i artefakty per zadanie
- Własny klucz API per provider — credentials nigdy nie opuszczają instancji DevilRoute
...
Możesz zarządzać każdym zasobem DevilRoute (providerzy, combo, klucze, ustawienia) przez HTTP, używając klucza zarządzania.
...
curl http://localhost:20128/api/providers \
  -H "Authorization: Bearer $DEVILROUTE_MANAGE_KEY"
...
curl http://localhost:20128/api/keys -H "Authorization: Bearer $DEVILROUTE_MANAGE_KEY"
curl -X POST http://localhost:20128/api/keys -H "Authorization: Bearer $DEVILROUTE_MANAGE_KEY" \
...
DevilRoute dostarcza wewnętrzne CLI (`devilroute …`) do setupu, diagnostyki i kontroli runtime.

```bash
devilroute setup                    # Interactive wizard (password, providers, combos)
devilroute setup --non-interactive  # CI-friendly
devilroute doctor                   # Health diagnostics (data dir, DB, providers, ports)
devilroute providers available      # List supported providers
devilroute providers list           # List configured connections
devilroute providers test <id>      # Live test a provider connection
devilroute combos list              # List combos
devilroute combos switch <name>     # Set default combo
devilroute models                   # List available models (--json, --search)
devilroute keys add | list | remove # Manage API keys from the terminal
devilroute backup                   # Snapshot config + DB
devilroute restore [<timestamp>]    # Restore from a snapshot
devilroute health                   # Detailed health (breakers, cache, memory)
devilroute quota                    # Provider quota usage
devilroute mcp status               # MCP server status
devilroute a2a status               # A2A server status
devilroute tunnel list|create|stop  # Cloudflare/Tailscale/ngrok tunnels
devilroute reset-password           # Reset the admin password
devilroute --mcp                    # Start MCP server over stdio
devilroute --port 3000              # Start the server on a custom port
```

Wskazówka: połącz `devilroute doctor --json` z narzędziem monitoringu, by alertować o niezdrowych providerach.

---

## 🖥️ Aplikacja desktopowa (Electron)

DevilRoute jest dostępny jako natywna aplikacja desktopowa na Windows, macOS i Linux.
...
| Zmienna               | Domyślnie | Opis                               |
| --------------------- | --------- | ---------------------------------- |
| `DEVILROUTE_PORT`      | `20128`   | Port serwera                       |
| `DEVILROUTE_MEMORY_MB` | `512`     | Limit sterty Node.js (64–16384 MB) |
