---
title: "Tryb zdalny — steruj zdalnym DevilRoute z laptopa"
version: 3.8.40
...
Uruchom CLI `devilroute` na laptopie, podczas gdy samo DevilRoute działa gdzie indziej
(VPS, serwer domowy, inna maszyna w Tailnet). Logujesz się raz przez
`devilroute connect`, a odtąd **każde** polecenie CLI celuje w ten zdalny
serwer — te same komendy, ten sam wynik, tylko wykonane względem zdalnego hosta.

Nie ma drugiego narzędzia do instalacji: tryb zdalny to zwykłe CLI `devilroute`
plus tokeny dostępu ze **scope**.

```bash
npm install -g devilroute                 # the normal CLI
devilroute connect 192.168.0.15           # log in (password → scoped token)
devilroute models list                    # ← now lists the REMOTE server's models
devilroute configure codex                # ← writes a local Codex profile from the remote catalog
```
...
```
your laptop                              remote DevilRoute (VPS)
┌────────────────────┐                   ┌───────────────────────────────┐
│ devilroute CLI      │  POST /api/cli/connect  (password → token)         │
...
- **Konteksty** przechowują po jednym serwerze (`~/.devilroute/config.json`, `chmod 600`).
  `devilroute contexts use <name>` przełącza aktywny serwer; `default` to lokalny.
...
```bash
devilroute connect 192.168.0.15
...
```bash
devilroute connect 192.168.0.15 --scope write
```

Opcje: `--port <p>` (gdy host nie ma portu), `--name <ctx>` (nazwa kontekstu),
`--scope read|write|admin`. Pełny URL jest honorowany bez zmian:
`devilroute connect https://omni.example.com`.

### Wcześniej wygenerowanym tokenem

Wygeneruj token ze scope w dashboardzie (lub przez `devilroute tokens create`) i
wklej go — hasło nie jest potrzebne:

```bash
devilroute connect 192.168.0.15 --key oma_live_xxxxxxxx
```
...
używającym dołączonego klienta desktop Antigravity, nie tylko w DevilRoute.)
...
Są dwa obsługiwane sposoby podłączenia Antigravity do zdalnego DevilRoute.
...
npx devilroute login antigravity
...
#
#   devilroute-cred-v1.eyJ2IjoxLCJ...
```

Następnie w **zdalnym** dashboardzie: **Providers → Antigravity → Connect** i
wklej blob `devilroute-cred-v1.…` w pole **Step 2** (akceptuje albo
URL callbacku, albo blob poświadczeń). DevilRoute dekoduje go, uruchamia onboarding Cloud Code
...
`redirect_uri` w upstreamowej aplikacji OAuth. DevilRoute nie może go zmienić — provider
...
DevilRoute nasłuchuje na loopbacku **serwera**. Otwórz dashboard pod adresem LAN
...
- **`20128`** (port dashboardu) sprawia, że origin jest prawdziwym localhostem, co w ogóle
  powoduje, że DevilRoute uruchamia serwer callback PKCE — origin LAN nigdy
...
```bash
devilroute tokens create --name "laptop" --scope write [--expires 30]
#   ↳ prints the secret ONCE — copy it now
devilroute tokens list                 # masked: id, name, scope, prefix, status, expiry
devilroute tokens revoke <id|prefix>   # revoke immediately
devilroute tokens scopes               # explain the three scopes
```
...
`devilroute configure` czyta żywy katalog modeli **aktywnego serwera** i zapisuje
konfigurację na **Twojej** maszynie.

```bash
devilroute configure codex
...
# non-interactive
devilroute configure codex --provider glm --model glm/glm-5.2 --name glm52
```

Zapisany profil odwołuje się do klucza inferencji przez zmienną env
(`DEVILROUTE_API_KEY`) — sekret nigdy nie trafia na dysk. Jednorazową
bazową konfigurację Codex (blok `[model_providers.devilroute]`) zobacz w
...
| ----------- | -------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Codex       | `devilroute setup-codex`    | profile `~/.codex/<name>.config.toml` (per model)                                                                                                                         |
| Claude Code | `devilroute setup-claude`   | `~/.claude/profiles/<name>/settings.json` (per model)                                                                                                                     |
| OpenCode    | `devilroute setup-opencode` | `~/.config/opencode/opencode.json` — provider `devilroute` openai-compatible z każdym modelem z katalogu (uruchom `opencode -m devilroute/<model>`)                         |
| Cline       | `devilroute setup-cline`    | `~/.cline/data/{globalState,secrets}.json` (tryb CLI) + wypisuje ustawienia rozszerzenia VS Code do wklejenia (OpenAI-compatible, Base URL **bez** `/v1`)                 |
| Kilo Code   | `devilroute setup-kilo`     | `~/.local/share/kilo/auth.json` (CLI) + ustawienia VS Code `kilocode.*` — OpenAI-compatible, Base URL **z** `/v1`                                                         |
| Continue    | `devilroute setup-continue` | `~/.continue/config.yaml` (VS Code/JetBrains + CLI `cn`) — `provider: openai`, `apiBase` **z** `/v1`, klucz przez `${{ secrets.DEVILROUTE_API_KEY }}`                      |
| Cursor      | `devilroute setup-cursor`   | wypisuje kroki w aplikacji (Settings → Models → Override OpenAI Base URL **z** `/v1` + klucz + model). Konfiguracja Cursor to nieprzezroczyste SQLite — tylko panel czatu |
| Roo Code    | `devilroute setup-roo`      | zapisuje JSON importu Roo (`~/.devilroute/roo-settings.json`) + ustawia `roo-cline.autoImportSettingsPath` + wypisuje kroki UI (OpenAI-compatible, Base URL **z** `/v1`)   |
| Crush       | `devilroute setup-crush`    | `~/.config/crush/crush.json` — provider `openai-compat`, `base_url` **z** `/v1`, klucz przez `$DEVILROUTE_API_KEY`                                                         |
| Goose       | `devilroute setup-goose`    | `~/.config/goose/config.yaml` (`GOOSE_PROVIDER=openai` + `OPENAI_HOST` **bez** `/v1` + `GOOSE_MODEL`) + przepis env                                                       |
| Aider       | `devilroute setup-aider`    | `~/.aider.conf.yml` (`openai-api-base` **bez** `/v1` + `model: openai/<id>`) + przepis env (`aider --message --yes`)                                                      |
| Qwen Code   | `devilroute setup-qwen`     | wpis V4 `modelProviders.openai` w `~/.qwen/settings.json` + `DEVILROUTE_API_KEY` w `~/.qwen/.env`                                                                          |
...
devilroute setup-opencode --only glm,kimi        # keep only matching models
opencode -m devilroute/glm/glm-5.2 "..."          # export DEVILROUTE_API_KEY first
```

> OpenCode ma też bogatszą integrację **plugin**: `devilroute setup opencode`
> (teraz świadome trybu zdalnego przez `--remote`) instaluje `@devilroute/opencode-plugin`.
> `setup-opencode` to lekka alternatywa openai-compatible. Klucz API
> jest odwoływany przez `{env:DEVILROUTE_API_KEY}` — nigdy nie zapisywany na dysk.

---

## Zarządzanie kontekstami (przełączanie między serwerami)

**Kontekst** to zapisany serwer (baseUrl + poświadczenie + scope). `devilroute connect`
tworzy jeden i czyni go aktywnym; odtąd każde polecenie go celuje. Zarządzaj i
przełączaj je przez `devilroute contexts`:

```bash
devilroute contexts list            # all contexts; the active one is marked ●
devilroute contexts current         # the active server, auth status, scope
```

```text
  | Name    | Base URL                  | Auth  | Scope | Description
● | vps     | http://100.67.86.91:20128 | token | admin | Remote DevilRoute (…)
...
```bash
devilroute contexts use vps         # → all commands now hit the remote VPS
devilroute tokens list              #   (runs against the VPS)

devilroute contexts use default     # → back to localhost
devilroute tokens list              #   (runs against the local server)
```

**Dodaj kontekst ręcznie** (zamiast `connect`), podejrzyj lub zmień nazwę:

```bash
devilroute contexts add staging --url https://staging.example.com:20128 \
  --access-token oma_live_xxxx --scope write --description "staging box"
devilroute contexts show staging    # full details for one context
devilroute contexts rename staging stg
```
...
```bash
devilroute contexts remove stg --yes
```
...
> odwołaj token na serwerze przez `devilroute tokens revoke <id>`, by faktycznie
> unieważnić dostęp.
...
```bash
devilroute contexts export --out contexts.json     # default: stdout
devilroute contexts import contexts.json            # overwrite; --merge to keep existing
```
...
devilroute connect 192.168.0.15                 # or: --key oma_live_xxxx  (no password)
devilroute contexts current                     # shows the remote server + scope

# 2. Use it — management commands now run against the remote
devilroute tokens create --name laptop --scope read   # mint a narrower token
devilroute tokens list                                 # masked list, from the remote

# 3. Switch back and forth
devilroute contexts use default                 # → local
devilroute contexts use 192-168-0-15            # → remote again (name from `contexts list`)
...
devilroute tokens revoke <id|prefix>            # kills access on the server
devilroute contexts remove 192-168-0-15 --yes   # drop the local context (even if active → falls back to default), no prompt
```
...
- `devilroute connect` korzysta z tej samej blokady brute-force logowania + logowania audytowego.
...
- Lokalny plik kontekstów to `~/.devilroute/config.json` (`chmod 600`); tokeny
