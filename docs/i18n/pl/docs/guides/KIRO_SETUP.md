...
Ten przewodnik opisuje dodawanie kont Kiro (hostowany w AWS asystent AI do kodowania) do DevilRoute,
...
## Jak DevilRoute to rozwiązuje (v3.8.0+)

Od v3.8.0 DevilRoute wywołuje `registerClient()` (AWS SSO OIDC) przy każdym
imporcie połączenia Kiro. Dzięki temu każde połączenie DevilRoute ma własną, dedykowaną
...
2. Wybierz jedną z opcji:
   ...
   - **Auto-Import** — kliknij przycisk; DevilRoute odczyta poświadczenia z
...
4. DevilRoute waliduje klucz i zapisuje połączenie.
...
(`clientSecretExpiresAt`). DevilRoute przechowuje ten znacznik czasu w `providerSpecificData`
...
- Upewnij się, że DevilRoute może dotrzeć do `https://oidc.us-east-1.amazonaws.com` (lub
...
- Klucz musi móc wywołać `ListAvailableProfiles`; w przeciwnym razie DevilRoute nie może
