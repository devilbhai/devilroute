...
Jak DevilRoute szacuje, rejestruje i raportuje koszt każdego żądania — oraz dlaczego
...
DevilRoute przypisuje koszt w USD do każdego completion, mnożąc liczbę tokenów przez
stawki cenowe modelu. Te liczby zasilają dashboard **Costs**, CLI
`devilroute cost` / `devilroute usage`, eksporty CSV/JSON oraz budżety per klucz API.

> **„Koszt” na dashboardzie to tracker oszczędności, a nie rachunek.** DevilRoute nigdy
...
- Zależy od tabeli cen DevilRoute dla każdego modelu. Model bez wpisu cenowego
...
3. **Zakodowane na stałe domyślne** — dostarczane z DevilRoute.
...
- Wydatki kluczy API są buforowane i flushowane partiami przez
  ...
  | Env var                             | Default | Cel                                             |
  | ----------------------------------- | ------- | ----------------------------------------------- |
  | `DEVILROUTE_SPEND_FLUSH_INTERVAL_MS` | `60000` | Interwał flush w milisekundach.                 |
  | `DEVILROUTE_SPEND_MAX_BUFFER_SIZE`   | `1000`  | Maks. liczba buforowanych wpisów przed flushem. |
...
CLI DevilRoute udostępnia komendy cost, usage i pricing (zarejestrowane w
...
```bash
devilroute cost                          # last 30d, grouped by provider
devilroute cost --period 7d              # last 7 days
devilroute cost --group-by model         # group by provider | model | combo | api-key | day
devilroute cost --since 2026-06-01 --until 2026-06-13
devilroute cost --api-key <key> --limit 50
```
...
```bash
devilroute usage analytics --period 30d [--provider <id>]   # per-provider cost summary
devilroute usage logs [--limit 100] [--follow] [--api-key <k>] [--search <q>]
devilroute usage quota [--provider <id>] [--check]
devilroute usage utilization [--api-key <k>]
devilroute usage history [--limit 100]
devilroute usage proxy-logs [--limit 100]

# Budgets
devilroute usage budget list
devilroute usage budget get [scope]
devilroute usage budget set <amount> [--scope global] [--period monthly]
devilroute usage budget reset [scope]
```

### `devilroute pricing`

```bash
devilroute pricing list [--provider <p>] [--model <m>] [--limit 200]
devilroute pricing get <model>
devilroute pricing sync [--provider <p>] [--force]   # POST /api/pricing/sync
devilroute pricing diff [--model <m>]
devilroute pricing defaults show
devilroute pricing defaults set [--input <p>] [--output <p>] [--cache-read <p>] [--cache-write <p>]
```
...
- **Wszystkie koszty pokazują $0 / „Legacy / Free”.** Używane modele nie mają wpisu
  ...
  `devilroute pricing sync`, albo ustaw ceny ręcznie na stronie Pricing / przez
  `PATCH /api/pricing`.
...
- **Wydatki opóźniają się względem czasu rzeczywistego.** Spend per klucz jest batchowany;
  obniż `DEVILROUTE_SPEND_FLUSH_INTERVAL_MS`, jeśli potrzebujesz świeższych liczb.
