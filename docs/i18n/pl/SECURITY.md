...
Jeśli odkryjesz lukę bezpieczeństwa w DevilRoute, zgłoś ją w odpowiedzialny sposób:

1. **NIE** otwieraj publicznego zgłoszenia (issue) na GitHub
2. Użyj [GitHub Security Advisories](https://github.com/diegosouzapw/DevilRoute/security/advisories/new)
...
DevilRoute wdraża wielowarstwowy model bezpieczeństwa:
...
DevilRoute dostarcza przeładowywalny na gorąco **rejestr guardrails** (`src/lib/guardrails/`) z 3 wbudowanymi guardrails uporządkowanymi według priorytetu:
...
Własne guardrails rejestruje się przez `registerGuardrail(new MyGuardrail())`. Model jest fail-open (wyjątki nigdy nie blokują ruchu). Rezygnacja per żądanie przez nagłówek `x-devilroute-disabled-guardrails`. → Zob. [`docs/security/GUARDRAILS.md`](docs/security/GUARDRAILS.md).
...
Opublikowany artefakt npm `devilroute` bundluje build Next.js `output: "standalone"`,
...
`DEVILROUTE_BUILD_PROFILE=minimal npm run build`. To zastępuje cztery
