...
**DevilRoute** — unified AI proxy/router. One endpoint, 290 LLM providers, auto-fallback.
...
DevilRoute has three related but distinct temporary-failure mechanisms. Keep their
...
- **Imports**: external → internal (`@/`, `@devilroute/open-sse`) → relative
...
repo** instead of polluting the main DevilRoute tree.
...
- **Path aliases**: `@/*` → `src/`, `@devilroute/open-sse` → `open-sse/`, `@devilroute/open-sse/*` → `open-sse/*`
- **Default port**: 20128 (API + dashboard on same port)
- **Data directory**: `DATA_DIR` env var, defaults to `~/.devilroute/`
...
DevilRoute has **~48 quality-gate scripts** (`scripts/check/` + `scripts/quality/`) wired
