# 🚀 DevilRoute — Gateway AI Gratis (Bahasa Indonesia)
...
_Hubungkan IDE atau alat CLI berbasis AI apa pun melalui DevilRoute — gateway API gratis untuk coding tanpa batas._
...
<sub>📡 Semua agen terhubung melalui <code>http://localhost:20128/v1</code> atau <code>http://cloud.devilroute.online/v1</code> — satu konfigurasi, model dan kuota tak terbatas</sub>
...
## 🎯 Apa yang Diselesaikan DevilRoute — 30 Masalah Nyata & Kasus Penggunaan

> **Setiap developer yang menggunakan alat AI menghadapi masalah ini setiap hari.** DevilRoute dibangun untuk menyelesaikannya semua — dari pembengkakan biaya hingga pemblokiran regional, dari alur OAuth yang rusak hingga operasi protokol dan observabilitas enterprise.
...
**Cara DevilRoute menyelesaikannya:**
...
**Bagaimana DevilRoute menyelesaikannya:**
...
- **Laporan Info Sistem** — `npm run system-info` menghasilkan `system-info.txt` dengan lingkungan lengkap Anda (versi Node, versi DevilRoute, OS, alat CLI, status Docker/PM2). Lampirkan saat melaporkan masalah untuk triase instan.
...
- **instal global npm** — `npm install -g devilroute && devilroute` — selesai
...
- Mulai cepat bawaan untuk `devilroute --mcp` dan orientasi klien
...
Pengguna Arch Linux dapat menginstal [AUR package](https://aur.archlinux.org/packages/devilroute-bin), yang menginstal DevilRoute dan menyediakan layanan pengguna systemd:
...
| Command                 | Description                                                     |
| ----------------------- | --------------------------------------------------------------- |
| `devilroute`             | Mulai server (`PORT=20128`, API dan dasbor pada port yang sama) |
| `devilroute --port 3000` | Set canonical/API port to 3000                                  |
| `devilroute --mcp`       | Mulai server MCP (stdio transport)                              |
...
Saat Anda tidak lagi memerlukan DevilRoute, kami menyediakan dua skrip cepat untuk penghapusan bersih:

| Command                  | Action                                                                                       |
| ------------------------ | -------------------------------------------------------------------------------------------- |
| `npm run uninstall`      | Menghapus aplikasi sistem tetapi **menyimpan DB dan konfigurasi Anda** di `~/.devilroute`.    |
| `npm run uninstall:full` | Menghapus aplikasi DAN secara permanen **menghapus semua konfigurasi, kunci, dan database**. |

> Catatan: Untuk menjalankan perintah ini, navigasikan ke folder proyek DevilRoute (jika Anda mengkloningnya) dan jalankan. Alternatifnya, jika diinstal secara global, Anda cukup menjalankan `npm uninstall -g devilroute`.
...
| `STREAM_IDLE_TIMEOUT_MS` | inherits `REQUEST_TIMEOUT_MS` | Kesenjangan maksimum antara potongan streaming sebelum DevilRoute membatalkan aliran SSE                                                                                      |
...
Untuk upstream yang kompatibel dengan Kode Claude (`anthropic-compatible-cc-*`), DevilRoute juga memperoleh header `X-Stainless-Timeout` keluar dari batas waktu pengambilan yang diselesaikan sehingga batas waktu baca sisi penyedia tetap selaras dengan konfigurasi env Anda.

Untuk reverse proxy pihak ketiga yang kompatibel dengan Claude Code, DevilRoute tetap menggunakan default
...
`cache_control`, DevilRoute tidak memasukkan penanda milik jembatan.
...
Untuk permintaan streaming, `FETCH_TIMEOUT_MS` hanya mencakup pengaturan koneksi/menunggu respons upstream pertama. Setelah aliran aktif, DevilRoute hanya akan dibatalkan pada keadaan terhenti sebenarnya (`STREAM_IDLE_TIMEOUT_MS`) atau tubuh Undici tidak aktif (`FETCH_BODY_TIMEOUT_MS`).

Jika Anda menjalankan DevilRoute di belakang Nginx, Caddy, Cloudflare, atau proksi terbalik lainnya, pastikan proksi tersebut
waktu tunggu juga lebih tinggi daripada waktu tunggu aliran/pengambilan DevilRoute Anda.
...
### 3) Arahkan alat pengkodean Anda ke DevilRoute
...
Untuk pengguna Void Linux, Anda dapat membuat paket asli menggunakan `xbps-src`. Simpan blok ini sebagai `srcpkgs/devilroute/template`:
...
- Terowongan Cepat tidak dipulihkan secara otomatis setelah DevilRoute atau kontainer dimulai ulang. Aktifkan kembali dari dasbor bila diperlukan.
...
- SQLite berjalan dalam mode WAL. `docker stop` harus dibiarkan selesai sehingga DevilRoute dapat memeriksa kembali perubahan terbaru ke `storage.sqlite`.
...
- Setel `CLOUDFLARED_BIN=/absolute/path/to/cloudflared` jika Anda ingin DevilRoute menggunakan biner yang sudah ada alih-alih mengunduhnya.

**Menggunakan Docker Compose dengan Caddy (HTTPS Auto-TLS):**

DevilRoute dapat diekspos dengan aman menggunakan penyediaan SSL otomatis Caddy. Pastikan data DNS A domain Anda mengarah ke IP server Anda.
...
> 🆕 **BARU!** DevilRoute kini tersedia sebagai **aplikasi desktop asli** untuk Windows, macOS, dan Linux.

Jalankan DevilRoute sebagai aplikasi desktop mandiri — tanpa terminal, tanpa browser, tanpa internet untuk model lokal. Aplikasi berbasis Electron meliputi:

- 🖥️ **Jendela Asli** — Jendela aplikasi khusus dengan integrasi baki sistem
- 🔄 **Mulai Otomatis** — Luncurkan DevilRoute saat login sistem
...
Saat diminimalkan, DevilRoute ada di baki sistem Anda dengan tindakan cepat:
...
**Tanpa biaya. Jangan pernah berhenti melakukan pengkodean.** Konfigurasikan ini sebagai satu kombo DevilRoute dan semua fallback terjadi secara otomatis — tidak pernah ada peralihan manual.
...
> Semua model di bawah **100% gratis tanpa memerlukan kartu kredit**. DevilRoute melakukan rute otomatis di antara keduanya ketika satu kuota habis — gabungkan semuanya untuk kombo $0 yang tidak dapat dipecahkan.
...
DevilRoute v3.6 dibangun sebagai platform operasional, bukan hanya proxy relai.
...
Ketika kuota, tarif, atau kesehatan gagal, DevilRoute secara otomatis berpindah ke kandidat berikutnya tanpa peralihan manual.
...
DevilRoute menyertakan kerangka evaluasi bawaan untuk menguji kualitas respons LLM terhadap rangkaian emas. Akses melalui **Analytics → Evals** di dasbor.

### Golden Set Bawaan

"DevilRoute Golden Set" yang dimuat sebelumnya berisi kasus uji untuk:
...
2. Jalankan `devilroute_get_health`.
3. Jalankan `devilroute_list_combos`.
...
**Kiat Pro:** Gunakan Opus untuk tugas kompleks, Soneta untuk kecepatan. DevilRoute melacak kuota per model!
...
- Perilaku rotasi: DevilRoute merutekan ke akun Codex berikutnya yang memenuhi syarat secara otomatis.
...
**Kiat Pro:** API yang kompatibel dengan OpenAI — bekerja secara lancar dengan terjemahan format DevilRoute!
...
> **Catatan:** OpenClaw hanya berfungsi dengan DevilRoute lokal. Gunakan `127.0.0.1` alih-alih `localhost` untuk menghindari masalah resolusi IPv6.
...
**Langkah 1:** Tambahkan DevilRoute sebagai penyedia khusus:
...
> **Tips:** Tambahkan model apa pun yang tersedia di titik akhir DevilRoute `/v1/models` Anda ke bagian `models`. Gunakan format `provider/model-id` dari dasbor DevilRoute Anda.
...
- Disegarkan secara otomatis oleh DevilRoute
...
- DevilRoute v1.0.6+ menyertakan validasi fallback melalui penyelesaian obrolan
...
> **⚠️ Penting bagi pengguna yang menjalankan DevilRoute di VPS, Docker, atau server jarak jauh mana pun**

Kredensial OAuth yang disertakan dalam DevilRoute didaftarkan **hanya untuk `localhost`**. Saat Anda mengakses DevilRoute di server jarak jauh (misalnya `https://devilroute.myserver.com`), Google menolak autentikasi dengan:
...
- Nama: apa pun yang Anda suka (mis. `DevilRoute Remote`)
...
1. DevilRoute membuka URL otorisasi Google
...
4. Tempelkan URL tersebut ke bidang yang ditampilkan di modal koneksi DevilRoute
5. Klik **"Hubungkan"**
...
DevilRoute memiliki **218+ fitur yang direncanakan** di berbagai fase pengembangan. Berikut adalah bidang-bidang utamanya:
