# DevilRoute — Panduan Mencopot Pemasangan (Bahasa Indonesia)
...
Panduan ini menjelaskan cara mencopot pemasangan DevilRoute dari sistem Anda secara bersih.

---

## Mencopot Pemasangan dengan Cepat (v3.6.2+)

DevilRoute menyediakan dua skrip bawaan untuk penghapusan yang bersih:
...
Perintah ini menghapus aplikasi DevilRoute tetapi **mempertahankan** basis data, konfigurasi, kunci API, dan pengaturan penyedia Anda di `~/.devilroute/`. Gunakan ini jika Anda berencana memasang ulang nanti dan ingin menyimpan pengaturan yang ada.
...
- Berkas cadangan
- Berkas log
- Semua berkas di direktori `~/.devilroute/`
...
- Buka `Settings → Apps → DevilRoute → Uninstall`
- Atau jalankan uninstaller NSIS dari direktori instalasi

**macOS:**

- Seret `DevilRoute.app` dari `/Applications` ke Trash
- Hapus data: `rm -rf ~/Library/Application Support/devilroute`

**Linux:**

- Hapus berkas AppImage
- Hapus data: `rm -rf ~/.devilroute`

### Instalasi dari Sumber (git clone)

```bash
# Remove the cloned directory
rm -rf /path/to/devilroute
...
DevilRoute menyimpan data di lokasi-lokasi berikut secara default:

| Platform      | Jalur Default                 | Pengganti                 |
| ------------- | ----------------------------- | ------------------------- |
| Linux         | `~/.devilroute/`               | `DATA_DIR` env var        |
| macOS         | `~/.devilroute/`               | `DATA_DIR` env var        |
| Windows       | `%APPDATA%/devilroute/`        | `DATA_DIR` env var        |
| Docker        | `/app/data/` (mounted volume) | `DATA_DIR` env var        |
| XDG-compliant | `$XDG_CONFIG_HOME/devilroute/` | `XDG_CONFIG_HOME` env var |
...
npm list -g devilroute 2>/dev/null

# Check for data directory
ls -la ~/.devilroute/ 2>/dev/null
