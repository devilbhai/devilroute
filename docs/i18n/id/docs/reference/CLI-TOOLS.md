# Panduan Pengaturan Alat CLI — DevilRoute (Bahasa Indonesia)
...
untuk menggunakan **DevilRoute** sebagai backend terpadu, memberikan manajemen kunci terpusat,
...
Claude / Codex / OpenCode / Cline / KiloCode / Continue / Kiro / Cursor / Copilot
           │
           ▼  (semua mengarah ke DevilRoute)
    http://YOUR_SERVER:20128/v1
           │
           ▼  (DevilRoute meneruskan ke penyedia yang tepat)
...
| **Antigravity**    | `antigravity` | internal   | mitm            | DevilRoute        |
...
## Langkah 1 — Dapatkan API Key DevilRoute

1. Buka dashboard DevilRoute → **API Manager** (`/dashboard/api-manager`)
...
export ANTHROPIC_API_KEY="sk-your-devilroute-key"
...
api_key = "sk-your-devilroute-key"
EOF
```
...
Atau gunakan dashboard DevilRoute → **CLI Tools → Cline → Apply Config**.
...
Atau gunakan dashboard DevilRoute → **CLI Tools → KiloCode → Apply Config**.
...
# CLI ini menggunakan autentikasinya sendiri — DevilRoute tidak diperlukan sebagai backend untuk Kiro CLI itu sendiri.
# Gunakan kiro-cli bersama DevilRoute untuk alat lainnya.
kiro-cli status
```
...
> **Catatan:** Cursor merutekan permintaan melalui cloudnya sendiri. Untuk integrasi DevilRoute,
> aktifkan **Cloud Endpoint** di Pengaturan DevilRoute dan gunakan URL domain publik Anda.

Melalui GUI: **Settings → Models → OpenAI API Key**

- Base URL: `https://your-domain.com/v1`
- API Key: kunci DevilRoute Anda

---

## Konfigurasi Otomatis Dashboard

Dashboard DevilRoute mengotomatiskan konfigurasi untuk sebagian besar alat:
...
**Droid** dan **OpenClaw** adalah agen AI yang dibangun langsung ke dalam DevilRoute — tidak perlu instalasi.
Keduanya berjalan sebagai rute internal dan menggunakan perutean model DevilRoute secara otomatis.
...
| ------------------------- | --------------------------------- | ------------------------------------------ |
| `Connection refused`      | DevilRoute tidak berjalan          | `pm2 start devilroute`                      |
...
```bash
# Instal semua CLI dan konfigurasi untuk DevilRoute (ganti dengan kunci dan URL server Anda)
DEVILROUTE_URL="http://localhost:20128/v1"
DEVILROUTE_KEY="sk-your-devilroute-key"
...
cat > ~/.claude/settings.json   <<< "{\"apiBaseUrl\":\"$DEVILROUTE_URL\",\"apiKey\":\"$DEVILROUTE_KEY\"}"
cat > ~/.codex/config.yaml      <<< "model: auto\napiKey: $DEVILROUTE_KEY\napiBaseUrl: $DEVILROUTE_URL"
cat >> ~/.bashrc << EOF
export OPENAI_BASE_URL="$DEVILROUTE_URL"
export OPENAI_API_KEY="$DEVILROUTE_KEY"
export ANTHROPIC_BASE_URL="$DEVILROUTE_URL"
export ANTHROPIC_API_KEY="$DEVILROUTE_KEY"
EOF

source ~/.bashrc
echo "✅ All CLIs installed and configured for DevilRoute"
```
