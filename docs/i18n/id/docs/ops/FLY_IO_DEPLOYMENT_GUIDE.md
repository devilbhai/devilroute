# Panduan Deployment DevilRoute di Fly.io
...
Dokumen ini menjelaskan metode deployment DevilRoute di Fly.io yang telah terbukti berhasil, mencakup dua skenario utama:
...
Dokumen ini disusun berdasarkan konfigurasi yang telah diverifikasi pada proyek saat ini, dengan nama aplikasi `devilroute`.
...
- Alamat akses: `https://devilroute.fly.dev/`
...
- `app = 'devilroute'` menentukan aplikasi Fly mana yang menjadi target deployment
...
- Jika sebelumnya Anda menggunakan nama lain seperti `oroute`, jangan sampai tertukar dengan `devilroute`
...
Jika Anda sudah mengganti nama aplikasi, ganti `devilroute` dengan nama Anda.
...
Parameter-parameter berikut telah digunakan secara nyata pada aplikasi `devilroute` saat ini:
...
- Berlaku untuk proyek saat ini yaitu `devilroute`
...
```powershell
flyctl secrets set INITIAL_PASSWORD=kata-sandi-kuat-anda -a devilroute
```
...
- Apakah aplikasi yang sedang dilihat adalah `devilroute`
...
Jika repositori saat ini adalah fork dan Anda ingin menyinkronkan pembaruan dari upstream `https://github.com/diegosouzapw/DevilRoute`, ikuti alur berikut.
...
- Anda membuka aplikasi yang salah, misalnya `oroute`, bukan `devilroute`
