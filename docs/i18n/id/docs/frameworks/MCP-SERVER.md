# Dokumentasi Server MCP DevilRoute (Bahasa Indonesia)
...
DevilRoute MCP sudah tersedia secara bawaan. Jalankan dengan:
...
| Alat                            | Deskripsi                                                         |
| :------------------------------ | :---------------------------------------------------------------- |
| `devilroute_get_health`          | Kesehatan gateway, pemutus sirkuit, uptime                        |
| `devilroute_list_combos`         | Semua combo yang dikonfigurasi beserta modelnya                   |
| `devilroute_get_combo_metrics`   | Metrik performa untuk combo tertentu                              |
| `devilroute_switch_combo`        | Ganti combo aktif berdasarkan ID/nama                             |
| `devilroute_check_quota`         | Status kuota per penyedia atau semua penyedia                     |
| `devilroute_route_request`       | Kirim penyelesaian chat melalui DevilRoute                         |
| `devilroute_cost_report`         | Analitik biaya untuk periode waktu tertentu                       |
| `devilroute_list_models_catalog` | Katalog model lengkap beserta kemampuannya                        |

## Alat Lanjutan (8)

| Alat                               | Deskripsi                                                              |
| :--------------------------------- | :--------------------------------------------------------------------- |
| `devilroute_simulate_route`         | Simulasi routing percobaan dengan pohon fallback                       |
| `devilroute_set_budget_guard`       | Anggaran sesi dengan tindakan degrade/block/alert                      |
| `devilroute_set_resilience_profile` | Terapkan preset conservative/balanced/aggressive                       |
| `devilroute_test_combo`             | Uji langsung semua model dalam combo melalui permintaan upstream nyata |
| `devilroute_get_provider_metrics`   | Metrik terperinci untuk satu penyedia                                  |
| `devilroute_best_combo_for_task`    | Rekomendasi kesesuaian tugas beserta alternatifnya                     |
| `devilroute_explain_route`          | Jelaskan keputusan routing yang lalu                                   |
| `devilroute_get_session_snapshot`   | Status sesi lengkap: biaya, token, kesalahan                           |
