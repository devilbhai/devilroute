...
**DevilRoute** — proksi/router AI yang bersatu. Satu titik akhir, 160+ penyedia LLM, auto-fallback.
...
DevilRoute mempunyai tiga mekanisme kegagalan sementara yang berkaitan tetapi berbeza. Pastikan skop mereka terpisah semasa menyahpepijat tingkah laku penghalaan. Lihat
...
- **Import**: luaran → dalaman (`@/`, `@devilroute/open-sse`) → relatif
...
- **Alias laluan**: `@/*` → `src/`, `@devilroute/open-sse` → `open-sse/`, `@devilroute/open-sse/*` → `open-sse/*`
- **Port lalai**: 20128 (API + papan pemuka pada port yang sama)
- **Direktori data**: `DATA_DIR` env var, lalai kepada `~/.devilroute/`
