...
**DevilRoute** — verenigde AI proxy/router. Eén eindpunt, 160+ LLM-providers, automatische fallback.
...
DevilRoute heeft drie gerelateerde maar verschillende mechanismen voor tijdelijke fouten. Houd hun
...
- **Imports**: extern → intern (`@/`, `@devilroute/open-sse`) → relatief
...
- **Padaliassen**: `@/*` → `src/`, `@devilroute/open-sse` → `open-sse/`, `@devilroute/open-sse/*` → `open-sse/*`
- **Standaardpoort**: 20128 (API + dashboard op dezelfde poort)
- **Gegevensdirectory**: `DATA_DIR` omgevingsvariabele, standaard `~/.devilroute/`
