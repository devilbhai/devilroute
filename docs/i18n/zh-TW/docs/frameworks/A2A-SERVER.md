---
title: "DevilRoute A2A 伺服器文件"
version: 3.8.40
lastUpdated: 2026-06-28
---

# DevilRoute A2A 伺服器文件

> Agent-to-Agent 協定 v0.3 — DevilRoute 作為智慧路由代理
...
回傳描述 DevilRoute 能力、技能及驗證需求的 Agent Card。
...
DevilRoute 提供 6 個 A2A 技能，配置於 `src/lib/a2a/taskExecution.ts::A2A_SKILL_HANDLERS`。每個技能模組位於 `src/lib/a2a/skills/`。

| 技能               | ID                    | 描述                                                                               | 標籤                        | 範例                                   |
| :----------------- | :-------------------- | :--------------------------------------------------------------------------------- | :-------------------------- | :------------------------------------- |
| Smart Routing      | `smart-routing`       | 透過 DevilRoute 的 combo 引擎 + 評分，將提示路由至最佳提供者/組合                       | routing, providers          | "Route this prompt via the best model" |
...
| List Capabilities  | `list-capabilities`   | 回傳完整的 42 項代理技能目錄，以 Markdown 表格呈現，附原始 SKILL.md 網址供注入上下文     | catalog, discovery, skills  | "List all DevilRoute capabilities"      |
...
`list-capabilities` 技能對於需要在發送 API 呼叫前探索 DevilRoute 能力的外部代理尤其有用。它回傳結構化的 Markdown 表格產出：
