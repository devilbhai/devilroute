...
DevilRoute 整合了三類 CLI 工具，分別對應三個專屬儀表板頁面：

| 頁面               | 路由                    | 概念                                                             | 數量       |
| ------------------ | ----------------------- | ---------------------------------------------------------------- | ---------- |
| **CLI 程式碼工具** | `/dashboard/cli-code`   | 指向 DevilRoute 的程式碼工具（客戶端 → CLI → DevilRoute → 提供者） | 21         |
| **CLI 代理工具**   | `/dashboard/cli-agents` | 指向 DevilRoute 的自動代理工具（相同流程，範圍更廣）              | 6          |
| **ACP 代理**       | `/dashboard/acp-agents` | DevilRoute 透過 stdio/ACP 以反向流程衍生的 CLI                    | 參見註冊表 |
...
ACP 代理（反向衍生流程）：
    客戶端請求 → DevilRoute → 透過 stdio/ACP 衍生 CLI → 回應
```
...
您無需手動編寫每個工具的設定檔。DevilRoute 為每個受支援的 CLI 提供了 `setup-*` 指令，可讀取執行中 DevilRoute（本機或遠端）的**即時**模型目錄，並在您的機器上寫入該工具的設定檔：
...
每個指令都接受 `--remote <url> --api-key <key>`（針對遠端 DevilRoute 設定本機工具）、`--dry-run`（預覽不寫入）和 `--port`。不支援模型自動探索的工具（Cline、Kilo、Roo、Goose、Aider、Gemini）需要 `--model <id>`（以及用於非互動執行的 `--yes`）。啟動器 `devilroute launch`（Claude Code）和 `devilroute launch-codex`（Codex）會以正確的環境變數注入來衍生 CLI，完全不寫入設定檔。
...
此頁面（從 `/dashboard/agents` 重新命名而來）顯示 DevilRoute 可以**衍生**為後端執行引擎（透過 stdio/ACP 協定）的 CLI。目錄獨立維護於 `src/lib/acp/registry.ts`，**不同於** `CLI_TOOLS`。
...
或使用 DevilRoute 儀表板 → **CLI 工具 → Cline → 套用設定**。
...
或使用 DevilRoute 儀表板 → **CLI 工具 → KiloCode → 套用設定**。
...
當 VS Code Insiders 設定為使用自訂端點模型，且您希望 DevilRoute 在無需自訂標頭欄位的情況下運作時使用。
...
- 將 `«redacted:sk-…»` 替換為在 DevilRoute 中建立的 API 金鑰。
...
# CLI 使用自己的認證機制 — Kiro CLI 本身不需要 DevilRoute 作為後端。
# 請將 kiro-cli 與 DevilRoute 搭配使用於其他工具。
kiro-cli status
```

至於 **Kiro IDE** 桌面應用程式，請使用 DevilRoute 在 `/dashboard/cli-tools → Kiro` 提供的 MITM 端點。

---

## 10. 內部 DevilRoute CLI

`devilroute` 二進位檔提供用於伺服器生命週期管理、設定、診斷和提供者管理的指令。進入點：`bin/devilroute.mjs`。

```bash
devilroute                              # 啟動伺服器（預設通訊埠 20128）
devilroute setup                        # 互動式設定精靈
devilroute doctor                       # 檢查設定、資料庫、通訊埠、執行環境
devilroute providers list               # 已設定的提供者連線
devilroute providers test-all           # 測試每個作用中連線
devilroute reset-password               # 重設管理員密碼
devilroute logs                         # 串流要求日誌
devilroute health                       # 詳細健康狀態（斷路器、快取、記憶體）
devilroute --version                    # 顯示版本
devilroute --help                       # 顯示所有指令
```
...
```bash
devilroute setup                        # 互動式設定精靈
devilroute setup --non-interactive      # CI/自動化模式（讀取環境變數 + 旗標）
devilroute setup --password '<value>'   # 直接設定管理員密碼
...
| 變數                | 用途                                                          |
| ------------------- | ------------------------------------------------------------- |
| `DEVILROUTE_API_KEY` | 提供者 API 金鑰（透過 Commander `.env()` 繫結至 `--api-key`） |
| `DATA_DIR`          | 覆寫 DevilRoute 資料目錄                                       |

所有其他非互動式輸入皆以旗標傳遞（非環境變數）：
`--password`、`--provider`、`--provider-name`、`--provider-base-url`、`--default-model`
（請參閱上方 `devilroute setup` 選項）。

### 診斷

```bash
devilroute doctor                       # 檢查設定、資料庫、通訊埠、執行環境、記憶體、運作狀態
devilroute doctor --json                # 機器可讀的 JSON
devilroute doctor --no-liveness         # 跳過 HTTP 健康狀態探測
devilroute doctor --host 0.0.0.0        # 覆寫運作狀態主機
devilroute doctor --liveness-url <url>  # 完整健康端點 URL 覆寫
```
...
```bash
devilroute providers available                       # DevilRoute 提供者目錄
devilroute providers available --search openai       # 依 ID/名稱/別名/類別過濾目錄
devilroute providers available --category api-key    # 依類別過濾（api-key、oauth、free 等）
devilroute providers available --json                # 機器可讀的 JSON

devilroute providers list                            # 已設定的提供者連線
devilroute providers list --json

devilroute providers test <id|name>                  # 測試一個已設定的連線
devilroute providers test-all                        # 測試每個作用中連線
devilroute providers validate                        # 僅限本機的結構驗證
```

> `providers available` 讀取 DevilRoute 目錄；`providers list/test/test-all/validate`
> 直接讀取本機 SQLite 資料庫，無需伺服器執行中。

### 復原與重設

```bash
devilroute reset-password                # 重設管理員密碼（亦可使用：devilroute-reset-password）
devilroute reset-encrypted-columns       # 顯示警告 + 加密憑證重設的試執行
devilroute reset-encrypted-columns --force  # 實際將 SQLite 中的加密憑證設為 null
```

### 憑證匯出（⚠ 請謹慎處理）

```bash
devilroute auth export                                 # 顯示警告 + 確認閘道 — 不會存取資料庫
devilroute auth export --force                          # 將所有連線的**解密後**憑證匯出至 stdout 為 JSON
devilroute auth export --force --id <id>                 # 僅匯出符合條件的連線
devilroute auth export --force --format env               # 輸出為 DEVILROUTE_<PROVIDER>_<FIELD>=<value> 格式
devilroute auth export --force --out creds.json           # 寫入檔案（以 0600 權限建立）
```
...
以下指令假設 DevilRoute 伺服器正在執行中，除非另有說明：

```bash
devilroute status                       # 完整的執行時期狀態
devilroute logs                         # 串流要求日誌（--json、--search、--follow）
devilroute config show                  # 顯示目前設定

devilroute provider list                # 列出可用提供者（providers list 的別名）
devilroute provider add                 # 將 DevilRoute 註冊為工具上的提供者
devilroute keys add | list | remove     # 管理 API 金鑰
devilroute models [provider]            # 列出模型（--json、--search）
devilroute combo list | switch | create | delete

devilroute backup                       # 快照設定 + 資料庫
devilroute restore                      # 從先前的快照還原

devilroute health                       # 詳細健康狀態（斷路器、快取、記憶體）
devilroute quota                        # 提供者配額使用情況
devilroute cache                        # 快取狀態
devilroute cache clear                  # 清除語意 + 簽章快取

devilroute mcp status | restart         # MCP 伺服器狀態 / 重新啟動
devilroute a2a status | card            # A2A 伺服器狀態 / 代理卡片

devilroute tunnel list | create | stop  # 管理通道（cloudflare/tailscale/ngrok）
devilroute env show | get <k> | set <k> <v>  # 檢查 / 設定環境變數（暫時性）

devilroute test                         # 提供者連線冒煙測試
devilroute update                       # 檢查更新
devilroute completion                   # 產生 Shell 補全
```
...
可直接貼上的 Token 化 DevilRoute URL 範例：
...
| 錯誤                               | 原因                 | 解決方式                                      |
| ---------------------------------- | -------------------- | --------------------------------------------- |
| `Connection refused`               | DevilRoute 未執行     | `devilroute serve`                             |
