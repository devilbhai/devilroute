...
**DevilRoute** — proxy/router AI thống nhất. Một điểm cuối, 160+ nhà cung cấp LLM, tự động chuyển tiếp.
...
DevilRoute có ba cơ chế tạm thời liên quan nhưng khác biệt về lỗi. Giữ cho phạm vi của chúng tách biệt khi gỡ lỗi hành vi định tuyến. Xem
...
- **Nhập khẩu**: bên ngoài → bên trong (`@/`, `@devilroute/open-sse`) → tương đối
...
- **Biểu thức đường dẫn**: `@/*` → `src/`, `@devilroute/open-sse` → `open-sse/`, `@devilroute/open-sse/*` → `open-sse/*`
- **Cổng mặc định**: 20128 (API + bảng điều khiển trên cùng một cổng)
- **Thư mục dữ liệu**: biến môi trường `DATA_DIR`, mặc định là `~/.devilroute/`
