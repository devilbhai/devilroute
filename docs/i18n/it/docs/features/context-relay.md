...
- before the active account is exhausted, DevilRoute generates a compact structured summary
- after authentication selects a different account for the same session, DevilRoute injects
...
If the active provider is enabled in `handoffProviders`, DevilRoute generates a structured
...
DevilRoute prepends the stored handoff as a system message. Injection happens only after the
...
At injection time, DevilRoute converts that payload into a `<context_handoff>` system
