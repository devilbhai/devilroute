# 🚀 DevilRoute — The Free AI Gateway (Português (Portugal))
...
[🌐 Website](https://devilroute.online) • [🚀 Quick Start](#-quick-start) • [💡 Features](#-key-features) • [📖 Docs](#-documentation) • [💰 Pricing](#-pricing-at-a-glance) • [💬 WhatsApp](https://chat.whatsapp.com/JI7cDQ1GyaiDHhVBpLxf8b?mode=gi_t)
...
_Connect any AI-powered IDE or CLI tool through DevilRoute — free API gateway for unlimited coding._
...
<sub>📡 All agents connect via <code>http://localhost:20128/v1</code> or <code>http://cloud.devilroute.online/v1</code> — one config, unlimited models and quota</sub>
...
This generates a `system-info.txt` with your Node.js version, DevilRoute version, OS details, installed CLI tools (qoder, gemini, claude, codex, antigravity, droid, etc.), Docker/PM2 status, and system packages — everything we need to reproduce your issue quickly. Attach the file directly to your GitHub issue.
...
## 🎯 What DevilRoute Solves — 30 Real Pain Points & Use Cases

> **Every developer using AI tools faces these problems daily.** DevilRoute was built to solve them all — from cost overruns to regional blocks, from broken OAuth flows to protocol operations and enterprise observability.
...
- **System Info Report** — `npm run system-info` generates `system-info.txt` with your full environment (Node version, DevilRoute version, OS, CLI tools, Docker/PM2 status). Attach it when reporting issues for instant triage.
...
- **npm global install** — `npm install -g devilroute && devilroute` — done
...
- Built-in quick-start for `devilroute --mcp` and client onboarding
...
```txt
1) Start MCP transport (`devilroute --mcp`) for tool-driven operations
...
>
> ```bash
> pnpm add -g devilroute@latest --allow-build=better-sqlite3 --allow-build=@swc/core
> devilroute
> ```
...
Arch Linux users can install the [AUR package](https://aur.archlinux.org/packages/devilroute-bin), which installs DevilRoute and provides a systemd user service:

```bash
yay -S devilroute-bin
systemctl --user enable --now devilroute.service
```

| Command                 | Description                                                 |
| ----------------------- | ----------------------------------------------------------- |
| `devilroute`             | Start server (`PORT=20128`, API and dashboard on same port) |
| `devilroute --port 3000` | Set canonical/API port to 3000                              |
| `devilroute --mcp`       | Start MCP server (stdio transport)                          |
| `devilroute --no-open`   | Don't auto-open browser                                     |
| `devilroute --help`      | Show help                                                   |

Optional split-port mode:

```bash
PORT=20128 DASHBOARD_PORT=20129 devilroute
...
When you no longer need DevilRoute, we provide two quick scripts for a clean removal:

| Command                  | Action                                                                              |
| ------------------------ | ----------------------------------------------------------------------------------- |
| `npm run uninstall`      | Removes the system app but **keeps your DB and configurations** in `~/.devilroute`.  |
| `npm run uninstall:full` | Removes the app AND permanently **erases all configurations, keys, and databases**. |

> Note: To run these commands, navigate to the DevilRoute project folder (if you cloned it) and run them. Alternatively, if globally installed, you can simply run `npm uninstall -g devilroute`.
...
| `STREAM_IDLE_TIMEOUT_MS` | inherits `REQUEST_TIMEOUT_MS` | Maximum gap between streaming chunks before DevilRoute aborts the SSE stream                                                                  |

Backward compatibility is preserved: existing `FETCH_TIMEOUT_MS`, `API_BRIDGE_PROXY_TIMEOUT_MS`, and other per-layer timeout vars still work and override the shared baseline.

For Claude Code-compatible upstreams (`anthropic-compatible-cc-*`), DevilRoute also derives the outbound `X-Stainless-Timeout` header from the resolved fetch timeout so provider-side read timeouts stay aligned with your env configuration.

For third-party Claude Code-compatible reverse proxies, DevilRoute keeps the default
...
`cache_control`, DevilRoute does not inject bridge-owned markers.
...
For streaming requests, `FETCH_TIMEOUT_MS` only covers connection setup / waiting for the first upstream response. Once the stream is active, DevilRoute will only abort on an actual stall (`STREAM_IDLE_TIMEOUT_MS`) or Undici body inactivity (`FETCH_BODY_TIMEOUT_MS`).

If you run DevilRoute behind Nginx, Caddy, Cloudflare, or another reverse proxy, make sure the proxy
timeouts are also higher than your DevilRoute stream/fetch timeouts.
...
### 3) Point your coding tool to DevilRoute
...
- `devilroute_get_health`
- `devilroute_list_combos`
...
For Void Linux users, you can build a native package using `xbps-src`. Save this block as `srcpkgs/devilroute/template`:

```bash
# Template file for 'devilroute'
pkgname=devilroute
version=3.4.1
revision=1
...
do_install() {
	vmkdir usr/lib/devilroute/.next

	vcopy .next/standalone/. usr/lib/devilroute/.next/standalone
	...
	for _d in \
		...
		touch "${DESTDIR}/usr/lib/devilroute/${_d}/.keep"
	done

	cat > "${WRKDIR}/devilroute" <<'EOF'
#!/bin/sh
export PORT="${PORT:-20128}"
export DATA_DIR="${DATA_DIR:-${XDG_DATA_HOME:-${HOME}/.local/share}/devilroute}"
export APP_LOG_TO_FILE="${APP_LOG_TO_FILE:-false}"
mkdir -p "${DATA_DIR}"
exec node /usr/lib/devilroute/.next/standalone/server.js "$@"
EOF
	vbin "${WRKDIR}/devilroute"
}
...
DevilRoute is available as a public Docker image on [Docker Hub](https://hub.docker.com/r/diegosouzapw/devilroute).
...
- Quick Tunnels are not auto-restored after an DevilRoute or container restart. Re-enable them from the dashboard when needed.
- Managed install currently supports Linux, macOS, and Windows on `x64` / `arm64`.
- Managed Quick Tunnels default to HTTP/2 transport to avoid noisy QUIC UDP buffer warnings in constrained container environments. Set `CLOUDFLARED_PROTOCOL=quic` or `auto` if you want a different transport.
- Docker images bundle system CA roots and pass them to managed `cloudflared`, which avoids TLS trust failures when the tunnel bootstraps inside the container.
- SQLite runs in WAL mode. `docker stop` should be allowed to finish so DevilRoute can checkpoint the latest changes back into `storage.sqlite`.
...
- Set `CLOUDFLARED_BIN=/absolute/path/to/cloudflared` if you want DevilRoute to use an existing binary instead of downloading one.

**Using Docker Compose with Caddy (HTTPS Auto-TLS):**

DevilRoute can be securely exposed using Caddy's automatic SSL provisioning. Ensure your domain's DNS A record points to your server's IP.
...
services:
  ...
  caddy:
    ...
    ports:
      - "80:80"
      - "443:443"
    command: caddy reverse-proxy --from https://your-domain.com --to http://devilroute:20128
...
> 🆕 **NEW!** DevilRoute is now available as a **native desktop application** for Windows, macOS, and Linux.

Run DevilRoute as a standalone desktop app — no terminal, no browser, no internet required for local models. The Electron-based app includes:

- 🖥️ **Native Window** — Dedicated app window with system tray integration
- 🔄 **Auto-Start** — Launch DevilRoute on system login
...
When minimized, DevilRoute lives in your system tray with quick actions:
...
**Zero cost. Never stops coding.** Configure this as one DevilRoute combo and all fallbacks happen automatically — no manual switching ever.
...
> All models below are **100% free with zero credit card required**. DevilRoute auto-routes between them when one quota runs out — combine them all for an unbreakable $0 combo.
...
DevilRoute v3.6 is built as an operational platform, not just a relay proxy.
...
When quota, rate, or health fails, DevilRoute automatically moves to the next candidate without manual switching.
...
DevilRoute includes a built-in evaluation framework to test LLM response quality against a golden set. Access it via **Analytics → Evals** in the dashboard.

### Built-in Golden Set

The pre-loaded "DevilRoute Golden Set" contains test cases for:
...
2. Run `devilroute_get_health`.
3. Run `devilroute_list_combos`.
...
**Pro Tip:** Use Opus for complex tasks, Sonnet for speed. DevilRoute tracks quota per model!
...
- Rotation behavior: DevilRoute routes to the next eligible Codex account automatically.
...
**Pro Tip:** OpenAI-compatible API — works seamlessly with DevilRoute's format translation!
...
Settings → Models → Advanced:
  ...
  OpenAI API Key: [from DevilRoute dashboard]
...
export OPENAI_API_KEY="your-devilroute-api-key"
...
> **Note:** OpenClaw only works with local DevilRoute. Use `127.0.0.1` instead of `localhost` to avoid IPv6 resolution issues.
...
Settings → API Configuration:
  ...
  Base URL: http://localhost:20128/v1
  API Key: [from DevilRoute dashboard]
  Model: if/kimi-k2-thinking
```

### OpenCode

**Step 1:** Add DevilRoute as a custom provider:

```bash
opencode
/connect
# Select "Other" → Enter ID: "devilroute" → Enter your DevilRoute API key
```
...
```bash
/models
# Select any DevilRoute model from the list
```

> **Tip:** Add any model available in your DevilRoute `/v1/models` endpoint to the `models` section. Use the format `provider/model-id` from your DevilRoute dashboard.
...
- DevilRoute v1.0.6+ includes fallback validation via chat completions
...
> **⚠️ Important for users running DevilRoute on a VPS, Docker, or any remote server**



The OAuth credentials bundled in DevilRoute are registered **for `localhost` only**. When you access DevilRoute on a remote server (e.g. `https://devilroute.myserver.com`), Google rejects the authentication with:
...
- Name: anything you like (e.g. `DevilRoute Remote`)
...
1. DevilRoute opens the Google authorization URL
...
4. Paste that URL into the field shown in the DevilRoute connection modal
5. Click **"Connect"**
...
As credenciais OAuth embutidas no DevilRoute estão cadastradas **apenas para `localhost`**. Quando você acessa o DevilRoute em um servidor remoto (ex: `https://devilroute.meuservidor.com`), o Google rejeita a autenticação com:
...
- Nome: escolha qualquer nome (ex: `DevilRoute Remote`)
...
1. O DevilRoute abrirá a URL de autorização do Google
2. Após você autorizar, o Google tentará redirecionar para `localhost` (que falha no servidor remoto)
3. **Copie a URL completa** da barra de endereço do seu browser (mesmo que a página não carregue)
4. Cole essa URL no campo que aparece no modal de conexão do DevilRoute
5. Clique em **"Connect"**
...
- **Website**: [devilroute.online](https://devilroute.online)
- **Package**: [npmjs.com/package/devilroute](https://www.npmjs.com/package/devilroute)
...
DevilRoute has **218+ features planned** across multiple development phases. Here are the key areas:
...
<div align="center">
  ...
  <br/>
  <sub><a href="https://devilroute.online">devilroute.online</a></sub>
</div>
