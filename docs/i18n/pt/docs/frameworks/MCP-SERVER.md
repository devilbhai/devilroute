# DevilRoute MCP Server Documentation (Português (Portugal))
...
| Tool                            | Description                              |
| :------------------------------ | :--------------------------------------- |
| `devilroute_get_health`          | Gateway health, circuit breakers, uptime |
| `devilroute_list_combos`         | All configured combos with models        |
| `devilroute_get_combo_metrics`   | Performance metrics for a specific combo |
| `devilroute_switch_combo`        | Switch active combo by ID/name           |
| `devilroute_check_quota`         | Quota status per provider or all         |
| `devilroute_route_request`       | Send a chat completion through DevilRoute |
| `devilroute_cost_report`         | Cost analytics for a time period         |
| `devilroute_list_models_catalog` | Full model catalog with capabilities     |

## Advanced Tools (8)

| Tool                               | Description                                                 |
| :--------------------------------- | :---------------------------------------------------------- |
| `devilroute_simulate_route`         | Dry-run routing simulation with fallback tree               |
| `devilroute_set_budget_guard`       | Session budget with degrade/block/alert actions             |
| `devilroute_set_resilience_profile` | Apply conservative/balanced/aggressive preset               |
| `devilroute_test_combo`             | Live-test all models in a combo via a real upstream request |
| `devilroute_get_provider_metrics`   | Detailed metrics for one provider                           |
| `devilroute_best_combo_for_task`    | Task-fitness recommendation with alternatives               |
| `devilroute_explain_route`          | Explain a past routing decision                             |
| `devilroute_get_session_snapshot`   | Full session state: costs, tokens, errors                   |
