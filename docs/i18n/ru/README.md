# 🚀 DevilRoute — Бесплатный AI-шлюз (Русский)
...
[**🚀 Быстрый старт**](#-быстрый-старт) • [**🎯 Комбо**](#-комбо--главная-фича) • [**🌐 Провайдеры**](#-278-ai-провайдеров--90-бесплатных) • [**🔌 CLI и MCP**](#-полный-cli--a2a-и-mcp) • [**🗜️ Сжатие**](#️-экономьте-1595-токенов--автоматически) • [**🌍 Сайт**](https://devilroute.online)

[💥 Обещание](#-обещание) • [🤔 Зачем](#-зачем-devilroute) • [🏆 Чем отличается](#-чем-devilroute-отличается) • [🤖 Совместимые CLI](#-совместимые-cli-и-агенты) • [🖥️ Где запускать](#️-где-запускается-devilroute--везде) • [🔒 Приватность](#-приватно-и-local-first) • [🎬 В деле](#-devilroute-в-деле) • [📚 Дальше](#-узнать-больше) • [📧 Поддержка](#-поддержка-и-сообщество)
...
> Собирать free-tier вручную — боль: десятки SDK, лимиты и непонятный остаток. DevilRoute сводит **документированные** free-tier **43 пулов / 460+ моделей** в одно честное число и показывает его live на `/dashboard/free-tiers`.
>
...
> Один endpoint. **278 провайдеров.** Код не останавливается — DevilRoute сам выбирает самый дешёвый рабочий вариант.
...
| ❌ Боль каждый день | ✅ Как решает DevilRoute |
|---|---|
...
> **Combo** — цепочка моделей, по которой DevilRoute ходит **сам**. Квота кончилась, провайдер упал, цена взлетела — комбо тихо уходит на следующий шаг. **Именно это делает DevilRoute «неубиваемым».** 🛡️

### ⚡ Без настройки — просто `auto`

Комбо создавать не обязательно. Поставьте модель `auto` (или вариант) — DevilRoute соберёт виртуальное комбо из подключённых провайдеров:
...
# 🏆 Чем DevilRoute отличается

</div>

| Фича | DevilRoute | Другие роутеры |
|---|---|---|
...
<sub>📊 Сравнение с LiteLLM, OpenRouter, Portkey → [`docs/comparison/DEVILROUTE_VS_ALTERNATIVES.md`](../../comparison/DEVILROUTE_VS_ALTERNATIVES.md)</sub>
...
- **🤖 One-command CLI setup** — `setup-*` для 12+ tools; `devilroute launch` / `launch-codex`.
...
- **💸 Cost telemetry** — `X-DevilRoute-*` headers, cache-HIT savings, per-key USD quotas.
- **🧠 Memory** — opt-in, int8 quantization, `x-devilroute-no-memory`.
...
<sub>📖 Setup 33 tools → [`docs/reference/CLI-TOOLS.md`](../../reference/CLI-TOOLS.md) · OpenCode plugin → [`@devilroute/opencode-provider`](https://www.npmjs.com/package/@devilroute/opencode-provider)</sub>
...
# 🖥️ Где запускается DevilRoute — везде
...
| 🧩 **OpenCode plugin** | `@devilroute/opencode-provider` | Нативная интеграция |
...
> Ваши ключи, ваша машина, ваши данные. DevilRoute — **локальный прокси**, без «звонков домой».

- 🏠 **100% на вашем железе** — npm, Docker, desktop или телефон. Нет cloud-hop DevilRoute.
...
> Это не «просто сервер» — **CLI-кокит** с **80+ командами** и открытыми agent-протоколами, чтобы AI управлял DevilRoute **сам**.

### ⌨️ Настоящий CLI

```bash
devilroute               # gateway + dashboard (порт 20128)
devilroute chat          # TUI-чат (/model /combo /skill /memory)
devilroute setup         # мастер первого запуска
devilroute doctor        # диагностика провайдеров, портов, native deps
```

### 🛰️ Remote mode — CLI здесь, DevilRoute на VPS

```bash
devilroute connect 192.168.0.15            # пароль → scoped token
devilroute models list                     # ← на REMOTE
devilroute configure codex                 # remote model → local Codex profile
devilroute tokens create --name ci --scope read
devilroute contexts use default            # ← обратно на local
```
...
|---|---|---|
| 🧰 **MCP (stdio)** | `devilroute --mcp` | Claude Desktop, Cursor, любой MCP client |
...
Precedence (high → low): header `x-devilroute-compression` › combo override › named profile › adaptive › panel default › off.
...
Должны появиться подключённые модели. 🎉 Дальше пишите код — DevilRoute сам роутит и делает fallback.
...
| Команда | Описание |
|---|---|
| `devilroute` | Сервер (`PORT=20128`, API + dashboard) |
| `devilroute --port 3000` | Порт 3000 |
| `devilroute --mcp` | MCP server (stdio) |
| `devilroute --no-open` | Не открывать браузер |
| `devilroute --help` | Справка |
...
| Команда | Действие |
|---|---|
| `npm run uninstall` | Убирает app, **сохраняет** `~/.devilroute` |
| `npm run uninstall:full` | Удаляет app **и** все ключи/БД |
| `npm uninstall -g devilroute` | Глобальный npm uninstall |
...
> 💡 «Cost» в дашборде — **tracker экономии**, не счёт DevilRoute. DevilRoute вам **не** выставляет счета.
...
| `DATA_DIR` | `~/.devilroute` | БД и конфиги |
| `REQUEST_TIMEOUT_MS` | `600000` | Базовый timeout |
| `STREAM_IDLE_TIMEOUT_MS` | inherits | Idle gap SSE |

**DevilRoute берёт деньги?** Нет — open-source на вашей машине. Платите только платным провайдерам.  
...
| [Remote Mode](../../guides/REMOTE-MODE.md) | CLI с ноутбука → DevilRoute на VPS |
...
**Сделано с ❤️ open-source сообществом · MIT License · [devilroute.online](https://devilroute.online)**

⭐ Если DevilRoute помог сэкономить — поставьте star репозиторию.
