...
title: "Arquitetura do DevilRoute"
version: 3.8.2
lastUpdated: 2026-05-13

---

# Arquitetura do DevilRoute
...
DevilRoute é um gateway de roteamento de IA local e um painel construído sobre Next.js.  
...
flowchart LR
    ...
    subgraph Router[Processo Local DevilRoute]
...
- Construtor de metadados de resposta DevilRoute: `src/domain/devilrouteResponseMeta.ts`
...
- arquivo: `${DATA_DIR}/storage.sqlite` (ou `$XDG_CONFIG_HOME/devilroute/storage.sqlite` quando definido, caso contrário `~/.devilroute/storage.sqlite`)
...
flowchart LR
    ...
    subgraph ContainerOrProcess[Runtime do DevilRoute]
...
> DevilRoute v3.8.0. Para a lista canônica e continuamente atualizada, consulte
...
- resposta final do cliente retornada pelo DevilRoute; respostas transmitidas são armazenadas na mesma forma de resumo compacto
...
1. `usageDb` e `localDb` compartilham a mesma política de diretório base (`DATA_DIR` -> `XDG_CONFIG_HOME/devilroute` -> `~/.devilroute`) com migração de arquivos legados.
...
5. O diretório `open-sse/` é publicado como o pacote de **workspace npm** `@devilroute/open-sse`. O código-fonte o importa via `@devilroute/open-sse/...` (resolvido pelo Next.js `transpilePackages`). Os caminhos de arquivo neste documento ainda usam o nome do diretório `open-sse/` para consistência.
...
- Construir imagem Docker: `docker build -t devilroute .`
