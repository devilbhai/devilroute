...
**DevilRoute** — birləşdirilmiş AI proxy/router. Bir uç nöqtə, 160+ LLM təminatçısı, avtomatik geri dönmə.
...
DevilRoute-un üç əlaqəli, lakin fərqli müvəqqəti uğursuzluq mexanizmi var. Marşrut davranışını düzəldərkən onların
...
- **İdxallar**: xarici → daxili (`@/`, `@devilroute/open-sse`) → nisbət
...
- **Yol aliasları**: `@/*` → `src/`, `@devilroute/open-sse` → `open-sse/`, `@devilroute/open-sse/*` → `open-sse/*`
- **Default port**: 20128 (API + dashboard eyni portda)
- **Məlumat qovluğu**: `DATA_DIR` env var, varsayılan `~/.devilroute/`
