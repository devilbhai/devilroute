...
> **TL;DR**: DevilRoute ships with built-in health monitoring, provider autopilot, quota tracking, and observability hooks. This guide covers the dashboard, alerts, and troubleshooting.
...
DevilRoute has **3 layers of monitoring**:
...
│  └─ MCP tools: devilroute_get_health, devilroute_get_session_snapshot │
└──────────────────────────────────────────────────────────────┘
```
...
DevilRoute supports **3 alert channels**:
...
Create a `~/.devilroute/dashboard.json`:
...
- DevilRoute's quota tracking may be **stale** — the provider's truth is upstream
...
- Run `sqlite3 ~/.devilroute/storage.sqlite "PRAGMA integrity_check;"`
- If "ok" — false alarm, the health check is being too strict
- If anything else — **stop DevilRoute** and follow the [disaster recovery guide](./DATABASE_GUIDE.md#disaster-recovery)
