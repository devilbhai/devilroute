...
```bash
sudo mkdir -p /opt/devilroute-ops
sudo cp scripts/ops/runner-janitor.sh /opt/devilroute-ops/
sudo chmod +x /opt/devilroute-ops/runner-janitor.sh
( sudo crontab -l 2>/dev/null; echo '*/30 * * * * /opt/devilroute-ops/runner-janitor.sh >> /var/log/runner-janitor.log 2>&1' ) | sudo crontab -
```
