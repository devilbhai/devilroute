...
1. `npm stage list devilroute` — find the stage id (also printed in the workflow summary).
...
`devilroute` in stage-only mode so a leaked long-lived token cannot `npm publish`
directly from anywhere — CI can only stage; only the owner's 2FA releases.

**Broken-artifact playbook (unchanged):** `npm deprecate devilroute@<bad> "<reason> — use <fixed>"`
...
- [ ] If source English docs changed, run `npm run i18n:run` (requires `DEVILROUTE_TRANSLATION_API_KEY` in `.env`) before tagging
...
- [ ] Test on fresh install: delete `~/.devilroute/devilroute.db` and run `npm run dev`
...
> **Operator note:** the remote VPS image directory remains `/usr/lib/node_modules/devilroute/app/`.
...
- [ ] `devilroute --tray` boots on macOS (systray2 installed into `~/.devilroute/runtime/`)
- [ ] `devilroute --tray` boots on Linux (requires DISPLAY; graceful error if not set)
- [ ] `devilroute --tray` boots on Windows (PowerShell NotifyIcon, no extra binaries)
- [ ] `devilroute config tray enable` creates autostart entry; disable removes it
- [ ] `npm install -g devilroute@<this-version>` runs postinstall without fatal exit
- [ ] Update path keeps optional deps: `devilroute update --apply` and the auto-updater
...
- [ ] `devilroute status` works with no `.env` (CLI token path, loopback only)
...
- [ ] All 10 `skills/devilroute*/SKILL.md` files are publicly fetchable via raw GitHub URL
