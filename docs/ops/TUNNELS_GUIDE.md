...
DevilRoute can expose its local server (`http://localhost:20128`) to the public
...
All three backends are managed in-process — DevilRoute starts/stops the underlying
...
- **Auto-install.** On first use, DevilRoute downloads the latest `cloudflared`
...
login` + `cloudflared tunnel route dns ...`) are not orchestrated by
  DevilRoute. URLs are ephemeral and will change every restart.
...
| Variable                                             | Purpose                                                                               |
| ---------------------------------------------------- | ------------------------------------------------------------------------------------- |
| `CLOUDFLARED_BIN`                                    | Override the binary path. If set and valid, DevilRoute uses it instead of downloading. |
...
must be configured in the ngrok dashboard — DevilRoute itself only forwards the
...
1. Install Tailscale (or let DevilRoute do it — see `install` endpoint below).
2. Sign in (`tailscale login` or via DevilRoute's `login` endpoint).
...
DevilRoute's MITM password cache (`getCachedPassword` / `setCachedPassword`) for
...
When you expose DevilRoute through a tunnel, the dashboard and OAuth flows must
...
and restart DevilRoute before initiating OAuth. For ephemeral Cloudflare Quick
...
than one backend simultaneously is allowed; DevilRoute will track each
independently.
...
DevilRoute attempts to auto-install on first use. If the install is blocked
