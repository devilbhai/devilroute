...
DevilRoute is heading from a monolithic router to a **modular AI platform**: a lightweight
...
| 3.8.53 | `chatCore.ts` decomposition · headless mode (`DEVILROUTE_HEADLESS=1`) · local candidate build/promote loop |
...
- **`stable/v3`** — the LTS line (3.9.x). Receives fixes, security patches, and provider
  updates. `npm install devilroute` (aka `latest`) stays on v3 during the whole v4 cycle.
...
- **`@devilroute/core`** (npm name stays `devilroute`) — just the engine: `/v1/*`, routing,
  combo/fallback, providers.
- **`@devilroute/sdk`** — one typed contract: hooks, extension points, two-phase lifecycle,
...
- **Modules** (`@devilroute/mod-*`) — cloud agents, traffic inspection (MITM), evals,
