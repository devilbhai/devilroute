...
1. **Lifecycle** (`RouterBackendLifecycle`) — _how the engine runs_:
   - `in-process` — runs inside the DevilRoute Node process (the native TS pipeline).
   - `supervised` — a local child process DevilRoute installs/starts/stops/health-checks
     via `ServiceSupervisor`, then consumes as a provider connection.
   - `external` — an HTTP endpoint DevilRoute dispatches to but does **not** manage
