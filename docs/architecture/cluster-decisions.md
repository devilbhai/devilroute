...
- Multi-replica deployment that needs shared vector state across `devilroute-1/2/3`.
...
| -------- | ----------------------- | ----------- | ----------------------------------------------------- |
| `qdrant` | `qdrant/qdrant:v1.12.4` | `6333` HTTP | HNSW index; persistent volume `devilroute_qdrant_data` |
...
- You run ≥3 `devilroute` replicas and want provider rotation centralised in a single Go process.
- You want a single audit/logging surface for upstream-provider requests across all replicas.
- You want horizontal scaling of the Tier-1 routing layer independent of the DevilRoute replicas.

**What it adds:**

| Service   | Image                            | Ports  | Notes                                                                   |
| --------- | -------------------------------- | ------ | ----------------------------------------------------------------------- |
| `bifrost` | `ghcr.io/maximhq/bifrost:1.5.21` | `8080` | Go-based Tier-1 router; persistent logs volume `devilroute_bifrost_logs` |
...
| **NATS**                             | **DROP** | Each `devilroute` replica is a single Node.js process; no multi-process pub/sub workload exists.        |
