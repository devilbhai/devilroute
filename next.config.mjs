...
const contentSecurityPolicy = [
  ...
  // DevilRoute is reached from a non-loopback host. Same-origin HTTP fetches stay covered
...
// DEVILROUTE_BUILD_PROFILE=minimal physically removes four optional privileged
...
// The resulting artifact is intended to be published as `devilroute-secure`
// for security-sensitive environments. See docs/security/SOCKET_DEV_FINDINGS.md.
const isMinimalBuild = process.env.DEVILROUTE_BUILD_PROFILE === "minimal";
...
const nextConfig = {
  ...
  // DevilRoute under https://host/devilroute/). Empty by default so root-path
  ...
  basePath: normalizeBasePath(process.env.DEVILROUTE_BASE_PATH),
  ...
  env: {
    NEXT_PUBLIC_DEVILROUTE_BASE_PATH: normalizeBasePath(process.env.DEVILROUTE_BASE_PATH),
  },
  distDir,
  ...
  turbopack: {
    ...
    resolveAlias: {
      ...
      // (Docker sets DEVILROUTE_MITM_STUB=1 — #3390 graceful degradation). The
  ...
  // DevilRoute is a proxy for AI APIs — request bodies routinely include
  ...
  experimental: {
    serverActions: {
      bodySizeLimit: process.env.DEVILROUTE_SERVER_ACTIONS_BODY_LIMIT || "50mb",
    },
    ...
    // Next's internal router proxy defaults to 30s when this is unset. DevilRoute
    ...
    // the internal `@devilroute/open-sse` workspace here: optimizePackageImports
  ...
  transpilePackages: ["@devilroute/open-sse", "@lobehub/icons", "fumadocs-ui", "fumadocs-core"],
  ...
  async headers() {
    return [
      ...
      // G-10: allow DevilRoute's own dashboard to embed the 9Router UI via our reverse proxy.
  ...
  async rewrites() {
    return [
      ...
      // so they hit the /api/[...devilrouteApiCatchAll] route (#6424) — which
