...
Emits [Langfuse](https://langfuse.com/) generation traces for every LLM completion routed through DevilRoute.
...
Copy the `examples/plugins/langfuse/` directory to your DevilRoute plugins path, or install directly from the marketplace UI.
...
- **Trace:** `devilroute:<model>` with `userId`, `provider`, `requestId` metadata
...
- API keys are stored in the plugin config, encrypted at rest by DevilRoute
...
- DevilRoute plugin SDK: `docs/frameworks/PLUGIN_SDK.md`
