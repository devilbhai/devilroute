...
export const DEVILROUTE_PEER_TRACE_HEADER = "X-DevilRoute-Peer-Trace";

type HeaderSource = Headers | Record<string, unknown> | null | undefined;
type PeerEnvironment = {
  DEVILROUTE_INSTANCE_ID?: string;
  DEVILROUTE_PEER_URLS?: string;
  DEVILROUTE_PEER_MAX_HOPS?: string;
};
...
function getInstanceId(env: PeerEnvironment): string | null {
  const value = env.DEVILROUTE_INSTANCE_ID?.trim() ?? "";
  return INSTANCE_ID_PATTERN.test(value) ? value : null;
}

function getMaxPeerHops(env: PeerEnvironment): number {
  const parsed = Number.parseInt(env.DEVILROUTE_PEER_MAX_HOPS ?? "", 10);
...
export function isConfiguredDevilRoutePeer(
  targetUrl: string,
  ...
  return (env.DEVILROUTE_PEER_URLS ?? "")
    .split(",")
...
export function inspectPeerRequest(
  ...
  const trace = parsePeerTrace(readHeader(headers, DEVILROUTE_PEER_TRACE_HEADER));
...
/**
 * Append this instance to the peer trace for an explicitly allowlisted DevilRoute URL.
...
export function applyPeerTraceHeader(
  ...
  const instanceId = getInstanceId(env);
  if (!instanceId || !isConfiguredDevilRoutePeer(targetUrl, env)) return false;

  const trace = parsePeerTrace(readHeader(clientHeaders, DEVILROUTE_PEER_TRACE_HEADER));
  if (!trace.includes(instanceId)) trace.push(instanceId);
  const traceHeaderLower = DEVILROUTE_PEER_TRACE_HEADER.toLowerCase();
  for (const key of Object.keys(outgoingHeaders)) {
    if (key.toLowerCase() === traceHeaderLower) delete outgoingHeaders[key];
  }
  outgoingHeaders[DEVILROUTE_PEER_TRACE_HEADER] = trace.join(",");
  return true;
}
