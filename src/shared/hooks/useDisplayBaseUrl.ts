...
/**
 * Resolve the deploy basePath for display URLs (e.g. `/devilroute`).
 *
 * Priority:
 * 1. Explicit `envBasePath` argument / `NEXT_PUBLIC_DEVILROUTE_BASE_PATH`
  ...
  const fromEnv = normalizeBasePath(
    envBasePath ??
      (typeof process !== "undefined"
        ? process.env.NEXT_PUBLIC_DEVILROUTE_BASE_PATH || process.env.DEVILROUTE_BASE_PATH
        : undefined)
  );
 ...
 * @param envBasePath       `NEXT_PUBLIC_DEVILROUTE_BASE_PATH` / `DEVILROUTE_BASE_PATH`
 */
export function resolveDisplayBaseUrl(
  ...
  // Example: NEXT_PUBLIC_BASE_URL=https://host/devilroute must not be collapsed to
  ...
  // reverse-proxy subpaths (DEVILROUTE_BASE_PATH).
 ...
 *   1. Public browser origin (+ DEVILROUTE_BASE_PATH when set) — proves the
 ...
 * `src/shared/utils/resolveDevilRouteBaseUrl.ts` instead.
 */
