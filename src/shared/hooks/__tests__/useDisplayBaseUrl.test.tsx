...
describe("useDisplayBaseUrl", () => {
  ...
  it("appends DEVILROUTE basePath when resolving a public browser origin", () => {
    expect(
      resolveDisplayBaseUrl(
    ...
    ).toBe("https://api.example.com/devilroute");
  });

  it("keeps a configured public URL that already includes the basePath", () => {
    expect(
      resolveDisplayBaseUrl(
        "https://api.example.com/devilroute",
        "https://api.example.com",
        "/devilroute/dashboard",
        "/devilroute"
      )
  ...
  it("derives basePath from NEXT_PUBLIC_BASE_URL when env basePath is unset", () => {
    expect(
      resolveDisplayBaseUrl("https://api.example.com/devilroute", "https://api.example.com", "/")
    ).toBe("https://api.example.com/devilroute");
  });

  it("normalizes basePath values without a leading slash", () => {
    expect(normalizeBasePath("devilroute")).toBe("/devilroute");
    expect(normalizeBasePath("/devilroute/")).toBe("/devilroute");
