...
// of `./outboundUrlGuard.ts` so the CLI (`devilroute setup-opencode`, loaded via tsx with no
...
export const PRIVATE_PROVIDER_URLS_ENV = "DEVILROUTE_ALLOW_PRIVATE_PROVIDER_URLS";
...
// cloud-metadata endpoints stay blocked. Defaults ON (DevilRoute is local-first); operators
// who only use public providers can disable it to restore strict SSRF blocking.
export const LOCAL_PROVIDER_URLS_ENV = "DEVILROUTE_ALLOW_LOCAL_PROVIDER_URLS";
 ...
 * (DevilRoute is local-first — local OpenAI-compatible providers should validate out of the
 * box). Disable via the `DEVILROUTE_ALLOW_LOCAL_PROVIDER_URLS` flag (DB toggle or env) to
 ...
 * provider URLs (`DEVILROUTE_ALLOW_PRIVATE_PROVIDER_URLS`, default OFF). Protocol and
