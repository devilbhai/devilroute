...
export const QWEN_CODE_ENV_KEY = "DEVILROUTE_API_KEY";
...
/**
 * Identifies only entries owned by DevilRoute. Generic custom endpoints are not
...
export const isManagedQwenCodeModel = (value: unknown): boolean => {
  ...
  if (value.envKey === QWEN_CODE_ENV_KEY || value.id === "devilroute") return true;

  return (
    typeof value.name === "string" &&
    value.name.endsWith(" (DevilRoute)") &&
...
export const buildQwenCodeModel = ({
  ...
  return {
    id: normalizedModel,
    name: `${String(modelName || normalizedModel).trim()} (DevilRoute)`,
...
/**
 * Merge one DevilRoute model into Qwen Code's current V4 settings contract.
...
export const mergeQwenCodeSettings = (
  ...
  if (next.selectedProvider === "devilroute") delete next.selectedProvider;

  return next;
};

export const findDevilRouteQwenCodeModel = (settings: unknown): JsonRecord | undefined => {
...
export const hasDevilRouteQwenCodeConfig = (settings: unknown): boolean =>
  findDevilRouteQwenCodeModel(settings) !== undefined;

/** Remove only DevilRoute-owned Qwen Code entries and selection state. */
...
const OWNED_ENV_LINE = /^\s*(?:export\s+)?DEVILROUTE_API_KEY\s*=/;
...
export const mergeQwenCodeEnv = (existing: unknown, apiKey: unknown): string => {
  ...
  lines.push(`${QWEN_CODE_ENV_KEY}=${JSON.stringify(String(apiKey || "sk_devilroute"))}`);
  return `${lines.join("\n")}\n`;
};
