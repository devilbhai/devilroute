...
export function resolveModelSyncInternalBaseUrl(_candidate?: string): string {
  ...
  const nativeTls = process.env.DEVILROUTE_INTERNAL_SCHEME === "https";
  const origin = nativeTls
    ? `https://localhost:${dashboardPort}`
    : `http://127.0.0.1:${dashboardPort}`;
  return `${origin}${normalizeInternalBasePath(process.env.DEVILROUTE_BASE_PATH)}`;
}
...
const globalState = globalThis as typeof globalThis & {
  __devilrouteModelSyncInternalAuthToken?: string;
};
...
function getInternalAuthToken(): string {
  if (!internalAuthToken) {
    internalAuthToken = globalState.__devilrouteModelSyncInternalAuthToken || randomUUID();
    globalState.__devilrouteModelSyncInternalAuthToken = internalAuthToken;
  }
...
export function isModelSyncInternalRequest(request: { headers: Headers }): boolean {
  if (!internalAuthToken && globalState.__devilrouteModelSyncInternalAuthToken) {
    internalAuthToken = globalState.__devilrouteModelSyncInternalAuthToken;
  }
 ...
 * @param apiBaseUrl — internal base URL to call DevilRoute's own API
