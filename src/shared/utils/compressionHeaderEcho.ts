 ...
 *
 * When a request carries `x-devilroute-compression`, docs promise the response echoes
 * `X-DevilRoute-Compression: <mode>; source=<source>`. Internal paths (idempotency
...
const COMPRESSION_REQUEST_HEADER = "x-devilroute-compression";
const COMPRESSION_RESPONSE_HEADER = DEVILROUTE_RESPONSE_HEADERS.compression;
...
/**
 * Wrap a Response so it carries `X-DevilRoute-Compression: <mode>; source=request-header`
 * when the request supplied `x-devilroute-compression` and the inner pipeline did not
