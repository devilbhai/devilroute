/**
 * Client/server helpers for Next.js `basePath` / `DEVILROUTE_BASE_PATH` deploys.
 *
 ...
 * Set via next.config `env.NEXT_PUBLIC_DEVILROUTE_BASE_PATH` from `DEVILROUTE_BASE_PATH`.
 */
export function getDeployBasePath(
  env: NodeJS.ProcessEnv = typeof process !== "undefined" ? process.env : ({} as NodeJS.ProcessEnv)
): string {
  return normalizeBasePath(
    env.NEXT_PUBLIC_DEVILROUTE_BASE_PATH || env.DEVILROUTE_BASE_PATH || ""
  );
}
 ...
 *
 * - Relative absolute paths: `/api/health/ping` → `/devilroute/api/health/ping`
 * - Absolute same-origin URLs: `https://host/api/x` → `https://host/devilroute/api/x`
