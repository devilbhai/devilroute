...
} from "@devilroute/open-sse/services/accountFallback.ts";
import { getModelInfo, getComboForModel } from "../services/model";
import { resolveBareModelToConnectionDefault } from "@devilroute/open-sse/services/model.ts";
import { errorResponse } from "@devilroute/open-sse/utils/error.ts";
import { getImageModelEntry } from "@devilroute/open-sse/config/imageRegistry.ts";
import { acceptHeaderForcesStream } from "@devilroute/open-sse/utils/aiSdkCompat.ts";
import { applyNoThinkingAlias } from "@devilroute/open-sse/utils/noThinkingAlias.ts";
import { resolveCcDiscoveryAliasStrip } from "@/lib/ccDiscoveryAliasResolve";
import { handleComboChat, shouldSkipConnDisable } from "@devilroute/open-sse/services/combo.ts";
import { mergeAbortSignals } from "@devilroute/open-sse/executors/base.ts";
import { resolveRequestAutoControls } from "@devilroute/open-sse/services/autoCombo/requestControls.ts";
import { resolveComboConfig } from "@devilroute/open-sse/services/comboConfig.ts";
import { injectHandoffIntoBody } from "@devilroute/open-sse/services/contextHandoff.ts";
import {
  HTTP_STATUS,
  ANTIGRAVITY_PRE_RESPONSE_TIMEOUT_CODE,
} from "@devilroute/open-sse/config/constants.ts";
import { getTargetFormat, detectFormatFromUrl } from "@devilroute/open-sse/services/provider.ts";
import {
  getModelsByProviderId,
  getModelTargetFormat,
  PROVIDER_ID_TO_ALIAS,
} from "@devilroute/open-sse/config/providerModels.ts";
...
import { connectionHasExtraKeys } from "@devilroute/open-sse/services/apiKeyRotator.ts";
import {
...
import { isSubscriptionQuotaText } from "@devilroute/open-sse/services/quotaTextCooldowns.ts";
...
import { cloneBoundedForLog } from "@devilroute/open-sse/utils/requestLogger.ts";
import { handleInternalUsageCommand } from "@/lib/usage/internalUsageCommand";
import {
  applyTaskAwareRouting,
  getTaskRoutingConfig,
} from "@devilroute/open-sse/services/taskAwareRouter.ts";
import {
  hasNativeWebSearchTool,
  resolveWebSearchRouteOverride,
} from "@devilroute/open-sse/services/webSearchRouting.ts";
import {
...
} from "@devilroute/open-sse/services/sessionManager.ts";
import { startQuotaMonitor } from "@devilroute/open-sse/services/quotaMonitor.ts";
import {
  isFallbackDecision,
  shouldUseFallback,
} from "@devilroute/open-sse/services/emergencyFallback.ts";
import {
  registerCodexConnection,
  registerCodexQuotaFetcher,
} from "@devilroute/open-sse/services/codexQuotaFetcher.ts";
import { registerBailianCodingPlanQuotaFetcher } from "@devilroute/open-sse/services/bailianQuotaFetcher.ts";
import { registerCrofUsageFetcher } from "@devilroute/open-sse/services/crofUsageFetcher.ts";
import { registerDeepseekQuotaFetcher } from "@devilroute/open-sse/services/deepseekQuotaFetcher.ts";
import { registerOpenrouterQuotaFetcher } from "@devilroute/open-sse/services/openrouterQuotaFetcher.ts";
import { registerOpencodeQuotaFetcher } from "@devilroute/open-sse/services/opencodeQuotaFetcher.ts";
import { registerGrokWebQuotaFetcher } from "@devilroute/open-sse/services/grokQuotaFetcher.ts";
import { registerGenericQuotaFetchers } from "@devilroute/open-sse/services/genericQuotaFetcher.ts";
import "@devilroute/open-sse/services/quotaTrackersBatch.ts";
import {
...
export async function handleChat(
  ...
  // DevilRoute-level 400 before any routing or upstream call (#5110, #6402).
  ...
  const requestedConnectionId = request.headers.get("x-devilroute-connection")?.trim() || null;
  if (sessionId) {
  ...
  requestAttemptLoop: while (true) {
    ...
    while (true) {
      ...
      if (
        ...
        body?._devilrouteSkipContextRelay !== true
      ) {
      ...
      if (runtimeOptions.sessionId && body?._devilrouteInternalRequest !== "context-handoff") {
