...
import { createBuiltinAutoCombo } from "@devilroute/open-sse/services/autoCombo/builtinCatalog.ts";
import * as log from "../utils/logger";
import { updateProviderCredentials } from "../services/tokenRefresh";
import {
  detectFormatFromEndpoint,
  getTargetFormat,
} from "@devilroute/open-sse/services/provider.ts";
import {
  getModelTargetFormat,
  PROVIDER_ID_TO_ALIAS,
} from "@devilroute/open-sse/config/providerModels.ts";
import { handleChatCore } from "@devilroute/open-sse/handlers/chatCore.ts";
import {
  errorResponse,
  modelCooldownResponse,
  providerCircuitOpenResponse,
  unavailableResponse,
} from "@devilroute/open-sse/utils/error.ts";
import { HTTP_STATUS } from "@devilroute/open-sse/config/constants.ts";
import {
  runWithProxyContext,
...
} from "@devilroute/open-sse/utils/proxyFetch.ts";
...
import { getRuntimeProviderProfile } from "@devilroute/open-sse/services/accountFallback.ts";
...
export async function safeLogEvents({
  ...
  try {
    ...
    try {
      ...
      const { proxyConfigToUrl } = await import("@devilroute/open-sse/utils/proxyDispatcher.ts");
...
export function withSessionHeader(response: Response, sessionId: string | null): Response {
  ...
  try {
    response.headers.set("X-DevilRoute-Session-Id", sessionId);
    return response;
  } catch {
    ...
    cloned.headers.set("X-DevilRoute-Session-Id", sessionId);
    return cloned;
  }
}
...
export function withSelectedConnectionHeader(
  ...
  try {
    response.headers.set("X-DevilRoute-Selected-Connection-Id", connectionId);
    return response;
  } catch {
    const cloned = new Response(response.body, {
      status: response.status,
    ...
    cloned.headers.set("X-DevilRoute-Selected-Connection-Id", connectionId);
    return cloned;
  }
}
