...
import { getQuotaScopeLabelForProvider } from "@devilroute/open-sse/services/antigravityQuotaFamily.ts";
import { getCreditsMode } from "@devilroute/open-sse/services/antigravityCredits.ts";
import {
...
import { isLocalProvider } from "@devilroute/open-sse/config/providerRegistry.ts";
import { COOLDOWN_MS, RateLimitReason } from "@devilroute/open-sse/config/constants.ts";
import {
  preflightQuota,
  isQuotaPreflightEnabled,
} from "@devilroute/open-sse/services/quotaPreflight.ts";
import { resolveResilienceSettings } from "@/lib/resilience/settings";
import { resolveModelLockoutSettings } from "@/lib/resilience/modelLockoutSettings";
import { syncHealthFromDB, type KeyHealth } from "@devilroute/open-sse/services/apiKeyRotator.ts";
import {
  classifyProviderError,
  PROVIDER_ERROR_TYPES,
} from "@devilroute/open-sse/services/errorClassifier.ts";
...
} from "@devilroute/open-sse/config/codexQuotaScopes.ts";
import {
  getProviderById,
  getProviderAlias,
  resolveProviderId,
...
export function extractSessionAffinityKey(
  ...
  const headerKey = normalizeSessionKey(
    readHeaderValue(headers, "x-codex-session-id") ??
      ...
      readHeaderValue(headers, "x-devilroute-session"),
    "header"
  );
 ...
 * - `Authorization: Bearer <key>` (OpenAI / DevilRoute / Codex CLI / Bearer clients)
 ...
 * rejected by per-route gates that compare against DevilRoute keys.
 *
 ...
 * Feature #1350: Supports DEVILROUTE_API_KEY / ROUTER_API_KEY env vars as
...
export async function isValidApiKey(apiKey: string) {
  ...
  const envKey = process.env.DEVILROUTE_API_KEY || process.env.ROUTER_API_KEY;
