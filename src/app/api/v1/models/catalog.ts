...
import { getAllEmbeddingModels } from "@devilroute/open-sse/config/embeddingRegistry";
import {
  getAllImageModels,
  isRegisteredImageModel,
} from "@devilroute/open-sse/config/imageRegistry";
import { getAllRerankModels } from "@devilroute/open-sse/config/rerankRegistry";
import { getAllAudioModels } from "@devilroute/open-sse/config/audioRegistry";
import { getAllModerationModels } from "@devilroute/open-sse/config/moderationRegistry";
import { getAllVideoModels } from "@devilroute/open-sse/config/videoRegistry";
import { getAllMusicModels } from "@devilroute/open-sse/config/musicRegistry";
import { REGISTRY } from "@devilroute/open-sse/config/providerRegistry";
import { CODEX_NATIVE_UNPREFIXED_MODELS } from "@devilroute/open-sse/services/model";
import { resolveNestedComboTargets } from "@devilroute/open-sse/services/combo";
import {
  ...
  isPaidTierAutoId,
} from "@devilroute/open-sse/services/autoCombo/builtinCatalog";
...
import { getTokenLimit } from "@devilroute/open-sse/services/contextManager";
...
import { buildErrorBody } from "@devilroute/open-sse/utils/error";
  ...
  try {
    ...
    if (apiKey) {
      ...
      if (keyMeta && keyMeta.allowedQuotas && keyMeta.allowedQuotas.length > 0) {
        ...
        // (DEVILROUTE_API_KEY / ROUTER_API_KEY per isValidApiKey). Those keys have no
