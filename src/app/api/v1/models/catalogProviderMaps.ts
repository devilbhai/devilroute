...
import { parseModel } from "@devilroute/open-sse/services/model";
...
/** True when `${prefix}/__devilroute_probe__` parses back to the given providerId. */
export function prefixRoutesToProvider(prefix: string, providerId: string): boolean {
  const parsed = parseModel(`${prefix}/__devilroute_probe__`);
