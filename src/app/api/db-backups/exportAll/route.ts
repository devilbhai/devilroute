...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const tempDir = path.join(os.tmpdir(), `devilroute-export-${timestamp}`);
    const zipPath = path.join(os.tmpdir(), `devilroute-full-backup-${timestamp}.zip`);
    ...
    try {
      ...
      const metadata = {
        ...
        format: "devilroute-full-backup-v1",
        contents: [
      ...
      return new NextResponse(archiveBuffer, {
        ...
        headers: {
          ...
          "Content-Disposition": `attachment; filename="devilroute-full-backup-${timestamp}.tar.gz"`,
