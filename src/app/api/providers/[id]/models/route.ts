...
import { getRegistryEntry } from "@devilroute/open-sse/config/providerRegistry.ts";
...
import { getStaticQoderModels } from "@devilroute/open-sse/services/qoderCli.ts";
import { deriveConfigFromRegistryModelsUrl } from "./discoveryConfig";
import {
  fetchGitHubCopilotModels,
  fetchGheCopilotModels,
} from "@devilroute/open-sse/services/githubCopilotModels.ts";
import { fetchKiroAvailableModels } from "@devilroute/open-sse/services/kiroModels.ts";
import {
  buildGlmCodingHeaders,
  buildGlmModelsUrl,
} from "@devilroute/open-sse/config/glmProvider.ts";
import { getImageProvider } from "@devilroute/open-sse/config/imageRegistry.ts";
import { getVideoProvider } from "@devilroute/open-sse/config/videoRegistry.ts";
import {
  discoverBedrockNativeModels,
  isBedrockNativeApiError,
} from "@devilroute/open-sse/services/bedrock.ts";
import {
  discoverPromptQlModels,
  PROMPTQL_FALLBACK_MODELS,
} from "@devilroute/open-sse/services/promptqlModels.ts";
import {
  discoverNotionWebModels,
  NOTION_WEB_FALLBACK_MODELS,
} from "@devilroute/open-sse/services/notionWebModels.ts";
import {
  AZURE_AI_DEFAULT_BASE_URL,
  buildAzureAiModelsUrl,
} from "@devilroute/open-sse/config/azureAi.ts";
import {
  DATAROBOT_DEFAULT_BASE_URL,
  buildDataRobotCatalogUrl,
  isDataRobotDeploymentUrl,
} from "@devilroute/open-sse/config/datarobot.ts";
import { OCI_DEFAULT_BASE_URL, buildOciModelsUrl } from "@devilroute/open-sse/config/oci.ts";
import {
  SAP_DEFAULT_BASE_URL,
  buildSapModelsUrl,
  getSapResourceGroup,
} from "@devilroute/open-sse/config/sap.ts";
import {
  WATSONX_DEFAULT_BASE_URL,
  buildWatsonxModelsUrl,
} from "@devilroute/open-sse/config/watsonx.ts";
import { getEmbeddingProvider } from "@devilroute/open-sse/config/embeddingRegistry.ts";
import { getRerankProvider } from "@devilroute/open-sse/config/rerankRegistry.ts";
import {
  getSpeechProvider,
  getTranscriptionProvider,
} from "@devilroute/open-sse/config/audioRegistry.ts";
import {
...
export async function GET(
  ...
  try {
    ...
    const buildDiscoveryErrorFallbackResponse = (
      ...
      // the local/cached catalog DevilRoute ships instead of surfacing a raw 503.
    ...
    if (provider === "vertex" || provider === "vertex-partner") {
      ...
      try {
        const { parseSAFromApiKey, getAccessToken } =
          await import("@devilroute/open-sse/executors/vertex.ts");
        if (accessToken) {
