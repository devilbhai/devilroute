    ...
    const stream = new ReadableStream({
      async start(controller) {
        ...
        try {
          ...
          try {
            await execFileAsync("pm2", ["restart", "devilroute", "--update-env"], {
              timeout: 30_000,
              cwd: PROJECT_ROOT,
            });
  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      try {
        ...
        send({ step: "install", status: "running", message: `Installing devilroute@${latest}...` });
          await execFileAsync(
            "npm",
            ["install", "-g", `devilroute@${latest}`, "--ignore-scripts", "--legacy-peer-deps"],
            buildNpmExecOptions(process.platform, { cwd: PROJECT_ROOT, timeoutMs: 300_000 })
          );
        send({ step: "install", status: "done", message: `Installed devilroute@${latest}` });
