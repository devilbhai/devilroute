 ...
 *
 * Re-issues the captured request through the local DevilRoute instance and
...
import { buildErrorBody, sanitizeErrorMessage } from "@devilroute/open-sse/utils/error.ts";
...
const DEVILROUTE_BASE = process.env.DEVILROUTE_BASE_URL ?? "http://127.0.0.1:20128";
...
export async function POST(_request: Request, { params }: Params): Promise<Response> {
  ...
  const url = `${DEVILROUTE_BASE}${entry.path}`;

  const replayHeaders: Record<string, string> = {
    "content-type": "application/json",
    "x-devilroute-source": "inspector-replay",
  };
