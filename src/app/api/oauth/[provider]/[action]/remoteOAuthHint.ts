 ...
 * If the operator drives the OAuth flow from a different machine (DevilRoute on
...
export function buildRemoteOAuthHint(hostHeader: string | null, port: number): RemoteOAuthHint {
  ...
  return {
    remoteHost: true,
    tunnelCommand: `ssh -L ${port}:127.0.0.1:${port} <user>@<devilroute-host>`,
    message:
      `DevilRoute appears to be running on a remote host (${hostHeader}). ` +
