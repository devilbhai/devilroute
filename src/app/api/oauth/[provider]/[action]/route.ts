...
import { runWithProxyContextOrDirect } from "@devilroute/open-sse/utils/proxyFetch.ts";
import {
  jsonObjectSchema,
...
function resolvePublicBaseUrl(request: Request): string {
  const env = process.env.NEXT_PUBLIC_BASE_URL || process.env.DEVILROUTE_PUBLIC_BASE_URL;
...
async function handleStartCallbackServer(
  ...
  try {
    ...
    // *different* machine (DevilRoute running on a remote host/VPS), the
