...
import { sanitizeErrorMessage } from "@devilroute/open-sse/utils/error";
...
const getBackupPath = () =>
  path.join(getLocalBackendDir(), "providers", "auth.json.devilroute-backup");
...
// ── Check if a base_url points to DevilRoute ──────────────────────────────
const isDevilRouteUrl = (baseUrl) => {
  if (!baseUrl) return false;
  return baseUrl.includes(":20128") || baseUrl.includes(":3000") || baseUrl.includes("devilroute");
};

// ── Check if DevilRoute is configured ─────────────────────────────────────
const hasDevilRouteConfig = (authFile) => {
  if (!authFile?.providers) return false;
  const provider = authFile.providers[PROVIDER_NAME];
  if (!provider) return false;
  return isDevilRouteUrl(provider.base_url);
};
...
export async function GET(request: Request) {
  ...
  try {
    ...
    // Detect if lmstudio is already configured for a non-DevilRoute endpoint
    let lmstudioConflict = false;
    if (provider && !isDevilRouteUrl(provider.base_url)) {
    ...
    return NextResponse.json({
      installed: true,
      config: authFile,
      hasDevilRoute: hasDevilRouteConfig(authFile),
      lmstudioConflict,
...
// ── POST - Apply DevilRoute as LM Studio provider + switch to local mode ──
/**
 ...
 * up a non-DevilRoute provider before it is overwritten. Extracted to keep POST under
 * the complexity gate.
 */
...
> {
  ...
  if (existingProvider && !isDevilRouteUrl(existingProvider.base_url) && !overwrite) {
  ...
  if (existingProvider && !isDevilRouteUrl(existingProvider.base_url)) {
...
export async function POST(request: Request) {
  ...
  try {
    ...
    // ── 1-2. Read auth.json, guard non-DevilRoute conflicts, back up before overwrite ──
    ...
    // Clean up legacy lc-devilroute provider if present
    if (authFile.providers?.["lc-devilroute"]) {
      delete authFile.providers["lc-devilroute"];
    }
    ...
    return NextResponse.json({
      success: true,
      message: "Settings applied. Restart Letta CLI, then use /model to select a DevilRoute model.",
      needsRestart: true,
    });
  } catch (error) {
    return NextResponse.json(
...
// ── DELETE - Remove DevilRoute configuration ──────────────────────────────
export async function DELETE(request: Request) {
  ...
  try {
    ...
    const message = restored
      ? "DevilRoute config removed. Your original LM Studio provider has been restored. Restart Letta CLI to take effect."
      : "DevilRoute config removed. Restart Letta CLI to take effect.";
