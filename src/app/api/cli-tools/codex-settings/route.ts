...
const parseToml = (content: string) => {
  ...
  content.split("\n").forEach((line) => {
    ...
    // Section header like [model_providers.devilroute]
...
// Check if config has DevilRoute settings
const hasDevilRouteConfig = (config: string | null) => {
  if (!config) return false;
  return (
    config.includes("openai_base_url") ||
    config.includes('model_provider = "devilroute"') ||
    config.includes("[model_providers.devilroute]")
  );
};
...
// POST - Update DevilRoute settings (merge with existing config)
export async function POST(request: Request) {
  ...
  try {
    ...
    // Update only DevilRoute related fields (api_key goes to auth.json, not config.toml)
    ...
    // Always create a custom provider to reliably pass wire_api and use DEVILROUTE_API_KEY
    parsed._root.model_provider = "devilroute";
    parsed._sections["model_providers.devilroute"] = {
      name: "DevilRoute",
...
// DELETE - Remove DevilRoute settings only (keep other settings)
export async function DELETE(request: Request) {
  ...
  try {
    ...
    // Remove DevilRoute related root fields
    delete parsed._root.openai_base_url;

    if (parsed._root.model_provider === "devilroute") {
      delete parsed._root.model;
      delete parsed._root.model_provider;
    }

    // Remove devilroute provider section
    delete parsed._sections["model_providers.devilroute"];
    ...
    return NextResponse.json({
      success: true,
      message: "DevilRoute settings removed successfully",
    });
  } catch (error) {
