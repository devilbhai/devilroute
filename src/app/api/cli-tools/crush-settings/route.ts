...
import { sanitizeErrorMessage } from "@devilroute/open-sse/utils/error.ts";
...
// the `devilroute setup-crush` CLI command agree on one canonical location.
...
/**
 * Crush's config uses a `providers.<id>` map. DevilRoute is registered under
 * the `devilroute` provider id as an `openai-compat` provider — same shape
...
const hasDevilRouteConfig = (settings: Record<string, unknown> | null): boolean => {
  if (!settings) return false;
  const providers = settings.providers as Record<string, unknown> | undefined;
  const devilroute = providers?.devilroute as Record<string, unknown> | undefined;
  return (
    !!devilroute &&
    devilroute.type === "openai-compat" &&
    typeof devilroute.base_url === "string" &&
    devilroute.base_url.length > 0
  );
};
...
export async function GET(request: Request) {
  ...
  try {
    ...
    return NextResponse.json({
      ...
      reason: runtime.reason,
      config,
      hasDevilRoute: hasDevilRouteConfig(config),
...
// POST — write DevilRoute settings to crush.json (providers.devilroute)
export async function POST(request: Request) {
  ...
  try {
    ...
    const provider: CrushProvider = {
      ...
      api_key: apiKey,
      models: [{ id: model, name: `DevilRoute: ${model}`, context_window: DEFAULT_CONTEXT_WINDOW }],
    };
...
// DELETE — remove DevilRoute provider from Crush config
export async function DELETE(request: Request) {
  ...
  try {
    ...
    return NextResponse.json({ success: true, message: "Crush DevilRoute settings removed" });
  } catch (err) {
    return NextResponse.json({ error: { message: sanitizeErrorMessage(err) } }, { status: 500 });
  }
}
