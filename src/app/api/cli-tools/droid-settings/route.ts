...
// Check if settings has DevilRoute customModels.
// Multi-model entries are stored as `custom:DevilRoute-0`, `custom:DevilRoute-1`, …
// (Ported from upstream PR decolua/9router#618.)
const hasDevilRouteConfig = (settings: any) => {
  if (!settings || !settings.customModels) return false;
  return settings.customModels.some(isDevilRouteCustomModel);
};
...
export async function GET(request: Request) {
  ...
  try {
    ...
    return NextResponse.json({
      ...
      reason: runtime.reason,
      settings,
      hasDevilRoute: hasDevilRouteConfig(settings),
...
// POST - Update DevilRoute customModels (merge with existing settings)
export async function POST(request: Request) {
  ...
  try {
    ...
    // Remove every existing DevilRoute config (multi-model: index 0..N)
    settings.customModels = settings.customModels.filter((m) => !isDevilRouteCustomModel(m));

    // Normalize baseUrl to ensure /v1 suffix
    const normalizedBaseUrl = baseUrl.endsWith("/v1") ? baseUrl : `${baseUrl}/v1`;

    // Build and prepend DevilRoute entries (one per requested model)
...
// DELETE - Remove DevilRoute customModels only (keep other settings)
export async function DELETE(request: Request) {
  ...
  try {
    ...
    // Remove DevilRoute customModels (every index, multi-model)
    if (settings.customModels) {
      settings.customModels = settings.customModels.filter((m) => !isDevilRouteCustomModel(m));
