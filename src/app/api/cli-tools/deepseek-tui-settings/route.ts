...
/**
 * Render the DevilRoute config block in DeepSeek TUI TOML format.
...
function renderDeepseekTuiConfig(baseUrl: string, apiKey: string, model: string): string {
  return [
    "# DeepSeek TUI config — managed by DevilRoute (plan 14)",
    "",
    "[openai]",
    `base_url = "${baseUrl}"`,
    `api_key = "${apiKey}"`,
...
// POST — write DevilRoute settings to DeepSeek TUI config.toml
...
// DELETE — remove DeepSeek TUI DevilRoute config
