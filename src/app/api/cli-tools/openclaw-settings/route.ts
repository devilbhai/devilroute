...
// Check if settings has DevilRoute config
const hasDevilRouteConfig = (settings: any) => {
  if (!settings || !settings.models || !settings.models.providers) return false;
  return !!settings.models.providers["devilroute"];
};
...
// POST - Update DevilRoute settings (merge with existing settings)
export async function POST(request: Request) {
  ...
  try {
    ...
    settings.agents.defaults.model.primary = `devilroute/${model}`;

    // Update models.providers.devilroute
    settings.models.providers["devilroute"] = {
...
export async function DELETE(request: Request) {
  ...
  try {
    ...
    // Remove DevilRoute from models.providers
    if (settings.models && settings.models.providers) {
      delete settings.models.providers["devilroute"];
    ...
    // Reset agents.defaults.model.primary if it uses devilroute
    if (settings.agents?.defaults?.model?.primary?.startsWith("devilroute/")) {
