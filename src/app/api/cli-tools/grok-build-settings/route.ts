...
// [model.devilroute] ... until the next [section] header or EOF
...
const PREV_DEFAULT_RE = /^# devilroute-prev-default = "([^"]*)"[ \t]*\r?\n?/m;
 ...
 * only the flat string fields DevilRoute itself writes.
 */
...
const buildModelSection = (model: string, baseUrl: string, apiKey: string): string => {
  const lines = [
    ...
    `name = "DevilRoute"`,
    `description = "Routed via DevilRoute gateway"`,
...
/** Insert/replace the `[model.devilroute]` section, preserving the rest of the file. */
...
const rememberPrevDefault = (toml: string): string => {
  ...
  const marker = `# devilroute-prev-default = "${current}"\n`;
...
const hasDevilRouteConfig = (modelCfg: GrokModelSection | null): boolean =>
...
// GET — check Grok Build CLI and return current [model.devilroute] config
export async function GET(request: Request) {
  ...
  try {
    ...
    return NextResponse.json({
      ...
      hasDevilRoute: hasDevilRouteConfig(model),
...
// POST — write the [model.devilroute] section into ~/.grok/config.toml and set it default
...
// DELETE — remove the [model.devilroute] section and restore the previous default
export async function DELETE(request: Request) {
  ...
  try {
    ...
    return NextResponse.json({
      success: true,
      message: "DevilRoute model slot removed from Grok Build",
    });
  } catch (err) {
    return NextResponse.json({ error: { message: sanitizeErrorMessage(err) } }, { status: 500 });
  }
