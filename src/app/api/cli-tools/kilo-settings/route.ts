...
// Check if DevilRoute OpenAI-compatible provider is configured
const hasDevilRouteConfig = (auth) => {
  if (!auth) return false;
  const routerEntry = auth["openai-compatible"] || auth["devilroute"];
  if (!routerEntry) return false;
  const baseUrl = routerEntry.baseUrl || routerEntry.baseURL || "";
  return (
    baseUrl.includes("localhost") || baseUrl.includes("127.0.0.1") || baseUrl.includes("devilroute")
  );
};
...
export async function GET(request: Request) {
  ...
  try {
    ...
    return NextResponse.json({
      ...
      hasDevilRoute: hasDevilRouteConfig(auth),
      authPath: AUTH_PATH,
    });
  } catch (error) {
...
// POST - Configure Kilo Code to use DevilRoute as OpenAI-compatible provider
export async function POST(request) {
  ...
  try {
    ...
    // Add/update DevilRoute as openai-compatible provider
...
// DELETE - Remove DevilRoute config from Kilo
export async function DELETE(request: Request) {
  ...
  try {
    ...
    // Remove DevilRoute provider
    delete auth["openai-compatible"];
    delete auth["devilroute"];
    ...
    return NextResponse.json({
      success: true,
      message: "DevilRoute settings removed from Kilo Code",
    });
  } catch (error) {
