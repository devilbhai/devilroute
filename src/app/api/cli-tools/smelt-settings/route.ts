...
/**
 * Check if the config file contains DevilRoute settings.
 */
const hasDevilRouteConfig = (settings: Record<string, unknown> | null): boolean => {
  ...
  return (
    ...
    settings._managedBy === "devilroute"
  );
};
...
// POST — write DevilRoute settings to Smelt config.json
export async function POST(request: Request) {
  ...
  try {
    ...
    // Merge DevilRoute settings (smelt uses OpenAI-compatible config)
...
// DELETE — remove DevilRoute settings from Smelt config
export async function DELETE(request: Request) {
  ...
  try {
    ...
    // Remove DevilRoute-managed fields
    ...
    return NextResponse.json({ success: true, message: "Smelt DevilRoute settings removed" });
  } catch (err) {
    return NextResponse.json(
      { error: { message: sanitizeErrorMessage(err) } },
      { status: 500 }
