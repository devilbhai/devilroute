...
/**
 * Render the DevilRoute config block in CodeWhale TOML format.
...
function renderCodewhaleConfig(baseUrl: string, apiKey: string, model: string): string {
  return [
    "# CodeWhale config — managed by DevilRoute (plan 14)",
    "",
    "[openai]",
    `base_url = "${baseUrl}"`,
    `api_key = "${apiKey}"`,
 ...
 */
const hasDevilRouteConfig = (content: string | null): boolean => {
  if (!content) return false;
  return content.includes("managed by DevilRoute");
};
...
// POST — write DevilRoute settings to CodeWhale's config.toml (primary), and
...
// DELETE — remove DevilRoute CodeWhale config (primary + legacy, if present)
