...
/**
 * Render the DevilRoute provider block in Forge TOML format.
...
function renderForgeConfig(baseUrl: string, apiKey: string, model: string): string {
  ...
  return [
    "# Forge config — managed by DevilRoute (plan 14)",
    "",
    "[openai]",
    `api_key = "${apiKey}"`,
 ...
 * Looks for the managed-by-DevilRoute marker comment.
 */
...
// POST — write DevilRoute settings to Forge config.toml
...
// DELETE — remove Forge DevilRoute config
