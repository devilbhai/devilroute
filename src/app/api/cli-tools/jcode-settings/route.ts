...
// POST — write DevilRoute settings to jcode config.json
export async function POST(request: Request) {
  ...
  try {
    ...
    // Merge DevilRoute settings (jcode uses OpenAI-compatible config)
...
// DELETE — remove DevilRoute settings from jcode config
export async function DELETE(request: Request) {
  ...
  try {
    ...
    return NextResponse.json({ success: true, message: "jcode DevilRoute settings removed" });
  } catch (err) {
    return NextResponse.json(
      { error: { message: sanitizeErrorMessage(err) } },
      { status: 500 }
