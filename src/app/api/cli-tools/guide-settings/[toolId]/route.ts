  ...
  try {
    switch (toolId) {
      ...
      case "opencode":
        ...
        // OpenCode reads ~/.config/opencode/opencode.json — write the DevilRoute settings there.
...
async function saveContinueConfig({ baseUrl, apiKey, model }) {
  ...
  // Build the DevilRoute model entry
  ...
  // Check if DevilRoute entry already exists and update it, or add new
  const existingIdx = models.findIndex(
    (m) =>
      m &&
      (m.devilrouteManaged === true ||
        normalizeApiBase(m.apiBase) === normalizedBaseUrl.toLowerCase() ||
        normalizeApiBase(m.apiBase).includes("devilroute") ||
 ...
 * keys, but make sure the DevilRoute provider entry is present and selected.
 */
async function saveHermesConfig({ baseUrl, apiKey, model }) {
  ...
  const nextConfig = {
    ...
    providers: {
      ...
      devilroute: {
        ...((existingConfig.providers && existingConfig.providers.devilroute) || {}),
        base_url: providerBaseUrl,
        api_key:
          apiKey ||
          (existingConfig.providers &&
            existingConfig.providers.devilroute &&
            existingConfig.providers.devilroute.api_key) ||
          "",
      },
    },
  };
