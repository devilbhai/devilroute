...
import { estimateTokens } from "@devilroute/open-sse/services/contextManager.ts";
import { adaptBodyForCompression } from "@devilroute/open-sse/services/compression/bodyAdapter.ts";
import type {
  CompressionConfig,
  CompressionResult,
} from "@devilroute/open-sse/services/compression/types.ts";
import { resolveCompressionSettings } from "@devilroute/open-sse/handlers/chatCore/compressionSettings.ts";
import {
  writeCompressionAnalytics,
  writeCompressionSkip,
} from "@devilroute/open-sse/handlers/chatCore/compressionAnalyticsWrite.ts";
...
export async function applyResponsesWsCompression(
  ...
  try {
    ...
    const { selectCompressionStrategy, applyCompressionAsync } = await import(
      "@devilroute/open-sse/services/compression/strategySelector.ts"
    );
