...
import { z } from "zod";
import { CodexExecutor } from "@devilroute/open-sse/executors/codex.ts";
...
import { logger } from "@devilroute/open-sse/utils/logger.ts";
import { resolveProxy } from "@devilroute/open-sse/utils/networkProxy.ts";
import { proxyConfigToUrl } from "@devilroute/open-sse/utils/proxyDispatcher.ts";
import {
...
function getBridgeSecret(): string {
  return process.env.DEVILROUTE_WS_BRIDGE_SECRET || "";
}
...
function getAuthRequest(body: JsonRecord): Request {
  ...
  const url = new URL(requestUrl, "http://devilroute.local");
...
async function resolveCodexRequestContext(body: JsonRecord) {
  if (!isFeatureFlagEnabled("DEVILROUTE_CODEX_WS_ENABLED")) {
    return {
...
async function prepare(body: JsonRecord) {
  ...
  if (reasoningDecision) {
    ...
    reasoningRouting = isRecord(withDirective._devilrouteReasoningRouteTrace)
      ? withDirective._devilrouteReasoningRouteTrace
      : null;
    responseBodyWithMemory = applyReasoningRuleDirective(withDirective) as JsonRecord;
    delete responseBodyWithMemory._devilrouteReasoningRouteTrace;
  }
  ...
  // Responses WebSocket too. The downstream client→DevilRoute hop works, but the
...
export async function POST(request: Request) {
  ...
  const receivedSecret = request.headers.get("x-devilroute-ws-bridge-secret") || "";
