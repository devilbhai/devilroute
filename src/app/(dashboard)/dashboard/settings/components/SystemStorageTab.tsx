...
export default function SystemStorageTab() {
  ...
  const [storageHealth, setStorageHealth] = useState({
    driver: "sqlite",
    dbPath: "~/.devilroute/storage.sqlite",
    sizeBytes: 0,
    retentionDays: {
      app: 7,
      call: 7,
  ...
  const handleExportJson = async () => {
    ...
    try {
      await fetchAndDownload(
        ...
        `devilroute-legacy-backup-${new Date().toISOString().replace(/[:.]/g, "-")}.json`,
        t("jsonExportFailed")
      );
    } catch (err) {
  ...
  const handleExport = async () => {
    ...
    try {
      await fetchAndDownload(
        ...
        `devilroute-backup-${new Date().toISOString().replace(/[:.]/g, "-")}.sqlite`,
        t("exportFailed")
      );
    } catch (err) {
      ...
      <div className="grid grid-cols-1 gap-3 mb-4">
        <div className="p-3 rounded-lg bg-bg border border-border">
          ...
          <p className="text-sm font-mono text-text-main break-all">
            {storageHealth.dbPath || "~/.devilroute/storage.sqlite"}
          </p>
        </div>
      </div>
