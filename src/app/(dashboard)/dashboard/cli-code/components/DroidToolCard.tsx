...
export default function DroidToolCard({
  ...
  // (#618) Match any custom:DevilRoute-<i> entry (multi-model).
  const isDevilRouteEntry = (m) => typeof m?.id === "string" && m.id.startsWith("custom:DevilRoute");

  const getConfigStatus = () => {
    if (!cliReady) return null;
    const currentConfig = droidStatus.settings?.customModels?.find(isDevilRouteEntry);
    ...
    try {
      ...
      const res = await fetch("/api/cli-tools/droid-settings", {
        ...
        body: JSON.stringify({
          ...
          apiKey: !cloudEnabled ? "sk_devilroute" : null,
          keyId: selectedKeyId,
  ...
  const getManualConfigs = () => {
    ...
    const keyToDisplay =
      selectedKeyObj?.key || (!cloudEnabled ? "sk_devilroute" : "<API_KEY_FROM_DASHBOARD>");
          ...
          {!checkingDroid && cliReady && (
            <>
              <div className="flex flex-col gap-2">
                {/* Current Base URL — first DevilRoute entry, any index (#618) */}
                {droidStatus?.settings?.customModels?.find(isDevilRouteEntry)?.baseUrl && (
                  <div className="flex items-center gap-2">
              ...
              <div className="flex items-center gap-2">
                ...
                <Button
                  ...
                  disabled={!droidStatus?.hasDevilRoute}
                  loading={restoring}
                >
