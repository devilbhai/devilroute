 ...
 *
 * Feature request: https://github.com/diegosouzapw/DevilRoute/issues/142
 */
export default function CopilotToolCard({
  ...
  const [selectedModels, setSelectedModels] = useState<Set<string>>(() => {
    ...
    try {
      const saved = localStorage.getItem("devilroute-copilot-selected-models");
  ...
  const [selectedApiKeyId, setSelectedApiKeyId] = useState(() => {
    if (typeof window !== "undefined") {
      const savedKey = localStorage.getItem("devilroute-cli-key-copilot");
  ...
  useEffect(() => {
    if (selectedModels.size > 0) {
      localStorage.setItem(
        "devilroute-copilot-selected-models",
  ...
  const generateConfig = () => {
    ...
    const config = {
      name: "DevilRoute",
      vendor: "azure",
      apiKey: `\${input:chat.lm.secret.devilroute}`,
      models,
    };

    const responsesConfig = {
      name: "DevilRoute-responses",
      vendor: "azure",
      apiKey: `\${input:chat.lm.secret.devilroute}`,
  ...
  const handleApiKeyChange = (value: string) => {
    ...
    if (value) localStorage.setItem("devilroute-cli-key-copilot", value);
  };
