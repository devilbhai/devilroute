...
export default function HermesAgentToolCard({
  ...
  const setRoleSelection = (roleId: string, model: string, provider = "DevilRoute") => {
    setSelections((prev) => ({ ...prev, [roleId]: { model, provider } }));
  };

  const applyToAll = (model: string) => {
    const newSel: Record<string, RoleSelection> = {};
    HERMES_ROLES.forEach((r) => (newSel[r.id] = { model, provider: "DevilRoute" }));
  ...
  const effectiveRoles = React.useMemo(() => {
    // If user has pending changes, treat selected roles as DevilRoute
    if (Object.keys(selections).length > 0) {
      const map: Record<string, any> = {};
      HERMES_ROLES.forEach((r) => {
        if (selections[r.id]) {
          map[r.id] = { usingDevilRoute: true };
  ...
  // Count of roles that are (or will be) routed via DevilRoute
  const configuredRolesCount = HERMES_ROLES.filter((role) => {
    // Pending selection always counts as DevilRoute intent
    if (selections[role.id]) return true;

    const info = effectiveRoles[role.id];
    if (!info) return false;

    // Support both shapes: detector shape (usingDevilRoute) and settings shape (provider + base_url)
    if (typeof info.usingDevilRoute === "boolean") {
      return info.usingDevilRoute;
    }
    return (
      info?.provider === "devilroute" ||
        ...
        <div className="mt-4 pt-4 border-t border-border flex flex-col gap-4">
          ...
          <div className="flex flex-col gap-2">
            {HERMES_ROLES.map((role) => {
              ...
              // - If user has selected something in this session (pending): show as via DevilRoute
              // - Else if current from disk: show real provider name + "(not DevilRoute)" or "DevilRoute"
              let badge: { label: string; pending: boolean; outsideDevilRoute: boolean } | null =
                null;

              if (sel) {
                // pending change made via the Select modal / quick apply → will be routed via DevilRoute
                const prov = sel.provider || "DevilRoute";
                badge = {
                  label: t("hermesViaDevilRoute", { provider: prov }),
                  pending: true,
                  outsideDevilRoute: false,
                };
              } else if (current) {
                const isOmni =
                  current?.provider === "devilroute" ||
                  (current?.base_url || "").includes("20128") ||
                  (current?.base_url || "").includes("localhost");

                if (isOmni) {
                  badge = { label: "DevilRoute", pending: false, outsideDevilRoute: false };
                } else {
                  const realProvider = current.provider || t("other");
                  badge = {
                    label: t("hermesNotDevilRoute", { provider: realProvider }),
                    pending: false,
                    outsideDevilRoute: true,
                  };
                }
              }
        ...
        onSelect={(model: any) => {
          if (modalRole) {
            ...
            if (modelValue) {
              ...
              const prov =
                (model && (model.provider || model.providerId || model.group)) || "DevilRoute";
