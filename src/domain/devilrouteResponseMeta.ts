...
import { DEVILROUTE_RESPONSE_HEADERS } from "@/shared/constants/headers";
...
export function getDevilRouteTokenCounts(usage: UsageLike): { input: number; output: number } {
...
export function formatDevilRouteCost(costUsd: unknown): string {
  const normalized = toFiniteNumber(costUsd);
  return normalized > 0 ? normalized.toFixed(10) : "0.0000000000";
}

/**
 * Build the `X-DevilRoute-Decision` composite header value: `strategy=<name>;
 ...
 * (same alias normalization the `X-DevilRoute-Provider` header already applies)
 ...
 */
export function buildDevilRouteDecisionHeaderValue({
  strategy = null,
  provider = null,
  latencyMs = 0,
}: {
...
export function buildDevilRouteResponseMetaHeaders({
  cacheHit = false,
  costUsd = 0,
   ...
   * Emitted as `X-DevilRoute-Cost-Saved` only when provided (omitted on normal
   ...
   * request) surfaced via `X-DevilRoute-Decision`. See #6022.
   */
  strategy?: string | null;
  usage?: UsageLike;
}): Record<string, string> {
  const tokens = getDevilRouteTokenCounts(usage);
  const headers: Record<string, string> = {
    [DEVILROUTE_RESPONSE_HEADERS.cacheHit]: toHeaderValue(String(cacheHit)),
    [DEVILROUTE_RESPONSE_HEADERS.latencyMs]: toHeaderValue(String(toNonNegativeInteger(latencyMs))),
    [DEVILROUTE_RESPONSE_HEADERS.responseCost]: toHeaderValue(formatDevilRouteCost(costUsd)),
    [DEVILROUTE_RESPONSE_HEADERS.tokensIn]: toHeaderValue(String(tokens.input)),
    [DEVILROUTE_RESPONSE_HEADERS.tokensOut]: toHeaderValue(String(tokens.output)),
    [DEVILROUTE_RESPONSE_HEADERS.version]: toHeaderValue(APP_CONFIG.version),
  };

  if (typeof model === "string" && model.trim().length > 0) {
    headers[DEVILROUTE_RESPONSE_HEADERS.model] = toHeaderValue(model);
  }

  if (typeof requestId === "string" && requestId.trim().length > 0) {
    headers[DEVILROUTE_RESPONSE_HEADERS.requestId] = toHeaderValue(requestId);
  }

  if (typeof provider === "string" && provider.trim().length > 0) {
    headers[DEVILROUTE_RESPONSE_HEADERS.provider] = toHeaderValue(getProviderAlias(provider));
  }
  ...
  if (costSavedUsd != null) {
    headers[DEVILROUTE_RESPONSE_HEADERS.costSaved] = toHeaderValue(
      formatDevilRouteCost(costSavedUsd)
    );
  }

  const attempts = toNonNegativeInteger(fallbackAttempts);
  if (attempts > 0) {
    headers[DEVILROUTE_RESPONSE_HEADERS.fallbackAttempts] = toHeaderValue(String(attempts));
  }

  const decisionValue = buildDevilRouteDecisionHeaderValue({ strategy, provider, latencyMs });
  if (decisionValue !== null) {
    headers[DEVILROUTE_RESPONSE_HEADERS.decision] = decisionValue;
  }

  return headers;
}

export function buildDevilRouteSseMetadataComment(
  options: Parameters<typeof buildDevilRouteResponseMetaHeaders>[0]
): string {
  const headers = buildDevilRouteResponseMetaHeaders(options);
...
/**
 * Single choke-point for attaching the X-DevilRoute-* response meta headers.
 ...
 */
export function attachDevilRouteMetaHeaders(
  headers: Headers | Record<string, string>,
  meta: Parameters<typeof buildDevilRouteResponseMetaHeaders>[0]
): void {
  const built = buildDevilRouteResponseMetaHeaders(meta);
...
/**
 * Attach the X-DevilRoute-* meta headers onto an already-built Response, ADDING
 ...
 */
export function attachDevilRouteMetaToResponse(
  response: Response,
  meta: Parameters<typeof buildDevilRouteResponseMetaHeaders>[0]
): Response {
  ...
  try {
    attachDevilRouteMetaHeaders(response.headers, meta);
    return response;
  } catch {
    ...
    attachDevilRouteMetaHeaders(cloned.headers, meta);
    return cloned;
  }
}
