...
import { getUsageForProvider } from "@devilroute/open-sse/services/usage.ts";
...
import { getCodexQuotaWindowFilterForModel } from "@devilroute/open-sse/config/codexQuotaScopes.ts";
import { getAntigravityQuotaFamily } from "@devilroute/open-sse/services/antigravityQuotaFamily.ts";
...
declare global {
  var __devilrouteQuotaCacheState: QuotaCacheState | undefined;
}

function getState(): QuotaCacheState {
  if (!globalThis.__devilrouteQuotaCacheState) {
    globalThis.__devilrouteQuotaCacheState = {
      cache: new Map(),
  ...
  return globalThis.__devilrouteQuotaCacheState;
}
