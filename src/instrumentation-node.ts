...
/**
 * Rename a Node process title so DevilRoute is identifiable in `ps`/`htop`
...
export function renameProcessTitle(currentTitle: string): string {
  ...
  return `devilroute${currentTitle.slice("next-server".length)}`;
}
...
function isBackgroundServicesDisabled(): boolean {
  const raw = process.env.DEVILROUTE_DISABLE_BACKGROUND_SERVICES;
  if (!raw) return false;
...
export async function registerNodejs(): Promise<void> {
  ...
  // Rename the process title so DevilRoute is identifiable in ps/htop instead
  ...
  await import("@devilroute/open-sse/index.ts");
  ...
  if (!isBackgroundServicesDisabled()) {
    ...
    const { initBatchProcessor } = await import("@devilroute/open-sse/services/batchProcessor");
    initBatchProcessor();
  ...
  try {
    ...
    if (settings.systemPrompt) {
      const { setSystemPromptConfig } =
        await import("@devilroute/open-sse/services/systemPrompt.ts");
    ...
    const { hydrateThinkingBudgetConfig } =
      await import("@devilroute/open-sse/services/thinkingBudget.ts");
    ...
    const { hydrateTaskRoutingConfig } =
      await import("@devilroute/open-sse/services/taskAwareRouter.ts");
  ...
  // DEVILROUTE_DISABLE_CREDENTIAL_HEALTH_CHECK and its cadence is tunable via
  ...
  if (!isBackgroundServicesDisabled()) {
    ...
    await Promise.allSettled([
      ...
      import("@devilroute/open-sse/services/autoRefreshDaemon")
      ...
      // Reads the schedule written by `devilroute backup auto enable` and fires
      ...
      // flag-gated auto-start (DEVILROUTE_ENABLE_LIVE_WS, default ON).
