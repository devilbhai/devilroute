  ...
  try {
    ...
    console.warn(
      `[DB] Cannot create data directory '${DATA_DIR}': ${msg}\n` +
        ...
        `[DB]   DATA_DIR=/path/to/writable/dir devilroute`
    );
  }
}
...
declare global {
  var __devilrouteDb: SqliteAdapter | undefined;
  ...
  var __devilrouteDbProbeRestoreCount: number | undefined;
  ...
  var __devilrouteDbOomFailureCount: number | undefined;
}

function getDb(): SqliteDatabase | null {
  return globalThis.__devilrouteDb ?? null;
}

function setDb(db: SqliteDatabase | null): void {
  if (db) {
    globalThis.__devilrouteDb = db;
  } else {
...
function shouldRunStartupDbHealthCheck(): boolean {
  if (process.env.DEVILROUTE_FORCE_DB_HEALTHCHECK === "1") return true;
...
function getDbHealthCheckIntervalMs(): number {
  const rawValue = process.env.DEVILROUTE_DB_HEALTHCHECK_INTERVAL_MS;
...
function startDbHealthCheckScheduler(db: SqliteDatabase) {
  ...
  dbHealthCheckTimer = setInterval(() => {
    try {
      ...
      runDbHealthCheck(db, {
        autoRepair: true,
        skipIntegrityCheck: process.env.DEVILROUTE_SKIP_DB_HEALTHCHECK === "1",
...
export function getDbInstance(): SqliteDatabase {
  ...
  if (!fs.existsSync(sqliteFile) && probeFailureBackups.length > 0) {
    ...
    if (
      (globalThis.__devilrouteDbProbeRestoreCount =
        (globalThis.__devilrouteDbProbeRestoreCount || 0) + 1) > 3
    ) {
      throw new Error(
  ...
  if (fs.existsSync(sqliteFile)) {
    try {
      ...
      if (
        ...
        if (
          (globalThis.__devilrouteDbOomFailureCount =
            (globalThis.__devilrouteDbOomFailureCount || 0) + 1) > 3
        ) {
          throw new Error(
  ...
  db.exec(`
    ...
    INSERT OR IGNORE INTO _devilroute_migrations (version, name)
  ...
  if (shouldRunStartupDbHealthCheck()) {
    const skipIntegrityCheck = process.env.DEVILROUTE_SKIP_DB_HEALTHCHECK === "1";
    if (skipIntegrityCheck) {
      console.log("[DB] Health check skipped (DEVILROUTE_SKIP_DB_HEALTHCHECK=1)");
    }
    runDbHealthCheck(db, {
      autoRepair: true,
      expectedSchemaVersion: "1",
 ...
 * can place it in ~/.devilroute/runtime/ without blocking a synchronous caller.
 *
