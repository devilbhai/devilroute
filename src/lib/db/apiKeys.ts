...
import { resolveModelAlias } from "@devilroute/open-sse/services/modelDeprecation.ts";
...
function isRedisAuthCacheEnabled(): boolean {
  return (
    process.env.DEVILROUTE_DISABLE_REDIS_AUTH_CACHE !== "1" &&
...
/**
 * Select an API key for internal DevilRoute operations (combo health checks,
...
export async function getApiKeyMetadata(
  ...
  if (isConfiguredEnvApiKey(key)) {
    ...
    // The deployment-time env key (`DEVILROUTE_API_KEY` / `ROUTER_API_KEY`)
