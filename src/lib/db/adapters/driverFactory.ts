...
declare global {
  var __devilrouteSqlJsAdapters: Map<string, SqliteAdapter> | undefined;
  var __devilrouteSqlJsInitPromises: Map<string, Promise<SqliteAdapter>> | undefined;
  var __devilrouteSqlJsPreInitErrors: Map<string, string> | undefined;
}

function getSqlJsCache(): Map<string, SqliteAdapter> {
  if (!globalThis.__devilrouteSqlJsAdapters) {
    globalThis.__devilrouteSqlJsAdapters = new Map();
  }
  return globalThis.__devilrouteSqlJsAdapters;
}

function getSqlJsPreInitErrorCache(): Map<string, string> {
  if (!globalThis.__devilrouteSqlJsPreInitErrors) {
    globalThis.__devilrouteSqlJsPreInitErrors = new Map();
  }
  return globalThis.__devilrouteSqlJsPreInitErrors;
}
...
function getSqlJsPendingCache(): Map<string, Promise<SqliteAdapter>> {
  if (!globalThis.__devilrouteSqlJsInitPromises) {
    globalThis.__devilrouteSqlJsInitPromises = new Map();
  }
  return globalThis.__devilrouteSqlJsInitPromises;
}
