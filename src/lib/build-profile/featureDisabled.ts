/**
 * Helper used by the `DEVILROUTE_BUILD_PROFILE=minimal` stubs to surface a
...
export class FeatureDisabledError extends Error {
  ...
  constructor(featureName: string) {
    super(
      `Feature "${featureName}" is disabled in this build (DEVILROUTE_BUILD_PROFILE=minimal). ` +
        `Install the full devilroute artifact instead of devilroute-secure if you need this feature.`
    );
