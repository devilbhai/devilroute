...
import { withRateLimit } from "@devilroute/open-sse/services/rateLimitManager";

const INTERNAL_ORIGIN = "http://devilroute.internal";
...
export function buildInternalChatRequest(
  ...
  return new Request(`${INTERNAL_ORIGIN}/v1/chat/completions`, {
    ...
    headers: {
      ...
      "X-DevilRoute-Compression": "off",
      "X-Request-Id": `model-test-${randomUUID()}`,
      ...(connectionId ? { "X-DevilRoute-Connection": connectionId } : {}),
    },
...
export async function runSingleModelTest(
  ...
  const testBody = isRerank
    ? {
        model: fullModelStr,
        query: "What is DevilRoute?",
        documents: [
          "DevilRoute routes AI requests across configured providers.",
