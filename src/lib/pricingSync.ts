...
// ─── Provider mapping: LiteLLM provider → DevilRoute aliases ─────
...
/**
 * Transform LiteLLM raw data → DevilRoute PricingByProvider format.
 *
 * Conversion: cost_per_token × 1_000_000 → $/1M tokens (DevilRoute format).
 ...
 */
export function transformToDevilRoute(raw: Record<string, LiteLLMModelInfo>): PricingByProvider {
  ...
  try {
    ...
    for (const source of validSources) {
      if (source === "litellm") {
        ...
        const transformed = transformToDevilRoute(raw);
