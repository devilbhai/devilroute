...
import { splitClaudeEffortSuffix } from "@devilroute/open-sse/config/providerModels.ts";
...
export function attachReasoningRuleDirective(
  ...
  body._devilrouteReasoningRouteTrace = {
...
export function applyReasoningRuleDirective(bodyInput: unknown): unknown {
  const source = asRecord(bodyInput);
  const directive = asRecord(source._devilrouteReasoningRule);
  if (!directive.id) return bodyInput;
  const body = { ...source };
  delete body._devilrouteReasoningRule;
