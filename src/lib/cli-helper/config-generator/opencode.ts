 ...
 * source is the user's OWN DevilRoute instance, so loopback/private hosts are the
...
/**
 * Fetch the live `/v1/models` catalog from DevilRoute. The catalog is the
 ...
 */
export async function fetchDevilRouteCatalog(
  baseUrl: string,
  apiKey: string,
  timeoutMs = 5_000
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error(
        `DevilRoute /v1/models returned ${response.status} ${response.statusText}`
      );
    }
...
/**
 * Generate a full `opencode.json` document for DevilRoute. The catalog is the
...
export async function generateOpencodeConfig(
  ...
  const providerId = options.providerId?.trim() || "devilroute";
  ...
  if (fetchCatalog) {
    const result = await fetchDevilRouteCatalog(baseURL, options.apiKey, timeoutMs);
  ...
  const provider: Record<string, unknown> = {
    name: existingProvider?.name ?? "DevilRoute",
