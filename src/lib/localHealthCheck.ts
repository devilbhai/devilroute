...
function getLHCState() {
  if (!globalThis.__devilrouteLocalHC) {
    globalThis.__devilrouteLocalHC = {
      initialized: false,
      sweepTimer: null,
      healthCache: new Map(),
  ...
  return globalThis.__devilrouteLocalHC;
}
...
function isLocalHealthCheckDisabled(): boolean {
  return (
    isEnvFlagEnabled("DEVILROUTE_DISABLE_LOCAL_HEALTHCHECK") ||
    isBuildProcess() ||
