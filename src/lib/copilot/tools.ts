/**
 * DevilRoute Copilot — Tool definitions
 *
 * Tools the copilot can execute to configure DevilRoute on behalf of the user,
...
// ── Helper: check if devilroute CLI is available ──────────────────────────────

function getDevilRouteCliPath(): string | null {
  try {
    const result = execSync("which devilroute 2>/dev/null || command -v devilroute 2>/dev/null", {
      encoding: "utf-8",
      timeout: 3000,
    }).trim();
    return result || null;
...
export const COPILOT_TOOLS: CopilotTool[] = [
  ...
  // ── CodeGraph Tools ──
  {
    ...
    description:
      "Search for symbols in the DevilRoute codebase by name (functions, classes, types, variables). Use this to understand how the app works internally.",
    parameters: [
      {
        name: "query",
        type: "string",
  ...
  },
  {
    name: "findCallees",
    description:
      "Find all functions/symbols that a specific function calls. Useful for understanding dependencies and code flow within DevilRoute.",
    parameters: [
      {
        name: "symbol",
        type: "string",
  ...
  // ── CLI Execution Tool ──
  {
    name: "runDevilRouteCli",
    description:
      "Execute an 'devilroute' CLI command to configure or query the DevilRoute app. Gives complete control over the app — use for advanced operations not covered by other tools. Common commands: devilroute list-keys, devilroute switch-combo [id], devilroute set-budget 10, devilroute set-strategy [id] priority, devilroute health, devilroute mcp (starts MCP server), devilroute db-health, devilroute reset-password.",
    parameters: [
      {
        name: "command",
        type: "string",
        description:
          "CLI command arguments (everything after 'devilroute'). Example: 'list-keys', 'switch-combo abc123', 'health'",
        required: true,
      },
    ],
    handler: async (args) => {
      ...
      const cliPath = getDevilRouteCliPath();
      if (!cliPath) return "devilroute CLI not found in PATH. Install DevilRoute first.";
