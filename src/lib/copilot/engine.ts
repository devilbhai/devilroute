/**
 * DevilRoute Copilot — Chat Engine
 *
...
function getKnowledgeResponse(query: string): string | null {
  ...
  if (
    ...
    return `## DevilRoute Architecture
...
Use \`createCombo\` tool or \`runDevilRouteCli\` to create them.`;
  }
...
DevilRoute supports 212+ providers across categories:
...
I can use CodeGraph to explore the DevilRoute codebase. Just ask me:
...
function getHelpResponse(): string {
  return `## DevilRoute Copilot — Comandos disponibles
...
### CLI
- "CLI health" → ejecuta \`devilroute health\`
- "CLI list-combos" → ejecuta \`devilroute list-combos\`
- "CLI set-budget 10" → ejecuta \`devilroute set-budget 10\`

### Conocimiento
- "Cómo funciona DevilRoute?" → explica la arquitectura
...
export async function processCopilotChat(request: CopilotRequest): Promise<CopilotResponse> {
  ...
  if (!intent) {
    ...
    return {
      message: `I understand you want help with DevilRoute.\n\n${getHelpResponse()}`,
    };
  }
...
> Crea un combo llamado "fallback-claude" con estrategia priority y targets: [{"provider":"claude-code","model":"claude-sonnet-4"},{"provider":"openai","model":"gpt-4o"}]`,
  ...
  if (intent.tool === "runDevilRouteCli") {
    ...
    return {
      message: result,
      toolCalls: [{ name: "runDevilRouteCli", args: intent.args, result }],
    };
  }
