...
function buildApiBody(skill: AgentSkill, sources: BuildSources): string {
  ...
  if (ops.length === 0) {
    ...
    for (const op of ops) {
      ...
      lines.push('  -H "Authorization: Bearer $DEVILROUTE_TOKEN"');
...
function buildCliBody(skill: AgentSkill, sources: BuildSources): string {
  ...
  lines.push("```bash");
  lines.push("npm install -g devilroute   # or: npx devilroute");
  lines.push("devilroute --version");
  lines.push("```");
  lines.push("");
  ...
  if (cmds.length === 0) {
    ...
    for (const cmd of cmds) {
      ...
      lines.push("**Example:**\n");
      lines.push("```bash");
      lines.push(`devilroute ${cmd.name}`);
      lines.push("```");
      lines.push("");
    }
  }
