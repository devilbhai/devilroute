...
import { isObsoleteKiroModelAlias } from "@devilroute/open-sse/services/kiroModels.ts";
...
// expose (as opposed to the flat `supportedThinkingEfforts` field DevilRoute's own
...
// Maps common upstream synonyms onto DevilRoute's canonical effort vocabulary
...
export function normalizeDiscoveredModels(models: unknown): SyncedAvailableModel[] {
  ...
  for (const item of items) {
    ...
    deduped.set(id, {
      ...
      ...(() => {
        // #7694/#8347: the flat field (DevilRoute's own import format) wins verbatim when
