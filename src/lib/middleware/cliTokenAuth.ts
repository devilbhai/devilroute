...
const HEADER_NAME = "x-devilroute-cli-token";
...
/**
 * Validates the CLI machine-id token sent by the local devilroute CLI.
 * Only accepted from loopback IPs. Disabled via DEVILROUTE_DISABLE_CLI_TOKEN=true.
 */
export async function isCliTokenAuthValid(request: Request): Promise<boolean> {
  if (process.env.DEVILROUTE_DISABLE_CLI_TOKEN === "true") return false;
