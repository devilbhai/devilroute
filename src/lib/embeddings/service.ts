import { handleEmbedding } from "@devilroute/open-sse/handlers/embeddings.ts";
import {
  parseEmbeddingModel,
...
} from "@devilroute/open-sse/config/embeddingRegistry.ts";
import { errorResponse, unavailableResponse } from "@devilroute/open-sse/utils/error.ts";
...
import { runWithProxyContext } from "@devilroute/open-sse/utils/proxyFetch.ts";
import { handleComboChat } from "@devilroute/open-sse/services/combo.ts";
...
import { attachDevilRouteMetaHeaders } from "@/domain/devilrouteResponseMeta";
...
export async function createEmbeddingResponse(
  ...
  if (result.success) {
    ...
    attachDevilRouteMetaHeaders(responseHeaders, {
      provider,
