...
/**
 * Endpoint suportado pelo Playground Studio. Reflete os 13 endpoints da API DevilRoute
...
export const API_KEY_PLACEHOLDER = "$DEVILROUTE_API_KEY";
...
function buildCurlSnippet(state: PlaygroundState): string {
  ...
  const lines: string[] = [
    `# Set your API key: export DEVILROUTE_API_KEY="your-key-here"`,
    `curl -s -X POST \\`,
    `  "${url}" \\`,
...
function buildPythonSnippet(state: PlaygroundState): string {
  ...
  const lines: string[] = [
    ...
    `import requests`,
    ``,
    `api_key = os.environ["DEVILROUTE_API_KEY"]`,
    ``,
    `url = "${url}"`,
    `headers = {`,
