...
/**
 * Resolve the real install directory of the globally-installed `devilroute` package — the directory
 * that owns `node_modules/better-sqlite3` and so is the correct cwd for `npm rebuild`.
 *
 * Replaces the hardcoded `${globalRoot}/devilroute/app` assumption (Bug 3, security-report v3.8.15):
 * the global package root is `${npm root -g}/devilroute`, not `/devilroute/app`. We probe the real
...
export async function resolveGlobalOmniroutePath(
  ...
  const packageRoot = path.join(globalRoot, "devilroute");
