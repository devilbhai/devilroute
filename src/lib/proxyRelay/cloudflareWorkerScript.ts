/**
 * Cloudflare Worker source emitter for the DevilRoute proxy relay.
 *
 ...
 *
 * The DevilRoute variant intentionally diverges from the upstream PR:
...
export function buildCloudflareWorkerUploadRequest(
  ...
  const boundary = `----DevilRouteCFWorker${randomUUID().replace(/-/g, "")}`;
  const CRLF = "\r\n";
  const parts: Buffer[] = [
    Buffer.from(
      `--${boundary}${CRLF}` +
...
export function buildCloudflareWorkerScript(relayAuth: string): string {
  ...
  return `// DevilRoute Cloudflare Worker proxy relay — generated at deploy time.
