...
import { parseModel, resolveCanonicalProviderModel } from "@devilroute/open-sse/services/model.ts";
import {
  MODEL_SPECS,
...
import { getUnsupportedParams } from "@devilroute/open-sse/config/providerRegistry.ts";
import {
  getLearnedThinkingCap,
  GEMINI_FALLBACK_THINKING_CAP,
} from "@devilroute/open-sse/services/learnedThinkingCaps.ts";
...
function reverseModelsDevProviders(provider: string): string[] {
  // models.dev may store capabilities under a different DevilRoute provider id
