 ...
 * Priority: system daemon socket → DevilRoute custom socket.
...
async function getActiveSocketPath(): Promise<string> {
  ...
  // Fallback to DevilRoute custom socket
...
async function getDefaultHostname() {
  try {
    ...
    const normalized = `devilroute-${machineId.slice(0, 8)}`.replace(/[^a-zA-Z0-9-]/g, "-");
