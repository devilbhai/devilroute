...
import { cooldownUntilMs } from "@devilroute/open-sse/services/accountFallback.ts";
...
declare global {
  var __devilrouteConnRecovery:
    | { initialized: boolean; interval: ReturnType<typeof setInterval> | null }
    | undefined;
}

function getRecoveryState() {
  if (!globalThis.__devilrouteConnRecovery) {
    globalThis.__devilrouteConnRecovery = { initialized: false, interval: null };
  }
  return globalThis.__devilrouteConnRecovery;
}
...
function isRecoverySchedulerDisabled(): boolean {
  return (
    isEnvFlagEnabled("DEVILROUTE_DISABLE_CONNECTION_RECOVERY") ||
    isEnvFlagEnabled("DEVILROUTE_DISABLE_BACKGROUND_SERVICES") ||
    isBuildProcess() ||
...
/**
 * Resolve the tick interval (ms) from DEVILROUTE_CONNECTION_RECOVERY_INTERVAL_MS,
...
export function resolveConnectionRecoveryIntervalMs(
  rawValue: string | undefined = typeof process !== "undefined"
    ? process.env.DEVILROUTE_CONNECTION_RECOVERY_INTERVAL_MS
    : undefined
): number {
