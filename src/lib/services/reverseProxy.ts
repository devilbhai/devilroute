 ...
 * can be iframed inside the DevilRoute dashboard without CORS issues.
 *
 * Security:
 ...
 *     to prevent credential leakage between DevilRoute and the embedded service.
 ...
 * Prevents DevilRoute session cookies and Authorization from leaking upstream.
 */
 ...
 *   DevilRoute origin, which would conflict with session management.
 * - content-security-policy / content-security-policy-report-only: the
 *   embedded service's CSP is irrelevant inside the DevilRoute iframe.
 * - x-frame-options: would block the iframe entirely if set to DENY/SAMEORIGIN
 *   by the embedded service (DevilRoute controls framing via its own CSP).
