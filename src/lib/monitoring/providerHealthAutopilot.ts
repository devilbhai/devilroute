...
import { clearProviderFailure, clearModelLock } from "@devilroute/open-sse/services/accountFallback";
...
export async function buildProviderHealthAutopilotReport(
  ...
  const [{ getAllCircuitBreakerStatuses }, { getAllModelLockouts }, quotaMonitor] =
    await Promise.all([
      ...
      import("@devilroute/open-sse/services/quotaMonitor.ts").catch(() => null),
    ]);
