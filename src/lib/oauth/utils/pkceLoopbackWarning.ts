 ...
 * browser, not on whatever host serves the DevilRoute dashboard.
 *
 ...
 *   DevilRoute *server's* loopback while the provider redirects the *browser's* loopback.
 */
...
export function buildPkceLoopbackMismatchWarning(provider: string): string {
  ...
  return (
    `DevilRoute is being accessed from a LAN IP, not true localhost. ${provider}'s OAuth app ` +
    `is registered with a fixed loopback redirect (${redirect}) that only resolves on the ` +
    "machine running this browser tab, not on the DevilRoute server — the login will silently " +
    "fail on the provider's side. Open the DevilRoute dashboard from true localhost instead " +
    "(SSH port-forward: ssh -L <port>:127.0.0.1:<port> <user>@<devilroute-host>, then browse to " +
