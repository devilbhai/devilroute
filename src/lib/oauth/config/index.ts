 ...
 * with the running DevilRoute server when saving tokens.
 */
...
export function getServerCredentials(): ServerCredentials {
  return {
    server: process.env.DEVILROUTE_SERVER || process.env.SERVER_URL || getDefaultApiServer(),
    token: process.env.DEVILROUTE_TOKEN || process.env.CLI_TOKEN || "",
    userId: process.env.DEVILROUTE_USER_ID || process.env.CLI_USER_ID || "cli",
  };
}
