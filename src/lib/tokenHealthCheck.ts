...
import {
  ...
  refreshCopilotToken,
} from "@devilroute/open-sse/services/tokenRefresh.ts";
...
function isHealthCheckDisabled(): boolean {
  return (
    isEnvFlagEnabled("DEVILROUTE_DISABLE_TOKEN_HEALTHCHECK") ||
    isBuildProcess() ||
 ...
 * (e.g. "codex,openai"). Targeted alternative to DEVILROUTE_DISABLE_TOKEN_HEALTHCHECK:
...
function getHealthCheckSkipProviders(): Set<string> {
  const raw = process.env.DEVILROUTE_HEALTHCHECK_SKIP_PROVIDERS || "";
  return new Set(
    raw
      .split(",")
...
async function shouldHideLogs(): Promise<boolean> {
  if (
    isEnvFlagEnabled("DEVILROUTE_HIDE_HEALTHCHECK_LOGS") ||
    isBuildProcess() ||
    isAutomatedTestProcess()
  ) {
    return true;
...
declare global {
  var __devilrouteTokenHC:
    | { initialized: boolean; interval: ReturnType<typeof setInterval> | null; sweeping: boolean }
    | undefined;
}
function getHCState() {
  if (!globalThis.__devilrouteTokenHC) {
    globalThis.__devilrouteTokenHC = { initialized: false, interval: null, sweeping: false };
  }
  return globalThis.__devilrouteTokenHC;
}
...
export async function checkConnection(conn) {
  ...
  // Codex). 9router did not have this background sweep; it was introduced in DevilRoute
