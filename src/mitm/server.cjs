...
const ROUTER_BASE_URL = (
  process.env.DEVILROUTE_BASE_URL ||
...
// Inspector. The token is injected by manager.ts (same value the DevilRoute
 ...
 * shape. The route-only namespace is reserved for client-facing DevilRoute model ids;
...
async function passthrough(req, res, bodyBuffer) {
  ...
  // Defense-in-depth loop guard (Gap 14). The x-devilroute-source header is the
...
async function intercept(req, res, bodyBuffer, override, sourceModel) {
  ...
  // The DevilRoute router uses these to distinguish AgentBridge traffic from
  ...
  try {
    ...
    const response = await fetch(forward.url, {
      ...
      headers: {
        ...
        "x-devilroute-source": "agent-bridge",
        "x-devilroute-agent": agentId,
      },
    ...
    if (!response.ok) {
      ...
      throw new Error(`DevilRoute ${response.status}: ${errText}`);
    }
...
async function startMitmServer() {
  ...
  const server = https.createServer(sslOptions, async (req, res) => {
    ...
    if (req.headers["x-devilroute-source"] === "devilroute") {
      vlog(1, `[MITM] → PASSTHROUGH (DevilRoute source loop)`);
