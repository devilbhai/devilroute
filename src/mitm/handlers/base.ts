 ...
 *   - forwarding to the DevilRoute router (Next.js API)
 *   - SSE piping
 ...
 * Routes through `@devilroute/open-sse/utils/error.sanitizeErrorMessage` (Hard Rule #12)
...
async function safeErrorMessage(err: unknown): Promise<string> {
  try {
    const mod = (await import("@devilroute/open-sse/utils/error")) as {
...
export abstract class MitmHandlerBase {
   ...
   *   3. Call `this.fetchRouter(...)` for the DevilRoute router round-trip.
  ...
  /**
   * Forward the prepared body to the DevilRoute router (Next.js API).
   * Adds AgentBridge correlation headers (`x-devilroute-source`, `x-devilroute-agent`)
  ...
  protected async fetchRouter(
    ...
    const base = process.env.DEVILROUTE_BASE_URL ?? "http://127.0.0.1:20128";
    ...
    return fetch(url, {
      ...
      headers: {
        ...
        "x-devilroute-agent": this.agentId,
