 ...
 * `thinkingConfig`, …). The DevilRoute router endpoint `/v1/chat/completions`
 ...
 *   - forward to `/v1/chat/completions` on the DevilRoute router,
 ...
 * @param model      resolved DevilRoute model string
  ...
  async intercept(
    ...
    try {
      ...
      if (!upstream.ok) {
        ...
        throw new Error(`DevilRoute ${upstream.status}: ${errText}`);
      }
