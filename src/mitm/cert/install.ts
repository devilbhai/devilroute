...
const LINUX_CERT_NAME = "devilroute-mitm.crt";
  ...
  return new Promise((resolve) => {
    exec(
      script,
      {
        ...
        env: {
          ...process.env,
          CERT_NAME: "DevilRoute MITM Root CA",
          CERT_PATH: certPath || "",
          ACTION: action,
        },
      },
...
export async function installCert(sudoPassword: string, certPath: string): Promise<void> {
  ...
  if (process.env.DEVILROUTE_SKIP_SYSTEM_TRUST === "1") {
    console.log("[cert] DEVILROUTE_SKIP_SYSTEM_TRUST=1 — skipping OS trust-store mutation");
    return;
  }
 ...
 * cert-path-agnostic and keep writing to the same `devilroute-mitm.crt`
 ...
 * cleanup needed. Distinct from TPROXY's own `devilroute-tproxy-ca.crt` slot
...
// They install / remove the DevilRoute MITM root CA from the OS trust store and
