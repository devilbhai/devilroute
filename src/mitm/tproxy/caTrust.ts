 ...
 * trust store under a DEDICATED slot (`devilroute-tproxy-ca.crt`), separate from
 * the static MITM cert (`devilroute-mitm.crt` in `cert/install.ts`), so the two
...
export const TPROXY_CA_CERT_NAME = "devilroute-tproxy-ca.crt";
...
export async function installTproxyCa(
  ...
  if (process.env.DEVILROUTE_SKIP_SYSTEM_TRUST === "1" && deps.run === undefined) {
    console.log("[tproxy-ca] DEVILROUTE_SKIP_SYSTEM_TRUST=1 — skipping OS trust-store mutation");
    return;
  }
