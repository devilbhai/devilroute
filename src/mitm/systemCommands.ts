 ...
 *
 * Slim Docker images (e.g. `node:24-trixie-slim` used by DevilRoute's runtime
...
/**
 * Truthy-env check for `DEVILROUTE_NO_SUDO`. Inlined (not imported from
...
export function isNoSudoEnv(): boolean {
  const value = process.env.DEVILROUTE_NO_SUDO;
 ...
 * the operator opts into root-less mode via `DEVILROUTE_NO_SUDO` (#6122). No
...
export function execFileWithPassword(
  ...
  // `DEVILROUTE_NO_SUDO` (root-less / user-namespace deployments — #6122), skip
...
export async function runElevatedPowerShell(script: string): Promise<string> {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "devilroute-elevate-"));
  const scriptName = `devilroute-elevate-${crypto.randomUUID()}.ps1`;
