...
⭐ Star the repo if DEVILROUTE helped you save money and make your work easier.
...
<table>
  ...
  <tr>
    ...
    <td align="center"><a href="#-why-devilroute">🤔 Why DevilRoute</a></td>
    <td align="center"><a href="#-what-sets-devilroute-apart">🏆 What Sets Apart</a></td>
  </tr>
  <tr>
  ...
  <tr>
    ...
    <td align="center"><a href="#%EF%B8%8F-where-devilroute-runs--anywhere">🖥️ Where It Runs</a></td>
    <td align="center"><a href="#-private--local-first">🔒 Private</a></td>
  </tr>
  <tr>
    <td align="right"><b>👀 See it</b></td>
    <td align="center"><a href="#-devilroute-in-action">🎬 In Action</a></td>
  ...
  <tr>
    ...
    <td align="center"><a href="#-support-devilroute">💚 Support / Donate</a></td>
...
<sub>Prefer a specific free backend? Call it directly, e.g. `oc/…` (OpenCode Free) or `felo/…` (Felo). Then graduate to `auto` and let DevilRoute pick.</sub>
...
> **Want to join as an Open Source Friend?** These are the companies that back open source and help keep DevilRoute moving — and we say publicly where every token they give us goes. Reach out: [diegosouza.pw@outlook.com](mailto:diegosouza.pw@outlook.com)
...
<table>
  <tr>
    ...
    <td>
      ...
      <br/><br/>
      <b>What Kimi's support powers:</b> Kimi's API credits power DevilRoute's AI-validated release pipeline — the <i>merge validation powered by Kimi K3</i> stage that reviews every pull request before it ships — plus day-to-day feature development. First-class Kimi support ships on both rails: the direct <a href="https://platform.kimi.ai?aff=devilroute">Kimi API</a> (<code>kimi-k3</code>) and the <a href="https://www.kimi.com/code?aff=devilroute">Kimi Code coding plan</a> (OAuth and API key). DevilRoute is also the first Brazilian open-source project in Kimi's support program. <a href="https://platform.kimi.ai?aff=devilroute"><b>Get a Kimi API key →</b></a>
    </td>
  </tr>
  <tr>
    <td align="center" width="150">
    ...
    <td>
      ...
      <br/><br/>
      <b>First-class support in DevilRoute:</b> Chat Completions, the native <code>/v1/responses</code> endpoint, vision, tool calling and 3 image models (<code>grok-imagine</code>, <code>nano-banana-pro</code>, <code>nano-banana-2</code>, reachable as <code>cheaperinference/&lt;model&gt;</code>). <a href="https://cheaperinference.com/?utm_source=devilroute"><b>Get an API key →</b></a>
    </td>
  </tr>
</table>

<sub>Links tagged <code>aff=devilroute</code> are partner links. They fund the project at no extra cost to you.</sub>
...
<sub><i>This section is for referral/coupon codes only. Sponsored partnerships live in <b>🤝 Supported by our Open Source Friends</b> above. DevilRoute has no sponsorship or partnership with the providers listed here — these are public coupons anyone can use.</i></sub>
...
<table>
  <tr>
    ...
    <td>
      <sub><b><a href="https://agentrouter.org/register?aff=70LM">AgentRouter</a></b> — affiliate signup · <b>$100 free credits</b> on signup (free server, expect higher latency — best for testing, not production). First-class support in DevilRoute since <b>v3.8.50</b>: Chat Completions, the Anthropic-compatible wire format and the OpenAI-compatible path. Available models include <code>claude-opus-4-8</code>, <code>claude-opus-5</code>, <code>gpt-5.6-sol</code> and more. <b><a href="https://agentrouter.org/register?aff=70LM">Grab your $100 →</a></b></sub>
      <br/><br/>
      <sub>⚠️ <i>Affiliate link — DevilRoute has no sponsorship or partnership with this provider.</i></sub>
    </td>
  </tr>
</table>

<sub>Know another provider with a generous free signup coupon that benefits DevilRoute users? Open an issue and we'll add it here.</sub>
...
No combo to create. Set your model to `auto` (or a variant) and DevilRoute builds a virtual combo from your connected providers, scored live:
...
```
00020101021126580014br.gov.bcb.pix01365d865059-bc44-483a-962d-43ceb80126eb5204000053039865802BR5922DEVILROUTE CONTRIBUICAO6006BRASIL62070503***630475DD
```
...
- **🤖 One-command CLI/agent setup** — `setup-*` configures 12+ coding tools; `devilroute launch` / `launch-codex` are zero-config. → [CLI Integrations](docs/guides/CLI-INTEGRATIONS.md)
- **🛰️ Remote mode** — drive a remote DevilRoute with scoped tokens (`connect` / `contexts` / `tokens`) + an `antigravity` OAuth helper for VPS installs. → [Remote Mode](docs/guides/REMOTE-MODE.md)
...
- **💸 Cost telemetry everywhere** — `X-DevilRoute-*` cost/usage headers on every endpoint, cache-HIT savings header, per-key USD spend quotas. → [API Reference](docs/reference/API_REFERENCE.md)
- **🧠 Memory you control** — off by default, opt-in int8 vector quantization + typed decay, per-request `x-devilroute-no-memory`. → [Memory](docs/frameworks/MEMORY.md)
...
- **📡 Routing transparency** — every response carries an `X-DevilRoute-Decision` header naming the strategy/provider/latency that served it, a new `cache-optimized` combo strategy + Auto-Combo `cacheAffinity` factor route repeat requests back to the connection holding the cached prefix, and a read-only `/v1/auto-combo/{channel}/candidates` endpoint exposes an `auto/*` channel's live candidate pool. → [Auto-Combo](docs/routing/AUTO-COMBO.md)
...
<sub>📖 Per-tool setup for all 33 tools (25 CLI Code's + 8 CLI Agents) → [`docs/reference/CLI-TOOLS.md`](docs/reference/CLI-TOOLS.md) · 🧩 OpenCode plugin → [`@devilroute/opencode-provider`](https://www.npmjs.com/package/@devilroute/opencode-provider)</sub>
...
## 🖥️ Where DevilRoute Runs — Anywhere
...
<table>
  ...
  <tr><td align="left" nowrap>🧩 <b>OpenCode plugin</b></td><td align="left" nowrap><code>@devilroute/opencode-provider</code></td><td align="left">Native OpenCode integration</td></tr>
...
> Beyond the server, DevilRoute is a **full command-line cockpit** with **80+ commands**, plus open agent protocols so an AI agent can drive it **on its own**.

### ⌨️ A real CLI (not just `start`)

```bash
devilroute               # serve gateway + dashboard (port 20128)
devilroute chat          # interactive TUI chat client (slash: /model /combo /skill /memory)
devilroute setup         # guided first-run wizard
devilroute doctor        # diagnose providers, ports, native deps
```

### 🛰️ Remote mode — run the CLI here, DevilRoute on a VPS

DevilRoute on a server? Drive it from your laptop with the **same CLI**. Log in once
with a scoped access token; every command then targets the remote.

```bash
devilroute connect 192.168.0.15            # password → scoped token, saved as a context
devilroute models list                     # ← runs against the REMOTE server
devilroute configure codex                 # ← picks a remote model, writes a local Codex profile
devilroute tokens create --name ci --scope read   # mint narrower tokens for other machines
devilroute contexts use default            # ← switch back to the local server
```
...
### 🤝 Connect an agent — and it controls DevilRoute itself

Expose DevilRoute over **MCP**, **A2A**, a **REST API**, **webhooks** or a **remote CLI** — any capable agent (or your own code) gets the keys to the whole gateway: routing, providers, combos, cache, compression, memory — autonomously. HTTP endpoints below are served under `http://localhost:20128`.

<table>
  <tr><th align="left">Interface</th><th align="left">Endpoint / command</th><th align="left">Use it for</th></tr>
  <tr><td align="left" nowrap>🧰 <b>MCP (stdio)</b></td><td align="left" nowrap><code>devilroute --mcp</code></td><td align="left">Plug into Claude Desktop, Cursor, any MCP client</td></tr>
  ...
  <tr><td align="left" nowrap>🛰️ <b>Remote CLI</b></td><td align="left" nowrap><code>devilroute connect <host></code></td><td align="left">Drive a remote instance with scoped access tokens</td></tr>
</table>

```bash
# Give Claude Code the full DevilRoute toolset over MCP:
...
> **Why use many tokens when few tokens do the trick?** Every request passes through DevilRoute's compression pipeline **transparently** — no client changes. It's now a **stack of 12 composable engines** that run in order and mix & match per routing combo — building on ideas from [RTK](https://github.com/rtk-ai/rtk), [Caveman](https://github.com/JuliusBrussee/caveman) (⭐ 90K+), [LLMLingua-2](https://github.com/microsoft/LLMLingua), and [Troglodita](https://github.com/leninejunior/troglodita) (PT-BR).
...
- **🎛️ Where compression is decided** _(precedence, high → low)_ — per-request `x-devilroute-compression` header › routing-combo override › active named profile › adaptive / auto-trigger › panel default › off. The applied plan echoes back in the `X-DevilRoute-Compression: <mode>; source=<source>` response header.
...
You should see your connected models listed. 🎉 That's it — start coding, and DevilRoute auto-routes & falls back for you.

If your client cannot send custom headers, DevilRoute also exposes tokenized compatibility aliases:
...
```bash
DEVILROUTE_SKIP_POSTINSTALL=1 npm install -g devilroute   # CI=1 also skips it
```
...
<table>
  ...
  <tr><td nowrap><b><a href="docs/guides/REMOTE-MODE.md">Remote Mode</a></b></td><td>Drive a remote DevilRoute (VPS) from your laptop CLI via scoped access tokens</td></tr>
  <tr><td nowrap><b><a href="docs/guides/CLAUDE-CODE-CONFIGURATION.md">Claude Code Config</a></b></td><td>Point Claude Code at DevilRoute (local/remote) with <code>launch</code> + per-model profiles</td></tr>
...
<table>
  ...
  <tr><td nowrap><b><a href="docs/guides/TERMUX_GUIDE.md">Termux Guide</a></b></td><td>Run DevilRoute on Android via Termux</td></tr>
...
> DevilRoute is shaped by a passionate open-source community. These individuals have made exceptional contributions that directly impact the quality, stability, and reach of the project. **Thank you.**
...
> 🙏 These contributors' features, bug fixes, and infrastructure improvements are a **core part** of what makes DevilRoute reliable and feature-rich. Every pull request, every test case, and every i18n translation file matters. Open source is built by people like them.
...
A heartfelt thank-you to the people who fund DevilRoute out of their own pocket — every contribution keeps the project free, independent and moving.
...
<table>
  <tr><th align="left">Project</th><th align="center">⭐</th><th align="left">How it inspired DevilRoute</th></tr>
...
**[⬆ Back to top](#-devilroute)** · Built with ❤️ for the open-source AI community.

<sub>DevilRoute v3.8.49 · Node ≥22.22.2 · MIT License · <a href="https://devilroute.online">devilroute.online</a></sub>
