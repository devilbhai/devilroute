 ...
 *   devilroute reset-password
 *
 * Non-interactive / scripted usage (piped stdin, e.g. CI or Docker):
 *   printf 'NewPass123\nNewPass123\n' | devilroute reset-password
 *   printf 'NewPass123' | devilroute reset-password --password-stdin
 *
 * Resets the admin password for DevilRoute.
...
async function main() {
  ...
  if (!passwordState.exists) {
    ...
    console.error(`   Make sure DevilRoute has been started at least once.`);
  ...
  console.log("   Restart DevilRoute for changes to take effect.\n");
}

main()
  .then(() => {
    // Explicit exit(0) so a caller that imports this module (bin/devilroute.mjs
    // routes `devilroute reset-password` here) terminates cleanly instead of
