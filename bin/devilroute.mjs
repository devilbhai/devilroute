...
// ahead of expensive self-heal hooks), adapted to DevilRoute's Commander CLI where the
...
// #7791: tsx's tsconfig-path resolution does not apply when DevilRoute is
// installed globally (files live under node_modules/devilroute/), so bare
...
function loadEnvFile() {
  ...
  if (process.env.DEVILROUTE_CLI_SKIP_REPO_ENV !== "1") {
...
// key or write ~/.devilroute/.env — running a read-only command should never
// mutate the data dir.
if (shouldProvisionStorageKey(process.argv)) {
  ...
  if (!process.env.STORAGE_ENCRYPTION_KEY) {
    ...
    // Writing only to ~/.devilroute (the container home, not a volume) silently lost the key on
    // container recreation, leaving the persisted encrypted DB undecryptable (regression of #1622).
    const dataDir = process.env.DATA_DIR || join(homedir(), ".devilroute");
    const envPath = join(dataDir, ".env");
...
// Apply --lang before Commander parses (program descriptions call t() during setup)
{
  ...
  const langEnv = process.env.DEVILROUTE_LANG;
...
process.on("exit", () => {
  if (process.env.DEVILROUTE_NO_UPDATE_NOTIFIER) return;
  ...
  if (_notifier.update) {
    _notifier.notify({
      ...
      message:
        ...
        "Run `npm install -g devilroute` or `devilroute update --apply`",
    });
  }
});
