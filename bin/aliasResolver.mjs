 ...
 *
 * Problem (#7791): when DevilRoute is installed via `npm i -g devilroute`, the
 * package files live under `node_modules/devilroute/`. tsx's tsconfig-path
 * resolution does not apply there, so specifiers like `@/shared/utils/featureFlags`
 * (declared in tsconfig.json `paths` as `@/* → ./src/*`) or
 * `@devilroute/open-sse/services/usage` fail with `ERR_MODULE_NOT_FOUND`.
 ...
 *
 * Each entry:
 *   prefix  — specifier prefix to match (e.g. `"@/"`, `"@devilroute/open-sse/"`)
 ...
 *             prefix *without* a trailing slash (e.g. `@devilroute/open-sse` →
...
export const ALIAS_MAP = [
  ...
  { prefix: "@devilroute/open-sse/", target: "open-sse", exact: false },
  { prefix: "@devilroute/open-sse", target: "open-sse", exact: true },
];
 ...
 *   "@/..."                     → <root>/src/...
 *   "@devilroute/open-sse/..."   → <root>/open-sse/...
 *   "@devilroute/open-sse"       → <root>/open-sse/index.*
 *
...
export function resolveAlias(specifier, root) {
  ...
  // Exact match (e.g. `@devilroute/open-sse` with no trailing path) →
