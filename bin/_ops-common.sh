# bin/_ops-common.sh — shared helpers for the DevilRoute ops runbook scripts.
#
...
ops_set_data_dir() {
  DEVILROUTE_DATA_DIR="$1"
  DEVILROUTE_SQLITE="${DEVILROUTE_DATA_DIR}/storage.sqlite"
  DEVILROUTE_BACKUPS_DIR="${DB_BACKUPS_DIR:-${DEVILROUTE_DATA_DIR}/db_backups}"
}
ops_set_data_dir "${DATA_DIR:-$HOME/.devilroute}"
...
ops_find_snapshot() {
  ...
  for cand in \
    "$id" \
    "$id/" \
    "$DEVILROUTE_BACKUPS_DIR/$id" \
    "$DEVILROUTE_BACKUPS_DIR/snapshot_$id"; do
  ...
  if [ -d "$DEVILROUTE_BACKUPS_DIR" ]; then
    for cand in "$DEVILROUTE_BACKUPS_DIR"/snapshot_*"$id"*; do
      [ -f "$cand/storage.sqlite" ] && { printf '%s\n' "$cand"; return 0; }
    done
  fi
  ops_die "no snapshot matching '$id' under $DEVILROUTE_BACKUPS_DIR (run bin/snapshot-data.sh first)"
}
