...
const RUNTIME_DIR = join(homedir(), ".devilroute", "runtime");
...
export async function loadSqliteRuntime() {
  ...
  try {
    ...
    console.warn(`[devilroute] runtime install failed: ${err.message}`);
  }
  ...
  if (existsSync(buildDir)) {
    ...
    if (nodeFile) {
      ...
      if (!magic || (magic !== expected && magic !== "macho-le" && magic !== "macho-fat")) {
        console.warn(
          `[devilroute] runtime sqlite binary magic mismatch (${magic} ≠ ${expected}) — skipping`
        );
        return null;
      }
    }
...
function ensureRuntimeDir() {
  ...
  if (!existsSync(pkg)) {
    writeFileSync(
      pkg,
      JSON.stringify({ name: "devilroute-runtime", private: true, type: "commonjs" }),
      "utf-8"
    );
  }
}
