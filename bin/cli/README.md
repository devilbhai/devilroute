# bin/cli — DevilRoute CLI internals

This directory contains the CLI runtime, helpers, and commands for the `devilroute` binary.
...
├── sqlite.mjs              ← openDevilRouteDb() — DB bootstrap
...
All HTTP calls to the DevilRoute server must go through this wrapper.
...
- `baseUrl` — override base URL (default: `DEVILROUTE_BASE_URL` env or `localhost:20128`)
- `apiKey` — override API key (default: `DEVILROUTE_API_KEY`)
...
Locale detection order: `DEVILROUTE_LANG` → `LC_ALL` → `LC_MESSAGES` → `LANG` → `en`.
...
2. `DEVILROUTE_LANG` environment variable
...
```bash
devilroute config lang set pt-BR       # saves to ~/.devilroute/.env
devilroute config lang list            # show all 42 available locales
devilroute config lang get             # show currently active locale
```

**One-time override:**

```bash
devilroute --lang de providers list    # run in German, not persisted
DEVILROUTE_LANG=ja devilroute status    # same effect via env
```
