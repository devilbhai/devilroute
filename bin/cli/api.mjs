...
export function getBaseUrl(opts = {}) {
  ...
  const envUrl = process.env.DEVILROUTE_BASE_URL;
  if (envUrl) return stripTrailingSlash(envUrl);

  // Resolve from the active context (canonical store + legacy profile fallback).
  // This is what makes "remote mode" work: `devilroute contexts use <remote>`
...
export async function buildHeaders(opts) {
  ...
  //
  // The active context's scoped token MUST win over the ambient DEVILROUTE_API_KEY:
  // `devilroute connect <remote>` saves the context's token, but users keep
  // DEVILROUTE_API_KEY in their shell. The global `--api-key` option is bound to
  // that env var (.env("DEVILROUTE_API_KEY")), so commands that spread
  ...
  const ambientKey = process.env.DEVILROUTE_API_KEY || null;
  ...
  const cliToken = opts.cliToken ?? process.env.DEVILROUTE_CLI_TOKEN ?? (await getCliToken());
...
export async function apiFetch(path, opts = {}) {
  ...
  const timeout =
    opts.timeout ?? (Number.parseInt(process.env.DEVILROUTE_HTTP_TIMEOUT_MS || "", 10) || 30000);
  const maxAttempts = opts.retry === false ? 1 : (opts.retryMax ?? RETRY_DEFAULTS.maxAttempts);
  const verbose = opts.verbose ?? process.env.DEVILROUTE_VERBOSE === "1";
