# DevilRoute CLI — Internal Conventions
...
```
devilroute keys add openai sk-xxx
devilroute combo switch fastest
devilroute memory search "react hooks"
```

**Not allowed**:

```
devilroute --add-key openai sk-xxx     # ❌ flag-as-verb
devilroute add-key openai sk-xxx       # ❌ hyphen at the top level
```
...
- Reads base URL from `DEVILROUTE_BASE_URL` env or `~/.devilroute/config.json`
  (active profile).
- Injects `Authorization: Bearer ${DEVILROUTE_API_KEY}` when available.
- Injects `x-devilroute-cli-token` when applicable (see task 8.12).
...
- Detection order: `--lang` flag → `DEVILROUTE_LANG` env → `LC_ALL` → `LC_MESSAGES` → `LANG` → `en`.
- Locale persisted via `config lang set <code>` — saves `DEVILROUTE_LANG` to `~/.devilroute/.env`.
