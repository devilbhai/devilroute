 ...
 *
 * Purely informational invocations must NOT create `~/.devilroute/.env` or write
 * a key — they never touch encrypted storage. Generating a 32-byte key and a
 * `.env` file just to print `devilroute --version` (or `--help`) is a surprising
 ...
 *
 * Returns TRUE for everything else, INCLUDING a bare `devilroute` (no args) — the
...
export function shouldProvisionStorageKey(argv) {
  ...
  // Bare `devilroute` runs the default `serve` command → must provision.
