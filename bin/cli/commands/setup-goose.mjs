/**
 * devilroute setup-goose — configure Goose (block/goose) for DevilRoute.
 *
...
export async function runSetupGooseCommand(opts = {}) {
  ...
  printHeading("DevilRoute → Goose (openai-compatible)");
...
export function registerSetupGoose(program) {
  program
    .command("setup-goose")
    .description("Configure Goose for DevilRoute: write ~/.config/goose/config.yaml + print the env recipe")
