/**
 * devilroute setup-kilo — configure Kilo Code to use DevilRoute.
 *
 ...
 * This writes BOTH (matching the DevilRoute dashboard) and prints the UI settings.
 *
...
/** Merge the DevilRoute openai-compatible provider into Kilo's CLI auth.json. */
export function buildKiloAuth(existing, { apiKey, baseUrl, model }) {
  const auth = { ...(existing || {}) };
  auth["openai-compatible"] = {
    ...(auth["openai-compatible"] || {}),
    apiKey: apiKey || "sk_devilroute",
    baseUrl,
    model,
  };
  return auth;
...
export function buildKiloVscodeSettings(existing, { apiKey, baseUrl, model }) {
  const s = { ...(existing || {}) };
  s["kilocode.customProvider"] = { name: "DevilRoute", baseURL: baseUrl, apiKey: apiKey || "sk_devilroute" };
...
export async function runSetupKiloCommand(opts = {}) {
  ...
  printHeading("DevilRoute → Kilo Code (OpenAI-compatible)");
  ...
  if (dryRun) {
    ...
    console.log(
      JSON.stringify(
        { "openai-compatible": { ...auth["openai-compatible"], apiKey: apiKey ? "set" : "sk_devilroute" } },
        null,
        2
      )
    );
...
export function registerSetupKilo(program) {
  program
    .command("setup-kilo")
    .description(
      "Configure Kilo Code for DevilRoute: write ~/.local/share/kilo/auth.json (CLI) + VS Code kilocode.* settings"
    )
