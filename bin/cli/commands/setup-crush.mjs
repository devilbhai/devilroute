/**
 * devilroute setup-crush — configure Crush (charmbracelet/crush) for DevilRoute.
 *
 ...
 * include /v1; the api_key may reference an env var (`$DEVILROUTE_API_KEY`) so the
...
const API_KEY_REF = "$DEVILROUTE_API_KEY";
...
export function resolveCrushTarget(opts = {}) {
  ...
  if (opts.remote) root = String(opts.remote).replace(/\/+$/, "");
    ...
    try {
      root = resolveActiveContext(opts.context ?? process.env.DEVILROUTE_CONTEXT)?.baseUrl;
    } catch {
      /* none */
    }
...
export function buildCrushProvider(modelIds, baseUrl) {
  ...
  for (const id of modelIds) {
    ...
    if (!cfg) continue;
    models.push({ id, name: `DevilRoute: ${id}`, context_window: cfg.ctx });
  }
  return {
    type: "openai-compat",
    base_url: baseUrl,
...
/** Merge the DevilRoute provider into an existing crush.json (preserve the rest). */
export function mergeCrushConfig(existing, provider) {
  const cfg = existing && typeof existing === "object" ? { ...existing } : {};
  cfg.providers = { ...(cfg.providers || {}), devilroute: provider };
  return cfg;
}
...
export async function runSetupCrushCommand(opts = {}) {
  ...
  printHeading("DevilRoute → Crush (openai-compat)");
  ...
  if (dryRun) {
    ...
    printInfo(`[dry-run] ${provider.models.length} model(s) under providers.devilroute → ${configPath}`);
    return 0;
  }
  mkdirSync(join(configPath, ".."), { recursive: true });
  writeFileSync(configPath, out, "utf8");
  printSuccess(`Wrote ${configPath} (${provider.models.length} models under providers.devilroute)`);
  printInfo("Provide the key (config references $DEVILROUTE_API_KEY):  export DEVILROUTE_API_KEY=...");
...
export function registerSetupCrush(program) {
  program
    .command("setup-crush")
    .description("Generate the DevilRoute openai-compat provider in ~/.config/crush/crush.json")
