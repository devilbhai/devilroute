/**
 * devilroute setup-codex — Remote-aware Codex CLI profile generator.
 *
 * Connects to a running DevilRoute instance (local or remote VPS), fetches the
 ...
 *   devilroute setup-codex --remote http://100.67.86.91:20128 --api-key sk-xxx
 *
...
export async function runSetupCodexCommand(opts = {}) {
  ...
  printHeading(`DevilRoute → Codex CLI profile generator`);
...
export function registerSetupCodex(program) {
  program
    ...
    .option(
      "--remote <url>",
      "Remote DevilRoute URL, e.g. http://100.67.86.91:20128 — fetches models from there"
    )
    .option(
      "--api-key <key>",
      "DevilRoute API key for the remote instance (defaults to DEVILROUTE_API_KEY env var)"
    )
