/**
 * devilroute setup-claude — Remote-aware Claude Code profile generator.
 *
 ...
 * fetches the live /v1/models catalog from a (possibly remote) DevilRoute and
 ...
 *
 * Launch a profile with:  devilroute launch --profile <name>
...
export function buildProfileSettings(modelId, baseUrl, cfg) {
  ...
  // NOTE: ANTHROPIC_AUTH_TOKEN is intentionally NOT written here — `devilroute
...
export async function runSetupClaudeCommand(opts = {}) {
  ...
  const baseUrl = (opts.remote ?? `http://localhost:${port}`)
    .replace(/\/+$/, "")
    .replace(/\/v1$/, "");
  const apiKey = opts.apiKey ?? opts["api-key"] ?? process.env.DEVILROUTE_API_KEY ?? "";
  const claudeHome = opts.claudeHome ?? opts["claude-home"] ?? join(os.homedir(), ".claude");
  const profilesRoot = join(claudeHome, "profiles");
  const dryRun = Boolean(opts.dryRun ?? opts["dry-run"]);

  printHeading("DevilRoute → Claude Code profile generator");
  ...
  try {
    ...
    printInfo(
      "Make sure DevilRoute is running and the --remote URL is correct.\n" +
        "You may also need --api-key if DevilRoute requires authentication."
    );
    return 1;
  }
  ...
  if (!dryRun) {
    ...
    console.log("\nTo use a profile:");
    console.log("  devilroute launch --profile <name>     # e.g. devilroute launch --profile glm52");
    console.log(
...
export function registerSetupClaude(program) {
  program
    ...
    .description(
      "Fetch the live model catalog from DevilRoute (local or remote VPS) and generate " +
