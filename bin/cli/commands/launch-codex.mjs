 ...
 *  in the shell can't shadow the devilroute provider (defense-in-depth). Mirrors
...
const NO_AUTH_SENTINEL = "devilroute-no-auth";
 ...
 * (`model_providers.devilroute.name="DevilRoute"` arrives unquoted and no longer
...
/**
 * Resolve the DevilRoute root base URL + auth for codex, honouring (in order):
...
export function resolveCodexTarget(opts = {}) {
  ...
  if (explicit) {
    ...
    let fromCtx;
    try {
      fromCtx = resolveActiveContext(opts.context ?? process.env.DEVILROUTE_CONTEXT)?.baseUrl;
    } catch {
      /* no context */
    }
    baseUrl = fromCtx
  ...
  if (!authToken) authToken = process.env.DEVILROUTE_API_KEY;
  return { baseUrl, authToken };
}

/** Health-check an DevilRoute root URL before launching Codex. */
 ...
 * DEVILROUTE_API_KEY (the provider env_key) to the resolved token or a sentinel.
...
export function buildCodexEnv(baseEnv, authToken) {
  ...
  env.DEVILROUTE_API_KEY = (authToken && String(authToken).trim()) || NO_AUTH_SENTINEL;
  return env;
}

/**
 * Codex `-c` flags that define the `devilroute` provider inline, so launch works
 * WITHOUT a pre-existing ~/.codex/config.toml. Mirrors free-claude-code.
 * @param {string} baseUrl  DevilRoute root URL (no /v1)
 * @returns {string[]}
 */
export function buildCodexProviderArgs(baseUrl) {
  return [
    "-c",
    tomlAssign("model_provider", "devilroute"),
    "-c",
    tomlAssign("model_providers.devilroute.name", "DevilRoute"),
    "-c",
    tomlAssign("model_providers.devilroute.base_url", `${baseUrl}/v1`),
    "-c",
    tomlAssign("model_providers.devilroute.env_key", "DEVILROUTE_API_KEY"),
    "-c",
    tomlAssign("model_providers.devilroute.wire_api", "responses"),
    "-c",
    tomlAssign("model_providers.devilroute.requires_openai_auth", false),
  ];
}
...
export async function runLaunchCodexCommand(opts = {}, codexArgs = []) {
  ...
  if (!(await healthCheck(baseUrl))) {
    console.error(
      (
        t("launch.notRunning") ||
        "DevilRoute is not reachable at {port}. Start it with 'devilroute serve'."
...
export function registerLaunchCodex(program) {
  program
    ...
    .description(
      t("launchCodex.description") || "Launch Codex CLI pointed at DevilRoute (local or remote VPS)"
    )
    .option("--port <port>", "Local DevilRoute port (ignored when --remote is set)", "20128")
    .option(
      "--remote <url>",
      "Remote DevilRoute base URL, e.g. http://192.168.0.15:20128 (overrides --port + context)"
    )
    .option("--profile <name>", "Codex profile to activate (passed as --profile <name>)")
    .option("-p, --p <name>", "Alias for --profile")
    .option(
      "--api-key <key>",
      "DevilRoute API key (overrides DEVILROUTE_API_KEY env var for this invocation)"
    )
