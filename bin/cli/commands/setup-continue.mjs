/**
 * devilroute setup-continue — configure Continue (continue.dev) for DevilRoute.
 *
 ...
 * `apiKey: ${{ secrets.DEVILROUTE_API_KEY }}` reference (secret never written to
...
const SECRET_REF = "${{ secrets.DEVILROUTE_API_KEY }}";
...
/**
 * Merge DevilRoute models into an existing Continue config object: drop any prior
...
export function mergeContinueConfig(existing, newModels, apiBase) {
  ...
  if (!cfg.name) cfg.name = "DevilRoute Config";
...
export async function runSetupContinueCommand(opts = {}) {
  ...
  printHeading("DevilRoute → Continue (config.yaml)");
  ...
  if (dryRun) {
    ...
    printInfo(`[dry-run] ${models.length} DevilRoute model(s) → ${configPath}`);
    return 0;
  }

  mkdirSync(join(configPath, ".."), { recursive: true });
  writeFileSync(configPath, out, "utf8");
  printSuccess(`Wrote ${configPath} (${models.length} DevilRoute models)`);
  printInfo("\nProvide the key (config.yaml references it, not stores it):");
  printInfo("  cn CLI:  export DEVILROUTE_API_KEY=...   (read from your shell)");
  printInfo("  IDE:     echo 'DEVILROUTE_API_KEY=...' >> ~/.continue/.env");
...
export function registerSetupContinue(program) {
  program
    ...
    .description(
      "Generate ~/.continue/config.yaml (Continue / cn CLI) from the DevilRoute model catalog"
    )
