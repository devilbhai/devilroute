/** Configure Qwen Code's OpenAI-compatible provider for DevilRoute. */
...
export function resolveQwenTarget(opts = {}) {
  ...
  if (!root || !(opts.apiKey ?? opts["api-key"])) {
    try {
      context = resolveActiveContext(opts.context ?? process.env.DEVILROUTE_CONTEXT);
    } catch {
  ...
  const apiKey =
    ...
    context?.accessToken ??
    context?.apiKey ??
    process.env.DEVILROUTE_API_KEY ??
    "sk_devilroute";
...
export async function runSetupQwenCommand(opts = {}) {
  ...
  printHeading("DevilRoute → Qwen Code (OpenAI-compatible)");
  ...
  try {
    ...
    if (dryRun) {
      ...
      printInfo(`[dry-run] credential → ${envPath} (DEVILROUTE_API_KEY)`);
      return 0;
    }
    ...
    printSuccess(`Updated ${envPath} (DEVILROUTE_API_KEY only)`);
...
export function registerSetupQwen(program) {
  program
    .command("setup-qwen")
    .description("Configure Qwen Code's upstream V4 modelProviders format for DevilRoute")
    .option("--port <port>", "Local DevilRoute port (ignored when --remote is set)", "20128")
    .option("--remote <url>", "Remote DevilRoute URL")
    .option("--api-key <key>", "DevilRoute API key")
