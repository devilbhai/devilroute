...
/**
 * `devilroute connect <host>` — remote mode.
 *
 * Logs into a remote DevilRoute server and saves the result as the active context
...
export async function runConnectCommand(host, opts = {}) {
  ...
  if (!baseUrl) {
    printError("A host is required, e.g. devilroute connect 192.168.0.15");
    return 2;
  }
  ...
  cfg.contexts[name] = {
    ...
    accessToken,
    scope,
    description: `Remote DevilRoute (${host})`,
  };
  ...
  printInfo("Switch back to local with: devilroute contexts use default");
  return 0;
}
...
export function registerConnect(program) {
  program
    ...
    .description(
      t("connect.description") || "Connect to a remote DevilRoute server and enter remote mode"
    )
