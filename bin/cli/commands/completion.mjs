...
function readCache() {
  try {
    ...
    if (process.env.DEVILROUTE_DEBUG_COMPLETION) {
      console.error("[devilroute completion] readCache failed:", err?.message ?? err);
    }
  }
  return null;
}
...
async function refreshCache(opts = {}) {
  ...
  try {
    ...
    if (process.env.DEVILROUTE_DEBUG_COMPLETION) {
      console.error("[devilroute completion] refreshCache failed:", err?.message ?? err);
    }
  }
  ...
  try {
    ...
    if (process.env.DEVILROUTE_DEBUG_COMPLETION) {
      console.error("[devilroute completion] writeCache failed:", err?.message ?? err);
    }
  }
  return data;
}
...
function installPath(shell) {
  const home = homedir();
  if (shell === "zsh") return join(home, ".zsh", "completions", "_devilroute");
  if (shell === "fish") return join(home, ".config", "fish", "completions", "devilroute.fish");
  return join(home, ".bash_completion.d", "devilroute");
}

function generateZshScript() {
  return `#compdef devilroute

# DevilRoute zsh completion (dynamic)
_devilroute_get_cache() {
  local key="$1"
  local cache="$HOME/.devilroute/completion-cache.json"
  ...
  if [[ $((now - mtime)) -gt 3600 ]]; then
    devilroute completion refresh --quiet >/dev/null 2>&1
  fi
...
_devilroute() {
  local -a commands
  commands=(
    'serve:Start the DevilRoute server'
    'stop:Stop the server'
    'restart:Restart the server'
    'setup:Configure DevilRoute'
  ...
  case $state in
    ...
    arg)
      case $words[1] in
        combo)
          case $words[2] in
            switch|delete|show)
              local -a combos
              combos=($(_devilroute_get_cache combos))
        ...
        providers|keys)
          case $words[2] in
            add|remove|test)
              local -a providers
              providers=($(_devilroute_get_cache providers))
      ...
      case $state in
        models)
          local -a models
          models=($(_devilroute_get_cache models))
...
compdef _devilroute devilroute
`;
}

function generateBashScript() {
  return `#!/bin/bash
# DevilRoute CLI bash completion (dynamic)
...
complete -F _devilroute devilroute
`;
}

function generateFishScript() {
  return `# DevilRoute CLI fish completion (dynamic)
complete -c devilroute -f
...
for cmd in $commands
  complete -c devilroute -n '__fish_is_nth_token 1' -a $cmd
end

# Subcommands
complete -c devilroute -n '__fish_seen_subcommand_from combo' -a 'list switch create delete show suggest'
complete -c devilroute -n '__fish_seen_subcommand_from keys' -a 'add list remove regenerate revoke reveal usage'
complete -c devilroute -n '__fish_seen_subcommand_from providers' -a 'available list test test-all'
complete -c devilroute -n '__fish_seen_subcommand_from config' -a 'list get set validate contexts'
complete -c devilroute -n '__fish_seen_subcommand_from completion' -a 'zsh bash fish install refresh'
complete -c devilroute -n '__fish_seen_subcommand_from open' -a 'combos providers api-manager cli-tools agents settings logs memory skills evals audit cost resilience'

# Dynamic completions from cache (requires python3)
function __devilroute_cache_get
  set -l key $argv[1]
  set -l cache "$HOME/.devilroute/completion-cache.json"
...
complete -c devilroute -n '__fish_seen_subcommand_from combo; and __fish_seen_subcommand_from switch delete' -a '(__devilroute_cache_get combos)'
complete -c devilroute -l model -a '(__devilroute_cache_get models)'
complete -c devilroute -l combo -a '(__devilroute_cache_get combos)'
`;
}
...
export function registerCompletion(program) {
  ...
  // Backward-compat: `devilroute completion <shell>` (positional arg form)
  comp
    .command("<shell>")
