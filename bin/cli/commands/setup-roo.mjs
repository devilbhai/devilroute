/**
 * devilroute setup-roo — configure Roo Code (RooVeterinaryInc.roo-cline) for DevilRoute.
 *
 ...
 * must support native OpenAI tool-calling (DevilRoute does).
 */
...
export function buildRooImport({ baseUrl, apiKey, model }) {
  return {
    providerProfiles: {
      currentApiConfigName: "DevilRoute",
      apiConfigs: {
        DevilRoute: {
          apiProvider: "openai",
          openAiBaseUrl: baseUrl,
          openAiApiKey: apiKey || "sk_devilroute",
          openAiModelId: model,
...
export async function runSetupRooCommand(opts = {}) {
  ...
  const importPath = opts.importPath ?? opts["import-path"] ?? join(os.homedir(), ".devilroute", "roo-settings.json");
  const vscodePath =
    opts.vscodeSettings ?? opts["vscode-settings"] ?? join(os.homedir(), ".config", "Code", "User", "settings.json");

  printHeading("DevilRoute → Roo Code (OpenAI-compatible)");
  ...
  if (dryRun) {
    console.log(`\n── [dry-run] ${importPath} ──`);
    console.log(JSON.stringify({ ...importDoc, providerProfiles: { ...importDoc.providerProfiles, apiConfigs: { DevilRoute: { ...importDoc.providerProfiles.apiConfigs.DevilRoute, openAiApiKey: apiKey ? "set" : "sk_devilroute" } } } }, null, 2));
  ...
  printInfo(`  API Key:   <your DEVILROUTE_API_KEY>`);
...
export function registerSetupRoo(program) {
  program
    .command("setup-roo")
    .description(
      "Configure Roo Code for DevilRoute: write a Roo import JSON + autoImport pointer + print UI steps"
    )
    ...
    .option("--import-path <path>", "Roo import JSON path (default: ~/.devilroute/roo-settings.json)")
