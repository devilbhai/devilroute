...
async function resolveProviderInput(opts, prompt, nonInteractive) {
  ...
  if (!apiKey) {
    throw new Error("Provider API key is required. Pass --api-key or DEVILROUTE_API_KEY.");
  }
...
export function registerSetup(program) {
  ...
  // Wire up `devilroute setup opencode` subcommand. Kept inside registerSetup
...
export async function runSetupCommand(opts = {}) {
  ...
  try {
    printHeading("DevilRoute Setup");
    const { db, dbPath } = await openDevilRouteDb();
