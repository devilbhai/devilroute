/**
 * devilroute setup-cursor — guide Cursor to use DevilRoute.
 *
...
export function buildCursorInstructions({ apiBase, models }) {
  const lines = [
    ...
    "  3. Set the OpenAI API Key to your DevilRoute key (DEVILROUTE_API_KEY)",
...
export async function runSetupCursorCommand(opts = {}) {
  ...
  printHeading("DevilRoute → Cursor");
...
export function registerSetupCursor(program) {
  program
    ...
    .description("Print the steps to point Cursor at DevilRoute (chat panel; Cursor config is not file-writable)")
