/**
 * devilroute setup opencode — Wire the bundled @devilroute/opencode-plugin
 * into a local OpenCode install.
 *
 * Closes the gap where `npm install -g devilroute` ships the plugin
 * inside the devilroute package (`@devilroute/opencode-plugin/dist/`) but
 ...
 *   3. Copies the built plugin into `<opencode>/plugins/devilroute/`.
 *   4. Creates or updates `opencode.json` with a single `plugin` entry
 *      pointing at the local copy (so OC ≥1.15 picks it up).
 *   5. Optionally runs `opencode auth login --provider devilroute`
...
// We walk up from this file to find the devilroute package root. The script
// lives at `<devilroute>/bin/cli/commands/setup-open-code.mjs`, so the
...
// The bundled plugin ships at PACKAGE_ROOT/@devilroute/opencode-plugin/
// (see root package.json `files`: ["@devilroute/", ...]). The env override
// exists so tests can point at a fixture without building the real plugin.
const BUNDLED_PLUGIN_DIR =
  process.env.DEVILROUTE_OPENCODE_PLUGIN_DIR || join(PACKAGE_ROOT, "@devilroute", "opencode-plugin");
...
/**
 * Locate the bundled @devilroute/opencode-plugin dist. The plugin may be
 * present in two states:
 *
 *   - Built (`dist/index.cjs` + `dist/index.js` exist) — preferred,
 *     ships from a published devilroute tarball after Step 8.8 of
...
function resolveBundledPlugin() {
  if (!existsSync(BUNDLED_PLUGIN_DIR)) {
    throw new Error(
      `Bundled @devilroute/opencode-plugin not found at ${BUNDLED_PLUGIN_DIR}.\n` +
        `This usually means devilroute was installed from a source tree that does not ` +
        `include the workspace package. Try reinstalling devilroute (npm install -g devilroute) ` +
        `or run \`cd @devilroute/opencode-plugin && npm install && npm run build\` from the source repo.`
    );
  }
  ...
  if (!existsSync(esmEntry)) {
    throw new Error(
      `@devilroute/opencode-plugin dist/ not built (looked for ${esmEntry}).\n` +
...
/**
 * Copy the plugin package into `<opencodeConfig>/plugins/devilroute/`. We
 ...
 * resolve the peer deps from the devilroute package's tree, which is
 * unreliable.
 */
function installPluginToOpenCode(pluginInfo, opencodeConfigDir) {
  const targetDir = join(opencodeConfigDir, "plugins", "devilroute");
...
function registerPluginInOpenCodeConfig({
    ...
    try {
      ...
      throw new Error(
        `Failed to parse existing ${configPath}: ${err.message}\n` +
          `Fix or remove the file manually, then re-run \`devilroute setup opencode\`.`
      );
    }
  }
  ...
  const entry = [
    `./plugins/devilroute/dist/index.js`,
    {
      providerId,
      baseURL,
  ...
  // drop a legacy `opencode-devilroute-auth` entry if present — that
  // package is the obsolete predecessor of @devilroute/opencode-plugin
  // and was the root cause of issue #3711.
  const filtered = plugins.filter((p) => {
    if (typeof p === "string") {
      return !p.includes("opencode-devilroute-auth");
    }
    if (Array.isArray(p) && p[1] && typeof p[1] === "object") {
      ...
      if (typeof p[0] === "string" && p[0].includes("opencode-devilroute-auth")) {
        return false;
      }
    }
    return true;
 ...
 *
 * @param {object} opts
 * @param {string} [opts.providerId="devilroute"]
...
export async function runSetupOpenCodeCommand(opts = {}) {
  const providerId = opts.providerId || "devilroute";
  // Remote-aware: explicit --remote/--base-url → active context → localhost.
  let baseURL = opts.remote || opts.baseURL || opts.baseUrl;
  if (!baseURL) {
    try {
      const ctx = resolveActiveContext(opts.context ?? process.env.DEVILROUTE_CONTEXT);
      baseURL = ctx?.baseUrl;
    } catch {
      /* no context */
    }
  ...
  printHeading("DevilRoute → OpenCode Plugin Setup");
...
/**
 * Register the `devilroute setup opencode` subcommand on the parent
 * `setup` command. Commander builds the doc/help from the chain, so
 * `devilroute setup --help` automatically shows the new subcommand.
 *
...
export function registerSetupOpenCode(setupCommand) {
  setupCommand
    ...
    .description(
      t("setup.opencode") ||
        "Install and register the bundled @devilroute/opencode-plugin with a local OpenCode install"
    )
    .option(
      "--provider-id <id>",
      "OpenCode provider id to register (default: devilroute)",
      "devilroute"
    )
    .option(
      "--base-url <url>",
      "DevilRoute base URL the plugin should talk to (default: active context or http://localhost:20128)"
    )
    .option(
      "--remote <url>",
      "Remote DevilRoute URL, e.g. http://192.168.0.15:20128 (overrides --base-url and the context)"
    )
