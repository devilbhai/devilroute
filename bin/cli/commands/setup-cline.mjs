/**
 * devilroute setup-cline — configure the Cline AI coding agent to use DevilRoute.
 *
 ...
 * writes the CLI-mode files (matching the DevilRoute dashboard) AND prints the
...
export function resolveClineTarget(opts = {}) {
  ...
  if (opts.remote) baseUrl = stripToRoot(opts.remote);
    ...
    try {
      baseUrl = stripToRoot(resolveActiveContext(opts.context ?? process.env.DEVILROUTE_CONTEXT)?.baseUrl);
    } catch {
      /* none */
    }
...
/** Merge DevilRoute openai-compatible settings into Cline's globalState (Plan + Act). */
...
export function buildClineSecrets(existing, { apiKey }) {
  return { ...(existing || {}), openAiApiKey: apiKey || "sk_devilroute" };
}
...
export async function runSetupClineCommand(opts = {}) {
  ...
  printHeading("DevilRoute → Cline (OpenAI-compatible)");
  ...
  if (dryRun) {
    ...
    console.log(`\n── [dry-run] ${secPath} ── (openAiApiKey: ${apiKey ? "set" : "sk_devilroute"})`);
  } else {
...
export function registerSetupCline(program) {
  program
    .command("setup-cline")
    .description(
      "Configure Cline for DevilRoute: write ~/.cline/data (CLI mode) + print VS Code extension settings"
    )
