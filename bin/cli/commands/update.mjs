...
export async function getLatestVersion(execFn = execFileAsync) {
  try {
    const { stdout } = await execFn("npm", ["view", "devilroute", "version", "--prefer-online"], {
      timeout: 15000,
    });
    return stdout.trim();
  } catch {
...
export async function createBackup() {
  ...
  const backupDir = path.join(homedir(), ".devilroute", "backups", `devilroute-${Date.now()}`);
  ...
  try {
    ...
    const files = ["devilroute.mjs", "cli", "nodeRuntimeSupport.mjs", "mcp-server.mjs"];
...
export async function runUpdateCommand(opts = {}) {
  ...
  if (showChangelog) {
    try {
      const { stdout } = await execFileAsync("npm", ["view", "devilroute", "changelog"], {
        timeout: 10000,
      });
      if (stdout.trim()) {
        console.log(stdout.trim());
      } else {
        console.log(`Changelog: https://github.com/your-org/devilroute/releases/tag/v${latest}`);
      }
    } catch {
      console.log(`Changelog: https://github.com/your-org/devilroute/releases/tag/v${latest}`);
    }
  ...
  printHeading("DevilRoute Update");
  ...
  if (checkOnly) {
    console.log("\n  Run `devilroute update --apply` to install automatically.");
    return 1; // exit 1 = outdated (useful for scripts)
  }

  if (dryRun) {
    console.log("\n  [DRY RUN] Would run: npm install -g devilroute@latest --include=optional");
    if (!skipBackup) console.log("  [DRY RUN] Would create backup in ~/.devilroute/backups/");
    return 0;
  }
  ...
  try {
    ...
    execSync("npm install -g devilroute@latest --include=optional", { stdio: "inherit" });
    printSuccess(`Updated to version ${latest}`);
    printInfo("Run `devilroute --version` to verify.");
    return 0;
  } catch (err) {
    printError(`Update failed: ${err.message}`);
    printInfo("Restore from backup:");
    const backupDir = path.join(homedir(), ".devilroute", "backups");
