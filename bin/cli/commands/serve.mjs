...
// URL scheme for the "DevilRoute is running" banner — flipped to https when
...
export function registerServe(program) {
  program
    ...
    .option(
      "--tls-cert <path>",
      t("serve.tls_cert") ||
        "Path to a TLS certificate (PEM) to serve HTTPS (also DEVILROUTE_TLS_CERT)"
    )
    .option(
      "--tls-key <path>",
      t("serve.tls_key") ||
        "Path to the TLS private key (PEM) to serve HTTPS (also DEVILROUTE_TLS_KEY)"
    )
    .action(async (opts) => {
      await runServe(opts);
    });
...
export async function runServe(opts = {}) {
  ...
  // Same prep as bin/devilroute.mjs — keep it here so a direct `runServe()` call
...
\x1b[0m`);
  ...
  if (!nodeSupport.nodeCompatible) {
    ...
    console.warn(`\x1b[33m  ⚠  Warning: You are running Node.js ${process.versions.node}.
     ...
     Or run:      devilroute runtime repair  (rebuilds into a user-writable runtime; works without a C++ toolchain)\x1b[0m
`);
  }
  ...
  if (!existsSync(serverJs)) {
    ...
    if (isMise) {
      console.error(
        "  \x1b[33m⚠ mise detected:\x1b[0m If you installed via `npm install -g devilroute`,"
      );
      console.error("    try: \x1b[36mnpx devilroute@latest\x1b[0m  (downloads a fresh copy)");
      console.error("    or:  \x1b[36mmise exec -- npx devilroute\x1b[0m");
    } else if (isNvm) {
      console.error(
        "  \x1b[33m⚠ nvm detected:\x1b[0m Try reinstalling after loading the correct Node version:"
      );
      console.error("    \x1b[36mnvm use --lts && npm install -g devilroute\x1b[0m");
    } else {
      console.error("  Try: \x1b[36mnpm install -g devilroute\x1b[0m  (reinstall)");
      console.error("  Or:  \x1b[36mnpx devilroute@latest\x1b[0m");
    }
    process.exit(1);
  }
  ...
  if (
    ...
    console.error(
      "  Or run: \x1b[36mdevilroute runtime repair\x1b[0m" +
  ...
  // of RAM under load. An explicit DEVILROUTE_MEMORY_MB still wins.
  const memoryLimit = resolveMaxOldSpaceMb(
    process.env.DEVILROUTE_MEMORY_MB,
  ...
  const tlsCert = opts.tlsCert ?? process.env.DEVILROUTE_TLS_CERT;
  const tlsKey = opts.tlsKey ?? process.env.DEVILROUTE_TLS_KEY;

  const env = {
    ...process.env,
    DEVILROUTE_PORT: String(port),
    ...
    // matches the OS-reported hostname (the auto-set signature). DEVILROUTE_SERVER_HOST
    ...
    HOSTNAME:
      process.env.DEVILROUTE_SERVER_HOST ||
    ...
    ...(tlsCert ? { DEVILROUTE_TLS_CERT: tlsCert } : {}),
    ...(tlsKey ? { DEVILROUTE_TLS_KEY: tlsKey } : {}),
  };
...
function runDaemon(serverJs, env, memoryLimit, dashboardPort, apiPort) {
  ...
  server.unref();
  console.log(`\x1b[32m✔ DevilRoute started in background (PID: ${server.pid})\x1b[0m`);
...
function runWithoutRecovery(serverJs, env, memoryLimit, dashboardPort, apiPort, noOpen, startedAt) {
  ...
  const shutdown = () => {
    console.log("\n\x1b[33m⏹ Shutting down DevilRoute...\x1b[0m");
...
async function runWithSupervisor(
  ...
  if (showLog) process.env.DEVILROUTE_SHOW_LOG = "1";
  ...
  try {
    ...
    const tray = await initTray({
      ...
      onShowLogs: () => {
        ...
        process.stdout.write(`[devilroute][tray] Logs at: ${dashboardUrl}/logs\n`);
      },
    });
    if (tray) {
    ...
    process.stderr.write(`[devilroute][tray] failed to start: ${err?.message ?? String(err)}\n`);
  }
}
...
async function onReady(dashboardPort, apiPort, noOpen, startedAt) {
  ...
  console.log(`
  \x1b[32m✔ DevilRoute is running!\x1b[0m \x1b[2m(started in ${elapsed}s)\x1b[0m
