...
const DEFAULT_NAME = "devilroute-redis";
const DEFAULT_PORT = "6379";
const DEFAULT_VOLUME = "devilroute-redis-data";
...
export function registerRedis(program) {
  const redis = program
    ...
    .description(
      t("redis.description") ||
        "Launch a 1-click local Redis container (Podman or Docker) for DevilRoute caching and quota tracking"
    );
...
export async function runRedisUpCommand(opts = {}) {
  ...
  try {
    ...
    info(`Set DEVILROUTE_REDIS_URL=redis://127.0.0.1:${port} in your .env to wire DevilRoute to it.`);
    return 0;
  } catch (err) {
...
export async function runRedisStatusCommand(opts = {}) {
  ...
  if (!running) {
    info(`Run 'devilroute redis up' to launch it.`);
  }
  return 0;
}