...
// ~/.devilroute/runtime by trayRuntime.ts (loadSystray). The previous inline
...
export async function initSystrayUnix(
  ...
  const items = [
    { title: `DevilRoute  •  port ${port}`, tooltip: "Server running", enabled: false },
    ...
    { title: "Quit DevilRoute", enabled: true },
  ];
  ...
  tray.ready().catch((err) => {
    process.stderr.write(`[devilroute][tray] systray2 failed: ${err?.message ?? String(err)}\n`);
  });
