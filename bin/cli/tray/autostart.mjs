...
const APP_LABEL = "com.devilroute.autostart";
const WIN_REG_VALUE = "DevilRoute";
const WIN_STARTUP_FILE = "DevilRoute.vbs";
const LINUX_SERVICE_NAME = "devilroute.service";
const LINUX_DESKTOP_NAME = "devilroute.desktop";

function resolveCliPath() {
  const candidates = [];
  if (process.argv[1]) candidates.push(process.argv[1]);
  try {
    const which = execSync("command -v devilroute 2>/dev/null", { encoding: "utf8" }).trim();
    if (which) candidates.push(which);
  } catch {
    // command -v unavailable
  }
  candidates.push(join(dirname(fileURLToPath(import.meta.url)), "..", "..", "devilroute.mjs"));

  for (const candidate of candidates) {
    if (!candidate) continue;
    try {
      const resolved = realpathSync(candidate);
      if (resolved.endsWith("devilroute.mjs") && existsSync(resolved)) return resolved;
    } catch {
      // try next candidate
    }
  }
...
function writeLinuxSystemdUnit(cliPath) {
  ...
  const envFile = join(userHomeDir(), ".devilroute", ".env");
  const lines = [
    "[Unit]",
    "Description=DevilRoute AI proxy router",
 ...
 * running process. When the running DevilRoute cli was itself spawned by the
...
/**
 * Builds the VBScript source that launches DevilRoute with WSH's Run method
