...
const DEVILROUTE_IPC_PORT_BASE = 29128;

export function initWinTray({ port, onQuit, onOpenDashboard, onShowLogs }) {
  if (process.platform !== "win32") return null;

  const ipcPort = DEVILROUTE_IPC_PORT_BASE + (port % 1000);
  const scriptPath = join(tmpdir(), `devilroute-tray-${process.pid}.ps1`);
...
$tray.Text = "DevilRoute - Port ${port}"
...
$mQuit = $menu.Items.Add("Quit DevilRoute")
