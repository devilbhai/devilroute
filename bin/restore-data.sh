#!/usr/bin/env bash
# bin/restore-data.sh — restore the DevilRoute SQLite data volume from a snapshot
...
Stop DevilRoute before running, and restart it afterwards.
EOF
}
...
ops_log "restore source: $snap → $DEVILROUTE_DATA_DIR"
ops_confirm "Overwrite storage.sqlite at $DEVILROUTE_DATA_DIR from $snap?" || ops_die "aborted"

# Pre-restore safety copy of the live data.
if [ -f "$DEVILROUTE_SQLITE" ]; then
  safety="$DEVILROUTE_BACKUPS_DIR/pre-restore_$(date -u +%Y%m%dT%H%M%SZ)"
  mkdir -p "$safety"
  if command -v sqlite3 >/dev/null 2>&1; then
    sqlite3 "$DEVILROUTE_SQLITE" "VACUUM INTO '$safety/storage.sqlite'" \
      || cp -a "$DEVILROUTE_SQLITE" "$safety/storage.sqlite"
  else
    cp -a "$DEVILROUTE_SQLITE" "$safety/storage.sqlite"
  fi
...
mkdir -p "$DEVILROUTE_DATA_DIR"
# Drop stale WAL/SHM so the restored DB is authoritative, then copy in.
rm -f "$DEVILROUTE_SQLITE" "${DEVILROUTE_SQLITE}-wal" "${DEVILROUTE_SQLITE}-shm"
cp -a "$snap/storage.sqlite" "$DEVILROUTE_SQLITE"

# Restore sibling DBs captured in the snapshot.
for f in "$snap"/*.sqlite; do
  [ -e "$f" ] || continue
  [ "$(basename "$f")" = "storage.sqlite" ] && continue
  cp -a "$f" "$DEVILROUTE_DATA_DIR/"
done

ops_log "restore complete — restart DevilRoute to pick up the restored data"
