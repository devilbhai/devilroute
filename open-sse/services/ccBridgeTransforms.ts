 ...
 *
 * Goal: ensure the final request body DevilRoute sends to Anthropic's
...
export interface InjectBillingHeaderOp {
   ...
   *   - devilroute-daystamp: sha256(YYYY-MM-DD + version).slice(0,3)
   */
  versionFormat: "ex-machina" | "devilroute-daystamp";
  /**
 ...
 * proven against the live DevilRoute deployment (call log
...
/**
 * Compute the `cc_version` suffix per the DevilRoute native-OAuth algorithm:
...
export function buildBillingHeaderValue(
  ...
  const suffix =
    ...
    (options.versionFormat === "devilroute-daystamp"
