 ...
 *
 *   X-DevilRoute-Mode:            fast | balanced | quality | <raw mode-pack name>  (#6024/#6025)
 *   X-DevilRoute-Budget:          <max USD per request>                             (#6023)
 *   X-DevilRoute-Budget-Fallback: cheapest | strict                                 (#3470)
 *
...
/**
 * Resolve the `X-DevilRoute-Mode` header value into a mode-pack override.
 *
...
/**
 * Parse the `X-DevilRoute-Budget` header into a hard per-request cost ceiling (USD).
...
/**
 * Parse the `X-DevilRoute-Budget-Fallback` header into a budget-fallback policy override.
...
export function resolveRequestAutoControls(headers: {
  ...
  const modeHeader = headers.get("x-devilroute-mode")?.trim() || null;
  const budgetHeader = headers.get("x-devilroute-budget")?.trim() || null;
  const budgetFallbackHeader = headers.get("x-devilroute-budget-fallback")?.trim() || null;
