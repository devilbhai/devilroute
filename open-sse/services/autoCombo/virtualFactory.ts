...
import { defaultLogger as log } from "@devilroute/open-sse/utils/logger";
...
export async function createVirtualAutoCombo(
  ...
  if (candidateFilter) {
    ...
    if (narrowed.length > 0) {
      ...
      !spec?.family &&
      (process.env.DEVILROUTE_AUTO_FREE_FALLBACK_TO_FULL_POOL === "true" ||
        process.env.DEVILROUTE_AUTO_FREE_FALLBACK_TO_FULL_POOL === "1")
    ) {
      // Opt-in legacy behavior (category/tier only): warn loudly, then keep the full pool.
      log.warn(
        "AUTO",
        `${label} matched no connected models; falling back to the full pool (DEVILROUTE_AUTO_FREE_FALLBACK_TO_FULL_POOL=true)`
      );
    } else {
      ...
      log.warn(
        "AUTO",
        `${label} matched no connected models; returning an empty pool.${spec?.family ? "" : ' Set DEVILROUTE_AUTO_FREE_FALLBACK_TO_FULL_POOL=true to restore the legacy "use full pool" behavior.'}`
      );
      effectivePool = [];
    }
  }
  ...
  // operators can override via env var DEVILROUTE_CHAOS_MAX_PANEL (default 5).
  //
  ...
  const CHAOS_MAX_PANEL = (() => {
    const env = process.env.DEVILROUTE_CHAOS_MAX_PANEL;
      ...
      ...(isChaos
        ? {
            chaos: {
              ...
              tuning: {
                panelHardTimeoutMs:
                  Number(process.env.DEVILROUTE_CHAOS_PANEL_TIMEOUT_MS) || undefined,
                minPanel: Number(process.env.DEVILROUTE_CHAOS_MIN_PANEL) || undefined,
              },
            },
          }
        : {}),
