 ...
 * not reliable here because DevilRoute may force a streaming envelope internally.
 */
 ...
 *
 * Supports BOTH wire formats DevilRoute may emit:
 ...
 * `x-devilroute-chaos-error` header and an error final chunk (status stays 200
...
export async function handleChaosChat(opts: {
  ...
  return new Response(stream, {
    ...
    headers: {
      ...
      "X-DevilRoute-Chaos": "true",
      "X-DevilRoute-Chaos-Panel": String(panel.length),
      "X-DevilRoute-Chaos-Primary": primaryModel ?? "",
    },
  });
}
