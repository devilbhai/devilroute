 ...
 * `DEVILROUTE_EMERGENCY_FALLBACK=false` (or `0`). Default remains enabled.
 */

import { isFeatureFlagEnabled } from "@/shared/utils/featureFlags";

const EMERGENCY_FALLBACK_FLAG_KEY = "DEVILROUTE_EMERGENCY_FALLBACK";
...
function isEmergencyFallbackRawEnvEnabled(): boolean {
  const raw = process.env.DEVILROUTE_EMERGENCY_FALLBACK;
...
export function shouldUseFallback(
  ...
  if (!isEmergencyFallbackEnvEnabled()) {
    return {
      shouldFallback: false,
      reason: "emergency fallback disabled via DEVILROUTE_EMERGENCY_FALLBACK",
    };
  }
