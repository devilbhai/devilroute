 ...
 * cannot reach the `tokenPlan/usage` endpoint), so DevilRoute self-tracks the
 ...
 * API to call. Instead we count the tokens DevilRoute itself routed to this
 ...
 * through DevilRoute, not the provider's own dashboard figure.
 */
export async function getXiaomiMimoUsage(connectionId: string) {
  ...
  try {
    ...
    return {
      plan: "Xiaomi MiMo Token Plan (DevilRoute-tracked)",
      quotas: {
