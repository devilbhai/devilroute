 ...
 * self-track pattern — DevilRoute sums the tokens it itself routed to this
 ...
 * MiMo self-track pattern above — DevilRoute sums the tokens it itself routed
...
export async function getXaiUsage(connectionId: string) {
  ...
  try {
    ...
    return {
      plan: "xAI / Grok (DevilRoute-tracked)",
      quotas: {
        monthly: {
          used,
          total: 0,
