 ...
 * with the connection's OAuth access token, falling back to DevilRoute's own
...
export async function getXaiOauthUsage(
  ...
  try {
    ...
    const used =
      ...
      0;
    return {
      plan: "xAI OAuth (Grok) · DevilRoute-tracked",
      message: "Live weekly quota unavailable; showing DevilRoute-routed token totals only.",
      quotas: {
        monthly: {
          used,
          total: 0,
