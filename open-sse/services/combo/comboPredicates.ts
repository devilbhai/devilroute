...
// #1731v2 guard: a provider circuit-breaker-open response (503 + `X-DevilRoute-Provider-Breaker`
// header / `provider_circuit_open` error code, see providerCircuitOpenResponse) is an DevilRoute
...
export function isProviderCircuitOpenResult(
  ...
  const breakerHeader = result.headers?.get?.("x-devilroute-provider-breaker");
