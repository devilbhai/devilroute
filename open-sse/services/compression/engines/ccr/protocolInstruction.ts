 ...
 * content — it can only call `devilroute_ccr_retrieve` if it (a) knows the tool
 ...
 * `devilroute_ccr_retrieve` (an MCP-capable caller). Plain OpenAI-compatible
 * callers that cannot reach the tool must never be told to call it.
 */

const CCR_RETRIEVE_TOOL_NAME = "devilroute_ccr_retrieve";
 ...
 * but only when:
 *  - the caller can actually reach `devilroute_ccr_retrieve` (see
