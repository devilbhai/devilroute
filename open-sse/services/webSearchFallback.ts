...
export const DEVILROUTE_WEB_SEARCH_FALLBACK_TOOL_NAME = "devilroute_web_search";
...
function buildFallbackTool(tool: JsonRecord, targetFormat?: string | null): JsonRecord {
  const name = DEVILROUTE_WEB_SEARCH_FALLBACK_TOOL_NAME;
...
// built-in web-search tool has to be converted to the devilroute_web_search function
...
export function supportsNativeWebSearchFallbackBypass({
  ...
  // native tool untouched instead of rewriting it to devilroute_web_search. Mirrors the
...
export function prepareWebSearchFallbackBody<T extends WebSearchFallbackBody>(
  ...
  if (!toolNames.has(DEVILROUTE_WEB_SEARCH_FALLBACK_TOOL_NAME)) {
  ...
  if (isBuiltInWebSearchToolChoice(body.tool_choice)) {
    ...
    nextBody.tool_choice = (
      isResponsesTarget
        ? { type: "function", name: DEVILROUTE_WEB_SEARCH_FALLBACK_TOOL_NAME }
        : { type: "function", function: { name: DEVILROUTE_WEB_SEARCH_FALLBACK_TOOL_NAME } }
    ) as T["tool_choice"];
  }
