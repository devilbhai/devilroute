 ...
 * synthetic `devilroute_web_fetch` function tool; the actual `/v1/web/fetch` call
...
export const DEVILROUTE_WEB_FETCH_FALLBACK_TOOL_NAME = "devilroute_web_fetch";
...
function buildFallbackTool(targetFormat?: string | null): JsonRecord {
  const name = DEVILROUTE_WEB_FETCH_FALLBACK_TOOL_NAME;
  const description = [
...
export function prepareWebFetchFallbackBody<T extends WebFetchFallbackBody>(
  ...
  if (!toolNames.has(DEVILROUTE_WEB_FETCH_FALLBACK_TOOL_NAME)) {
  ...
  if (isBuiltInWebFetchToolChoice(body.tool_choice)) {
    nextBody.tool_choice = (
      isResponsesTarget
        ? { type: "function", name: DEVILROUTE_WEB_FETCH_FALLBACK_TOOL_NAME }
        : { type: "function", function: { name: DEVILROUTE_WEB_FETCH_FALLBACK_TOOL_NAME } }
    ) as T["tool_choice"];
  }
  ...
  return {
    ...
    fallback: {
      enabled: true,
      toolName: DEVILROUTE_WEB_FETCH_FALLBACK_TOOL_NAME,
