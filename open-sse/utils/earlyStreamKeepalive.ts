 ...
 * DevilRoute, however, holds the streaming response until `ensureStreamReadiness`
...
const KEEPALIVE_FRAME = ENCODER.encode(": devilroute-keepalive\n\n");
...
export const OPENAI_KEEPALIVE_FRAME = ENCODER.encode(
  'data: {"id":"devilroute-keepalive","object":"chat.completion.chunk","created":0,"model":"devilroute","choices":[{"index":0,"delta":{},"finish_reason":null}]}\n\n'
);
...
const RESPONSES_STARTUP_ITEM_ID = "rs_devilroute_keepalive";
const STARTUP_THINKING_TEXT = "DevilRoute: got request, sending to provider";
...
export type EarlyStreamKeepaliveOptions = {
   ...
   * (`: devilroute-keepalive`). Anthropic-format routes (/v1/messages) must pass
