 ...
 * `stream`. Without this hint, DevilRoute previously routed those requests with
...
export function resolveStreamFlag(
  ...
  // DevilRoute accumulates the provider stream and converts it to JSON for the client
  ...
  // DevilRoute fallback would force SSE upstream and fail JSON-only providers as
  ...
  // No explicit stream param — preserve DevilRoute's streaming default unless
