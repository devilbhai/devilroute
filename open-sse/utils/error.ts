 ...
 * `x-devilroute-combo-*` headers and a `diagnostics` field in the OpenAI-shaped
 ...
 * carries a `recovery` hint it is mirrored as `x-devilroute-recovery-action` /
 * `x-devilroute-recovery-next-step` / `x-devilroute-retry-after-seconds` headers and as a
...
export function errorResponseWithComboDiagnostics(
  ...
  const headers: Record<string, string> = {
    ...
    "x-devilroute-combo-pool-size": String(safe.poolSize),
    "x-devilroute-combo-attempted": String(safe.attempted),
    "x-devilroute-combo-excluded": excludedHeader,
    "x-devilroute-combo-terminal-reason": toHeaderSafeAscii(safe.terminalReason.slice(0, 200)),
  };

  if (safe.recovery) {
    headers["x-devilroute-recovery-action"] = safe.recovery.action;
    ...
    headers["x-devilroute-recovery-next-step"] = toHeaderSafeAscii(safe.recovery.next_step).slice(
      0,
      128
    );
    if (
      ...
      headers["x-devilroute-retry-after-seconds"] = String(safe.recovery.retry_after_seconds);
    }
  }
  ...
  return new Response(
    ...
    }),
    {
      ...
      headers: {
        "Content-Type": "application/json",
        "Retry-After": String(retryAfterSec),
        "X-DevilRoute-Provider-Breaker": "open",
      },
    }
  );
}
