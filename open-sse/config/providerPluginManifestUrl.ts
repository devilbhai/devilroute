export const PROVIDER_PLUGIN_MANIFEST_HEADER = "X-DevilRoute-Provider-Manifest-Url";
...
export function resolveProviderPluginManifestUrl(origin?: string | null): string {
  const configured = process.env.DEVILROUTE_PROVIDER_MANIFEST_URL?.trim();
  ...
  const protocol = process.env.DEVILROUTE_PUBLIC_PROTOCOL || "http";
