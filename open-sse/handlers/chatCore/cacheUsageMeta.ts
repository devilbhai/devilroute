 ...
 * shapes), and attach an `_devilroute` meta blob to a log payload. Side-effect-free; behaviour is
...
export function attachLogMeta(
  ...
  if (!payload || typeof payload !== "object" || Array.isArray(payload)) {
    return { _devilroute: compactMeta, _payload: payload ?? null };
  }
  const existing =
    payload._devilroute &&
    typeof payload._devilroute === "object" &&
    !Array.isArray(payload._devilroute)
      ? payload._devilroute
      : {};
  return {
    ...payload,
