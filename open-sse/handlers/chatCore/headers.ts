 ...
 * `x-devilroute-no-memory` header. Mirrors the existing `x-devilroute-no-cache`
...
export function isNoMemoryRequested(
  ...
  const value = (getHeaderValueCaseInsensitive(headers, "x-devilroute-no-memory") || "")
    .trim()
    .toLowerCase();
...
/**
 * Per-request compression override via the `x-devilroute-compression` header. Mirrors the
 * `x-devilroute-no-memory` convention (#4290). Returns the raw trimmed value, or null when
...
export function resolveCompressionHeader(
  ...
  const value = (getHeaderValueCaseInsensitive(headers, "x-devilroute-compression") || "").trim();
  return value || null;
}

/**
 * Per-request opt-in to unconditionally strip `reasoning_content` from the
 * non-streaming JSON response via the `x-devilroute-strip-reasoning` header.
...
export function isStripReasoningRequested(
  ...
  const value = (getHeaderValueCaseInsensitive(headers, "x-devilroute-strip-reasoning") || "")
    .trim()
    .toLowerCase();
  return value === "true" || value === "1" || value === "yes";
}
