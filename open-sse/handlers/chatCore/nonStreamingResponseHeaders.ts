 ...
 * cache-MISS JSON response — the static Content-Type + cache marker, the DevilRoute meta headers
...
import { attachDevilRouteMetaHeaders as defaultAttachMeta } from "@/domain/devilrouteResponseMeta";
...
export function buildNonStreamingResponseHeaders(
  ...
  deps: { attachDevilRouteMetaHeaders: typeof defaultAttachMeta; now: () => number } = {
    attachDevilRouteMetaHeaders: defaultAttachMeta,
    now: Date.now,
  }
  ...
  deps.attachDevilRouteMetaHeaders(responseHeaders, {
  ...
  if (args.compressionResponseMeta) {
    responseHeaders[DEVILROUTE_RESPONSE_HEADERS.compression] = args.compressionResponseMeta;
  }
