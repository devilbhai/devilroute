...
} from "@/domain/devilrouteResponseMeta";
import { DEVILROUTE_RESPONSE_HEADERS } from "@/shared/constants/headers";
import { defaultLogger } from "@devilroute/open-sse/utils/logger";
 ...
 * This budget includes each header name, separator, value, and trailing CRLF. DevilRoute's own
...
function isDevilRouteInternalHeader(headerName: string): boolean {
  return headerName.toLowerCase().startsWith("x-devilroute-");
}
 ...
 *
 * If DevilRoute re-emits those headers from an App Router route handler, Next
...
export function buildStreamingResponseHeaders(
  ...
  providerHeaders.forEach((value, key) => {
    ...
    if (
      ...
      isDevilRouteInternalHeader(normalized)
    ) {
      return;
    }
    candidates.push({
  ...
  const responseHeaders: Record<string, string> = {
    ...
    [DEVILROUTE_RESPONSE_HEADERS.cache]: "MISS",
  };
  attachDevilRouteMetaHeaders(responseHeaders, meta);
