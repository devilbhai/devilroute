...
declare global {
  var __devilrouteMcpAuditDb: AuditDatabase | null | undefined;
}
...
function getCachedAuditDb(): AuditDatabase | null {
  return globalThis.__devilrouteMcpAuditDb ?? null;
}

function setCachedAuditDb(database: AuditDatabase | null): void {
  globalThis.__devilrouteMcpAuditDb = database;
}
  ...
  if (!nodeSqliteFallbackAvailable()) {
    console.error(
      `[MCP Audit] better-sqlite3 native binding unavailable and Node ${process.version} ` +
        ...
        "`npm rebuild better-sqlite3` in the devilroute install root."
    );
    return null;
  }
 ...
 * Uses the same SQLite database as the main DevilRoute app.
 *
 * Driver priority:
...
async function getDb(): Promise<AuditDatabase | null> {
  ...
  try {
    ...
    const dbPath = process.env.DATA_DIR
      ...
      : join(homedir(), ".devilroute", "storage.sqlite");
...
export async function logToolCall(
  ...
  try {
    ...
    const apiKeyId = process.env.DEVILROUTE_API_KEY_ID || null;
