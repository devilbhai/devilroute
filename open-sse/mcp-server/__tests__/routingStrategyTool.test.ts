...
describe("devilroute_set_routing_strategy MCP tool schema", () => {
  it("should be registered in MCP_TOOLS", () => {
    const tool = MCP_TOOLS.find((t) => t.name === "devilroute_set_routing_strategy");
    expect(tool).toBeDefined();
    expect(tool?.phase).toBe(2);
  });

  it("should be available in MCP_TOOL_MAP", () => {
    expect(MCP_TOOL_MAP["devilroute_set_routing_strategy"]).toBeDefined();
  });
