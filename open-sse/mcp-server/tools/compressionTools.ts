/**
 * DevilRoute MCP Compression Tools — Manage and monitor prompt compression.
 *
 * Tools:
 *   1. devilroute_compression_status   — Get compression config, analytics, and cache stats
 *   2. devilroute_compression_configure — Update compression settings
...
export async function handleCompressionStatus(
  ...
  try {
    ...
    await logToolCall("devilroute_compression_status", args, result, duration, true);
...
export async function handleCompressionConfigure(
  ...
  try {
    ...
    const duration = Date.now() - start;
    await logToolCall("devilroute_compression_configure", args, result, duration, true);
...
export async function handleCcrStoreTool(
  ...
  await logToolCall("devilroute_ccr_store", auditInput, output, Date.now() - start, true);
  return output;
}
...
export async function handleCcrRetrieveTool(
  ...
  if ((!args.mode || args.mode === "full") && metadata.bytes > MAX_CCR_MCP_FULL_BYTES) {
    ...
    await logToolCall("devilroute_ccr_retrieve", args, output, Date.now() - start, true);
    return output;
  }
...
export async function handleCcrInspectTool(
  ...
  await logToolCall("devilroute_ccr_inspect", args, output, Date.now() - start, Boolean(metadata));
  return output;
}
...
export async function handleCcrListTool(
  ...
  await logToolCall("devilroute_ccr_list", args, output, Date.now() - start, true);
  return output;
}
...
export async function handleCcrDeleteTool(
  ...
  await logToolCall("devilroute_ccr_delete", args, output, Date.now() - start, true);
  return output;
}
...
export async function handleCcrStatsTool(
  ...
  await logToolCall("devilroute_ccr_stats", args, output, Date.now() - start, true);
  return output;
}
...
export async function handleRtkDiscover(
  ...
  await logToolCall("devilroute_rtk_discover", args, result, Date.now() - start, true);
  return result;
}
...
export async function handleRtkLearn(
  ...
  await logToolCall("devilroute_rtk_learn", args, result, Date.now() - start, true);
  return result;
}
...
export const compressionTools = {
  ...
  devilroute_ccr_inspect: {
    name: "devilroute_ccr_inspect",
  ...
  devilroute_ccr_delete: {
    name: "devilroute_ccr_delete",
  ...
  devilroute_ccr_stats: {
    name: "devilroute_ccr_stats",
    description:
  ...
  devilroute_rtk_discover: {
    name: "devilroute_rtk_discover",
    description:
  ...
  devilroute_rtk_learn: {
    name: "devilroute_rtk_learn",
    description:
