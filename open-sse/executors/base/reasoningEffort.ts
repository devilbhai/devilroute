...
/**
 * Map DevilRoute's reasoning-effort inputs onto the binary thinking switch exposed by
...
export function supportsMaxEffortForProvider(provider: string, model: string): boolean {
  ...
  // normalized to xhigh (the DevilRoute-internal top tier) and rejected by the
...
// DevilRoute carries the requested effort on up to three shapes:
//   1. top-level `reasoning_effort`        — OpenAI / DevilRoute-internal
...
export function sanitizeReasoningEffortForProvider(
  ...
  // ONLY as {high, max} (its own top tier is literally "max"). DevilRoute's internal
