...
function consumeResponsesStoreMarker(body: Record<string, unknown>): unknown {
  const marker = body._devilrouteResponsesStore;
  delete body._devilrouteResponsesStore;
  return marker;
}

/**
 * Global Codex WebSocket kill-switch (feature flag DEVILROUTE_CODEX_WS_ENABLED,
...
function isCodexWsGloballyEnabled(): boolean {
  try {
    return isFeatureFlagEnabled("DEVILROUTE_CODEX_WS_ENABLED");
  } catch {
    return true;
  }
}
...
export function isCodexResponsesWebSocketRequired(_model: string, credentials: unknown): boolean {
  ...
  // DevilRoute is an HTTP→SSE gateway — WebSocket transport is unnecessary and
...
function codexDropNonstandardEvents(): boolean {
  const v = process.env.DEVILROUTE_CODEX_DROP_NONSTANDARD_EVENTS;
...
// streaming Response when the DEVILROUTE_CODEX_DROP_NONSTANDARD_EVENTS kill-switch is on.
...
export function encodeResponseSseEvent(raw: string): { sse: string; terminal: boolean } {
  ...
  // DEVILROUTE_CODEX_DROP_NONSTANDARD_EVENTS (the HTTP transport is handled
...
export class CodexExecutor extends BaseExecutor {
  ...
  transformRequest(
    ...
    // These are often injected by DevilRoute's fallback logic for store=true,
    ...
    const RESPONSES_API_ALLOWLIST = new Set([
      ...
      "text",
      // Internal markers used by DevilRoute pipeline
      "_devilrouteResponsesStore",
    ]);
