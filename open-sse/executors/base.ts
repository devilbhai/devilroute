...
/**
 * Strip the DevilRoute provider prefix from versioned built-in tool model
 ...
 * leaked DevilRoute prefix makes Anthropic reject the request. Mutates in place.
 */
      ...
      try {
        ...
        if (
          ...
          if (Array.isArray(tb.tools)) {
            ...
            // Also strip DevilRoute provider prefix from versioned built-in tool
          ...
          //   x-devilroute-effort:   low | medium | high | xhigh | max | off
          //   x-devilroute-thinking: adaptive | off
          // A header value applies only when the corresponding body field is
          // not already set; "off" force-strips the field.
          const headerEffort = (
            clientHeaders?.["x-devilroute-effort"] ?? clientHeaders?.["X-DevilRoute-Effort"]
          )
            ?.trim()
            .toLowerCase();
          const headerThinking = (
            clientHeaders?.["x-devilroute-thinking"] ?? clientHeaders?.["X-DevilRoute-Thinking"]
          )
            ?.trim()
            .toLowerCase();
          ...
          if (effThinking === "adaptive") {
            ...
            // must opt in with x-devilroute-thinking; force-injecting adaptive thinking
          ...
          // Anthropic correlate accounts behind one DevilRoute.
          ...
          // is pinned to the captured CLI wire image, not DevilRoute's Node.
