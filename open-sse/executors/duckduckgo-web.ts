...
function shouldUseBrowserBacked(): boolean {
  ...
  const poolFlag = process.env.DEVILROUTE_BROWSER_POOL;
 ...
 * retired id DevilRoute historically advertised to the current wire id served by
...
export class DuckDuckGoWebExecutor extends BaseExecutor {
  ...
  async execute(input: ExecuteInput) {
    ...
    // Browser-backed path: opt-in via DEVILROUTE_BROWSER_POOL=on or
