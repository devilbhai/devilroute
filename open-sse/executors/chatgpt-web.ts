...
// DevilRoute model ID → ChatGPT internal slug. The public ChatGPT Web catalog
// keeps DevilRoute's historical dot-form IDs (e.g. "gpt-5.5-pro"), while
...
function configuredProPollTimeoutMs(): number {
  const raw = Number(process.env.DEVILROUTE_CGPT_WEB_PRO_TIMEOUT_MS);
  if (!Number.isFinite(raw) || raw <= 0) return DEFAULT_PRO_POLL_TIMEOUT_MS;
  return Math.floor(raw);
}

function configuredProPollIntervalMs(): number {
  const raw = Number(process.env.DEVILROUTE_CGPT_WEB_PRO_POLL_INTERVAL_MS);
 ...
 * dozen "DevilRoute" entries clutter the sidebar and can interact with
 ...
 * chat but returned to DevilRoute as a bare "completed without image markdown".
 */
 ...
 * often reach DevilRoute from a container while the user's browser needs a
...
function derivePublicBaseUrl(
  ...
  const explicitPublicBase = normalizePublicBaseUrl(process.env.DEVILROUTE_PUBLIC_BASE_URL);
  if (explicitPublicBase) {
    log?.debug?.("CGPT-WEB", `derivePublicBaseUrl: using DEVILROUTE_PUBLIC_BASE_URL`);
    return explicitPublicBase;
  }

  const headerBase = deriveHeaderBaseUrl(clientHeaders);
  const configuredBase =
    normalizePublicBaseUrl(process.env.DEVILROUTE_BASE_URL) ||
...
/**
 * Download a chatgpt.com signed image URL and re-serve it from DevilRoute's
 ...
 * and return a browser-fetchable DevilRoute URL.
 */
...
function configuredAsyncImageTimeoutMs(): number {
  const raw = Number(process.env.DEVILROUTE_CGPT_WEB_IMAGE_TIMEOUT_MS);
...
function makeImageResolver(ctx: ResolverContext): ImageResolver {
  ...
  return async (assetPointer, conversationId, parentMessageId) => {
    ...
    if (fileId) {
      ...
      // DevilRoute's short-lived image cache.
      //
    ...
    if (signedUrl) {
      ...
      // so we materialize the bytes into our own cache and emit an DevilRoute
