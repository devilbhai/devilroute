...
function resolveKimiProtocol(
  ...
  const targetFormat = credentials?.providerSpecificData?._devilrouteKimiTargetFormat;
...
function getThinkingPolicy(credentials: ProviderCredentials): KimiThinkingPolicy {
  return (asRecord(credentials.providerSpecificData?._devilrouteKimiThinking) ||
