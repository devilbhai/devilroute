...
// (bin/devilroute.mjs, proxyFetch.ts, proxyDispatcher.ts) where redirecting DATA_DIR
...
// but every process resolves DATA_DIR to the SAME default (~/.devilroute) when the env
...
if (!process.env.DATA_DIR) {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "devilroute-test-"));
...
process.env.DEVILROUTE_SKIP_SYSTEM_TRUST = "1";
...
process.env.DEVILROUTE_SKIP_DNS_WRITE = "1";
