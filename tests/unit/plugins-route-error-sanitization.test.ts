 ...
 *   2. Import and use `buildErrorBody` from `@devilroute/open-sse/utils/error`.
 *
...
for (const { rel, label } of PLUGIN_ROUTES) {
  ...
  test(`${label}: imports buildErrorBody from @devilroute/open-sse/utils/error`, () => {
    const src = readRoute(rel);
    assert.match(
      src,
      /import \{[^}]*buildErrorBody[^}]*\} from ["']@devilroute\/open-sse\/utils\/error["']/,
      `${rel}: must import buildErrorBody from @devilroute/open-sse/utils/error`
    );
  });
