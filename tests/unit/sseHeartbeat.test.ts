...
function withEnv(value: string | undefined, fn: () => void) {
  const prev = process.env.DEVILROUTE_SSE_COMMENTS;
  try {
    if (value === undefined) delete process.env.DEVILROUTE_SSE_COMMENTS;
    else process.env.DEVILROUTE_SSE_COMMENTS = value;
    fn();
  } finally {
    if (prev === undefined) delete process.env.DEVILROUTE_SSE_COMMENTS;
    else process.env.DEVILROUTE_SSE_COMMENTS = prev;
  }
}
...
test("createSseHeartbeatTransform suppresses COMMENT heartbeats when DEVILROUTE_SSE_COMMENTS=off", async () => {
  const prev = process.env.DEVILROUTE_SSE_COMMENTS;
  process.env.DEVILROUTE_SSE_COMMENTS = "off";
  try {
