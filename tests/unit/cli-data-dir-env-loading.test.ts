...
const BIN = path.join(ROOT, "bin", "devilroute.mjs");
...
test("CLI data-dir resolver preserves an existing legacy ~/.devilroute before XDG/app data", async () => {
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "devilroute-cli-data-dir-"));
  const home = path.join(tmp, "home");
  const legacyDir = path.join(home, ".devilroute");
...
test("CLI startup loads later non-conflicting .env files without overriding earlier values", () => {
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "devilroute-cli-env-layers-"));
  const home = path.join(tmp, "home");
  const dataDir = path.join(tmp, "data");
  const cwd = path.join(tmp, "cwd");
  const appDataDir =
    process.platform === "win32"
      ? path.join(tmp, "appdata", "devilroute")
      : path.join(home, ".devilroute");
  ...
  try {
    ...
    fs.writeFileSync(
      ...
      "DEVILROUTE_BASE_URL=https://data.example/v1\n",
      "utf-8"
    );
    fs.writeFileSync(
      path.join(appDataDir, ".env"),
      ["DEVILROUTE_BASE_URL=https://appdata.example/v1", "DEVILROUTE_HTTP_TIMEOUT_MS=1234", ""].join(
        "\n"
      ),
      "utf-8"
    );
    fs.writeFileSync(
      path.join(cwd, ".env"),
      ["DEVILROUTE_BASE_URL=https://cwd.example/v1", "PORT=34567", ""].join("\n"),
      "utf-8"
    );
    ...
    const env = {
      ...
      DEVILROUTE_NO_UPDATE_NOTIFIER: "1",
    };

    const current = readCliEnvShow(env, cwd);
    assert.equal(current.DEVILROUTE_BASE_URL, "https://data.example/v1");
    assert.equal(current.DEVILROUTE_HTTP_TIMEOUT_MS, "1234");
