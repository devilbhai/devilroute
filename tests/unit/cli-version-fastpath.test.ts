...
const argv = (...args: string[]) => ["node", "devilroute", ...args];
...
test("devilroute CLI --version fast-path prints ONLY the version, skipping bootstrap output", async () => {
  const pkg = JSON.parse(
    readFileSync(join(process.cwd(), "package.json"), "utf8")
  ) as { version: string };

  const { stdout } = await execFileAsync(process.execPath, ["bin/devilroute.mjs", "--version"], {
    cwd: process.cwd(),
