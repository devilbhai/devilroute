...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "devilroute-model-sync-"));
...
test("modelSyncScheduler resolves only loopback origins and uses the dashboard port", async () => {
  const previous = {
    DEVILROUTE_PORT: process.env.DEVILROUTE_PORT,
    PORT: process.env.PORT,
    DASHBOARD_PORT: process.env.DASHBOARD_PORT,
    BASE_URL: process.env.BASE_URL,
    NEXT_PUBLIC_BASE_URL: process.env.NEXT_PUBLIC_BASE_URL,
    NEXT_PUBLIC_APP_URL: process.env.NEXT_PUBLIC_APP_URL,
    DEVILROUTE_BASE_PATH: process.env.DEVILROUTE_BASE_PATH,
    DEVILROUTE_INTERNAL_SCHEME: process.env.DEVILROUTE_INTERNAL_SCHEME,
    DEVILROUTE_TLS_CERT: process.env.DEVILROUTE_TLS_CERT,
    DEVILROUTE_TLS_KEY: process.env.DEVILROUTE_TLS_KEY,
  };
  process.env.DEVILROUTE_PORT = "20128";
  process.env.PORT = "22128";
  process.env.DASHBOARD_PORT = "22128";
  process.env.BASE_URL = "https://attacker.example";
  delete process.env.NEXT_PUBLIC_BASE_URL;
  delete process.env.NEXT_PUBLIC_APP_URL;
  process.env.DEVILROUTE_BASE_PATH = "/devilroute/";
  delete process.env.DEVILROUTE_INTERNAL_SCHEME;
  delete process.env.DEVILROUTE_TLS_CERT;
  delete process.env.DEVILROUTE_TLS_KEY;

  try {
    const scheduler = await loadScheduler("trusted-loopback-origin");
    assert.equal(scheduler.getModelSyncInternalBaseUrl(), "http://127.0.0.1:22128/devilroute");
    assert.equal(
      scheduler.resolveModelSyncInternalBaseUrl("https://attacker.example/steal"),
      "http://127.0.0.1:22128/devilroute"
    );
    assert.equal(
...
test("modelSyncScheduler uses the listener-declared TLS scheme without trusting candidates", async () => {
  ...
  delete process.env.NEXT_PUBLIC_APP_URL;
  process.env.DEVILROUTE_BASE_PATH = "/devilroute";
  process.env.DEVILROUTE_INTERNAL_SCHEME = "https";
  delete process.env.DEVILROUTE_TLS_CERT;
  delete process.env.DEVILROUTE_TLS_KEY;
  ...
  try {
    ...
    assert.equal(
      ...
      "https://localhost:22128/devilroute"
    );
    assert.equal(
...
test("runtime launchers publish the actual internal listener scheme", () => {
  ...
  assert.match(runNext, /DEVILROUTE_INTERNAL_SCHEME\s*=\s*["']http["']/);
  assert.match(standalone, /DEVILROUTE_INTERNAL_SCHEME\s*=\s*tlsOptions\s*\?\s*["']https["']/);
});
...
test("initCloudSync skips auto initialization during build and test processes unless explicitly re-enabled", () => {
  ...
  assert.equal(
    initCloudSync.shouldSkipCloudSyncInitialization(
      {
        NODE_ENV: "test",
        DEVILROUTE_ENABLE_RUNTIME_BACKGROUND_TASKS: "1",
      },
      ["node", "--test"]
    ),
    false
