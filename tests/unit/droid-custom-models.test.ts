...
test("buildDroidCustomModels emits one entry per model with sequential ids", () => {
  ...
  assert.equal(out[0].id, "custom:DevilRoute-0");
  ...
  assert.equal(out[0].apiKey, "sk_devilroute");
  assert.equal(out[1].id, "custom:DevilRoute-1");
...
test("buildDroidCustomModels promotes activeModel to index 0", () => {
  ...
  assert.equal(out[2].id, "custom:DevilRoute-2");
...
test("isDevilRouteCustomModel matches any custom:DevilRoute-<i> id (multi-model)", () => {
  assert.equal(isDevilRouteCustomModel({ id: "custom:DevilRoute-0" }), true);
  assert.equal(isDevilRouteCustomModel({ id: "custom:DevilRoute-1" }), true);
  assert.equal(isDevilRouteCustomModel({ id: "custom:DevilRoute-42" }), true);
  assert.equal(isDevilRouteCustomModel({ id: "custom:Other-0" }), false);
  assert.equal(isDevilRouteCustomModel({ id: 42 as unknown }), false);
  assert.equal(isDevilRouteCustomModel(null), false);
  assert.equal(isDevilRouteCustomModel(undefined), false);
  assert.equal(isDevilRouteCustomModel({}), false);
});
