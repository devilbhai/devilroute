...
const DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "devilroute-ca-datadir-"));
...
// A throwaway self-signed cert (CN=DevilRoute Test CA), valid to 2036.
...
const dir = fs.mkdtempSync(path.join(os.tmpdir(), "devilroute-ca-test-"));
...
test("#3488 valid PEM cert → 200 ok with subject", async () => {
  ...
  assert.match(json.subject, /DevilRoute Test CA/);
});
