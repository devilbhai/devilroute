...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "devilroute-command-code-auth-"));
...
test("Command Code auth assist keeps auth URL callback on CLI localhost contract", async () => {
  process.env.DEVILROUTE_PUBLIC_BASE_URL = "https://devilroute.example.com/base-path";
