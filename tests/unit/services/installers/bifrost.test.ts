...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "devilroute-bifrost-installer-"));
const FAKE_BIN_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "devilroute-bifrost-fake-bin-"));
...
test("install creates host package.json structure", async () => {
  ...
  assert.equal(parsedHost.name, "devilroute-bifrost-host");
