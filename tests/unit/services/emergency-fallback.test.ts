...
const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "devilroute-emergency-fallback-"));
...
function resetTestState() {
  ...
  delete process.env.DEVILROUTE_EMERGENCY_FALLBACK;
  resetEmergencyFallbackEnvCache();
...
function withEnv(value: string | undefined, fn: () => void) {
  const previous = process.env.DEVILROUTE_EMERGENCY_FALLBACK;
  if (value === undefined) {
    delete process.env.DEVILROUTE_EMERGENCY_FALLBACK;
  } else {
    process.env.DEVILROUTE_EMERGENCY_FALLBACK = value;
  }
  resetEmergencyFallbackEnvCache();
  try {
    fn();
  } finally {
    restoreEnv("DEVILROUTE_EMERGENCY_FALLBACK", previous);
    resetEmergencyFallbackEnvCache();
  }
}
...
function withFeatureFlagOverride(value: string, fn: () => void) {
  try {
    setFeatureFlagOverride("DEVILROUTE_EMERGENCY_FALLBACK", value);
    resetEmergencyFallbackEnvCache();
    fn();
  } finally {
    removeFeatureFlagOverride("DEVILROUTE_EMERGENCY_FALLBACK");
    resetEmergencyFallbackEnvCache();
  }
}
...
test("DEVILROUTE_EMERGENCY_FALLBACK=false disables the 402 redirect", () => {
  withEnv("false", () => {
    assert.equal(isEmergencyFallbackEnvEnabled(), false);
    const decision = shouldUseFallback(402, "", false);
    assert.equal(decision.shouldFallback, false);
    assert.match(decision.reason, /DEVILROUTE_EMERGENCY_FALLBACK/);
  });
});

test("DEVILROUTE_EMERGENCY_FALLBACK=0 disables the budget-keyword redirect", () => {
  withEnv("0", () => {
