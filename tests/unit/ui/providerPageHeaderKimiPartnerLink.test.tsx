...
describe("ProviderPageHeader — Kimi partner-link note", () => {
  ...
  it.each([
    ["moonshot", "Kimi", "https://platform.kimi.ai?aff=devilroute"],
    ["kimi-coding", "Kimi Code CLI", "https://www.kimi.com/code?aff=devilroute"],
    ["kimi-web", "Kimi Web", "https://www.kimi.com/code?aff=devilroute"],
    ...
    expect(link?.getAttribute("title")).toBe(
      "Partner link — supports DevilRoute at no extra cost to you"
    );
    expect(link?.getAttribute("aria-label")).toBe(
      `${name} — Partner link — supports DevilRoute at no extra cost to you`
    );
    expect(el.textContent).toContain("Partner link — supports DevilRoute at no extra cost to you");
  });
