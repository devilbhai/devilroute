...
test("isCodexResponsesWebSocketRequired: DEVILROUTE_CODEX_WS_ENABLED=false forces HTTP even with codexTransport=websocket", () => {
  ...
  const prev = process.env.DEVILROUTE_CODEX_WS_ENABLED;
  process.env.DEVILROUTE_CODEX_WS_ENABLED = "false";
  try {
    ...
    if (prev === undefined) delete process.env.DEVILROUTE_CODEX_WS_ENABLED;
    else process.env.DEVILROUTE_CODEX_WS_ENABLED = prev;
    __setCodexWebSocketTransportForTesting(undefined);
  }
});
...
test("CodexExecutor.transformRequest preserves store-enabled responses state when explicitly enabled", () => {
  ...
  const body = {
    _nativeCodexPassthrough: true,
    _devilrouteResponsesStore: true,
    instructions: "keep this",
  ...
  assert.equal(result._devilrouteResponsesStore, undefined);
...
test("Codex internal websocket bridge rejects non-object JSON payloads", async () => {
  await withEnv({ DEVILROUTE_WS_BRIDGE_SECRET: "bridge-secret" }, async () => {
    const { POST } = await import("../../src/app/api/internal/codex-responses-ws/route.ts");

    const response = await POST(
      new Request("http://devilroute.local/api/internal/codex-responses-ws", {
        method: "POST",
        headers: {
          "content-type": "application/json",
          "x-devilroute-ws-bridge-secret": "bridge-secret",
        },
