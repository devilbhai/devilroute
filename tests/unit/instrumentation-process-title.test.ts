...
test("renameProcessTitle renames a bare 'next-server' title to 'devilroute'", () => {
  assert.equal(renameProcessTitle("next-server"), "devilroute");
});

test("renameProcessTitle preserves a version suffix after 'next-server'", () => {
  assert.equal(renameProcessTitle("next-server (v16.2.9)"), "devilroute (v16.2.9)");
});
...
test("renameProcessTitle is idempotent when called again on an already-renamed title", () => {
  ...
  assert.equal(once, "devilroute (v16.2.9)");
  assert.equal(twice, "devilroute (v16.2.9)");
});
