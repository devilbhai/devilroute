 ...
 * `Cannot find package '@/shared'` when DevilRoute is installed globally
 * (issue #7791).
 *
...
describe("aliasResolver.resolveAlias — @devilroute/open-sse aliases", () => {
  it("exposes ALIAS_MAP with three entries matching tsconfig paths", () => {
    ...
    // @devilroute/open-sse/ (subpath)
    assert.equal(ALIAS_MAP[1].prefix, "@devilroute/open-sse/");
    assert.equal(ALIAS_MAP[1].target, "open-sse");
    assert.equal(ALIAS_MAP[1].exact, false);
    // @devilroute/open-sse (exact package name)
    assert.equal(ALIAS_MAP[2].prefix, "@devilroute/open-sse");
    assert.equal(ALIAS_MAP[2].target, "open-sse");
    assert.equal(ALIAS_MAP[2].exact, true);
  });

  it("resolves @devilroute/open-sse (bare) to open-sse/index.ts", () => {
    const got = resolveAlias("@devilroute/open-sse", REPO_ROOT);
    assert.ok(got, "expected non-null URL for @devilroute/open-sse");
    assert.ok(got.startsWith("file://"), "must be a file URL");
    const fsPath = fileURLToPath(got);
    assert.ok(
  ...
  it("resolves @devilroute/open-sse/services/usage to open-sse/services/usage.ts", () => {
    const got = resolveAlias("@devilroute/open-sse/services/usage", REPO_ROOT);
  ...
  it("resolves @devilroute/open-sse/utils/proxyFetch to open-sse/utils/proxyFetch.ts", () => {
    const got = resolveAlias("@devilroute/open-sse/utils/proxyFetch", REPO_ROOT);
    assert.ok(got, "expected non-null URL");
    const fsPath = fileURLToPath(got);
    assert.ok(
  ...
  it("returns null for non-existent @devilroute/open-sse/* paths", () => {
    assert.equal(resolveAlias("@devilroute/open-sse/does/not/exist", REPO_ROOT), null);
  });

  it("returns null for @devilroute/other (unmatched scope)", () => {
    assert.equal(resolveAlias("@devilroute/other", REPO_ROOT), null);
    assert.equal(resolveAlias("@devilroute/other/pkg", REPO_ROOT), null);
  });

  it("rejects path-traversal via @devilroute/open-sse/../../etc/passwd", () => {
    assert.equal(resolveAlias("@devilroute/open-sse/../../etc/passwd", REPO_ROOT), null);
  });
});
...
describe("aliasResolver end-to-end (#7791 regression)", () => {
  function runChild(script) {
    const result = spawnSync(process.execPath, ["--input-type=module", "-e", script], {
      ...
      env: {
        ...
        DEVILROUTE_CLI_SKIP_REPO_ENV: "1",
      },
      encoding: "utf8",
    });
  ...
  it("resolves @/shared/network/outboundUrlGuard from a child process", () => {
    ...
    // bin/devilroute.mjs does (await import("tsx/esm") first, then register).
    const script = `
