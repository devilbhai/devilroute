...
test("buildCodexEnv strips stale OpenAI/Codex creds from the child env (defense-in-depth)", () => {
  ...
  assert.equal(env.DEVILROUTE_API_KEY, "oma_live_x");
...
test("buildCodexEnv uses a no-auth sentinel when no token is given", () => {
  ...
  assert.equal(env.DEVILROUTE_API_KEY, "devilroute-no-auth");
});
...
test("buildCodexProviderArgs defines the devilroute provider inline (works without config.toml)", () => {
  const args = buildCodexProviderArgs("http://vps:20128");
  const joined = args.join(" ");
  assert.ok(joined.includes('model_provider="devilroute"'));
  assert.ok(joined.includes('model_providers.devilroute.base_url="http://vps:20128/v1"'));
  assert.ok(joined.includes('model_providers.devilroute.env_key="DEVILROUTE_API_KEY"'));
  assert.ok(joined.includes('model_providers.devilroute.wire_api="responses"'));
  assert.ok(joined.includes("model_providers.devilroute.requires_openai_auth=false"));
