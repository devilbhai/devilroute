...
test("buildRooImport produces an openai-compatible provider profile (baseUrl /v1, model)", () => {
  ...
  const cfg = d.providerProfiles.apiConfigs.DevilRoute;
  ...
  assert.equal(d.providerProfiles.currentApiConfigName, "DevilRoute");
});
test("buildRooImport falls back to a placeholder key", () => {
  assert.equal(buildRooImport({ baseUrl: "http://x/v1", apiKey: "", model: "m" }).providerProfiles.apiConfigs.DevilRoute.openAiApiKey, "sk_devilroute");
});
test("buildRooVscodeAutoImport sets the pointer, preserving other settings", () => {
  const s = buildRooVscodeAutoImport({ "editor.tabSize": 2 }, "/home/u/.devilroute/roo-settings.json");
  assert.equal(s["editor.tabSize"], 2);
  assert.equal(s["roo-cline.autoImportSettingsPath"], "/home/u/.devilroute/roo-settings.json");
});
