...
test("quoteCodexArgs: exact win32 encoding (golden)", () => {
  const golden: Array<[string, string]> = [
    ["-c", '^^^"-c^^^"'],
    [
      'model_providers.devilroute.base_url="http://localhost:20128/v1"',
      '^^^"model_providers.devilroute.base_url=\\^^^"http://localhost:20128/v1\\^^^"^^^"',
    ],
    [
      "model_providers.devilroute.requires_openai_auth=false",
      '^^^"model_providers.devilroute.requires_openai_auth=false^^^"',
    ],
...
test("quoteCodexArgs does not mutate the caller's array", () => {
  const input = ["-c", "model_provider=\"devilroute\""];
  quoteCodexArgs(input, "win32");
  assert.deepEqual(input, ["-c", 'model_provider="devilroute"']);
});
...
test(
  ...
  async () => {
    const dir = mkdtempSync(join(tmpdir(), "devilroute-codex-argv-"));
    try {
