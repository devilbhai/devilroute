...
test.before(() => {
  tmpDir = mkdtempSync(join(tmpdir(), "devilroute-autostart-linux-"));
...
test("resolveCliPath finds devilroute.mjs from argv", async () => {
  ...
  const unitPath = join(tmpDir, ".config", "systemd", "user", "devilroute.service");
  const desktopPath = join(tmpDir, ".config", "autostart", "devilroute.desktop");
  ...
  if (existsSync(unitPath)) {
    ...
    assert.match(unit, /ExecStart=.*devilroute\.mjs.*serve --no-open/m);
