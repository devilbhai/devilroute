...
test.before(() => {
  tmpDir = mkdtempSync(join(tmpdir(), "devilroute-autostart-win-"));
...
test("Windows enable/disable writes and removes VBS in Startup folder", async () => {
  ...
  const vbsPath = join(startupDir, "DevilRoute.vbs");
  ...
  assert.match(vbs, /serve --no-open --tray/, "launches DevilRoute tray server");
