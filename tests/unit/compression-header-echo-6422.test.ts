/**
 * #6422 — X-DevilRoute-Compression response header echo.
 *
 * Docs promise that when a request supplies `x-devilroute-compression`, the response
 * echoes `X-DevilRoute-Compression: <mode>; source=<source>`. Internal early-returns
...
function makeRequest(headerValue: string | null): {
  ...
  return {
    headers: {
      get(name: string) {
        if (name.toLowerCase() === "x-devilroute-compression") return headerValue;
        return null;
      },
    },
  };
...
describe("compressionHeaderEcho (#6422)", () => {
  ...
  it("echoes the request header value onto the response when missing", () => {
    ...
    assert.equal(wrapped.headers.get("X-DevilRoute-Compression"), "engine:rtk; source=request-header");
    assert.equal(wrapped.status, 200);
  });

  it("normalizes off / default / engine:* to lowercase", () => {
    assert.equal(
      withCompressionHeaderEcho(new Response(""), "OFF").headers.get("X-DevilRoute-Compression"),
      "off; source=request-header"
    );
    assert.equal(
      withCompressionHeaderEcho(new Response(""), "Default").headers.get("X-DevilRoute-Compression"),
  ...
  it("preserves operator casing on named-combo values", () => {
    ...
    assert.equal(wrapped.headers.get("X-DevilRoute-Compression"), "MyCombo; source=request-header");
  });

  it("never overwrites an existing X-DevilRoute-Compression header set by the inner pipeline", () => {
    const inner = new Response("", {
      headers: {
        "X-DevilRoute-Compression": "stacked; source=routing; tokens=100->42; rules: rtk-nl x2",
      },
    });
    const wrapped = withCompressionHeaderEcho(inner, "engine:rtk");
    assert.equal(
      wrapped.headers.get("X-DevilRoute-Compression"),
  ...
  it("is a no-op when the request did not supply the header", () => {
    ...
    assert.equal(wrapped.headers.get("X-DevilRoute-Compression"), null);
