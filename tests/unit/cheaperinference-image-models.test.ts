...
import { IMAGE_PROVIDERS, parseImageModel } from "@devilroute/open-sse/config/imageRegistry.ts";
...
test("cheaperinference image models are NOT in the chat registry", async () => {
  const { REGISTRY } = await import("@devilroute/open-sse/config/providers/index.ts");
