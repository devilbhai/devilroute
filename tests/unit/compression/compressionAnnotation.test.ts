...
describe("formatCompressionAnnotation", () => {
  ...
  it("is ASCII-only so it survives HTTP header (X-DevilRoute-Compression) construction", () => {
    // Regression: the annotation is appended to the X-DevilRoute-Compression response
    ...
    assert.doesNotThrow(() => new Headers({ "X-DevilRoute-Compression": value }));
    const res = new Response(null, { headers: { "X-DevilRoute-Compression": value } });
    assert.equal(res.headers.get("X-DevilRoute-Compression"), value);
  });
  ...
  it("bounds high-cardinality rule telemetry before it reaches an HTTP response header", () => {
    ...
    assert.doesNotThrow(
      () => new Response(null, { headers: { "X-DevilRoute-Compression": annotation } })
    );
  });
