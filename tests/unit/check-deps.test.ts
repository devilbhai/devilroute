...
test("6A.8: discoverManifests finds root and workspace package.json files", () => {
  ...
  assert.ok(
    manifests.includes("@devilroute/opencode-plugin/package.json"),
    "@devilroute/opencode-plugin/package.json must be included"
  );
  assert.ok(
    manifests.includes("@devilroute/opencode-provider/package.json"),
    "@devilroute/opencode-provider/package.json must be included"
  );
});
