...
test("wantsProgress detects X-DevilRoute-Progress header correctly", () => {
  const headersObj = { "x-devilroute-progress": "true" };
  assert.equal(wantsProgress(new Headers(headersObj)), true);

  const headersMap = new Map([["x-devilroute-progress", "true"]]);
  assert.equal(wantsProgress(headersMap), true);

  const headersPlain = { "x-devilroute-progress": "true" };
  assert.equal(wantsProgress(headersPlain), true);

  assert.equal(wantsProgress({ "x-devilroute-progress": "false" }), false);
