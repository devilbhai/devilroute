...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "devilroute-sem-cache-"));
...
const { DEVILROUTE_RESPONSE_HEADERS } = await import("../../src/shared/constants/headers.ts");
const { calculateCost } = await import("../../src/lib/usage/costCalculator.ts");
const { formatDevilRouteCost } = await import("../../src/domain/devilrouteResponseMeta.ts");
...
test("checkSemanticCache returns null when the x-devilroute-no-cache header forces a bypass", async () => {
  // The no-cache header makes isCacheableForRead return false even with temperature:0.
  const { args } = makeBaseArgs({
    semanticCacheEnabled: true,
    body: { model: "gpt-4o", messages: [{ role: "user", content: "hi" }], temperature: 0 },
    clientRawRequest: { headers: { "x-devilroute-no-cache": "true" } },
  });
...
test("checkSemanticCache returns a non-streaming JSON HIT with cache headers + logging side effects", async () => {
  ...
  assert.equal(res.headers.get(DEVILROUTE_RESPONSE_HEADERS.cache), "HIT", "X-DevilRoute-Cache: HIT");
  assert.equal(
    res.headers.get(DEVILROUTE_RESPONSE_HEADERS.cacheHit),
    "true",
...
test("checkSemanticCache returns a streaming SSE HIT (text/event-stream) when stream=true", async () => {
  ...
  assert.equal(res.headers.get(DEVILROUTE_RESPONSE_HEADERS.cache), "HIT");
  const bodyText = await res.text();
...
test("checkSemanticCache HITs even when the cached body has no usage (cost falls back to 0)", async () => {
  ...
  assert.equal(
    res.headers.get(DEVILROUTE_RESPONSE_HEADERS.responseCost),
    "0.0000000000",
...
// X-DevilRoute-Response-Cost must therefore be 0 (so billing consumers don't charge
// for cache hits), while the original cost is surfaced via X-DevilRoute-Cost-Saved.

test("checkSemanticCache HIT bills 0 incremental cost and reports the original cost in X-DevilRoute-Cost-Saved", async () => {
  clearCache();
  ...
  const expectedSaved = formatDevilRouteCost(
  ...
  assert.equal(
    res.headers.get(DEVILROUTE_RESPONSE_HEADERS.costSaved),
    expectedSaved,
    "X-DevilRoute-Cost-Saved reflects the original cost the cache avoided"
  );
});
