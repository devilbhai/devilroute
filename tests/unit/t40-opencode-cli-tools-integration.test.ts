...
test("T40: OpenCode config path resolves per-platform", () => {
  ...
  // Windows (NOT %APPDATA%) — DevilRoute must write where OpenCode reads.
...
test("T40: OpenCode config generator includes endpoint and selected API key", () => {
  ...
  assert.equal(mergedConfig.provider.devilroute.options.baseURL, "http://localhost:20128/v1");
  assert.equal(mergedConfig.provider.devilroute.options.apiKey, "sk_test_opencode");
});
...
test("T40: OpenCode config document uses current provider schema", () => {
  ...
  assert.ok(configDocument.provider.devilroute);
  assert.equal(configDocument.provider.devilroute.npm, "@ai-sdk/openai-compatible");
  assert.equal(configDocument.provider.devilroute.options.baseURL, "http://localhost:20128/v1");
  assert.equal(configDocument.provider.devilroute.options.apiKey, "sk_test_opencode");
  assert.deepEqual(Object.keys(configDocument.provider.devilroute.models), [
    "cc/claude-sonnet-4-20250514",
    "gg/gemini-2.5-pro",
  ]);
  assert.equal(
    configDocument.provider.devilroute.models["cc/claude-sonnet-4-20250514"].name,
    "Claude Sonnet 4.5"
  );
  assert.equal(
    configDocument.provider.devilroute.models["gg/gemini-2.5-pro"].name,
    "Gemini 2.5 Pro"
  );
...
test("T40: OpenCode merge preserves unrelated config and updates only provider.devilroute", () => {
  const mergedConfig = mergeOpenCodeConfig(
    {
  ...
  assert.deepEqual(mergedConfig.provider.devilroute.models, {
