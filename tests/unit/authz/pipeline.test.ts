...
const ORIGINAL_DEVILROUTE_PUBLIC_BASE_URL = process.env.DEVILROUTE_PUBLIC_BASE_URL;
...
const ORIGINAL_DEVILROUTE_TRUST_PROXY = process.env.DEVILROUTE_TRUST_PROXY;
const ORIGINAL_DEVILROUTE_PEER_STAMP_TOKEN = process.env.DEVILROUTE_PEER_STAMP_TOKEN;
...
function resetEnvironment() {
  ...
  delete process.env.DEVILROUTE_PUBLIC_BASE_URL;
  delete process.env.NEXT_PUBLIC_BASE_URL;
  delete process.env.NEXT_PUBLIC_APP_URL;
  delete process.env.DEVILROUTE_TRUST_PROXY;
  delete process.env.DEVILROUTE_PEER_STAMP_TOKEN;
  globalThis.__devilrouteShutdown = { init: false, shuttingDown: false, activeRequests: 0 };
}
...
test.after(() => {
  ...
  if (ORIGINAL_DEVILROUTE_PUBLIC_BASE_URL === undefined)
    delete process.env.DEVILROUTE_PUBLIC_BASE_URL;
  else process.env.DEVILROUTE_PUBLIC_BASE_URL = ORIGINAL_DEVILROUTE_PUBLIC_BASE_URL;
  ...
  if (ORIGINAL_DEVILROUTE_TRUST_PROXY === undefined) delete process.env.DEVILROUTE_TRUST_PROXY;
  else process.env.DEVILROUTE_TRUST_PROXY = ORIGINAL_DEVILROUTE_TRUST_PROXY;
  if (ORIGINAL_DEVILROUTE_PEER_STAMP_TOKEN === undefined) {
    delete process.env.DEVILROUTE_PEER_STAMP_TOKEN;
  } else {
    process.env.DEVILROUTE_PEER_STAMP_TOKEN = ORIGINAL_DEVILROUTE_PEER_STAMP_TOKEN;
  }
  globalThis.__devilrouteShutdown = { init: false, shuttingDown: false, activeRequests: 0 };
});
...
test("runAuthzPipeline redirects unauthenticated dashboard pages to login", async () => {
  ...
  assert.equal(response.headers.get("x-devilroute-route-class"), "MANAGEMENT");
...
// DEVILROUTE_BASE_PATH. Next.js strips the basePath from nextUrl.pathname
...
test("runAuthzPipeline prefixes the root-to-dashboard redirect with basePath when set", async () => {
  ...
  const req = new NextRequest("http://localhost/devilroute/", {
    nextConfig: { basePath: "/devilroute" },
  });

  const response = await pipeline.runAuthzPipeline(req, { enforce: true });

  assert.equal(response.status, 307);
  assert.equal(response.headers.get("location"), "http://localhost/devilroute/dashboard");
});

test("runAuthzPipeline prefixes the dashboard login redirect with basePath when set", async () => {
  await forceAuthRequired();

  const req = new NextRequest("http://localhost/devilroute/dashboard", {
    nextConfig: { basePath: "/devilroute" },
  });
  ...
  assert.equal(response.status, 307);
  assert.equal(response.headers.get("location"), "http://localhost/devilroute/login");
  assert.equal(response.headers.get("x-devilroute-route-class"), "MANAGEMENT");
});
...
test("runAuthzPipeline allows onboarding when login is required but no password exists", async () => {
  ...
  assert.equal(response.headers.get("x-devilroute-route-class"), "PUBLIC");
});
...
test("runAuthzPipeline rejects oversized API bodies before auth", async () => {
  ...
  assert.equal(response.headers.get("x-devilroute-route-class"), "CLIENT_API");
  assert.ok(response.headers.get("x-request-id"));
  assert.equal(
...
test("runAuthzPipeline rejects new API requests during shutdown drain", async () => {
  globalThis.__devilrouteShutdown = { init: true, shuttingDown: true, activeRequests: 0 };
...
test("runAuthzPipeline accepts dashboard mutations from configured public origin", async () => {
  ...
  const response = await pipeline.runAuthzPipeline(
    request("http://devilroute:20128/api/providers/health-autopilot/actions", {
      method: "POST",
      headers: {
...
test("runAuthzPipeline rejects dashboard mutations from invalid browser origin", async () => {
  ...
  assert.match(body.error.message, /DEVILROUTE_PUBLIC_BASE_URL/);
});
