...
import { resolveAutoStrategyOrder } from "@devilroute/open-sse/services/combo/resolveAutoStrategy.ts";
...
// also supports a per-request `X-DevilRoute-Mode` override (relayOptions.mode) that can
...
test("per-request X-DevilRoute-Mode override changes the EFFECTIVE weights used for fallback ranking, not just selection", async () => {
