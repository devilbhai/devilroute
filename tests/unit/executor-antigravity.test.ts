...
test("AntigravityExecutor.buildHeaders includes native headers without DevilRoute internals", () => {
  const executor = new AntigravityExecutor();
  ...
  assert.equal(headers["X-DevilRoute-Source"], undefined);
});

test("Antigravity header scrub removes DevilRoute internal headers", () => {
  const headers = scrubProxyAndFingerprintHeaders({
    Authorization: "Bearer ag-token",
    "X-DevilRoute-Source": "devilroute",
    "X-DevilRoute-No-Cache": "true",
    "X-Forwarded-For": "127.0.0.1",
  });

  assert.equal(headers.Authorization, "Bearer ag-token");
  assert.equal(headers["X-DevilRoute-Source"], undefined);
  assert.equal(headers["X-DevilRoute-No-Cache"], undefined);
...
test("AntigravityExecutor.transformRequest allows body project overrides when the env flag is enabled", async () => {
  ...
  await withEnv("DEVILROUTE_ALLOW_BODY_PROJECT_OVERRIDE", "1", async () => {
    const result = await executor.transformRequest(
      "antigravity/gemini-2.5-pro",
      {
        project: "body-project",
