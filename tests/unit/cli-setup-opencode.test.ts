 ...
 *
 * `devilroute setup opencode` wires the bundled @devilroute/opencode-plugin into a
 * local OpenCode install: copies the built plugin into `<config>/plugins/devilroute/`
 * and registers a tuple entry in `opencode.json` (idempotent, replacing the legacy
 * `opencode-devilroute-auth` entry from issue #3711).
 *
 * The command resolves the bundled plugin at module load, so the
 * DEVILROUTE_OPENCODE_PLUGIN_DIR fixture override MUST be set before the import.
...
process.env.DEVILROUTE_OPENCODE_PLUGIN_DIR = FAKE_PLUGIN_DIR;
...
function makeFakePluginDist() {
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({ name: "@devilroute/opencode-plugin", version: "0.0.0-test" })
  );
...
describe("devilroute setup opencode", () => {
  before(() => {
    console.log = () => {};
    console.info = () => {};
    console.warn = () => {};
  ...
  it("installs the plugin and registers a tuple entry honouring --base-url (camelCased by Commander)", async () => {
    ...
    assert.ok(fs.existsSync(path.join(CONFIG_DIR, "plugins", "devilroute", "dist", "index.js")));
    assert.ok(fs.existsSync(path.join(CONFIG_DIR, "plugins", "devilroute", "package.json")));

    const cfg = readConfig();
    assert.ok(Array.isArray(cfg.plugin));
    assert.equal(cfg.plugin.length, 1);
    const [modulePath, options] = cfg.plugin[0];
    assert.equal(modulePath, "./plugins/devilroute/dist/index.js");
    assert.equal(options.providerId, "devilroute");
    assert.equal(
      options.baseURL,
      "http://10.0.0.5:20128",
  ...
  it("is idempotent: re-running updates the entry in place instead of duplicating it", async () => {
    ...
    const omniEntries = cfg.plugin.filter(
      (p: unknown) =>
        Array.isArray(p) && (p[1] as { providerId?: string })?.providerId === "devilroute"
    );
  ...
  it("removes the legacy opencode-devilroute-auth entry (#3711) and preserves unrelated plugins", async () => {
    const cfgPath = path.join(CONFIG_DIR, "opencode.json");
    fs.writeFileSync(
      cfgPath,
      JSON.stringify({
        plugin: [
          "opencode-devilroute-auth",
    ...
    assert.ok(!flat.includes("opencode-devilroute-auth"), "legacy entry must be dropped");
    assert.ok(flat.includes('"providerId":"other"'), "unrelated plugin entries must survive");
    assert.equal(cfg.plugin.length, 2, "other + devilroute");
  });
