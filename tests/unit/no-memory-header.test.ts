...
// `x-devilroute-no-memory` header (mirrors `x-devilroute-no-cache`).
...
test("isNoMemoryRequested is true for truthy header values (plain object)", () => {
  for (const v of ["true", "1", "yes", "TRUE", "Yes", " true "]) {
    assert.equal(
      isNoMemoryRequested({ "x-devilroute-no-memory": v }),
      true,
...
test("isNoMemoryRequested is case-insensitive on the header NAME", () => {
  assert.equal(isNoMemoryRequested({ "X-DevilRoute-No-Memory": "true" }), true);
});

test("isNoMemoryRequested works with a Headers instance", () => {
  const h = new Headers();
  h.set("x-devilroute-no-memory", "1");
...
test("isNoMemoryRequested is false when the header is absent / empty / falsy", () => {
  ...
  assert.equal(isNoMemoryRequested({ "x-devilroute-no-memory": "" }), false);
  assert.equal(isNoMemoryRequested({ "x-devilroute-no-memory": "false" }), false);
  assert.equal(isNoMemoryRequested({ "x-devilroute-no-memory": "0" }), false);
  assert.equal(isNoMemoryRequested({ "x-devilroute-no-memory": "no" }), false);
});

test("getHeaderValueCaseInsensitive still resolves the header (sanity)", () => {
  assert.equal(
    getHeaderValueCaseInsensitive({ "x-devilroute-no-memory": "true" }, "x-devilroute-no-memory"),
    "true"
  );
});
