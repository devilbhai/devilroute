...
const DUMMY_HOME = path.join(os.tmpdir(), "devilroute-guide-settings-test-" + Date.now());
...
type HermesConfig = {
  ...
  providers?: { devilroute?: { base_url?: string; api_key?: string } };
};
...
test("guide-settings POST creates new hermes config.yaml if it doesn't exist", async () => {
  ...
  assert.equal(content.model?.provider, "devilroute");
  assert.equal(content.model?.base_url, "http://my-omni/v1");
  assert.equal(content.providers?.devilroute?.base_url, "http://my-omni/v1");
  assert.ok(String(content.providers?.devilroute?.api_key || "").startsWith("sk-"));
});
...
test("guide-settings POST writes OpenCode config with current schema and multi-model selection", async () => {
  ...
  assert.equal(content.provider.devilroute.npm, "@ai-sdk/openai-compatible");
  assert.equal(content.provider.devilroute.options.baseURL, "http://my-omni/v1");
  assert.ok(content.provider.devilroute.options.apiKey.startsWith("sk-"));
  assert.deepEqual(Object.keys(content.provider.devilroute.models), [
...
test("guide-settings POST preserves existing OpenCode config fields while only updating provider.devilroute", async () => {
  await fs.mkdir(path.dirname(OPENCODE_CONFIG_PATH), { recursive: true });
  await fs.writeFile(
    OPENCODE_CONFIG_PATH,
    `{
  ...
  assert.equal(content.provider.devilroute.options.baseURL, "http://my-omni/v1");
  assert.ok(content.provider.devilroute.options.apiKey.startsWith("sk-"));
  assert.deepEqual(content.provider.devilroute.models, {
