...
test("#7357: async image_gen pointer delivered via update_content.messages[] should resolve to markdown (currently lost → 502)", async () => {
  ...
  const previousTimeout = process.env.DEVILROUTE_CGPT_WEB_IMAGE_TIMEOUT_MS;
  process.env.DEVILROUTE_CGPT_WEB_IMAGE_TIMEOUT_MS = "300"; // keep the probe fast
  ...
  try {
    ...
    // update_content.messages[] (plural) shape. DevilRoute should extract it,
    ...
    if (previousTimeout === undefined) delete process.env.DEVILROUTE_CGPT_WEB_IMAGE_TIMEOUT_MS;
    else process.env.DEVILROUTE_CGPT_WEB_IMAGE_TIMEOUT_MS = previousTimeout;
  }
});
...
test(
  ...
  async () => {
    ...
    process.env.DEVILROUTE_CGPT_WEB_IMAGE_TIMEOUT_MS = "300";
