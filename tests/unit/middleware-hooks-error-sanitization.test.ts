 ...
 * "SQLITE_ERROR: near /var/lib/devilroute/..."). Guard against
...
for (const path of ROUTES) {
  test(`${path} imports sanitizeErrorMessage from open-sse/utils/error`, () => {
    ...
    assert.ok(
      /import\s*\{[^}]*\bsanitizeErrorMessage\b[^}]*\}\s*from\s*["']@devilroute\/open-sse\/utils\/error["']/.test(
        source
      ),
      `expected ${path} to import sanitizeErrorMessage from "@devilroute/open-sse/utils/error"`
    );
  });
