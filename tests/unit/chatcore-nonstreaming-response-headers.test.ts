...
// #3501). attachDevilRouteMetaHeaders + now are injected so the static headers, the meta payload,
...
function makeDeps(now = 1000) {
  ...
  const deps = {
    attachDevilRouteMetaHeaders: (headers: Record<string, string>, meta: Record<string, unknown>) => {
      metaCalls.push({ headers, meta });
      headers["x-devilroute-meta"] = "attached";
    },
    now: () => now,
