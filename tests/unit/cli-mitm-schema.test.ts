...
// Regression test: null apiKey + unresolvable keyId must yield the sentinel 'sk_devilroute',
...
test("sentinel guard condition catches sk_devilroute and null", () => {
  const SENTINEL = "sk_devilroute";
  // Simulate what the route guard checks: (!apiKey || apiKey === 'sk_devilroute')
  ...
  assert.equal(shouldReject("sk_devilroute"), true, "sentinel must be rejected");
