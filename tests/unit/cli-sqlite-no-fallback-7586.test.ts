...
// #7586: `devilroute doctor`'s "Database" and "Storage/encryption" checks call
...
test("#7586: readDatabaseHealth() falls back to node:sqlite when better-sqlite3 is unavailable", async (t) => {
  ...
  seed.exec(
    "CREATE TABLE _devilroute_migrations (version TEXT PRIMARY KEY, name TEXT NOT NULL, applied_at TEXT);" +
      "INSERT INTO _devilroute_migrations (version, name, applied_at) VALUES ('001', 'initial_schema', datetime('now'));"
  );
  seed.close();

  // Exercise the CLI helper's own health check — the exact function
  // `devilroute doctor` calls (bin/cli/commands/doctor.mjs:checkDatabase ->
