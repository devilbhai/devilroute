...
// forwards. On the stdio transport `devilroute_ccr_retrieve` therefore fell back
...
// the registered devilroute_ccr_retrieve handler with an `extra` carrying that
...
test("static tool loops forward `extra` so stdio callers keep their scope/identity (#6178)", async () => {
  ...
  const retrieve = getRegisteredHandler(server, "devilroute_ccr_retrieve");
