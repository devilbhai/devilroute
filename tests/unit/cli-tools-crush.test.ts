...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "devilroute-crush-settings-"));
...
test("crush-settings POST: writes crush.json with an openai-compat providers.devilroute block", async () => {
  ...
  try {
    ...
    if (res.status === 200) {
      ...
      if (fs.existsSync(configPath)) {
        ...
        const provider = written.providers?.devilroute;
        assert.ok(provider, "providers.devilroute must be written");
...
test("crush-settings DELETE: removes only the devilroute provider entry", async () => {
  ...
  try {
      ...
      JSON.stringify({
        providers: {
          devilroute: {
            ...
            api_key: "sk-test",
            models: [
              { id: "openai/gpt-5", name: "DevilRoute: openai/gpt-5", context_window: 128000 },
            ],
          },
    ...
    if (res.status === 200) {
      ...
      if (fs.existsSync(configPath)) {
        const written = JSON.parse(fs.readFileSync(configPath, "utf-8"));
        assert.equal(written.providers?.devilroute, undefined);
