...
test("C2 header contract — server.cjs intercept must inject x-devilroute-source and x-devilroute-agent", async () => {
  ...
  assert.match(
    src,
    /"x-devilroute-source":\s*"agent-bridge"/,
    'server.cjs must inject "x-devilroute-source: agent-bridge"'
  );
  assert.match(
    src,
    /"x-devilroute-agent":\s*agentId/,
    'server.cjs must inject "x-devilroute-agent: <id>" from the host→agent map'
  );
