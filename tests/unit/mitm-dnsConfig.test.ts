/**
 * Unit tests: DEVILROUTE_SKIP_DNS_WRITE guard on addDNSEntries / removeDNSEntries.
 *
 ...
 * DEVILROUTE_SKIP_DNS_WRITE=1 by default, so the guard tests are inherently
 ...
 * installed (slim containers) or under `DEVILROUTE_NO_SUDO` (#6122) — so the
...
function guardEnv(value: string | undefined): () => void {
  const prev = process.env.DEVILROUTE_SKIP_DNS_WRITE;
  if (value === undefined) {
    delete process.env.DEVILROUTE_SKIP_DNS_WRITE;
  } else {
    process.env.DEVILROUTE_SKIP_DNS_WRITE = value;
  }
  return () => {
    if (prev === undefined) delete process.env.DEVILROUTE_SKIP_DNS_WRITE;
    else process.env.DEVILROUTE_SKIP_DNS_WRITE = prev;
  };
}
...
test("addDNSEntries: returns early when DEVILROUTE_SKIP_DNS_WRITE=1", async () => {
  resetSpawnCalls();
...
test("addDNSEntries: proceeds when DEVILROUTE_SKIP_DNS_WRITE=0", async () => {
  resetSpawnCalls();
  const { addDNSEntries } = await importDnsConfig();
...
test("removeDNSEntries: returns early when DEVILROUTE_SKIP_DNS_WRITE=1", async () => {
  resetSpawnCalls();
...
test("removeDNSEntries: proceeds when DEVILROUTE_SKIP_DNS_WRITE=0", async () => {
  resetSpawnCalls();
  const { removeDNSEntries } = await importDnsConfig();
  const restore = guardEnv("0");
  try {
