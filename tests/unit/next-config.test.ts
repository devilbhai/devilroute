...
test("next config declares Turbopack aliases, runtime assets and server externals", async () => {
  ...
  // #6344: the @/mitm/manager stub alias is OPT-IN (DEVILROUTE_MITM_STUB=1, Docker only).
...
test("Turbopack aliases @/mitm/manager to the stub ONLY when DEVILROUTE_MITM_STUB=1 (#6344)", async () => {
  const original = process.env.DEVILROUTE_MITM_STUB;
  try {
    delete process.env.DEVILROUTE_MITM_STUB;
    const { default: def } = await loadNextConfig("mitm-default");
    assert.equal(def.turbopack.resolveAlias["@/mitm/manager"], undefined);

    process.env.DEVILROUTE_MITM_STUB = "1";
    ...
    if (original === undefined) delete process.env.DEVILROUTE_MITM_STUB;
    else process.env.DEVILROUTE_MITM_STUB = original;
  }
});
...
test("optimizePackageImports excludes the internal @devilroute/open-sse workspace (build-OOM guard)", async () => {
  // Regression guard: adding the internal `@devilroute/open-sse` workspace to
  ...
  assert.ok(
    !list.includes("@devilroute/open-sse"),
    "do NOT add the internal @devilroute/open-sse workspace to optimizePackageImports — it OOMs the production build"
  );
