...
test("generateMitmCa produces a CA certificate (basicConstraints CA, key+cert PEM)", async () => {
  const ca = await generateMitmCa("DevilRoute MITM CA (test)");
  ...
  assert.match(x.subject, /DevilRoute MITM CA \(test\)/);
});
...
test("DynamicCertStore caches one SecureContext per hostname", async () => {
  const store = new DynamicCertStore("DevilRoute MITM CA (test)");
