...
// ─── #3476: discoverability of `devilroute runtime repair` ──────────────────────
//
// When better-sqlite3's native binding mismatches the Node ABI, DevilRoute prints
...
// A self-heal command already exists (`devilroute runtime repair`); these tests
...
test("serve.mjs ABI-mismatch hint source mentions `runtime repair`", () => {
  ...
  assert.match(
    src,
    /runtime repair/,
    "serve.mjs should surface `devilroute runtime repair` as a recovery hint"
  );
});
