...
test("discoverPlugins retorna array vazio se nenhum plugin instalado (diretório não existe)", async () => {
  ...
  const orig = process.env.DEVILROUTE_PLUGIN_PATH;
  process.env.DEVILROUTE_PLUGIN_PATH = join(tmpdir(), `no-such-dir-${Date.now()}`);
  try {
    const plugins = await discoverPlugins();
    assert.ok(Array.isArray(plugins));
  } finally {
    if (orig === undefined) delete process.env.DEVILROUTE_PLUGIN_PATH;
    else process.env.DEVILROUTE_PLUGIN_PATH = orig;
  }
});

test("discoverPlugins descobre plugin com package.json válido", async () => {
  const { discoverPlugins } = await import("../../bin/cli/plugins.mjs");
  const pluginDir = join(tmpdir(), `devilroute-plugins-test-${Date.now()}`);
  const pkgDir = join(pluginDir, "devilroute-cmd-test-hello");
  mkdirSync(pkgDir, { recursive: true });
  writeFileSync(
    join(pkgDir, "package.json"),
    JSON.stringify({
      name: "devilroute-cmd-test-hello",
      version: "1.0.0",
      type: "module",
      main: "index.mjs",
    })
  ...
  const orig = process.env.DEVILROUTE_PLUGIN_PATH;
  process.env.DEVILROUTE_PLUGIN_PATH = pluginDir;
  try {
    const plugins = await discoverPlugins();
    assert.ok(
      plugins.some((p) => p.name === "devilroute-cmd-test-hello"),
...
test("discoverPlugins ignora pacotes sem prefixo devilroute-cmd-", async () => {
  const { discoverPlugins } = await import("../../bin/cli/plugins.mjs");
  const pluginDir = join(tmpdir(), `devilroute-plugins-test-${Date.now()}`);
...
test("loadPlugins não quebra CLI quando plugin tem erro de load (try/catch)", async () => {
  ...
  const pluginDir = join(tmpdir(), `devilroute-plugins-test-${Date.now()}`);
  const pkgDir = join(pluginDir, "devilroute-cmd-broken");
  mkdirSync(pkgDir, { recursive: true });
  writeFileSync(
    join(pkgDir, "package.json"),
    JSON.stringify({
      name: "devilroute-cmd-broken",
      version: "1.0.0",
      type: "module",
      main: "broken.mjs",
    })
...
test("loadPlugins carrega plugin válido e chama register()", async () => {
  const { loadPlugins } = await import("../../bin/cli/plugins.mjs");
  const pluginDir = join(tmpdir(), `devilroute-plugins-test-${Date.now()}`);
  const pkgDir = join(pluginDir, "devilroute-cmd-valid");
  mkdirSync(pkgDir, { recursive: true });
  writeFileSync(
    join(pkgDir, "package.json"),
    JSON.stringify({
      name: "devilroute-cmd-valid",
      version: "1.0.0",
      type: "module",
      main: "index.mjs",
    })
...
test("exemplo devilroute-cmd-hello existe e tem register()", () => {
  const examplePath = join(ROOT, "examples", "devilroute-cmd-hello", "index.mjs");
...
test("docs/frameworks/PLUGINS.md existe", () => {
  ...
  assert.ok(src.includes("devilroute-cmd"), "deve mencionar devilroute-cmd");
