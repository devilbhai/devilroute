...
function createTempRoot(): string {
  return fs.mkdtempSync(path.join(os.tmpdir(), "devilroute-sync-env-"));
}
...
test("syncEnv creates .env from .env.example and generates install-time secrets", () => {
  ...
  // find the user's real DB at ~/.devilroute/ during tests
  ...
  try {
    ...
    assert.match(envContent, /^MACHINE_ID_SALT=devilroute-/m);
