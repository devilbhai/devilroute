...
test("completion bash outputs bash script", async () => {
  ...
  assert.equal(result, 0);
  assert.ok(output.includes("_devilroute"));
});
...
test("completion zsh outputs zsh script", async () => {
  ...
  assert.equal(result, 0);
  assert.ok(output.includes("#compdef devilroute"));
});
...
test("completion fish outputs fish script", async () => {
  ...
  assert.equal(result, 0);
  assert.ok(output.includes("complete -c devilroute"));
});
...
test("env get returns 0 and prints env value", async () => {
  process.env.__DEVILROUTE_TEST_KEY__ = "hello";
  const { runEnvGetCommand } = await import("../../bin/cli/commands/env.mjs");
  const lines: string[] = [];
  const originalLog = console.log;
  console.log = (msg: string) => lines.push(msg);
  const result = await runEnvGetCommand("__DEVILROUTE_TEST_KEY__");
  console.log = originalLog;
  delete process.env.__DEVILROUTE_TEST_KEY__;
  assert.equal(result, 0);
