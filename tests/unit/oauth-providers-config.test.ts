...
test("custom Google OAuth callbacks preserve the requested callback path and query", () => {
  const redirectUri = resolveBrowserOAuthRedirectUri(
    ...
    { ...publicBaseEnv, NEXT_PUBLIC_BASE_URL: "https://devilroute.example.com/base" }
  );

  assert.equal(redirectUri, "https://devilroute.example.com/base/auth/callback?source=popup");
});
