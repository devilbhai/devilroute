...
test("provider manifest URL uses explicit env override", () => {
  const previous = process.env.DEVILROUTE_PROVIDER_MANIFEST_URL;
  process.env.DEVILROUTE_PROVIDER_MANIFEST_URL = "http://sidecar.local/manifest.json";
  try {
    assert.equal(
    ...
    if (previous === undefined) {
      delete process.env.DEVILROUTE_PROVIDER_MANIFEST_URL;
    } else {
      process.env.DEVILROUTE_PROVIDER_MANIFEST_URL = previous;
    }
  }
});
