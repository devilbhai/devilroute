...
//      bypass the check (DEVILROUTE_MAX_PENDING_MIGRATIONS=0) — e.g. after
...
test(
  "abort message tells the operator to set DEVILROUTE_MAX_PENDING_MIGRATIONS=0 to bypass (#6260)",
  serial,
  async () => {
    ...
    try {
      ...
      assert.match(message, /DEVILROUTE_MAX_PENDING_MIGRATIONS=0/);
    } finally {
      db.close();
    }
  }
