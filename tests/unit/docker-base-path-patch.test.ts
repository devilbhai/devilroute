...
test("patchBasePathLiterals rewrites empty basePath literals", () => {
  ...
  const output = patchBasePathLiterals(input, "/devilroute");
  assert.match(output, /basePath:"\/devilroute"/);
  assert.match(output, /"basePath":"\/devilroute"/);
});

test("patchJsonManifestFile updates nested basePath fields", () => {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "devilroute-basepath-"));
  const filePath = path.join(dir, "routes-manifest.json");
  fs.writeFileSync(
    filePath,
    JSON.stringify({ basePath: "", nested: { basePath: "" } }, null, 2)
  );
  assert.equal(patchJsonManifestFile(filePath, "/devilroute"), true);
  const parsed = JSON.parse(fs.readFileSync(filePath, "utf8"));
  assert.equal(parsed.basePath, "/devilroute");
  assert.equal(parsed.nested.basePath, "/devilroute");
});

test("patchStandaloneBasePath rewrites a root-path standalone tree", () => {
  const appRoot = fs.mkdtempSync(path.join(os.tmpdir(), "devilroute-standalone-"));
  ...
  fs.writeFileSync(path.join(appRoot, "BUILD_DEVILROUTE_BASE_PATH"), "\n");

  const result = patchStandaloneBasePath({
    appRoot,
    fromBasePath: "",
    toBasePath: "/devilroute",
  });
  ...
  assert.match(fs.readFileSync(path.join(distRoot, "server", "chunk.js"), "utf8"), /\/devilroute/);
});
