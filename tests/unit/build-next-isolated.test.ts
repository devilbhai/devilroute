...
async function withTempDir(fn) {
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "devilroute-build-next-isolated-"));
...
test("resolveNextBuildEnv honors the DEVILROUTE_BUILD_MEMORY_MB override", () => {
  const env = resolveNextBuildEnv({ DEVILROUTE_BUILD_MEMORY_MB: "6144" });
...
test("getTransientBuildPaths only moves _tasks when explicitly enabled", () => {
  const paths = getTransientBuildPaths("/repo", { DEVILROUTE_BUILD_MOVE_TASKS: "1" });
