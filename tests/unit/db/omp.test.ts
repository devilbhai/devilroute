 ...
 * (~/.omp/agent/agent.db) directly, per request — NOT DevilRoute's own DB.
...
describe("db/omp.ts — getOmpCredentials", () => {
  it("returns hasDevilRoute:false without throwing when the omp DB file does not exist", () => {
    assert.ok(!fs.existsSync(getOmpDbPath()), "precondition: no DB file yet");
    const creds = getOmpCredentials(PROVIDER_ID);
    assert.deepEqual(creds, { hasDevilRoute: false, baseUrl: null, apiKey: null });
  });

  it("returns hasDevilRoute:false when the DB exists but has no matching row", () => {
    seedOmpDb();
    const creds = getOmpCredentials(PROVIDER_ID);
    assert.deepEqual(creds, { hasDevilRoute: false, baseUrl: null, apiKey: null });
  });

  it("returns hasDevilRoute:false gracefully when the schema itself is missing (corrupt/foreign DB)", () => {
    const dbPath = getOmpDbPath();
    fs.mkdirSync(path.dirname(dbPath), { recursive: true });
...
describe("db/omp.ts — saveOmpCredentials + getOmpCredentials round trip", () => {
  it("persists apiKey/baseUrl so a subsequent read sees them", () => {
    ...
    const creds = getOmpCredentials(PROVIDER_ID);
    assert.equal(creds.hasDevilRoute, true);
...
describe("db/omp.ts — deleteOmpCredentials", () => {
  it("removes the row so a subsequent get reports hasDevilRoute:false", () => {
    seedOmpDb();
    saveOmpCredentials(PROVIDER_ID, "sk-test-omp-key", "http://localhost:20128/v1");
    assert.equal(getOmpCredentials(PROVIDER_ID).hasDevilRoute, true);
