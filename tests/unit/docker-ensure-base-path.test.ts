...
function makeStandaloneRoot(basePath = "") {
  const appRoot = fs.mkdtempSync(path.join(os.tmpdir(), "devilroute-docker-base-"));
  ...
  fs.writeFileSync(path.join(appRoot, "BUILD_DEVILROUTE_BASE_PATH"), `${basePath}\n`);
  return appRoot;
}

test("ensureDockerBasePath is a no-op when runtime matches the baked sentinel", () => {
  const appRoot = makeStandaloneRoot("");
  const result = ensureDockerBasePath({ appRoot, env: { DEVILROUTE_BASE_PATH: "" } });
  assert.equal(result.action, "noop");
});

test("ensureDockerBasePath patches a root image when a subpath is configured", () => {
  const appRoot = makeStandaloneRoot("");
  const result = ensureDockerBasePath({ appRoot, env: { DEVILROUTE_BASE_PATH: "/devilroute" } });
  assert.equal(result.action, "patched");
  assert.equal(fs.readFileSync(path.join(appRoot, "BUILD_DEVILROUTE_BASE_PATH"), "utf8"), "/devilroute\n");
  const manifest = JSON.parse(
    fs.readFileSync(path.join(appRoot, ".build", "next", "routes-manifest.json"), "utf8")
  );
  assert.equal(manifest.basePath, "/devilroute");
});

test("ensureDockerBasePath rejects unset runtime on subpath images", () => {
  const appRoot = makeStandaloneRoot("/devilroute");
  assert.throws(
