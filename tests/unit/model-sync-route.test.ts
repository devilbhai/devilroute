...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "devilroute-model-sync-route-"));
...
test("model sync route falls back to in-process discovery when internal self-fetch throws", async () => {
  ...
  const expectedPort = process.env.DEVILROUTE_PORT || process.env.PORT || "20128";
