...
// ─── transformToDevilRoute ────────────────────────────────

describe("transformToDevilRoute", () => {
  test("converts LiteLLM per-token pricing to DevilRoute per-million format", () => {
    const raw = {
      "openai/gpt-4o": {
    ...
    const result = transformToDevilRoute(raw);
...
describe("pricing merge precedence", () => {
  test("user overrides > synced > defaults conceptual order", () => {
    ...
    const synced = transformToDevilRoute(raw);
