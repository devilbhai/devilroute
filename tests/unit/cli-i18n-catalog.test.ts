...
test("i18n.mjs detecta locale por DEVILROUTE_LANG", async () => {
  const { resetForTests, detectLocale } = await import("../../bin/cli/i18n.mjs");
  const orig = process.env.DEVILROUTE_LANG;
  process.env.DEVILROUTE_LANG = "pt-BR";
  resetForTests();
  const locale = detectLocale();
  assert.equal(locale, "pt-BR");
  if (orig === undefined) delete process.env.DEVILROUTE_LANG;
  else process.env.DEVILROUTE_LANG = orig;
  resetForTests();
});
...
test("i18n.mjs usa fallback en quando locale não existe", async () => {
  const { resetForTests, detectLocale } = await import("../../bin/cli/i18n.mjs");
  const orig = process.env.DEVILROUTE_LANG;
  process.env.DEVILROUTE_LANG = "xx-FAKE";
  resetForTests();
  const locale = detectLocale();
...
test("t() usa pt-BR quando disponível", async () => {
  ...
  assert.ok(result.includes("devilroute serve"), `Esperava mensagem pt-BR, obteve: ${result}`);
  resetForTests();
});
