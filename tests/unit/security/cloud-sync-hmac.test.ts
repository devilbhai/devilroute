 ...
 * overwrite accessToken / refreshToken unless DEVILROUTE_CLOUD_SYNC_SECRETS=true.
...
test("verifyCloudSignature accepts a valid HMAC", async () => {
  ...
  const TEST_HMAC_KEY = crypto.createHash("sha256").update("devilroute-test").digest("hex");
  process.env.DEVILROUTE_CLOUD_SYNC_SECRET = TEST_HMAC_KEY;
...
test("verifyCloudSignature falls through (legacy mode) when secret is unset", async () => {
  delete process.env.DEVILROUTE_CLOUD_SYNC_SECRET;
  // Force re-import so module constants pick up the cleared env.
  delete (globalThis as any).__devilroute_cloudSync_cache;
  const mod = await import("../../../src/lib/cloudSync.ts");
  const body = JSON.stringify({ data: { providers: {} } });
