...
test("status discrepancy shows both DevilRoute and provider statuses", () => {
  const html = renderToStaticMarkup(
    React.createElement(RequestLoggerDetail, {
      log: {
        status: 504,
  ...
  assert.notEqual(
    html.indexOf("DevilRoute returned 504"),
    -1,
    "Should indicate DevilRoute returned its own status"
  );
});
