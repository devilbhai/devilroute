...
describe("resolveApiKey", () => {
  ...
  it("falls back to sk_devilroute when both are null", async () => {
    mockKeyStore.clear();

    const result = await resolveApiKey(null, null);
    assert.equal(result, "sk_devilroute");
  });

  it("falls back to sk_devilroute when both are undefined", async () => {
    mockKeyStore.clear();
