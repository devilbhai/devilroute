...
- **refactor(rollup):** god-file split ([#200](https://github.com/x/y/pull/200), [#201](https://github.com/x/y/pull/201) — thanks @erin); editorconfig ([#202](https://github.com/x/y/pull/202) — thanks @frank). — thanks @devilbhai
...
test("noise handles and the maintainer are excluded from the contributor map", () => {
  ...
  assert.ok(!agg.has("devilbhai"), "maintainer is rendered separately, not in the map");
...
test("renderContributors emits an alphabetical table with maintainer last", () => {
  ...
  assert.equal(handles[handles.length - 1], "devilbhai", "maintainer is last");
