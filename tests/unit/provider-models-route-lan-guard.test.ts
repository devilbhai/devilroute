...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "devilroute-lan-guard-models-"));
...
const originalAllowPrivateProviderUrls = process.env.DEVILROUTE_ALLOW_PRIVATE_PROVIDER_URLS;
const originalAllowLocalProviderUrls = process.env.DEVILROUTE_ALLOW_LOCAL_PROVIDER_URLS;

async function resetStorage() {
  globalThis.fetch = originalFetch;
  if (originalAllowPrivateProviderUrls === undefined) {
    delete process.env.DEVILROUTE_ALLOW_PRIVATE_PROVIDER_URLS;
  } else {
    process.env.DEVILROUTE_ALLOW_PRIVATE_PROVIDER_URLS = originalAllowPrivateProviderUrls;
  }
  if (originalAllowLocalProviderUrls === undefined) {
    delete process.env.DEVILROUTE_ALLOW_LOCAL_PROVIDER_URLS;
  } else {
    process.env.DEVILROUTE_ALLOW_LOCAL_PROVIDER_URLS = originalAllowLocalProviderUrls;
  }
...
test("#6939: getProviderOutboundGuard() and getProviderValidationGuard() agree for LAN hosts under the default local-first setting", () => {
  // Default settings: DEVILROUTE_ALLOW_LOCAL_PROVIDER_URLS unset (ON by default),
  // DEVILROUTE_ALLOW_PRIVATE_PROVIDER_URLS unset (OFF by default).
  delete process.env.DEVILROUTE_ALLOW_LOCAL_PROVIDER_URLS;
  delete process.env.DEVILROUTE_ALLOW_PRIVATE_PROVIDER_URLS;
...
test("#6939: LAN model-list fetch is still blocked when the local-first default is explicitly disabled", async () => {
  delete process.env.DEVILROUTE_ALLOW_PRIVATE_PROVIDER_URLS;
  process.env.DEVILROUTE_ALLOW_LOCAL_PROVIDER_URLS = "false";
