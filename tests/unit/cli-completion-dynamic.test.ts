...
test("runCompletionCommand bash retorna 0 e string não-vazia", async () => {
  ...
  assert.ok(out.includes("devilroute"), "bash script should mention devilroute");
  assert.ok(out.includes("_devilroute"), "bash script should define _devilroute function");
});
...
test("runCompletionCommand fish retorna 0", async () => {
  ...
  const out = chunks.join("");
  assert.ok(out.includes("devilroute"), "fish script should mention devilroute");
});
...
test("completion scripts incluem combos/providers/models no cache dinamicamente", async () => {
  ...
  const out = chunks.join("");
  assert.ok(
    out.includes("completion-cache.json") || out.includes("devilroute_get_cache"),
