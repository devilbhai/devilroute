...
const TEST_DATA_DIR = mkdtempSync(join(tmpdir(), "devilroute-sse-shim-"));
...
test("src/sse service wrappers delegate to open-sse and shared infrastructure", () => {
  ...
  assert.match(tokenRefreshSource, /@devilroute\/open-sse\/services\/tokenRefresh\.ts/);
  assert.match(modelSource, /@devilroute\/open-sse\/services\/model\.ts/);
