 ...
 * Helper.app bundles from build.productName ("DevilRoute"). The old code used
 * app.getName() (package.json `name` = "devilroute-desktop"); the two diverged, so it
...
const EXEC = "/Applications/DevilRoute.app/Contents/MacOS/DevilRoute";

describe("#7941 — resolveDarwinHelperExecutable()", () => {
  it("resolves the productName-derived framework Helper even though app.getName() would say 'devilroute-desktop'", () => {
    const frameworkHelper = path.join(
      path.dirname(EXEC),
      "..",
      "Frameworks",
      "DevilRoute Helper.app",
      "Contents",
      "MacOS",
      "DevilRoute Helper"
    );
  ...
  it("prefers the sibling (unsuffixed) Helper next to the main binary when present", () => {
    const sibling = path.join(path.dirname(EXEC), "DevilRoute Helper");
    const result = resolveDarwinHelperExecutable({
      execPath: EXEC,
