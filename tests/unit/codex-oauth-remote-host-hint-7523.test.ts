...
// server binds the SERVER's loopback (localhost:PORT). When DevilRoute runs on a
...
test("remote Host → returns the reverse-tunnel hint with the exact callback port", () => {
  ...
  assert.equal(hint.tunnelCommand, "ssh -L 1455:127.0.0.1:1455 <user>@<devilroute-host>");
...
test("remote Host honours a random callback port (Windsurf/Devin OS-assigned port)", () => {
  const hint = buildRemoteOAuthHint("devilroute.example.com", 54321);
  assert.ok(hint.remoteHost === true);
  assert.equal(hint.tunnelCommand, "ssh -L 54321:127.0.0.1:54321 <user>@<devilroute-host>");
});
