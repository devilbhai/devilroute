...
function withTempEnv(fn) {
  ...
  const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "devilroute-bootstrap-test-"));
  ...
  try {
    fn({ tempRoot, tempCwd, tempHome, dataDir: path.join(tempHome, ".devilroute") });
  } finally {
...
test("bootstrapEnv prefers ~/.devilroute/.env over server.env", () => {
  withTempEnv(({ dataDir }) => {
