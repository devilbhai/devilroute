...
const originalPoolFlag = process.env.DEVILROUTE_BROWSER_POOL;
...
beforeEach(() => {
  ...
  delete process.env.DEVILROUTE_BROWSER_POOL;
...
afterEach(() => {
  ...
  restoreEnv("DEVILROUTE_BROWSER_POOL", originalPoolFlag);
  __resetClaudeWebSessionForTesting();
  __resetClaudeWebBrowserTemplatesForTesting();
});
...
describe("Claude Web executor transport orchestration", () => {
  it("returns buffered output and generated state from an exact direct request", async () => {
    ...
    assert.match(
      execution.response.headers.get("X-DevilRoute-Claude-Web-Assistant-Message-Uuid") ?? "",
      /^[a-f0-9-]+$/
    );
  });
  ...
  it("falls back from a direct challenge to the same scoped browser request when enabled", async () => {
    process.env.DEVILROUTE_BROWSER_POOL = "on";
