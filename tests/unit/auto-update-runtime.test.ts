...
describe("getAutoUpdateConfig", () => {
  it("defaults to npm or source mode locally", () => {
    const config = autoUpdate.getAutoUpdateConfig({ DATA_DIR: "/tmp/devilroute" });
    assert.ok(config.mode === "npm" || config.mode === "source");
    assert.equal(config.repoDir, "/workspace/devilroute");
  ...
  it("reads docker-compose settings from env", () => {
    const config = autoUpdate.getAutoUpdateConfig({
      ...
      AUTO_UPDATE_REPO_DIR: "/srv/devilroute",
      AUTO_UPDATE_COMPOSE_FILE: "/srv/devilroute/docker-compose.yml",
      AUTO_UPDATE_COMPOSE_PROFILE: "base",
      AUTO_UPDATE_SERVICE: "devilroute-base",
    ...
    assert.equal(config.repoDir, "/srv/devilroute");
    assert.equal(config.composeFile, "/srv/devilroute/docker-compose.yml");
    assert.equal(config.composeProfile, "base");
    assert.equal(config.composeService, "devilroute-base");
...
describe("buildSourceUpdateScript", () => {
  it("includes git checkout, env sync, and rebuild steps", () => {
    ...
    assert.match(script, /pm2 restart devilroute --update-env \|\| true/);
  });
});
