...
// coletava tests/unit/docs/) e apodreceu — filtrava dirs `devilroute*`, mas os skills
...
// invariante de uso é "referencia as env vars ($DEVILROUTE_URL/DEVILROUTE_KEY) OU
// comandos da CLI (`devilroute …`)" — 3 skills (health/keys/batches) usam só a CLI.
...
test("each skill dir has SKILL.md with frontmatter", async () => {
  ...
  for (const dir of dirs) {
    ...
    if (!dir.startsWith("omni-")) {
      assert.ok(
        content.includes("DEVILROUTE_") || content.includes("devilroute "),
        `${dir}: missing usage references (DEVILROUTE_* env vars or devilroute CLI commands)`
      );
    }
  }
});
