...
test("generateCert() issues a cert whose SAN list covers all 4 antigravity hosts", async (t) => {
  // Isolate DATA_DIR so this test never touches (or reuses) a real ~/.devilroute cert.
  const tmpDataDir = fs.mkdtempSync(path.join(os.tmpdir(), "devilroute-mitm-cert-6494-"));
