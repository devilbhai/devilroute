...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "devilroute-image-route-auth-"));
...
test("v1 image generation POST rejects an invalid presented API key", async () => {
  const originalDevilRouteApiKey = process.env.DEVILROUTE_API_KEY;
  const originalRequireApiKey = process.env.REQUIRE_API_KEY;
  process.env.DEVILROUTE_API_KEY = "valid-image-route-key";
  process.env.REQUIRE_API_KEY = "true";
  ...
  try {
    ...
    if (originalDevilRouteApiKey === undefined) {
      delete process.env.DEVILROUTE_API_KEY;
    } else {
      process.env.DEVILROUTE_API_KEY = originalDevilRouteApiKey;
    }
    if (originalRequireApiKey === undefined) {
      delete process.env.REQUIRE_API_KEY;
    } else {
