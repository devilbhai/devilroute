...
test("Dockerfile exposes DEVILROUTE_BASE_PATH as a builder-stage build arg", () => {
  const dockerfile = fs.readFileSync(path.join(REPO_ROOT, "Dockerfile"), "utf8");
  assert.match(dockerfile, /ARG DEVILROUTE_BASE_PATH/);
  assert.match(dockerfile, /ENV DEVILROUTE_BASE_PATH=\$DEVILROUTE_BASE_PATH/);
});

test("docker-compose forwards DEVILROUTE_BASE_PATH into image builds", () => {
  const compose = fs.readFileSync(path.join(REPO_ROOT, "docker-compose.yml"), "utf8");
  assert.match(compose, /DEVILROUTE_BASE_PATH: \$\{DEVILROUTE_BASE_PATH:-\}/);
  assert.match(compose, /- DEVILROUTE_BASE_PATH=\$\{DEVILROUTE_BASE_PATH:-\}/);
});
...
test("Hard Rule #13: shell basePath helpers never interpolate into sed/awk", () => {
  ...
  for (const rel of shellFiles) {
    ...
    assert.doesNotMatch(
      source,
      /\b(?:sed|awk)\b.*\$\{?DEVILROUTE_BASE_PATH/,
      `${rel} must not expand DEVILROUTE_BASE_PATH into sed/awk`
    );
    assert.doesNotMatch(
      source,
      /(?:sed|awk).*DEVILROUTE_BASE_PATH/,
      `${rel} must not pass DEVILROUTE_BASE_PATH on a sed/awk command line`
    );
  }
  ...
  assert.match(ensure, /process\.env\.DEVILROUTE_BASE_PATH/);
});
