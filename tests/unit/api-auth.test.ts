...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "devilroute-api-auth-"));
...
test("getApiKeyMetadata recognizes DEVILROUTE_API_KEY environment variable", async () => {
  const envKey = "sk-test-env-key-" + Date.now();
  process.env.DEVILROUTE_API_KEY = envKey;
...
test("requireManagementAuth returns null for DEVILROUTE_API_KEY env passthrough", async () => {
  await setupAuth();
