...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "devilroute-settings-ext-"));
...
function cleanupGlobalDb() {
  try {
    if ((globalThis as any).__devilrouteDb?.open) {
      (globalThis as any).__devilrouteDb.close();
    }
  } catch {}
  delete (globalThis as any).__devilrouteDb;
}
