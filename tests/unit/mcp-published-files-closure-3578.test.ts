...
// #3578 — `devilroute --mcp` crashed on npm installs with ERR_MODULE_NOT_FOUND for
...
function resolveImport(fromFile: string, spec: string): string | null {
  ...
  else if (spec.startsWith("@devilroute/open-sse/"))
    base = path.join("open-sse", spec.slice("@devilroute/open-sse/".length));
  else if (spec === "@devilroute/open-sse") base = path.join("open-sse", "index");
