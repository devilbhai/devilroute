...
const DEVILROUTE_BIN = path.join(ROOT, "bin", "devilroute.mjs");
...
// ~/.devilroute so DATA_DIR is the only data directory in play.
function baseEnv(dataDir: string, isolatedHome: string): NodeJS.ProcessEnv {
  ...
  return {
    ...
    DATA_DIR: dataDir,
    HOME: isolatedHome,
    // Give the CLI a key so bin/devilroute.mjs never warns/provisions.
...
function mkHome(): string {
  return fs.mkdtempSync(path.join(os.tmpdir(), "devilroute-reset-home-"));
}
function mkDataDir(): string {
  return fs.mkdtempSync(path.join(os.tmpdir(), "devilroute-reset-data-"));
}

// #6261: `devilroute reset-password` must be a real subcommand (not "unknown
...
test("devilroute reset-password subcommand applies the reset over piped stdin (#6261, #6258)", async () => {
  const dataDir = mkDataDir();
  const home = mkHome();
  try {
    const dbPath = seedDb(dataDir);
    const res = spawnSync("node", [DEVILROUTE_BIN, "reset-password"], {
...
test("devilroute-reset-password applies the reset over piped two-line stdin (#6258)", async () => {
  const dataDir = mkDataDir();
  const home = mkHome();
  try {
    const dbPath = seedDb(dataDir);
...
test("devilroute-reset-password --password-stdin reads the whole stdin as the password (#6258)", async () => {
  const dataDir = mkDataDir();
  const home = mkHome();
  try {
    const dbPath = seedDb(dataDir);
