...
DEVILROUTE_URL="${DEVILROUTE_URL:-http://localhost:20128}"
MODEL="${1:-${DEVILROUTE_MODEL:-}}"
AUTH_TOKEN="${DEVILROUTE_AUTH_TOKEN:-dummy}"
FAILURES=0
...
echo "=== DevilRoute Tool Call Integrity E2E Test ==="
echo "URL: $DEVILROUTE_URL | Model: $MODEL"

echo
echo "[TEST 1] Non-streaming tool call integrity..."
response=$(curl -fsS -X POST "$DEVILROUTE_URL/v1/chat/completions" \
...
stream_output=$(curl -fsS -X POST "$DEVILROUTE_URL/v1/chat/completions" \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer $AUTH_TOKEN" \
  -d @- <<JSON
{
...
models=$(curl -fsS "$DEVILROUTE_URL/v1/models" \
  -H "Authorization: Bearer $AUTH_TOKEN" \
