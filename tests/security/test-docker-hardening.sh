...
IMAGE_NAME="${1:-devilroute-local-hardened}"
CONTAINER_NAME="${2:-devilroute-hardening-test}"
HOST_PORT="${3:-20129}"
...
docker build -t "${IMAGE_NAME}" "${WORKDIR}" >/tmp/devilroute_hardening_build.log
echo "      Build done."
...
docker run -d \
  ...
  -e DATA_DIR=/app/data \
  "${IMAGE_NAME}" >/tmp/devilroute_hardening_container_id.txt
