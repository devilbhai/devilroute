...
const BASE_URL = process.env.DEVILROUTE_BASE_URL || "http://localhost:20128";
const API_KEY = process.env.DEVILROUTE_API_KEY || "";
  ...
  it(
    ...
    async () => {
      const transport = new StdioClientTransport({
        ...
        env: {
          ...process.env,
          DEVILROUTE_BASE_URL: BASE_URL,
          DEVILROUTE_API_KEY: API_KEY,
      ...
      try {
        ...
        expect(toolNames).toContain("devilroute_get_health");
        expect(toolNames).toContain("devilroute_list_combos");
      ...
      const auditRes = await apiFetch("/api/mcp/audit?limit=50&tool=devilroute_get_health");
      ...
      if (auditRes.status === 200) {
        ...
        expect(entries.some((entry: any) => entry.toolName === "devilroute_get_health")).toBe(true);
      }
    },
    TEST_TIMEOUT_MS * 2
  );
