 ...
 *
 * Live benchmark: how well do the free-tier models DevilRoute exposes handle
 ...
 *
 * Environment:
 *   DEVILROUTE_API_KEY  — required (else test skips)
...
test(
  ...
  async () => {
    ...
    // DevilRoute's routing) is broken, not that every free provider failed at
    // once.
    assert.ok(
      totalSucceeded > 0,
