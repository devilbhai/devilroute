...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "devilroute-codewhale-settings-"));
...
test("codewhale-settings POST: writes primary ~/.codewhale/config.toml for a fresh install", async () => {
  ...
  try {
    ...
    if (res.status === 200) {
      ...
      assert.ok(content.includes("managed by DevilRoute"), "Config should have DevilRoute marker");
...
test("codewhale-settings GET: falls back to legacy ~/.deepseek/config.toml when primary is absent", async () => {
  ...
  try {
    ...
    const body = await res.json();
    if (body.config) {
      assert.ok(body.config.includes("managed by DevilRoute"));
      assert.equal(body.hasDevilRoute, true);
    }
  } finally {
    process.env.HOME = origHome;
    fs.rmSync(tmpHome, { recursive: true, force: true });
