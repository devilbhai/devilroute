...
// completely independent of the per-request `x-devilroute-compression` header/mode. The internal
...
// a request carrying `x-devilroute-compression: off` must NOT get the styles system message
...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "devilroute-testmodel-compression-"));
...
test("chatCore: x-devilroute-compression: off suppresses Output Styles injection even with the operator's global style enabled (#6240)", async () => {
  ...
  assert.match(plainFirstMessage?.content ?? "", /DevilRoute Output Styles/);

  // The "Test model" connection test sends x-devilroute-compression: off — must be clean.
  const testModelBody = await runChatCore({
    provider,
    model,
    connectionId: connection.id,
    headers: new Headers({ "x-devilroute-compression": "off" }),
  });
  ...
  const anyMessageHasMarker = (testModelBody?.messages ?? []).some((m) =>
    (m?.content ?? "").includes("DevilRoute Output Styles")
  );
  assert.equal(
    anyMessageHasMarker,
    false,
