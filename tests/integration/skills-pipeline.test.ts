...
import { DEVILROUTE_WEB_SEARCH_FALLBACK_TOOL_NAME } from "../../open-sse/services/webSearchFallback.ts";
...
test("web_search fallback converts built-in tools for unsupported providers and executes search", async () => {
  ...
  globalThis.fetch = async (url, init = {}) => {
    ...
    if (urlStr.includes("google.serper.dev/search")) {
      ...
      return new Response(
        JSON.stringify({
          organic: [
            {
              title: "DevilRoute Search Result",
              link: "https://example.com/devilroute",
              snippet: "Fresh DevilRoute web search fallback result",
            },
          ],
        }),
    ...
    return buildOpenAIToolCallResponse({
      toolName: DEVILROUTE_WEB_SEARCH_FALLBACK_TOOL_NAME,
      toolCallId: "call_web_search",
      argumentsObject: {
        query: "latest devilroute release notes",
        max_results: 3,
      },
    });
  };
  ...
  const response = await handleChat(
    buildRequest({
      ...
      body: {
        ...
        stream: false,
        messages: [
          { role: "user", content: "Search the web for the latest DevilRoute release notes" },
        ],
  ...
  assert.equal(upstreamBodies[0].tools[0].function.name, DEVILROUTE_WEB_SEARCH_FALLBACK_TOOL_NAME);
  ...
  assert.equal(output.results[0].title, "DevilRoute Search Result");
});
...
test("web_search fallback preserves Responses API output by appending function_call_output", async () => {
  ...
  globalThis.fetch = async (url, init = {}) => {
    ...
    return buildOpenAIToolCallResponse({
      ...
      argumentsObject: {
        query: "latest devilroute roadmap",
      },
    });
  };
        ...
        input: [
          {
            ...
            role: "user",
            content: [
              { type: "input_text", text: "Search the web for the latest DevilRoute roadmap" },
            ],
          },
        ],
  ...
  assert.equal(functionCall.name, DEVILROUTE_WEB_SEARCH_FALLBACK_TOOL_NAME);
