...
//      - request carries x-devilroute-session-id header
...
// reads "x-session-id" (not "x-devilroute-session-id") and prefixes the result with "ext:".
...
//     internal summary request (identified by _devilrouteInternalRequest flag in body)
...
function installHandoffAwareFetch() {
  ...
  globalThis.fetch = async (url: any, init: any = {}) => {
    ...
    if (bodyObj._devilrouteInternalRequest === "universal-handoff") {
...
// Without the x-devilroute-session-id header, sessionId = null → block is skipped.
test("context-relay universal handoff: does NOT fire when x-devilroute-session-id header is absent", async () => {
  ...
  assert.equal(
    handoff,
    null,
    "handoff must NOT be written when x-devilroute-session-id header is absent (sessionId gate)"
  );
