...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "devilroute-jcode-settings-"));
  ...
  try {
    ...
    if (res.status === 200) {
      ...
      if (fs.existsSync(configPath)) {
        ...
        assert.equal(written._managedBy, "devilroute");
...
// ── Test 5: DELETE → removes DevilRoute fields ────────────────────────────────

test("jcode-settings DELETE: removes DevilRoute fields from existing config", async () => {
