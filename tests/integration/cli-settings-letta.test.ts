 ...
 *
 * Letta configures DevilRoute as its "lmstudio" provider (localModelDiscovery:
...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "devilroute-letta-settings-"));
...
// ── Test 6: POST refuses to overwrite an existing non-DevilRoute lmstudio config ──
...
// ── Test 7: DELETE → removes the DevilRoute lmstudio config ──────────────────
