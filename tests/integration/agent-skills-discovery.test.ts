 ...
 *  3. MCP tool devilroute_agent_skills_list handler returns 44 entries.
...
// ── §3: MCP tool devilroute_agent_skills_list ─────────────────────────────────

test("MCP devilroute_agent_skills_list handler returns count 45 (44 + config)", async () => {
...
test("MCP devilroute_agent_skills_list result has all 42 IDs", async () => {
  const result = await agentSkillTools.devilroute_agent_skills_list.handler({});
