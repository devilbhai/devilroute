 ...
 *  2. Zero devilroute-* folders remain (post-prune: old devilroute-* skill dirs were removed).
...
// ── §2: No devilroute-* directories remain ────────────────────────────────────

test("skills/ has zero devilroute-* directories (all pruned)", () => {
  ...
  const omniRouteDirs = entries
    .filter((e) => e.isDirectory() && e.name.startsWith("devilroute-"))
    .map((e) => e.name);
  assert.deepEqual(
    omniRouteDirs,
    [],
    `Found devilroute-* directories that should have been pruned: ${omniRouteDirs.join(", ")}`,
  );
});
