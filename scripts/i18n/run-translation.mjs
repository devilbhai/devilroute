#!/usr/bin/env node
/**
 * DevilRoute — Docs translation pipeline (hash-based, incremental).
 *
 ...
 *   DEVILROUTE_TRANSLATION_API_URL     e.g. https://cloud.devilroute.dev/v1
 *   DEVILROUTE_TRANSLATION_API_KEY     bearer token (kept out of logs)
 *   DEVILROUTE_TRANSLATION_MODEL       e.g. cx/gpt-5.6-sol
 *   DEVILROUTE_TRANSLATION_TIMEOUT_MS  optional, default 60000
 *   DEVILROUTE_TRANSLATION_CONCURRENCY optional, default 4
 */
...
function backendConfig() {
  const apiUrl = requireEnv("DEVILROUTE_TRANSLATION_API_URL").replace(/\/$/, "");
  const apiKey = requireEnv("DEVILROUTE_TRANSLATION_API_KEY");
  const model = requireEnv("DEVILROUTE_TRANSLATION_MODEL");
  const timeoutMs = Number(process.env.DEVILROUTE_TRANSLATION_TIMEOUT_MS || 60000);
...
async function main() {
  ...
  if (!opts.dryRun) {
    ...
    else backend.concurrency = Number(process.env.DEVILROUTE_TRANSLATION_CONCURRENCY || 4);
    logInfo(
      `backend: ${backend.apiUrl} (model=${backend.model}, concurrency=${backend.concurrency}, timeout=${backend.timeoutMs}ms)`
    );
  } else {
    const apiUrl = (process.env.DEVILROUTE_TRANSLATION_API_URL || "").replace(/\/$/, "");
