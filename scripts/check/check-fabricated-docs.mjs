...
//   3. CLI commands `devilroute ...`     → must exist in bin/cli/commands/ or bin/
...
const ENV_VAR_ALLOWLIST = new Set([
  ...
  "VERBOSE",
  "LOG_LEVEL",
  "PORT", // generic, not DevilRoute-specific
  "DATA_DIR",
  "REQUIRE_API_KEY",
  "DEVILROUTE_BUILD_PROFILE", // build-time only
  "DEVILROUTE_BUILD_SHA",
  "DEVILROUTE_URL", // used by ad-hoc tooling, validated elsewhere
  "DEVILROUTE_KEY", // ditto
  ...
  // subprocess — never read via `process.env.X` in DevilRoute's own source, so the
  ...
  "OPENAI_BASE_URL", // env var DevilRoute passes to downstream CLIs (AGENT_PROTOCOLS_GUIDE.md)
...
const ENDPOINT_ALLOWLIST = new Set([
  ...
  // paths on the UPSTREAM service (Claude.ai web, Blackbox), not DevilRoute routes.
...
// Env var helper wrappers used across DevilRoute — envInt(NAME, 5), envBool(NAME),
...
export function buildCodebaseIndex(root = ROOT) {
  ...
  // documented DevilRoute env var (including external-CLI / docker / electron vars
  ...
  // Set of `devilroute <subcommand>` strings that exist in bin/
...
const COARSE_PATTERNS = {
  ...
  // devilroute <verb> <sub> ... — only on the same line, captures first 2 tokens
  cliCmd: /\bdevilroute\s+([a-z][a-z0-9-]+)(?:\s+([a-z][a-z0-9-]+))?/g,
...
export function scanDocFile(absPath, index, root = ROOT) {
  ...
  // 3) CLI commands: `devilroute foo bar` — only flag when the line is in
  //    a code-like context (inside backticks or a shell block). Bare prose
  //    like "we use devilroute and..." is not a command claim.
  for (const m of textNoCode.matchAll(COARSE_PATTERNS.cliCmd)) {
    ...
    const isShellLike = /^[ \t]*\$\s|^```sh|^```bash|^```shell|`devilroute/.test(lineText);
    if (!isShellLike) continue;
    if (/example|placeholder|tbd/i.test(lineText)) continue;
    findings.push({
      kind: "cli-cmd",
      value: `devilroute ${sub}`,
      line: ln,
      msg: `devilroute subcommand '${sub}' not registered in bin/`,
    });
  }
  ...
  for (const m of textNoCode.matchAll(COARSE_PATTERNS.fileRef)) {
    ...
    // (`@devilroute/opencode-provider/src/index.ts`). Neither resolves from repo root.
...
export function formatHumanReport(result) {
  ...
  const KIND_LABELS = {
    ...
    "cli-cmd": "devilroute subcommands not registered",
