...
function findDb(dataDir) {
  const candidates = ["storage.sqlite", "devilroute.sqlite", "data.sqlite"];
...
async function bootAndProbe({ prefix, dataDir, port, expectVersion, label }) {
  const binPath = path.join(prefix, "bin", "devilroute");
  ...
  const child = spawn(binPath, ["serve", "--port", String(port)], {
    env: {
      ...
      DEVILROUTE_SKIP_SYSTEM_TRUST: "1",
    },
...
function resolvePreviousVersion(current, explicit) {
  ...
  const out = execFileSync("npm", ["view", "devilroute", "dist-tags.latest"], { encoding: "utf8" });
  const latest = out.trim();
  if (!latest) throw new Error("could not resolve devilroute@latest from npm");
  if (latest === current) {
    ...
    const all = JSON.parse(execFileSync("npm", ["view", "devilroute", "versions", "--json"], { encoding: "utf8" }));
...
async function main() {
  ...
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "devilroute-install-upgrade-"));
  const failures = [];
  const warnings = [];
