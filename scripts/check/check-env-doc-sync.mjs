 ...
 *
 * Enforces that every env var referenced in DevilRoute source code appears in
...
const IGNORE_FROM_CODE = new Set([
  ...
  "LOCALAPPDATA",
  "XDG_CONFIG_HOME",
  // XDG Base Directory cache root — read (never defined by DevilRoute) so the
  // Android/Termux serve path can honor an operator-set cache location (#8519).
  "XDG_CACHE_HOME",
  "USERPROFILE",
  "PREFIX",
  // X11 display server — set by the OS/session manager, not DevilRoute config.
  "DISPLAY",
  ...
  // bundler is active — framework-internal. The DevilRoute-facing knob is
  // DEVILROUTE_USE_TURBOPACK (scripts/dev/run-next.mjs), which IS documented.
  "TURBOPACK",
  "NODE_TEST_CONTEXT",
  "VITEST",
  // Instruction snippet shown to users (Traffic Inspector HttpProxySnippetCard) — not DevilRoute config.
  ...
  // points at DevilRoute). A downstream client-tool var, not an DevilRoute server
  ...
  // (scripts/check/check-openapi-breaking.mjs) — a build/check signal, not DevilRoute runtime config.
  "BASE_REF",
  ...
  // pull_request.{head,base}.sha — never DevilRoute runtime config, and meaningless in a .env.
  "HEAD_REF",
  "HEAD_SHA",
  "BASE_SHA",
  ...
  // a CI-only signal, never an DevilRoute runtime config (Phase 7.10).
  "PR_BODY",
  // CLI machine-id token opt-out (server-side flag; not user-configurable via .env).
  "DEVILROUTE_DISABLE_CLI_TOKEN",
  ...
  // ("http://192.168.0.15:20128" / null), never DevilRoute runtime config (#5151).
  "COMBO_LIVE_BASE_URL",
  "COMBO_LIVE_API_KEY",
  ...
  // These are external signals set by the host OS or cloud provider — not DevilRoute config.
  "CODESPACES",
  ...
  // internals, never DevilRoute runtime config.
  "VERIFY_DEADLINE_S",
  "VERIFY_PORT",
  "VERIFY_VERSION",
  // Doctor diagnostic flags (no runtime behavior yet — placeholders).
  "DEVILROUTE_DOCTOR_HOST",
  "DEVILROUTE_DOCTOR_LIVENESS_URL",
  "DEVILROUTE_PROVIDER_CATALOG_PATH",
  "DEVILROUTE_PROVIDER_TEST_MODEL",
  // Test-only opt-out: instructs bin/devilroute.mjs to skip auto-loading the
  // repository .env so isolation tests get a deterministic environment.
  "DEVILROUTE_CLI_SKIP_REPO_ENV",
  ...
  // A dev/ops measurement tool, never DevilRoute runtime config.
  "DEVILROUTE_EVAL_CREDENTIALS",
  ...
  "DEVILROUTE_INTERNAL_SCHEME",
  // Source typo / placeholder.
  "OMNIROUT",
  // Static config alias path (the canonical var is DEVILROUTE_PAYLOAD_RULES_PATH).
  "PAYLOAD_RULES_PATH",
  // Node.js module resolution path — OS/Node internal, not an DevilRoute config var.
  ...
  "NVIDIA_BASE_URL",
  "NVIDIA_MODEL",
  // XDG standard data directory — set by OS/desktop session, not DevilRoute config.
  ...
  "DEVILROUTE_OPENCODE_PLUGIN_DIR",
]);
...
const DOC_ONLY_ALLOWLIST = new Set([
  ...
  "DEVILROUTE_CRYPT_KEY",
  "DEVILROUTE_API_KEY_BASE64",
