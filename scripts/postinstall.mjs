#!/usr/bin/env node
/**
 * Post-install warm-up for DevilRoute native runtimes.
 *
 * Runs after the main `scripts/build/postinstall.mjs` binary-copy step.
 * Tries to pre-resolve better-sqlite3 into ~/.devilroute/runtime/ so that
...
if (
  process.env.DEVILROUTE_SKIP_POSTINSTALL === "1" ||
  process.env.CI === "true" ||
  process.env.CI === "1"
) {
  process.exit(0);
...
(async () => {
  try {
    ...
    console.warn(`[devilroute] postinstall warm-up skipped: ${err?.message ?? err}`);
  }
})();
