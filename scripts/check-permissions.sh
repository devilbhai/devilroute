...
# If DEVILROUTE_MEMORY_MB is set, build NODE_OPTIONS dynamically so the
# user can tune heap size via environment without editing the Dockerfile.
if [ -n "$DEVILROUTE_MEMORY_MB" ]; then
  export NODE_OPTIONS="${NODE_OPTIONS:-} --max-old-space-size=${DEVILROUTE_MEMORY_MB}"
fi

# Hard Rule #13: never interpolate DEVILROUTE_BASE_PATH (or any runtime path)
...
if [ -d "$DATA_PATH" ] && [ ! -w "$DATA_PATH" ]; then
  ...
  if [ "${CONTAINER_HOST:-}" = "podman" ]; then
    ...
    echo "  https://github.com/diegosouzapw/DevilRoute/blob/main/contrib/podman/README.md#data-directory-permissions-by-topology"
  else
