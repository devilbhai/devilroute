...
/**
 * Compares DEVILROUTE_BASE_PATH at container start against BUILD_DEVILROUTE_BASE_PATH
...
const SENTINEL = "BUILD_DEVILROUTE_BASE_PATH";
...
export function ensureDockerBasePath(opts = {}) {
  ...
  const runtime = normalizeBasePath(opts.env?.DEVILROUTE_BASE_PATH ?? process.env.DEVILROUTE_BASE_PATH);
  ...
  if (!runtime && baked) {
    throw new Error(
      `This DevilRoute image was built for subpath ${baked}, but DEVILROUTE_BASE_PATH is unset. ` +
        `Set DEVILROUTE_BASE_PATH=${baked} or rebuild without the build-arg.`
    );
  }
  ...
  try {
    ...
    console.error(
      "[ensure-docker-base-path] Rebuild the image with the same subpath, e.g.\n" +
        "  docker compose build --build-arg DEVILROUTE_BASE_PATH=/devilroute"
    );
    process.exit(1);
  }
}
