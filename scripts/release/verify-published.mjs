...
set -euo pipefail
echo "[verify-published] npm i -g devilroute@\${VERIFY_VERSION} (public registry)"
npm install -g "devilroute@\${VERIFY_VERSION}"
export DATA_DIR=/tmp/devilroute-data JWT_SECRET=verify-published-secret-with-sufficient-length API_KEY_SECRET=verify-published-api-key-secret DISABLE_SQLITE_AUTO_BACKUP=true DEVILROUTE_SKIP_SYSTEM_TRUST=1
mkdir -p "\$DATA_DIR"
devilroute serve --port "\$VERIFY_PORT" &
node -e '
...
function main() {
  ...
  console.log(`[verify-published] clean-container verify of devilroute@${version}…`);
  ...
  console.error(`[verify-published] ❌ FAILED (exit ${r.status}) — consider: npm deprecate devilroute@${version} "<reason>"`);
  process.exit(1);
}
