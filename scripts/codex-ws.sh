#!/usr/bin/env bash
# =============================================================================
# codex-ws — roda a OpenAI Codex CLI contra um DevilRoute LOCAL usando o
#            transporte Responses-over-WebSocket (em vez do HTTP da Cloud).
#
# POR QUE ESTE WRAPPER EXISTE
# ---------------------------
# O DevilRoute expõe um proxy WebSocket para a API de Responses do Codex em
...
#     account"). É preciso mandar o id "puro" -> "gpt-5.5". (O DevilRoute, no
...
#   DEVILROUTE_WS_BASE   base URL do DevilRoute local   (default abaixo)
#   DEVILROUTE_WS_MODEL  modelo codex (id puro)        (default "gpt-5.5")
#   DEVILROUTE_LOCAL_KEY API key (qualquer valor se REQUIRE_API_KEY=false)
...
DEVILROUTE_WS_BASE="${DEVILROUTE_WS_BASE:-http://127.0.0.1:20128/v1}"  # base do DevilRoute local
DEVILROUTE_WS_MODEL="${DEVILROUTE_WS_MODEL:-gpt-5.5}"                  # id PURO (sem "codex/")
...
export DEVILROUTE_LOCAL_KEY="${DEVILROUTE_LOCAL_KEY:-local}"
...
# Gerado por codex-ws.sh — config isolada para o WS local do DevilRoute.
model = "$DEVILROUTE_WS_MODEL"
model_provider = "devilroute-local"

[model_providers.devilroute-local]
name = "DevilRoute Local (WS)"
base_url = "$DEVILROUTE_WS_BASE"   # a URL WebSocket é derivada desta base pela CLI
...
env_key = "DEVILROUTE_LOCAL_KEY"   # a CLI lê a key Bearer desta env var
...
overrides=(-c model="$DEVILROUTE_WS_MODEL" -c model_provider="devilroute-local")
