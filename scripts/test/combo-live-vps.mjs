 ...
 *
 * Phase-3 VPS HTTP scenario driver for DevilRoute combo routing.
...
// Known approximate input cost ($/M tokens) from DevilRoute's default-pricing constants.
...
//
// DevilRoute's sortModelsByCost uses getPricingForModel which merges
...
async function scenarioCostOptimized(healthy) {
  ...
  try {
    ...
    // Both direct match and provider-substring match are accepted since DevilRoute
...
//   404, DevilRoute calls recordProviderCooldown(providerA, undefined) — a provider-
...
//   1. SSH to read STORAGE_ENCRYPTION_KEY from /root/.devilroute/.env
//   2. Encrypt a wrong API key using scryptSync(key,"devilroute-field-encryption-v1",32)+AES-256-GCM
...
// ---------------------------------------------------------------------------
async function main() {
  console.log("=== DevilRoute combo-live-vps scenario driver ===");
