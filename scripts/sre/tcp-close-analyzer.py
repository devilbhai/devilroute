#!/usr/bin/env python3
"""
tcp-close-analyzer.py — dependency-free pcap analyzer for DevilRoute<->Caddy
...
(the devilroute-dev container)?

Why this exists: the dashboard's HTTP-level status (499 "Request aborted")
only tells us DevilRoute's own executor detected a dropped connection — it
doesn't tell us whether the underlying TCP socket was actually closed by the
far end, or whether DevilRoute itself is the one tearing it down (e.g. an
idle/read timeout on DevilRoute's side that then gets misreported as a client
...
container's OWN namespace via its PID instead:
    ...
    PID=$(podman inspect devilroute-dev --format '{{.State.Pid}}')
    sudo nsenter -t "$PID" -n tcpdump -i any -w /tmp/devilroute-capture.pcap \\
        'host <devilroute-container-ip> and port 20128'

Find the container's IP first with:
    podman inspect devilroute-dev --format '{{.NetworkSettings.Networks}}'

That captures all traffic between Caddy and the devilroute-dev container on
...
again), then Ctrl+C the tcpdump. Make the file readable:
    ...
    sudo chmod 644 /tmp/devilroute-capture.pcap

Then run this script against it:

    python3 scripts/sre/tcp-close-analyzer.py /tmp/devilroute-capture.pcap
...
<pcap>.streams.jsonl next to the input file), each with:
  ...
  - correlationId / requestId, if an `x-correlation-id:` / `x-request-id:`
    ...
    DevilRoute doesn't echo these on every hop, so this is a bonus, not the
...
That captures all traffic between Caddy and the devilroute-dev container —
...
Then run:
    ...
    python3 scripts/sre/tcp-close-analyzer.py /tmp/devilroute-capture.pcap --find "<marker>"
"""
    )
