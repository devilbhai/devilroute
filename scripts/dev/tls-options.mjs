...
//
// Pure, dependency-light helpers for DevilRoute's opt-in native HTTPS/TLS serving
 ...
 *
 * Reads `DEVILROUTE_TLS_CERT` and `DEVILROUTE_TLS_KEY` (filesystem paths). Returns
...
export function resolveTlsOptions(
  ...
  const certPath = typeof env?.DEVILROUTE_TLS_CERT === "string" ? env.DEVILROUTE_TLS_CERT.trim() : "";
  const keyPath = typeof env?.DEVILROUTE_TLS_KEY === "string" ? env.DEVILROUTE_TLS_KEY.trim() : "";
  ...
  if (!certPath || !keyPath) {
    warn(
      `[devilroute][tls] HTTPS not enabled: both DEVILROUTE_TLS_CERT and DEVILROUTE_TLS_KEY ` +
  ...
  try {
    ...
    warn(
      `[devilroute][tls] HTTPS not enabled: could not read TLS cert/key ` +
