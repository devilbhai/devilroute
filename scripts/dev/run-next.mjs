...
if (fs.existsSync(rootAppDir) && fs.statSync(rootAppDir).isDirectory()) {
  ...
  console.error("Please rename or delete the root 'app/' directory before starting DevilRoute.\n");
  process.exit(1);
}
...
process.env.DEVILROUTE_INTERNAL_SCHEME = "http";

const { dashboardPort } = runtimePorts;
const hostname = process.env.HOST || "0.0.0.0";
// Turbopack by default in dev (matches the Next 16 CLI default and the production
// build default in build-next-isolated.mjs); DEVILROUTE_USE_TURBOPACK=0 is the
// webpack escape hatch.
const useTurbopack = dev && mergedEnv.DEVILROUTE_USE_TURBOPACK !== "0";
