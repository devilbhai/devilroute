...
const bootstrapMode = process.env.DEVILROUTE_E2E_BOOTSTRAP_MODE || "auth";
const playwrightPassword =
  process.env.DEVILROUTE_E2E_PASSWORD || process.env.INITIAL_PASSWORD || "devilroute-e2e-password";
const testServerEnv = {
  ...
  NEXT_PUBLIC_DEVILROUTE_E2E_MODE: process.env.NEXT_PUBLIC_DEVILROUTE_E2E_MODE || "1",
  DEVILROUTE_DISABLE_BACKGROUND_SERVICES:
    process.env.DEVILROUTE_DISABLE_BACKGROUND_SERVICES || "true",
  DEVILROUTE_DISABLE_TOKEN_HEALTHCHECK: process.env.DEVILROUTE_DISABLE_TOKEN_HEALTHCHECK || "true",
  DEVILROUTE_DISABLE_LOCAL_HEALTHCHECK: process.env.DEVILROUTE_DISABLE_LOCAL_HEALTHCHECK || "true",
  DEVILROUTE_HIDE_HEALTHCHECK_LOGS: process.env.DEVILROUTE_HIDE_HEALTHCHECK_LOGS || "true",
  ...(bootstrapMode === "open"
    ? {
        INITIAL_PASSWORD: "",
        DEVILROUTE_E2E_PASSWORD: "",
        DEVILROUTE_API_KEY: "",
      }
    : {
        INITIAL_PASSWORD: playwrightPassword,
        DEVILROUTE_E2E_PASSWORD: playwrightPassword,
      }),
  ...(process.env.DEVILROUTE_USE_TURBOPACK
    ? {
        DEVILROUTE_USE_TURBOPACK: process.env.DEVILROUTE_USE_TURBOPACK,
      }
    : {}),
};
...
export function shouldUseWebpackForPlaywrightDev({ mode, env }) {
  ...
  return mode === "dev" && env.DEVILROUTE_USE_TURBOPACK === "0";
}
...
async function runBuildForStart() {
  ...
  if (process.env.DEVILROUTE_PLAYWRIGHT_SKIP_BUILD === "1") return;
