...
function normalizeEndpoint(rawEndpoint) {
  ...
  let parsed;
  try {
    parsed = new URL(endpoint, "http://devilroute.local");
  } catch {
    return null;
  }

  if (parsed.origin !== "http://devilroute.local") {
    return null;
  }
...
class WebSocketSession {
  ...
  async executeRequest(requestId, endpoint, payload, abortController) {
    const headers = {
      ...
      "content-type": "application/json",
      "x-devilroute-ws-session-id": this.sessionId,
      "x-devilroute-ws-request-id": requestId,
    };
