...
export const LINUX_EXECUTABLE_NAMES = ["devilroute-desktop", "devilroute", "DevilRoute"];
...
function discoverMacExecutable() {
  ...
  const candidates = [
    ...
    join(distDir, "mac", "DevilRoute.app", "Contents", "MacOS", "DevilRoute"),
    join(distDir, "mac-arm64", "DevilRoute.app", "Contents", "MacOS", "DevilRoute"),
  ];
...
function discoverWindowsExecutable() {
  const distDir = join(ROOT, "electron", "dist-electron");
  const candidates = [
    join(distDir, "win-unpacked", "DevilRoute.exe"),
    join(distDir, "win-x64-unpacked", "DevilRoute.exe"),
    join(distDir, "win-arm64-unpacked", "DevilRoute.exe"),
  ];

  return (
    candidates.find((candidate) => existsSync(candidate)) ||
    findExecutableByName(distDir, ["DevilRoute.exe"]) ||
    candidates[0]
  );
}
...
async function assertPortIsFree(url) {
  try {
    ...
    throw new Error(
      `Smoke URL already responded with HTTP ${response.status}: ${url}. Stop the existing DevilRoute process before running this check.`
    );
  } catch (error) {
...
async function ensureSmokeEnvDirs(smokeEnv, dataDir) {
  ...
  if (platform() === "win32" && smokeEnv.APPDATA) {
    for (const subdir of ["devilroute-desktop", "DevilRoute", "devilroute"]) {
...
async function main() {
  ...
  if (!existsSync(appExecutable)) {
    throw new Error(
      `Packaged DevilRoute executable not found at ${appExecutable}. Build it first with \`npm run build:<target> --prefix electron\` or set ELECTRON_SMOKE_APP_EXECUTABLE.`
    );
  }
  ...
  const dataDir =
    ...
    (await mkdtemp(join(tmpdir(), "devilroute-electron-smoke-")));
  const removeDataDir =
