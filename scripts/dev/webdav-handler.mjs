/**
 * WebDAV file server handler for DevilRoute.
 *
...
// Key derivation: scryptSync(secret, "devilroute-field-encryption-v1", 32)
// ─────────────────────────────────────────────────────────────────────────────

const ENC_PREFIX = "enc:v1:";
const STATIC_SALT = "devilroute-field-encryption-v1";
const KEY_LENGTH = 32;
...
/**
 * Decrypt a value that may be encrypted with the DevilRoute enc:v1: format.
...
function getDefaultDataDir() {
  ...
  const legacyDir = path.join(homeDir, ".devilroute"); // getLegacyDotDataDir()

  // 2) Preserve the legacy ~/.devilroute path if it already exists (avoid data loss).
  try {
  ...
  if (typeof xdgConfigHome === "string" && xdgConfigHome.trim().length > 0) {
    return path.join(path.resolve(xdgConfigHome.trim()), "devilroute");
  }

  // 5) Default → legacy ~/.devilroute (even if it does not exist yet).
  return legacyDir;
}
...
function sendUnauthorized(res) {
  sendError(res, 401, "Unauthorized", {
    "WWW-Authenticate": 'Basic realm="DevilRoute WebDAV"',
  });
}
...
async function handlePut(req, res, absPath) {
  ...
  const tmpPath = absPath + ".devilroute-webdav-tmp-" + Date.now();
  let writeStream;
  try {
