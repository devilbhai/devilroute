...
const CRYPTO_SECRETS = {
  ...
  // Generated in bin/devilroute.mjs:ensureStorageEncryptionKey() and persisted to
  // ~/.devilroute/.env to survive across upgrades. This prevents credential loss
  // when upgrading DevilRoute (issue #1622).
  MACHINE_ID_SALT: () => `devilroute-${randomBytes(8).toString("hex")}`,
};
...
function resolveDataDir(env = process.env) {
  ...
  if (process.platform === "win32") {
    ...
    return join(appData, "devilroute");
  }

  const xdg = env.XDG_CONFIG_HOME?.trim();
  if (xdg) return join(resolve(xdg), "devilroute");

  return join(homedir(), ".devilroute");
}
        ...
        if (ENCRYPTION_BOUND_KEYS.has(key) && dbHasEncrypted) {
          log(
            `⚠️  ${key} NOT generated — encrypted credentials exist in DB. ` +
              `Restore your previous key via ~/.devilroute/server.env, ~/.devilroute/.env, ` +
