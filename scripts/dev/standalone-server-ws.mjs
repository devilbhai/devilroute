...
// Opt-in native HTTPS (#5242). Resolved once at boot: when both DEVILROUTE_TLS_CERT
// and DEVILROUTE_TLS_KEY point at readable files we terminate TLS on the same
...
process.env.DEVILROUTE_INTERNAL_SCHEME = tlsOptions ? "https" : "http";
if (tlsOptions) {
  console.log(`[devilroute][tls] HTTPS enabled — terminating TLS with cert=${tlsOptions.certPath}`);
}

process.env.DEVILROUTE_WS_BRIDGE_SECRET ||= randomUUID();
...
function getProxy(server) {
  ...
  const proxy = createResponsesWsProxy({
    ...
    bridgeSecret: process.env.DEVILROUTE_WS_BRIDGE_SECRET,
  });
...
http.createServer = function createServerWithResponsesWs(...args) {
  ...
  // (#7003). This wrapper is what `devilroute serve` / Docker / Electron actually
