 ...
 * `DEVILROUTE_MEMORY_MB`, mirroring `devilroute serve`. Clamped to [64, 16384];
 * invalid/unset → fallback (512). The standalone launcher uses this so
 * DEVILROUTE_MEMORY_MB can override the Docker image's NODE_OPTIONS fallback
 ...
 * when `DEVILROUTE_MEMORY_MB` is unset. A fixed 512MB default crashed boxes with
 ...
 * explicit DEVILROUTE_MEMORY_MB override always wins.
 ...
 * (`--max-old-space-size=…`). Used to decide whether `devilroute serve` may
 ...
 * the user already exported. #5238: `devilroute serve` used to UNCONDITIONALLY
...
export function withRuntimePortEnv(env, runtimePorts) {
  ...
  return {
    ...env,
    DEVILROUTE_PORT: String(basePort),
