 ...
 *
 * When `DEVILROUTE_BUILD_BACKEND_ONLY=1` (or `DEVILROUTE_BUILD_PROFILE=backend`) is set,
 * `build-next-isolated.mjs` calls `stubDashboardPages()` BEFORE `next build` and
 * `restoreDashboardPages()` in a `finally` afterward.
 *
 * WHY: DevilRoute embedders that only consume the HTTP API (`/api/*`, `/v1/*`, `/v1beta/*`)
...
export const BACKEND_ONLY_STUB_MARKER =
  "/* devilroute:backend-only-stub (auto-restored after build) */";
...
/**
 * Strip a leading `"use server"` module directive. Some DevilRoute API Route Handlers
