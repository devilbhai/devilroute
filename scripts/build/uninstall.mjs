...
const uninstallAlreadyInProgress =
  process.env.DEVILROUTE_SKIP_UNINSTALL_HOOK === "1" ||
  process.env.npm_lifecycle_event === "uninstall";

console.log("🛑 DevilRoute Uninstaller");
console.log("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━");

// 1. Stop PM2 process if it exists
try {
  console.log("Stopping and removing background PM2 processes...");
  execSync("pm2 delete devilroute 2>/dev/null", { stdio: "ignore" });
} catch {
...
const dataDir = process.env.DATA_DIR || path.join(os.homedir(), ".devilroute");
...
if (uninstallAlreadyInProgress) {
  ...
  try {
    execSync("npm uninstall -g devilroute", {
      stdio: "inherit",
      env: {
        ...process.env,
        DEVILROUTE_SKIP_UNINSTALL_HOOK: "1",
      },
    });
    console.log("\n✅ DevilRoute has been successfully uninstalled from your system.");
    if (!fullUninstall) {
