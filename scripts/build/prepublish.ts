...
/**
 * DevilRoute — Prepublish Build Script
 *
...
console.log("🔨 DevilRoute — Building for npm publish...\n");
...
const cliSrcFile = join(ROOT, "bin", "devilroute.ts");
const cliDestFile = join(ROOT, "bin", "devilroute.mjs");
  ...
  try {
    execFileSync(
      ...
      [
        ...
        "--packages=external",
        "--format=esm",
        "--outfile=bin/devilroute.mjs",
      ],
      { cwd: ROOT, stdio: "inherit" }
    );
    chmodSync(cliDestFile, 0o755);
    console.log("  ✅ CLI Entrypoint bundled to bin/devilroute.mjs");
  } catch (err: any) {
    console.warn("  ⚠️  CLI bundle error:", err.message);
  }
}

// ── Step 8.8: Build @devilroute/opencode-plugin ──────────────
// The plugin ships bundled inside the devilroute npm package (see root
// package.json "files": ["@devilroute/", ...]). Its built `dist/` MUST be
// present in the publish tarball so `devilroute setup opencode` can copy it
...
const opencodePluginSrc = join(ROOT, "@devilroute", "opencode-plugin");
...
if (existsSync(opencodePluginSrc) && existsSync(join(opencodePluginSrc, "package.json"))) {
  ...
  if (!pluginAlreadyBuilt) {
    console.log("\n  🔨 Building @devilroute/opencode-plugin (tsup)...");
    try {
      ...
      console.log("  ✅ @devilroute/opencode-plugin bundled to @devilroute/opencode-plugin/dist/");
    } catch (err: any) {
      console.error("  ❌ Failed to build @devilroute/opencode-plugin:", err.message);
      console.error("     The published package would be missing the plugin dist.");
      console.error(
        "     Run `cd @devilroute/opencode-plugin && npm install && npm run build` to debug."
      );
      process.exit(1);
    }
  } else {
    console.log("  ✅ @devilroute/opencode-plugin dist/ already present (skipping rebuild)");
  }
  ...
  if (existsSync(pluginNodeModules)) {
    ...
    console.log("  🧹 Removed @devilroute/opencode-plugin/node_modules (hard link guard)");
  }
} else {
  console.log("  ⏭️  @devilroute/opencode-plugin not found in workspace (skipping build)");
}
