#!/usr/bin/env node
/**
 * DevilRoute — Zero-Config Bootstrap
 *
 ...
 *   3. Preferred config .env  (DATA_DIR/.env -> ~/.devilroute/.env -> ./.env)
...
function writeEnvFile(filePath, env) {
  const lines = [
    "# Auto-generated by DevilRoute bootstrap — do not delete",
...
export function bootstrapEnv({ dataDirOverride, quiet = false } = {}) {
  ...
  // This keeps run-next / run-standalone consistent with `bin/devilroute.mjs`.
  //
  ...
  if (needsPersist) {
    merged.DEVILROUTE_BOOTSTRAPPED = "true";
  }
        ...
        if (row) {
          ...
          if (ciphertext?.startsWith("enc:v1:")) {
            ...
            if (parts.length >= 5) {
              ...
              // Legacy salt (fallback): scryptSync(secret, "devilroute-field-encryption-v1", 32)
              const legacySalt = "devilroute-field-encryption-v1";
              ...
              if (!keyMatched) {
                ...
                log(
                  "   Either restore your previous key via ~/.devilroute/server.env or ~/.devilroute/.env,"
                );
                log(
                  "   or run: devilroute reset-encrypted-columns --force  (wipes credentials, keeps provider config)"
                );
              }
            }
          }
