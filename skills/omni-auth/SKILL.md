---
name: omni-auth
description: Manage API key authentication and session tokens. Start here to authenticate requests via Bearer token, obtain session cookies, and configure login requirements for the DevilRoute API.
---
...
<!-- skill:custom-start -->
<!-- Migrated from skills/devilroute/SKILL.md (preserved curated content) -->
...
```bash
export DEVILROUTE_URL="http://localhost:20128"      # or VPS / tunnel URL
export DEVILROUTE_KEY="sk-..."                       # from Dashboard → API Keys
```

All requests: `${DEVILROUTE_URL}/v1/...` with `Authorization: Bearer ${DEVILROUTE_KEY}`.

Verify: `curl $DEVILROUTE_URL/api/health` → `{"ok":true}`

## Discover models

```bash
curl $DEVILROUTE_URL/v1/models                  # chat/LLM (default)
curl $DEVILROUTE_URL/v1/models/image            # image-gen
curl $DEVILROUTE_URL/v1/models/tts              # text-to-speech
curl $DEVILROUTE_URL/v1/models/embedding        # embeddings
curl $DEVILROUTE_URL/v1/models/web              # web search + fetch
curl $DEVILROUTE_URL/v1/models/stt              # speech-to-text
```
...
- `401` → set/refresh `DEVILROUTE_KEY` (Dashboard → API Keys)
