---
name: omni-cli-tools
description: "Manage CLI tool integrations exposed via the API. List, configure, and invoke CLI tool plugins that extend DevilRoute's automation surface."
---
...
Read Crush CLI DevilRoute config

Local-only. Reads the DevilRoute provider block in Crush's config.
...
Write Crush CLI DevilRoute config

Local-only. Registers DevilRoute as an `openai-compat` provider in Crush's config.
...
Remove DevilRoute from Crush CLI config

Local-only. Removes the DevilRoute provider block from Crush's config.
...
Read CodeWhale CLI DevilRoute config

Local-only. Reads the DevilRoute config block from `~/.codewhale/config.toml` (with `~/.deepseek/config.toml` legacy fallback).
...
Write CodeWhale CLI DevilRoute config

Local-only. Writes the DevilRoute config block in CodeWhale TOML format.
...
Remove DevilRoute from CodeWhale CLI config

Local-only. Removes the DevilRoute config block from CodeWhale's config.
