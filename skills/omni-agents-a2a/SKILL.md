...
description: "Interact with DevilRoute via JSON-RPC 2.0 agent-to-agent protocol. 6 built-in A2A skills: smart-routing, quota-management, provider-discovery, cost-analysis, health-report, list-capabilities."
---
...
<!-- skill:custom-start -->
<!-- Migrated from skills/devilroute-a2a/SKILL.md (preserved curated content) -->
...
```bash
curl $DEVILROUTE_URL/.well-known/agent.json
```
...
```bash
curl -X POST $DEVILROUTE_URL/a2a \
...
- `401` → missing/invalid `DEVILROUTE_KEY`
