...
<!-- skill:custom-start -->
<!-- Migrated from skills/devilroute-routing/SKILL.md (preserved curated content) -->
...
```bash
curl $DEVILROUTE_URL/api/combos \
...
```bash
curl -X POST $DEVILROUTE_URL/api/combos \
  -H "Authorization: Bearer $DEVILROUTE_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "name": "my-combo",
...
```bash
# Activate
curl -X PUT $DEVILROUTE_URL/api/combos/{id}/toggle \
  -H "Authorization: Bearer $DEVILROUTE_KEY" \
  -d '{ "enabled": true }'
```
...
```bash
curl $DEVILROUTE_URL/api/combos/{id}/metrics \
  -H "Authorization: Bearer $DEVILROUTE_KEY"
```
...
```bash
curl -X POST $DEVILROUTE_URL/api/routing/simulate \
  -H "Authorization: Bearer $DEVILROUTE_KEY" \
  -H "Content-Type: application/json" \
...
```
devilroute_list_combos     → list all combos
devilroute_switch_combo    → enable/disable a combo
devilroute_set_routing_strategy → change strategy at runtime
devilroute_simulate_route  → dry-run routing decision
devilroute_best_combo_for_task → get recommendation by task type
```
