...
curl -X POST https://localhost:20128/api/v1/chat/completions \
  -H "Authorization: Bearer $DEVILROUTE_TOKEN"
...
Mistral OCR–compatible document OCR endpoint. Accepts a JSON body referencing a document/image and returns extracted text. Success responses carry the `X-DevilRoute-*` cost-telemetry headers.
...
OpenAI Whisper–compatible audio translation (multipart/form-data). Unlike `/api/v1/audio/transcriptions`, output is always English regardless of the source language. Success responses carry the `X-DevilRoute-*` cost-telemetry headers.
...
<!-- skill:custom-start -->
<!-- Aggregated from: devilroute-chat, devilroute-image, devilroute-tts, devilroute-stt, devilroute-embeddings, devilroute-web-search, devilroute-web-fetch -->
...
- `POST $DEVILROUTE_URL/v1/chat/completions` — OpenAI format
- `POST $DEVILROUTE_URL/v1/messages` — Anthropic Messages format
- `POST $DEVILROUTE_URL/v1/responses` — OpenAI Responses API

### Discover

```bash
curl $DEVILROUTE_URL/v1/models | jq '.data[].id'
```
...
```bash
curl -X POST $DEVILROUTE_URL/v1/chat/completions \
...
```bash
curl -X POST $DEVILROUTE_URL/v1/messages \
  -H "Authorization: Bearer $DEVILROUTE_KEY" \
...
- `POST $DEVILROUTE_URL/v1/images/generations` — Text-to-image
- `POST $DEVILROUTE_URL/v1/images/edits` — Image edit (mask)
- `POST $DEVILROUTE_URL/v1/images/variations` — Variations

### Discover

```bash
curl $DEVILROUTE_URL/v1/models/image | jq '.data[]'
```
...
```bash
curl -X POST $DEVILROUTE_URL/v1/images/generations \
  -H "Authorization: Bearer $DEVILROUTE_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "model": "dall-e-3",
...
- `POST $DEVILROUTE_URL/v1/audio/speech` — returns binary audio (mp3/opus/wav/flac)

### Discover

```bash
curl $DEVILROUTE_URL/v1/models/tts | jq '.data[]'
```
...
```bash
curl -X POST $DEVILROUTE_URL/v1/audio/speech \
  -H "Authorization: Bearer $DEVILROUTE_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "model": "tts-1",
    "input": "Hello from DevilRoute.",
    "voice": "alloy",
    "response_format": "mp3"
  }' --output speech.mp3
```
...
- `POST $DEVILROUTE_URL/v1/audio/transcriptions` — multipart upload, returns text
- `POST $DEVILROUTE_URL/v1/audio/translations` — transcribe + translate to English

### Discover

```bash
curl $DEVILROUTE_URL/v1/models/stt | jq '.data[]'
```

### Example

```bash
curl -X POST $DEVILROUTE_URL/v1/audio/transcriptions \
  -H "Authorization: Bearer $DEVILROUTE_KEY" \
  -F "file=@audio.mp3" \
  -F "model=whisper-1" \
...
- `POST $DEVILROUTE_URL/v1/embeddings`

### Discover

```bash
curl $DEVILROUTE_URL/v1/models/embedding | jq '.data[]'
```
...
```bash
curl -X POST $DEVILROUTE_URL/v1/embeddings \
  -H "Authorization: Bearer $DEVILROUTE_KEY" \
  -H "Content-Type: application/json" \
  -d '{
...
- `POST $DEVILROUTE_URL/v1/web/search` — unified search format

### Discover

```bash
curl $DEVILROUTE_URL/v1/models/web | jq '.data[] | select(.kind == "webSearch")'
```

### Example

```bash
curl -X POST $DEVILROUTE_URL/v1/web/search \
  -H "Authorization: Bearer $DEVILROUTE_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "model": "tavily/search",
    "query": "DevilRoute github latest release",
    "max_results": 5,
    "include_answer": true
  }'
```
...
- `POST $DEVILROUTE_URL/v1/web/fetch`

### Discover

```bash
curl $DEVILROUTE_URL/v1/models/web | jq '.data[] | select(.kind == "webFetch")'
```

### Example

```bash
curl -X POST $DEVILROUTE_URL/v1/web/fetch \
  -H "Authorization: Bearer $DEVILROUTE_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "model": "jina/reader",
