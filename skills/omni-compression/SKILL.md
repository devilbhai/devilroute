...
<!-- skill:custom-start -->
<!-- Migrated from skills/devilroute-compression/SKILL.md (preserved curated content) -->
...
DevilRoute compresses token payloads before forwarding to providers. No code changes required — set it once, it applies to all requests transparently.
...
```bash
curl $DEVILROUTE_URL/api/settings/compression \
...
```bash
curl -X PUT $DEVILROUTE_URL/api/settings/compression \
  -H "Authorization: Bearer $DEVILROUTE_KEY" \
  -H "Content-Type: application/json" \
...
```bash
curl -X POST $DEVILROUTE_URL/api/compression/preview \
  -H "Authorization: Bearer $DEVILROUTE_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "mode": "rtk",
...
When DevilRoute is used with browser/Playwright MCP tools, it automatically compresses verbose accessibility-tree tool results. Enabled by default; configure thresholds:
...
```
devilroute_compression_status     → current settings + savings analytics
devilroute_compression_configure  → update mode/threshold/language
devilroute_set_compression_engine → switch engine at runtime
```
