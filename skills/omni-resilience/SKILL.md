...
<!-- skill:custom-start -->
<!-- Migrated from skills/devilroute-monitoring/SKILL.md (preserved curated content) -->

# DevilRoute — Monitoring & Health
...
```bash
curl $DEVILROUTE_URL/api/health \
...
```bash
curl $DEVILROUTE_URL/api/monitoring/health \
  -H "Authorization: Bearer $DEVILROUTE_KEY"
```
...
```bash
curl $DEVILROUTE_URL/api/providers/metrics \
  -H "Authorization: Bearer $DEVILROUTE_KEY"
```
...
## Via MCP (if DevilRoute is your MCP server)

```
devilroute_get_health            → full system health snapshot
devilroute_get_provider_metrics  → p50/p95/p99 + circuit state per provider
devilroute_get_session_snapshot  → cost, tokens, errors for current session
devilroute_check_quota           → quota balance + percent remaining + reset time
devilroute_db_health_check       → diagnose + auto-repair database drift
```

## Quota check

```bash
curl $DEVILROUTE_URL/api/quota \
  -H "Authorization: Bearer $DEVILROUTE_KEY"
```
...
```bash
curl -X POST $DEVILROUTE_URL/api/budget/guard \
  -H "Authorization: Bearer $DEVILROUTE_KEY" \
...
DevilRoute logs every MCP tool call to `mcp_audit` table. Query via API:

```bash
curl "$DEVILROUTE_URL/api/mcp/status" \
  -H "Authorization: Bearer $DEVILROUTE_KEY"
```
...
- `503` on health endpoint → DevilRoute is starting up; retry in 5s
