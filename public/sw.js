const CACHE_NAME = "devilroute-pwa-v2";
const APP_SHELL = [
  "/",
  "/offline",
...
self.addEventListener("push", (event) => {
  ...
  try {
    ...
    data = { title: "DevilRoute", body: event.data?.text() || "New notification" };
  }

  const title = data.title || "DevilRoute";
  const options = {
    body: data.body || "",
    icon: data.icon || "/icon-512.png",
    badge: data.badge || "/icon-192.png",
    tag: data.tag || "devilroute-default",
    data: {
