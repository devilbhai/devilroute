...
Run DevilRoute with **podman compose** on Linux, macOS, or Windows, or with
...
```bash
cd /path/to/devilroute
podman build --target runner-base -t devilroute:base .
# For web-cookie providers (gemini-web, claude-web, claude-turnstile):
podman build --target runner-web -t devilroute:web .
# For CLI tool support:
podman build --target runner-cli -t devilroute:cli .
```

### 2. Copy Quadlet files to the systemd directory

```bash
mkdir -p ~/.config/containers/systemd/devilroute
cp contrib/podman/*.container ~/.config/containers/systemd/devilroute/
cp contrib/podman/*.network ~/.config/containers/systemd/devilroute/
cp contrib/podman/*.volume ~/.config/containers/systemd/devilroute/
```

### 3. Mount the project .env for secrets

Edit `~/.config/containers/systemd/devilroute/devilroute.container` and
uncomment/replace the `EnvironmentFile` line with the absolute path to
your project `.env`:

```
EnvironmentFile=/home/USER/code/docker/DevilRoute/.env
```
...
systemctl --user start devilroute-redis
systemctl --user start devilroute
```

### 5. Verify

```bash
systemctl --user status devilroute
...
```bash
journalctl --user -u devilroute -f
```
...
The application profiles use local image names such as `devilroute:base`; those
...
```bash
podman volume create devilroute-data
podman run -d --name devilroute \
  --env-file .env \
  -e DATA_DIR=/app/data \
  -p 20128:20128 \
